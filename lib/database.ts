...
export function isSQL(value: any): value is SQLStatement {
  ...
  return (
    ...
    (value?.constructor.name === 'SQLStatement' &&
      Array.isArray(value.sql) &&
...
export abstract class Database<
  Pool extends mysql.Pool | pg.Pool = mysql.Pool | pg.Pool,
  OkPacket = unknown,
> {
  connection: Pool;
  prefix: string;
  ...
  queryOne<T = ResultRow>(): (
    strings: TemplateStringsArray,
    ...rest: SQLValue[]
  ) => Promise<T | undefined>;
  queryOne<T = ResultRow>(sql?: SQLStatement) {
    if (!sql)
      return (strings: any, ...rest: any) => this.queryOne<T>(new SQLStatement(strings, rest));
  ...
  queryExec(sql?: SQLStatement) {
    if (!sql)
      return (strings: any, ...rest: any) => this.queryExec(new SQLStatement(strings, rest));
...
export class DatabaseTable<Row, DB extends Database> {
  ...
  constructor(db: DB, name: string, primaryKeyName: (keyof Row & string) | null = null) {
    this.db = db;
  ...
  selectAll<T = Row>(
    entries?: (keyof Row & string)[] | SQLStatement
  ...
  deleteAll(): (strings: TemplateStringsArray, ...rest: SQLValue[]) => Promise<OkPacketOf<DB>> {
    return (strings, ...rest) =>
      this.queryExec()`DELETE FROM "${this.name}" ${new SQLStatement(strings, rest)}`;
  }
  deleteOne(): (strings: TemplateStringsArray, ...rest: SQLValue[]) => Promise<OkPacketOf<DB>> {
    return (strings, ...rest) =>
      this.queryExec()`DELETE FROM "${this.name}" ${new SQLStatement(strings, rest)} LIMIT 1`;
  }
  eval<T>(): (strings: TemplateStringsArray, ...rest: SQLValue[]) => Promise<T | undefined> {
    return (strings, ...rest) =>
      this.queryOne<{
        result: T;
      }>()`SELECT ${new SQLStatement(strings, rest)} AS result FROM "${this.name}" LIMIT 1`.then(
        row => row?.result
      );
  }
  ...
  upsert(partialRow: PartialOrSQL<Row>, partialUpdate = partialRow, where?: SQLStatement) {
    if (this.db.type === 'pg') {
      return this.queryExec()`INSERT INTO "${this.name}" (${partialRow as any}) ON CONFLICT (${
        this.primaryKeyName
      }) DO UPDATE ${partialUpdate as any} ${where}`;
    }
    return this.queryExec()`INSERT INTO "${this.name}" (${partialRow as any}) ON DUPLICATE KEY UPDATE ${partialUpdate as any} ${where}`;
  }
...
export class PGDatabase extends Database<pg.Pool, { affectedRows: number | null }> {
  ...
  override _queryExec(query: string, values: BasicSQLValue[]) {
    return this.connection
      .query<never>(query, values)
      .then(res => ({ affectedRows: res.rowCount }));
  }
  override escapeId(id: string) {
