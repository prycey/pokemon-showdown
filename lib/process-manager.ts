...
export function exec(
  args: [string, ...string[]],
  execOptions?: child_process.ExecFileOptions
): Promise<{ stderr: string; stdout: string }>;
...
export class QueryProcessWrapper<T, U> implements ProcessWrapper {
  ...
  safeJSON(obj: string): any {
    ...
    try {
      ...
      (global as any).Monitor?.crashlog?.(e, `a ${path.basename(this.file)} process`, {
        result: obj,
      });
      return undefined;
    }
...
export abstract class ProcessManager<T extends ProcessWrapper = ProcessWrapper> {
  ...
  releaseCrashed(process: T) {
    ...
    void Promise.reject(
      new Error(
        `Process ${this.basename} ${process.getProcess().pid} crashed and had to be restarted`
      )
    );
...
> {
  ...
  listen() {
    ...
    process.on('message', (message: string) => {
      ...
      void Promise.resolve(this._query(JSON.parse(message))).then(response =>
