...
export class ReadStream {
  ...
  constructor(
    optionsOrStreamLike: { [k: string]: any } | NodeJS.ReadableStream | string | Buffer = {}
  ) {
  ...
  _read(size = 0): void | Promise<void> {
    throw new Error(
      `ReadStream needs to be subclassed and the _read function needs to be implemented.`
    );
  }
  ...
  async next(byteCount: number | null = null) {
    ...
    return { value, done: value === null } as
...
export class WriteStream {
  ...
  _write(chunk: Buffer | string): void | Promise<void> {
    throw new Error(
      `WriteStream needs to be subclassed and the _write function needs to be implemented.`
    );
  }
