...
export class PostgresDatabase {
  ...
  async ensureMigrated(opts: MigrationOptions) {
    ...
    try {
      ...
      await this.query(
        `CREATE TABLE db_info (name TEXT NOT NULL, key TEXT NOT NULL, value TEXT NOT NULL)`
      );
    }
    if (!value) {
      // means nothing inserted - create row
      value = '0';
      await this.query('INSERT INTO db_info (name, key, value) VALUES ($1, $2, $3)', [
        opts.table,
        'version',
        value,
      ]);
    ...
    if (curVer !== value) {
      ...
      for (const n of files) {
        ...
        await this.query(`UPDATE db_info SET value = $1 WHERE key = 'version' AND name = $2`, [
          `${n}`,
          opts.table,
        ]);
      }
