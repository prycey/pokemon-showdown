  ...
  'node_modules/@nodelib/fs.walk/out/readers/async.js'(exports) {
    ...
    var AsyncReader = class extends reader_1.default {
      ...
      _handleEntry(entry, base) {
        ...
        if (base !== void 0) {
          entry.path = common2.joinPathSegments(
            base,
            entry.name,
        ...
        if (
          ...
          common2.isAppliedFilter(this._settings.deepFilter, entry)
        ) {
...
var require_ci_info = __commonJS({
  'node_modules/ci-info/index.js'(exports) {
    ...
    exports.isCI = !!(
      env2.CI !== 'false' && // Bypass all checks if CI env is explicitly set to 'false'
...
var require_picocolors = __commonJS({
  'node_modules/picocolors/picocolors.js'(exports, module) {
    ...
    var isColorSupported =
      !(!!env2.NO_COLOR || argv.includes('--no-color')) &&
      (!!env2.FORCE_COLOR ||
        argv.includes('--color') ||
        p.platform === 'win32' ||
        ((p.stdout || {}).isTTY && env2.TERM !== 'dumb') ||
        !!env2.CI);
    var formatter =
...
var require_lib2 = __commonJS({
  'node_modules/@babel/code-frame/lib/index.js'(exports) {
    ...
    function highlight(text) {
      ...
      let highlighted = '';
      for (const { type: type2, value } of tokenize2(text)) {
        if (type2 in defs) {
          highlighted += value
            .split(NEWLINE$1)
            .map(str2 => defs[type2](str2))
            .join('\n');
        } else {
          highlighted += value;
        }
  ...
  'node_modules/ignore/index.js'(exports, module) {
    ...
    var Ignore = class {
      ...
      _test(originalPath, cache3, checkUnignored, slices) {
        ...
        checkPath(path13, originalPath, this._strictPathCheck ? throwError2 : RETURN_FALSE);
      ...
      _t(path13, cache3, checkUnignored, slices) {
        ...
        return (cache3[path13] = parent.ignored
          ? parent
          : this._rules.test(path13, checkUnignored, MODE_IGNORE));
      }
      ignores(path13) {
    ...
    var isPathValid = path13 =>
      checkPath(path13 && checkPath.convert(path13), path13, RETURN_FALSE);
    if (
      // Detect `process` so that it can run in browsers.
      typeof process !== 'undefined' &&
      ((process.env && process.env.IGNORE_TEST_WIN32) || process.platform === 'win32')
    ) {
      const makePosix = str2 =>
        /^\\\\\?\\/.test(str2) || /["<>|\u0000-\u001F]+/u.test(str2)
          ? str2
          : str2.replace(/\\/g, '/');
      checkPath.convert = makePosix;
      const REGEX_TEST_WINDOWS_PATH_ABSOLUTE = /^[a-z]:\//i;
      checkPath.isNotRelative = path13 =>
        REGEX_TEST_WINDOWS_PATH_ABSOLUTE.test(path13) || isNotRelative(path13);
    }
...
Diff.prototype = {
  diff: function diff(oldString, newString) {
    ...
    var maxExecutionTime =
      (_options$timeout = options8.timeout) !== null && _options$timeout !== void 0
        ? _options$timeout
        : Infinity;
    ...
    if (bestPath[0].oldPos + 1 >= oldLen && newPos + 1 >= newLen) {
      return done(
        buildValues(self, bestPath[0].lastComponent, newString, oldString, self.useLongestToken)
      );
    }
    ...
    function execEditLength() {
      for (
        var diagonalPath = Math.max(minDiagonalToConsider, -editLength);
        ...
        if (basePath.oldPos + 1 >= oldLen && newPos + 1 >= newLen) {
          return done(
            buildValues(self, basePath.lastComponent, newString, oldString, self.useLongestToken)
          );
        } else {
  ...
  extractCommon: function extractCommon(basePath, newString, oldString, diagonalPath, options8) {
    ...
    while (
      newPos + 1 < newLen &&
      oldPos + 1 < oldLen &&
      this.equals(oldString[oldPos + 1], newString[newPos + 1], options8)
    ) {
      newPos++;
      oldPos++;
      commonCount++;
...
function replacePrefix(string, oldPrefix, newPrefix) {
  if (string.slice(0, oldPrefix.length) != oldPrefix) {
    throw Error(
      'string '
        .concat(JSON.stringify(string), " doesn't start with prefix ")
...
function replaceSuffix(string, oldSuffix, newSuffix) {
  ...
  if (string.slice(-oldSuffix.length) != oldSuffix) {
    throw Error(
      'string '
        .concat(JSON.stringify(string), " doesn't end with suffix ")
...
jsonDiff.castInput = function (value, options8) {
  var undefinedReplacement = options8.undefinedReplacement,
    _options$stringifyRep = options8.stringifyReplacer,
    stringifyReplacer =
      _options$stringifyRep === void 0
        ? function (k, v) {
...
function structuredPatch(oldFileName, newFileName, oldStr, newStr, oldHeader, newHeader, options8) {
  ...
  if (!options8.callback) {
    ...
    var _options = options8,
      _callback = _options.callback;
    diffLines(
      oldStr,
      newStr,
      _objectSpread2(
        _objectSpread2({}, options8),
        {},
        {
    ...
    var _loop = function _loop2() {
      ...
      if (current.added || current.removed) {
        ...
        if (!oldRangeStart) {
          ...
          if (prev) {
            curRange =
              options8.context > 0 ? contextLines(prev.lines.slice(-options8.context)) : [];
...
function formatPatch(diff2) {
  ...
  ret.push(
    '--- ' +
      diff2.oldFileName +
      (typeof diff2.oldHeader === 'undefined' ? '' : '	' + diff2.oldHeader)
  );
  ret.push(
    '+++ ' +
      diff2.newFileName +
      (typeof diff2.newHeader === 'undefined' ? '' : '	' + diff2.newHeader)
  );
...
function _supportsColor(haveStream, { streamIsTTY, sniffFlags = true } = {}) {
  ...
  if ('CI' in env) {
    ...
    if (
      ['TRAVIS', 'APPVEYOR', 'GITLAB_CI', 'BUILDKITE', 'DRONE'].some(sign2 => sign2 in env) ||
...
var applyOptions = (object, options8 = {}) => {
  if (
    options8.level &&
    !(Number.isInteger(options8.level) && options8.level >= 0 && options8.level <= 3)
  ) {
...
var commonDeprecatedHandler = (keyOrPair, redirectTo, { descriptor }) => {
  ...
  if (redirectTo) {
    messages2.push(
      `we now treat it as ${source_default.blue(typeof redirectTo === 'string' ? descriptor.key(redirectTo) : descriptor.pair(redirectTo))}`
    );
  }
...
function getListDescription({ text, list }, printWidth) {
  ...
  if (list) {
    descriptions.push(
      [`- ${source_default.blue(list.title)}:`]
        .concat(
          list.values.map(valueDescription =>
...
var IntegerSchema = class extends NumberSchema {
  ...
  validate(value, utils) {
    return (
      utils.normalizeValidateResult(super.validate(value, utils), value) === true && isInt(value)
    );
  }
};
  ...
  _applyNormalization(options8, newOptions) {
    ...
    for (const key2 of knownKeys) {
      ...
      if ('remain' in redirectResult) {
        ...
        newOptions[key2] =
          key2 in newOptions
            ? schema2.overlap(newOptions[key2], remainingValue, this._utils)
            : remainingValue;
...
var Searcher = class {
  ...
  async search(startDirectory, { shouldCache }) {
    ...
    for (const directory of iterate_directory_up_default(
      ...
      result = await __privateMethod(this, _Searcher_instances, searchInDirectory_fn).call(
        this,
        directory,
        shouldCache
      );
...
var safeLastIndexOf = (string, searchString, index) =>
...
var _JSONError = class _JSONError extends Error {
  ...
  get message() {
    ...
    return `${__privateGet(this, _message)}${fileName ? ` in ${fileName}` : ''}${
      codeFrame
        ? `
...
var generateCodeFrame = (string, location, highlightCode = true) =>
  (0, import_code_frame.codeFrameColumns)(string, { start: location }, { highlightCode });
var getErrorLocation = (string, message) => {
  ...
  if (index === string.length) {
    const { line: line4, column: column3 } = indexToLineColumn(string, string.length - 1, {
      oneBased: true,
    });
...
var _TomlDate = class _TomlDate extends Date {
  ...
  isLocal() {
    return (
      !__privateGet(this, _hasDate) || !__privateGet(this, _hasTime) || !__privateGet(this, _offset)
    );
  }
  isDate() {
    return __privateGet(this, _hasDate) && !__privateGet(this, _hasTime);
  ...
  toISOString() {
    ...
    let offset =
      +__privateGet(this, _offset).slice(1, 3) * 60 + +__privateGet(this, _offset).slice(4, 6);
...
function extractValue(str2, ptr, end, depth) {
  ...
  return [parseValue(slice[0], str2, ptr), endPtr];
}
...
function formatList(array2, type2 = 'and') {
  return array2.length < 3
    ? array2.join(` ${type2} `)
    : `${array2.slice(0, -1).join(', ')}, ${type2} ${array2[array2.length - 1]}`;
}
...
function resolve2(specifier, parent) {
  ...
  try {
    ...
    const exception2 =
      ...
      error;
    if (
      (exception2.code === 'ERR_UNSUPPORTED_DIR_IMPORT' ||
        exception2.code === 'ERR_MODULE_NOT_FOUND') &&
      typeof exception2.url === 'string'
    ) {
  ...
  if (Array.isArray(merged.plugins)) {
    merged.plugins = merged.plugins.map(value =>
      typeof value === 'string' && value.startsWith('.')
        ? path9.resolve(path9.dirname(result.configFile), value)
        : value
    );
  }
  return merged;
...
async function createIsIgnoredFunction(ignoreFiles, withNodeModules) {
  ...
  const isIgnoredFunctions = (
    await Promise.all(
      ignoreFiles.map(ignoreFile => createSingleIsIgnoredFunction(ignoreFile, withNodeModules))
    )
  ).filter(Boolean);
...
function getLanguageByFileName(languages2, file) {
  ...
  return (
    ...
    languages2.find(({ extensions }) =>
      extensions == null ? void 0 : extensions.some(extension => basename.endsWith(extension))
    )
  );
}
function getLanguageByLanguageName(languages2, languageName) {
  ...
  return (
    ...
    languages2.find(({ aliases }) => (aliases == null ? void 0 : aliases.includes(languageName))) ??
    languages2.find(({ extensions }) =>
...
function getLanguageByInterpreter(languages2, file) {
  ...
  return languages2.find(({ interpreters }) =>
    interpreters == null ? void 0 : interpreters.includes(interpreter)
  );
}
...
async function getParser(file, options8) {
  ...
  return (
    (config == null ? void 0 : config.parser) ??
  ...
  function printDoc(doc3, index, parentParts) {
    ...
    if (doc3.type === DOC_TYPE_IF_BREAK) {
      return (
        ...
        (doc3.groupId
          ? (!doc3.flatContents ? ', ""' : '') + `, { groupId: ${printGroupId(doc3.groupId)} }`
          : '') +
        ')'
      );
    }
...
function getSortedChildNodes(node, options8) {
  ...
  ).flatMap(node2 => (canAttachComment(node2) ? [node2] : getSortedChildNodes(node2, options8)));
  childNodes.sort(
...
function attachComments(ast, options8) {
  ...
  for (const [index, context] of decoratedComments.entries()) {
    ...
    if (
      options9.parser === 'json' ||
      options9.parser === 'json5' ||
      options9.parser === 'jsonc' ||
      options9.parser === '__js_expression' ||
      options9.parser === '__ts_expression' ||
      options9.parser === '__vue_expression' ||
      options9.parser === '__vue_ts_expression'
    ) {
...
function printCommentsSeparately(path13, options8) {
  ...
  path13.each(() => {
    ...
    if (leading) {
      ...
      printedTrailingComment = printTrailingComment(path13, options8, printedTrailingComment);
...
function optionInfoToSchema(optionInfo, { isCLI, optionInfos, FlagSchema }) {
  ...
  if (optionInfo.exception) {
    parameters.validate = (value, schema2, utils) =>
      optionInfo.exception(value) || schema2.validate(value, utils);
  } else {
    parameters.validate = (value, schema2, utils) =>
      value === void 0 || schema2.validate(value, utils);
  }
  ...
  if (isCLI && !optionInfo.array) {
    ...
    parameters.preprocess = (value, schema2, utils) =>
      schema2.preprocess(
        originalPreprocess(
          Array.isArray(value)
            ? at_default(
...
async function printAstToDoc(ast, options8) {
  ...
  const doc2 = await callPluginPrintFunction(path13, options8, mainPrint, void 0, embeds);
...
function callPluginPrintFunction(path13, options8, printPath, args, embeds) {
  ...
  if (
    ...
    (!printer.willPrintOwnComments || !printer.willPrintOwnComments(path13, options8))
  ) {
...
function findNodeAtOffset(node, offset, options8, predicate, parentNodes = [], type2) {
  ...
  if (
    offset > end ||
    offset < start ||
    (type2 === 'rangeEnd' && offset === start) ||
    (type2 === 'rangeStart' && offset === end)
  ) {
    return;
  }
...
function isJsSourceElement(type2, parentType) {
  return (
    ...
    (type2 === 'Directive' ||
      type2 === 'TypeAlias' ||
      type2 === 'TSExportAssignment' ||
      type2.startsWith('Declare') ||
      type2.startsWith('TSDeclare') ||
      type2.endsWith('Statement') ||
      type2.endsWith('Declaration'))
  );
}
...
function calculateRange(text, opts, ast) {
  ...
  const endNodeAndParents =
    ...
    isAllWhitespace
      ? startNodeAndParents
      : findNodeAtOffset(ast, end, opts, node => isSourceElement(opts, node), [], 'rangeEnd');
...
function getPreferredQuote(text, preferredQuoteOrPreferSingleQuote) {
  const preferred =
    preferredQuoteOrPreferSingleQuote === true || preferredQuoteOrPreferSingleQuote === SINGLE_QUOTE
      ? SINGLE_QUOTE
      : DOUBLE_QUOTE;
