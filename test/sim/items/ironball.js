...
describe('Iron Ball', () => {
  ...
  it("should reduce halve the holder's speed", () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Smeargle', ability: 'owntempo', item: 'ironball', moves: ['bestow'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Aerodactyl', ability: 'pressure', moves: ['stealthrock'] }],
    });
    const target = battle.p2.active[0];
    assert.sets(
      () => target.getStat('spe'),
      battle.modify(target.getStat('spe'), 0.5),
      () => battle.makeChoices('move bestow', 'move stealthrock')
    );
  });
  ...
  it('should ground Pokemon that are airborne', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Smeargle', ability: 'owntempo', moves: ['spore'] }],
    });
    battle.setPlayer('p2', {
      team: [
        {
