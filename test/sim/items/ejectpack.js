...
describe(`Eject Pack`, () => {
  ...
  it.skip(`should wait until after all other end-turn effects have resolved before switching out the holder`, () => {
    ...
    assert(
      powerConstructIndex < ejectPackIndex,
      'Eject Pack should not activate before Power Construct'
    );
  });
  ...
  it.skip(`should not activate when another switching effect was triggered as part of the move`, () => {
    ...
    assert.species(
      battle.p2.active[0],
      'Zeraora',
      `Zeraora should not have switched out with its Eject Pack.`
    );
    assert.species(
      battle.p2.active[1],
      'Wynaut',
      `Mew should have switched out with its Eject Button.`
    );
  });
