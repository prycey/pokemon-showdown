...
describe('Flame Orb', () => {
  ...
  it('should not trigger when entering battle', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Breloom', ability: 'technician', moves: ['bulletseed'] }],
    });
  ...
  it('should trigger after one turn', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Ursaring', ability: 'guts', item: 'flameorb', moves: ['protect'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Magikarp', ability: 'swiftswim', moves: ['splash'] }],
    });
