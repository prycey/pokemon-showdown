...
describe('Protective Pads', () => {
  ...
  it(`should prevent ability-changing abilities triggered by contact from acting`, () => {
    ...
    const mummyActivationMessages = battle.log.filter(
      logStr => logStr.startsWith('|-activate|') && logStr.includes('Mummy')
    );
    assert.equal(mummyActivationMessages.length, 1, `Mummy should activate only once`);
    assert(
      mummyActivationMessages[0].includes('Cofagrigus'),
      `Source of Mummy activation should be included`
    );
    assert.false(
      mummyActivationMessages[0].includes('Sturdy'),
      `Attacker's ability should not be revealed`
    );
  });
  ...
  it(`should not stop Pickpocket`, () => {
    ...
    assert.equal(
      battle.p2.active[0].item,
      'protectivepads',
      `Target should receive stolen Protective Pads`
    );
  });
  ...
  it(`should not start Perish Body on either Pokemon`, () => {
    ...
    battle.makeChoices();
    assert.false(
      battle.p1.active[0].volatiles['perishsong'],
      'Perish Body should not have activated on Wynaut due to Protective Pads.'
    );
    assert.false(
      battle.p2.active[0].volatiles['perishsong'],
      'Perish Body should not have activated on Cursola due to Protective Pads.'
    );
  ...
  it(`should not protect against Gulp Missile when using a contact move`, () => {
    ...
    assert.equal(
      wynaut.hp,
      wynaut.maxhp - Math.floor(wynaut.maxhp / 4),
      `Wynaut should be damaged by Gulp Missile, but not Rocky Helmet`
    );
