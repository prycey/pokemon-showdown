...
describe('Ability Shield', () => {
  ...
  it(`should protect the holder's ability against ability-changing moves`, () => {
    ...
    battle.makeChoices();
    assert(
      battle.log.some(line => line.includes('Ability Shield')),
      `Ability Shield should trigger a block message`
    );
  ...
  it(`should only protect the holder`, () => {
    ...
    battle.makeChoices();
    assert(
      battle.log.every(line => !line.includes('Ability Shield')),
      `Ability Shield should not trigger a block message`
    );
  ...
  it(`should protect the holder's ability against Neutralizing Gas`, () => {
    ...
    assert(
      battle.log.some(line => line.includes('Neutralizing Gas')),
      `Neutralizing Gas should trigger`
    );
    assert(
      battle.log.some(line => line.includes('Ability Shield')),
      `Ability Shield should trigger a block message`
      ...
      [
        {
          ...
          ability: 'levitate',
          item: 'abilityshield',
          moves: ['gastroacid', 'trick', 'shadowball'],
        },
      ],
    ]);
  ...
  it(`should not prevent forme changes from changing the holder's ability`, () => {
    ...
    assert.equal(
      battle.p1.active[0].ability,
      'embodyaspectteal',
      `Ogerpon's ability should change to Embody Aspect`
    );
  });
