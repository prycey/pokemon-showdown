...
describe('Rock Head', () => {
  ...
  it('should block recoil from most moves', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Aerodactyl', ability: 'rockhead', moves: ['doubleedge'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Registeel', ability: 'clearbody', moves: ['rest'] }],
    });
    assert.false.hurts(battle.p1.active[0], () =>
      battle.makeChoices('move doubleedge', 'move rest')
    );
  });
  ...
  it('should not block recoil from Struggle', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Aerodactyl', ability: 'rockhead', moves: ['roost'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Sableye', ability: 'prankster', moves: ['taunt'] }],
    });
  ...
  it('should not block crash damage', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Rampardos', ability: 'rockhead', moves: ['jumpkick'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Sableye', ability: 'prankster', moves: ['taunt'] }],
    });
