...
describe('Sturdy', () => {
  ...
  it('should give the user an immunity to OHKO moves', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Aron', level: 1, ability: 'sturdy', moves: ['sleeptalk'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Kyogre', ability: 'noguard', moves: ['sheercold'] }],
    });
    assert.false.hurts(battle.p1.active[0], () =>
      battle.makeChoices('move sleeptalk', 'move sheercold')
    );
  });
  ...
  it('should allow its user to survive an attack from full HP', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Paras', ability: 'sturdy', moves: ['sleeptalk'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Charizard', ability: 'drought', moves: ['fusionflare'] }],
    });
  ...
  it('should allow its user to survive a confusion damage hit from full HP', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Shedinja', ability: 'sturdy', moves: ['absorb'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Klefki', ability: 'prankster', moves: ['confuseray'] }],
    });
  ...
  it('should not trigger on recoil damage', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Shedinja', ability: 'sturdy', moves: ['doubleedge'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Klefki', ability: 'prankster', moves: ['reflect'] }],
    });
  ...
  it('should not trigger on residual damage', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Shedinja', ability: 'sturdy', moves: ['sleeptalk'] }],
    });
    battle.setPlayer('p2', {
  ...
  it('should be suppressed by Mold Breaker', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Reshiram', ability: 'turboblaze', moves: ['fusionflare'] }],
    });
    battle.makeChoices('move sleeptalk', 'move fusionflare');
    assert.fainted(battle.p1.active[0]);
  });
