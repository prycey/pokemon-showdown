...
describe('Wonder Guard', () => {
  ...
  it('should make the user immune to damaging attacks that are not super effective', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Aerodactyl', ability: 'wonderguard', moves: ['sleeptalk'] }],
    });
    battle.setPlayer('p2', {
      team: [
        {
          species: 'Smeargle',
          ability: 'owntempo',
          moves: ['knockoff', 'flamethrower', 'thousandarrows', 'moonblast'],
        },
      ],
    });
    for (let i = 1; i <= 4; i++) {
    ...
    assert.false.hurts(battle.p1.active[0], () =>
      battle.makeChoices('move sleeptalk', 'move thousandarrows')
    );
  });
  ...
  it('should not make the user immune to status moves', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Abra', ability: 'wonderguard', moves: ['teleport'] }],
    });
    battle.setPlayer('p2', {
      team: [
        {
          species: 'Smeargle',
          ability: 'noguard',
          moves: ['poisongas', 'screech', 'healpulse', 'gastroacid'],
        },
      ],
    });
    ...
    assert.hurtsBy(wwTarget, -Math.floor(wwTarget.maxhp / 8), () =>
      battle.makeChoices('move teleport', 'move healpulse')
    );
  ...
  it('should be suppressed by Mold Breaker', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Zekrom', ability: 'wonderguard', moves: ['sleeptalk'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Reshiram', ability: 'turboblaze', moves: ['fusionflare'] }],
    });
    assert.hurts(battle.p1.active[0], () =>
