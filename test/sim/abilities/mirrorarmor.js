...
describe('Mirror Armor', () => {
  ...
  it('should bounce boosts back to the source', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Corviknight', ability: 'mirrorarmor', moves: ['endure'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Machop', ability: 'noguard', moves: ['rocktomb', 'leer'] }],
    });
  ...
  it('should activate, but silently, if the source has fainted', () => {
    ...
    for (const line of battle.log) {
      assert.false(
        line.includes('Mirror Armor'),
        `Expected no Mirror Armor message, but got the following message:\n${line}`
      );
    }
  });
});
