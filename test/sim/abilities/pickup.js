...
describe('Pickup', () => {
  ...
  it('should pick up a consumed item', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Gourgeist', ability: 'pickup', moves: ['flamethrower'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Paras', ability: 'dryskin', item: 'sitrusberry', moves: ['endure'] }],
    });
  ...
  it('should pick up flung items', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Gourgeist', ability: 'pickup', moves: ['endure'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Clefairy', ability: 'unaware', item: 'airballoon', moves: ['fling'] }],
    });
  ...
  it('should not pick up an item that was knocked off', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Ambipom', ability: 'pickup', moves: ['knockoff'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Machamp', ability: 'noguard', item: 'choicescarf', moves: ['bulkup'] }],
    });
    battle.makeChoices('move knockoff', 'move bulkup');
    assert.false.holdsItem(
      battle.p1.active[0],
      'Pick Up should not retrieve knocked off Choice Scarf'
    );
  });
  ...
  it('should not pick up a popped Air Balloon', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Ambipom', ability: 'pickup', moves: ['fakeout'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Scizor', ability: 'swarm', item: 'airballoon', moves: ['roost'] }],
    });
  ...
  it('should not pick up items from Pokemon that have switched out and back in', () => {
    ...
    assert.false.holdsItem(
      battle.p1.active[0],
      'Pick Up should not retrieve Eject Button of returned Clefable'
    );
  });
  ...
  it('should not pick up items from Pokemon that have switched out', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Gourgeist', ability: 'pickup', moves: ['shadowsneak', 'synthesis'] }],
    });
    battle.setPlayer('p2', {
      team: [
  ...
  it('should not pick up items that were already retrieved', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Ambipom', ability: 'pickup', moves: ['return'] }],
    });
    battle.setPlayer('p2', {
      team: [
  ...
  it('should not pick up items from non-adjacent allies and enemies', () => {
    ...
    battle.makeChoices(
      'move protect, move curse, move extremespeed 2',
      'move flamecharge 2, move rest, move splash'
    );
    assert.false.holdsItem(battle.p1.active[0]);
  });
});
