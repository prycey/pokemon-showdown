...
describe('Simple', () => {
  ...
  it('should double all stat boosts', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Gyarados', ability: 'moxie', moves: ['splash'] }],
    });
...
describe('Simple [Gen 4]', () => {
  ...
  it('should double the effect of stat boosts', () => {
    ...
    assert.sets(
      () => target.getStat('def'),
      2 * target.getStat('def'),
      () => battle.makeChoices('move defensecurl', 'move splash')
    );
  ...
  it('should double the effect of stat boosts passed by Baton Pass', () => {
    ...
    assert.equal(
      battle.p1.active[0].getStat('atk'),
      Math.floor(0.5 * battle.p1.active[0].getStat('atk', true))
    );
  });
