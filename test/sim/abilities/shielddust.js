...
describe('Shield Dust', () => {
  ...
  it('should not block secondary effects that affect the user of the move', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Ledian', ability: 'ironfist', moves: ['poweruppunch'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Dustox', ability: 'shielddust', moves: ['roost'] }],
    });
  ...
  it('should block added effects from Fling', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Ledian', ability: 'ironfist', item: 'petayaberry', moves: ['fling'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Dustox', ability: 'shielddust', moves: ['roost'] }],
    });
  ...
  it('should not block secondary effects on attacks used by the Pokemon with the ability', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Ledian', ability: 'shielddust', moves: ['poweruppunch', 'strugglebug'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Clefable', ability: 'unaware', moves: ['softboiled'] }],
    });
  ...
  it('should be negated by Mold Breaker', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Pinsir', ability: 'moldbreaker', moves: ['strugglebug'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Dustox', ability: 'shielddust', moves: ['roost'] }],
    });
  ...
  it.skip(`should only prevent Sparkling Aria from curing burn if there is only one target`, () => {
    ...
    assert.equal(
      battle.p2.active[0].status,
      'brn',
      `Shield Dust should prevent cured burn if it's the only target`
    );
    ...
    assert.equal(
      battle.p2.active[0].status,
      '',
      `Shield Dust should not prevent cured burn if it's one of many targets`
    );
  });
});
