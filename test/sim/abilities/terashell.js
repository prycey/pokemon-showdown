...
describe('Tera Shell', () => {
  ...
  it(`should take not very effective damage when it is at full health`, () => {
    ...
    assert.bounded(
      damage,
      [14, 16],
      `Tera Shell should yield not very effective damage roll, actual damage taken is ${damage}`
    );

    battle.makeChoices();
    assert.bounded(
      terapagos.maxhp - terapagos.hp - damage,
      [28, 33],
      `Tera Shell should not reduce damage, because Terapagos-Terastal was not at full HP`
    );
  });
  ...
  it('All hits of multi-hit move should be not very effective', () => {
    ...
    assert.bounded(
      damage,
      [15, 18],
      `Tera Shell should yield not very effective damage for both all hits of multi-hit move, actual damage taken is ${damage}`
    );
  });
  ...
  it(`should be suppressed by Gastro Acid`, () => {
    ...
    assert.bounded(
      damage,
      [28, 33],
      `Tera Shell should not reduce damage, because Tera Shell should be suppressed`
    );
  });
  ...
  it(`should not work if the user's species is not currently Terapagos-Terastal`, () => {
    ...
    assert.bounded(
      damage,
      [51, 60],
      `Tera Shell should not have activated because current species is not Terapagos`
    );
    ...
    assert.bounded(
      damage,
      [33, 39],
      `Tera Shell should have activated because current species is Terapagos`
    );
  });
  ...
  it(`should not weaken the damage from Struggle`, () => {
    ...
    assert.bounded(
      damage,
      [27, 32],
      `Tera Shell should not have reduced the damage Struggle dealt`
    );
  });
  ...
  it(`should not continue to weaken attacks after taking damage from a Future attack`, () => {
    ...
    assert.bounded(
      damage,
      [59, 70],
      `Tera Shell should have reduced the damage Future Sight dealt`
    );
    ...
    assert.bounded(
      damage,
      [59, 70],
      `Tera Shell should not have reduced the damage Wicked Blow dealt`
    );
  });
  ...
  it.skip(`should activate, but not weaken, moves with fixed damage`, () => {
    ...
    assert(
      battle.log[battle.lastMoveLine + 1].endsWith('Tera Shell'),
      `Tera Shell should have activated on Seismic Toss`
    );
    ...
    assert.equal(
      damage,
      Math.floor(terapagos.maxhp / 2),
      `Tera Shell should not have reduced the damage Super Fang dealt`
    );
    assert(
      battle.log[battle.lastMoveLine + 1].endsWith('Tera Shell'),
      `Tera Shell should have activated on Super Fang`
    );
    ...
    assert(
      battle.log[battle.lastMoveLine + 1].endsWith('Tera Shell'),
      `Tera Shell should have activated on Counter`
    );
    ...
    assert.equal(
      damage,
      shuckle.maxhp,
      `Tera Shell should not have reduced the damage Final Gambit dealt`
    );
    assert(
      battle.log[battle.lastMoveLine + 1].endsWith('Tera Shell'),
      `Tera Shell should have activated on Final Gambit`
    );
    ...
    assert(
      battle.log[battle.lastMoveLine + 1].endsWith('Tera Shell'),
      `Tera Shell should have activated on Sheer Cold`
    );
  });
});
