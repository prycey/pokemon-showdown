...
describe(`Emergency Exit`, () => {
      ...
      [
        {
          ...
          ability: 'ironbarbs',
          moves: ['stealthrock', 'spikes', 'dragonascent'],
        },
      ],
    ]);
  ...
  it.skip('should request switch-out after taking recoil and dragging in an opponent', () => {
    ...
    assert(
      dragIndex < abilityIndex,
      'Stufful should be dragged in before Emergency Exit activates'
    );
    assert.equal(battle.requestState, 'switch');
  });
      ...
      [
        {
          species: 'Ferrothorn',
          ability: 'ironbarbs',
          moves: ['stealthrock', 'spikes', 'protect'],
        },
      ],
    ]);
    battle.makeChoices('move uturn', 'move stealthrock');
  ...
  it.skip(`should request switchout between hazards`, () => {
    ...
    assert.equal(
      volcarona.hp,
      Math.floor(volcarona.maxhp / 2),
      'Emergency Exit should trigger before Spikes damage.'
    );
    assert.equal(battle.requestState, 'switch');
  });
  ...
  it.skip(`should request switchout between residual damage`, () => {
    ...
    assert.equal(
      amoonguss.hp,
      expectedHP,
      `Amoonguss should have taken damage before Golisopod can be replaced.`
    );
