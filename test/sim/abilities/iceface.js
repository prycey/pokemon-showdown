...
describe('Ice Face', () => {
  ...
  it(`should not work while Transformed`, () => {
    ...
    assert.species(
      transformedEiscue,
      'Eiscue',
      `Transformed Eiscue should not have changed to Eiscue-Noice after taking physical damage`
    );
    ...
    assert.species(
      transformedEiscue,
      'Eiscue-Noice',
      `Transformed Eiscue should not have changed to Eiscue after hail was set`
    );
  });
  ...
  it(`should not trigger if the Pokemon was KOed by Max Hailstorm`, () => {
    ...
    const hasMultipleActivates =
      firstIndex !== -1 && firstIndex !== battleLog.lastIndexOf(activate);
    assert.false(
      hasMultipleActivates,
