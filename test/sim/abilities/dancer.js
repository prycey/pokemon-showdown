...
describe('Dancer', () => {
  ...
  it(`should not copy Teeter Dance when all targets are confused`, () => {
    ...
    assert.holdsItem(
      wynaut,
      `Persim Berry should not be consumed, because Dancer did not activate`
    );
  ...
  it(`should not copy a Dance move that failed for other reasons`, () => {
    ...
    battle.makeChoices(
      'move dragondance, move featherdance 1',
      'move revelationdance -2, move finalgambit 1'
    );
    assert.fullHP(
      oricorio,
      `Nothing should target Oricorio because Revelation Dance failed from Wonder Guard`
    );
    assert.statStage(
      wynaut,
      'atk',
      0,
      `Wynaut's attack should not have changed from either Feather Dance or Dragon Dance, because both failed`
    );
  });
  ...
  it('should target the user of a Dance move unless it was an ally attacking an opponent', () => {
      ...
      [
        {
          ...
          ability: 'dancer',
          item: 'laggingtail',
          moves: ['sleeptalk', 'protect', 'teeterdance'],
        },
        {
          species: 'Oricorio',
          level: 99,
          ability: 'heatproof',
          moves: ['fierydance', 'sleeptalk'],
        },
      ],
      [
    ...
    assert.hurts(opponentTargetingAlly, () =>
      battle.makeChoices('move sleeptalk, move sleeptalk', 'move fierydance 2, move sleeptalk')
    );

    const opponentTargetingOpponent = battle.p2.active[0];
    assert.hurts(opponentTargetingOpponent, () =>
      battle.makeChoices('move sleeptalk, move sleeptalk', 'move fierydance -2, move sleeptalk')
    );

    const allyTargetingDancer = battle.p1.active[1];
    assert.hurts(allyTargetingDancer, () =>
    ...
    assert.hurts(opponentTargetedByAlly, () =>
      battle.makeChoices('move sleeptalk, move fierydance 2', 'move sleeptalk, move sleeptalk')
    );
