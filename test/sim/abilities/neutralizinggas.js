...
describe('Neutralizing Gas', () => {
  ...
  it('should prevent switch-in abilities from activating', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Gyarados', ability: 'intimidate', moves: ['splash'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Weezing', ability: 'neutralizinggas', moves: ['toxicspikes'] }],
    });
  ...
  it('should negate self-healing abilities', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Weezing', ability: 'neutralizinggas', moves: ['toxic'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Breloom', ability: 'poisonheal', moves: ['swordsdance'] }],
    });
  ...
  it('should negate abilities that suppress item effects', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Weezing', ability: 'neutralizinggas', moves: ['reflect'] }],
    });
    battle.setPlayer('p2', {
      team: [
  ...
  it('should negate abilities that modify boosts', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Weezing', ability: 'neutralizinggas', moves: ['spore'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Shuckle', ability: 'contrary', moves: ['sleeptalk', 'superpower'] }],
    });
  ...
  it('should negate abilities that modify move type', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Gengar', ability: 'neutralizinggas', moves: ['laserfocus'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Sylveon', ability: 'pixilate', moves: ['hypervoice'] }],
    });
  ...
  it('should negate abilities that damage the attacker', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Weezing-Galar', ability: 'neutralizinggas', moves: ['payback'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Ferrothorn', ability: 'ironbarbs', moves: ['rockpolish'] }],
    });
    battle.makeChoices();
    assert.fullHP(battle.p1.active[0]);
  });
  ...
  it(`should negate Primal weather Abilities`, () => {
    ...
    assert.false(
      battle.field.isWeather('desolateland'),
      `Desolate Land should be negated, turning off the weather`
    );
    battle.makeChoices('auto', 'switch 2');
  ...
  it('should not activate Imposter if Neutralizing Gas leaves the field', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Ditto', ability: 'imposter', moves: ['sleeptalk'] }],
    });
  ...
  it(`should not reactivate instances of Embody Aspect that had previously activated`, () => {
    ...
    assert.statStage(
      ...
      'atk',
      4,
      `Ogerpon-Hearthflame-Tera's Embody Aspect should not have been reactivated`
    );
    assert.statStage(
      wellspring,
      'spd',
      1,
      `Ogerpon-Wellspring-Tera's Embody Aspect should not have activated twice`
    );
    assert.statStage(
      cornerstone,
      'def',
      1,
      `Ogerpon-Cornerstone-Tera's Embody Aspect should have been activated by Neutalizing Gas ending`
    );
  });
