...
describe('Frisk', () => {
  ...
  it(`should reveal opposing Pokemon's items`, () => {
    ...
    assert(
      log.indexOf('Spectrier|Choice Specs') > -1,
      "Frisk should have revealed Spectrier's Choice Specs"
    );
    assert(
      log.indexOf('Glastrier|Choice Band') > -1,
      "Frisk should have revealed Glastrier's Choice Band"
    );
  });
  ...
  it(`should not reveal opposing fainted Pokemon's items`, () => {
    ...
    battle.makeChoices();
    assert.false(
      battle.log.find(line => line.startsWith('|-item|')),
