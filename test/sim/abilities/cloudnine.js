...
describe('Cloud Nine', () => {
  ...
  it('should treat the weather as none for the purposes of formes, moves and abilities', () => {
    ...
    assert.false.hurts(weatherSuppressor, () =>
      battle.makeChoices('move sunnyday', 'move solarbeam')
    ); // Solar Beam must charge
  ...
  it('should negate the damage-dealing effects of Sandstorm', () => {
    ...
    assert.false.hurts(battle.p2.active[0], () =>
      battle.makeChoices('move dragondance', 'move calmmind')
    );
  });
  ...
  it("should not negate Desolate Land's ability to prevent other weathers from activating", () => {
    ...
    assert.constant(
      () => battle.weather,
      () => battle.makeChoices('move raindance', 'move sunnyday')
    );
  });
