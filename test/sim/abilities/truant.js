...
describe('Truant', () => {
  ...
  it('should prevent the user from acting the turn after using a move', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Slaking', ability: 'truant', moves: ['scratch'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Steelix', ability: 'sturdy', moves: ['endure'] }],
    });
  ...
  it('should not allow the user to act the turn it wakes up, if it moved the turn it fell asleep', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Slaking', ability: 'truant', moves: ['scratch', 'rest'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Steelix', ability: 'sturdy', moves: ['endure', 'quickattack'] }],
    });
    const pokemon = battle.p2.active[0];
  ...
  it('should allow the user to act the turn it wakes up, if it was loafing the turn it fell asleep', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Slaking', ability: 'truant', moves: ['scratch', 'irondefense'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Steelix', ability: 'sturdy', moves: ['endure', 'spore'] }],
    });
  ...
  it('should cause two-turn moves to fail', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Slaking', ability: 'truant', moves: ['razorwind'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Steelix', ability: 'sturdy', moves: ['endure'] }],
    });
  ...
  it('should prevent a newly-Mega Evolved Pokemon from acting if given the ability', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Steelix', ability: 'sturdy', item: 'steelixite', moves: ['heavyslam'] }],
    });

    assert.false.hurts(battle.p1.active[0], () =>
      battle.makeChoices('move entrainment', 'move heavyslam mega')
    );
  });
});
