...
describe('Comatose', () => {
  ...
  it('should make the user immune to status conditions', () => {
      ...
      [
        {
          species: 'Smeargle',
          ability: 'noguard',
          moves: ['spore', 'glare', 'willowisp', 'toxic'],
        },
      ],
    ]);
    ...
    for (let i = 1; i <= 4; i++) {
      assert.constant(
        () => comatoseMon.status,
        () => battle.makeChoices('move shadowclaw', 'move ' + i)
      );
    }
  });
  ...
  it('should cause Rest to fail', () => {
    ...
    assert.constant(
      () => comatoseMon.status,
      () => battle.makeChoices('move rest', 'move aquajet')
    );
  });
  ...
  it('should allow the use of Snore and Sleep Talk as if the user were asleep', () => {
      ...
      [
        {
          ...
          item: 'normaliumz',
          ability: 'comatose',
          moves: ['snore', 'sleeptalk', 'brickbreak'],
        },
      ],
    ...
    assert.hurts(
      defender,
      () => battle.makeChoices('move snore', 'move endure'),
  ...
  it('should cause the user to be damaged by Dream Eater as if it were asleep', () => {
    ...
    assert.hurts(battle.p1.active[0], () =>
      battle.makeChoices('move shadowclaw', 'move dreameater')
    );
  });
