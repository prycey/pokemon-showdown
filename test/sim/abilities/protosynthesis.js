...
describe('Protosynthesis', () => {
  ...
  it(`should boost the user's highest stat except HP while Sunny Day is active`, () => {
    ...
    battle.makeChoices();
    assert(
      !tail.volatiles['protosynthesis'],
      `Scream Tail's SpD should stop being boosted when Sun ends`
    );
  });
  ...
  it(`should be activated by Booster Energy when Sunny Day is not active`, () => {
    ...
    const tail = battle.p1.active[0];
    assert.equal(
      tail.volatiles['protosynthesis'].bestStat,
      'spd',
      `Scream Tail's SpD should have been boosted by Protosynthesis in Sun`
    );
    assert.equal(
      tail.volatiles['protosynthesis'].fromBooster,
      undefined,
      `Scream Tail's Protosynthesis should not have been activated by Booster Energy in Sun`
    );
    // change to rain
    battle.makeChoices();
    assert(
      tail.volatiles['protosynthesis'].fromBooster,
      `Scream Tail's Protosynthesis should have been activated by Booster Energy in Rain`
    );
    ...
    assert(
      !!tail.volatiles['protosynthesis'],
      `Scream Tail's Protosynthesis activated by Booster Energy should still be active when Sun ends`
    );
  });
  ...
  it(`should not be prevented from activating if the user holds Utility Umbrella`, () => {
    ...
    assert.equal(
      tail.volatiles['protosynthesis'].bestStat,
      'spd',
      `Scream Tail's SpD should have been boosted by Protosynthesis in Sun while holding Utility Umbrella`
    );
  });
  ...
  it(`should be deactiviated by weather suppressing abilities`, () => {
    ...
    assert(
      !tail.volatiles['protosynthesis'],
      `Scream Tail should not have remained boosted by Protosynthesis because a weather supressing ability started even though Sun was active`
    );
  });
  ...
  it(`should not activate if weather is suppressed`, () => {
    ...
    assert.equal(
      tail.volatiles['protosynthesis'],
      undefined,
      `Scream Tail should not have been boosted by Protosynthesis because a weather supressing ability was active when Sun started`
    );
  });
  ...
  it(`should activate when weather supression ends`, () => {
    ...
    assert.equal(
      tail.volatiles['protosynthesis'].bestStat,
      'spd',
      `Scream Tail should have been boosted by Protosynthesis because a weather supressing ability ended while Sun was active`
    );
  });
