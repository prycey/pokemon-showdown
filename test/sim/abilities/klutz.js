...
describe('Klutz', () => {
  ...
  it('should negate residual healing events', () => {
    ...
    assert.hurtsBy(
      klutzMon,
      Math.floor(klutzMon.maxhp / 2),
      () => battle.makeChoices('move bellydrum', 'move shadowsneak'),
  ...
  it('should prevent items from being consumed', () => {
    ...
    assert.constant(
      () => klutzMon.item,
      () => battle.makeChoices('move endure', 'move psychic')
    );
  ...
  it('should not ignore item effects that prevent item removal', () => {
    ...
    assert.constant(
      () => klutzMon.item,
      () => battle.makeChoices('move calmmind', 'move trick')
    );
  });
