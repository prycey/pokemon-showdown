...
describe('Costar', () => {
  ...
  it("should copy the teammate's crit ratio on activation", () => {
    ...
    assert(
      !flamigo.volatiles['focusenergy'],
      'Costar should copy having no volatile crit modifiers when re-activated.'
    );
  });
  ...
  it('should copy both positive and negative stat changes', () => {
    ...
    assert.statStage(
      ...
      'atk',
      2,
      "A pokemon should copy the target's positive stat changes (atk) when switching in with Costar."
    );
    assert.statStage(
      flamigo,
      'spa',
      2,
      "A pokemon should copy the target's positive stat changes (spa) when switching in with Costar."
    );
    assert.statStage(
      flamigo,
      'spe',
      2,
      "A pokemon should copy the target's positive stat changes (spe) when switching in with Costar."
    );
    assert.statStage(
      flamigo,
      'def',
      -1,
      "A pokemon should copy the target's negative stat changes (def) when switching in with Costar."
    );
    assert.statStage(
      flamigo,
      'spd',
      -1,
      "A pokemon should copy the target's negative stat changes (spd) when switching in with Costar."
    );
  });
