...
describe('Normalize', () => {
  ...
  it("should change most of the user's moves to Normal-type", () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Delcatty', ability: 'normalize', moves: ['grassknot'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Latias', ability: 'colorchange', moves: ['endure'] }],
    });
  ...
  it('should not change Hidden Power to Normal-type', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Delcatty', ability: 'normalize', moves: ['hiddenpowerfighting'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Latias', ability: 'colorchange', moves: ['endure'] }],
    });
  ...
  it('should not change Judgment to Normal-type if the user is holding a Plate', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Delcatty', ability: 'normalize', item: 'zapplate', moves: ['judgment'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Latias', ability: 'colorchange', moves: ['endure'] }],
    });
  ...
  it('should not change Weather Ball to Normal-type if sun, rain, or hail is an active weather', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Latias', ability: 'colorchange', moves: ['sunnyday'] }],
    });
