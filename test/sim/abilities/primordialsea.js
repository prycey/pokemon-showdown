...
describe('Primordial Sea', () => {
  ...
  it('should activate the Primordial Sea weather upon switch-in', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Kyogre', ability: 'primordialsea', moves: ['helpinghand'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Abra', ability: 'magicguard', moves: ['teleport'] }],
    });
  ...
  it('should increase the damage (not the basePower) of Water-type attacks', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Kyogre', ability: 'primordialsea', moves: ['waterpledge'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Blastoise', ability: 'torrent', moves: ['splash'] }],
    });
  ...
  it('should cause Fire-type attacks to fail', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Charizard', ability: 'blaze', moves: ['flamethrower'] }],
    });
  ...
  it('should not cause Fire-type Status moves to fail', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Charizard', ability: 'noguard', moves: ['willowisp'] }],
    });
    assert.sets(
      () => battle.p1.active[0].status,
      'brn',
      () => battle.makeChoices('move helpinghand', 'move willowisp')
    );
  });
  ...
  it('should be treated as Rain Dance for any forme, move or ability that requires it', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Kyogre', ability: 'primordialsea', moves: ['sonicboom'] }],
    });
    battle.setPlayer('p2', {
      team: [
    ...
    assert.equal(
      myActive[0].getStat('spe'),
      2 * myActive[0].storedStats['spe'],
      "Kingdra's Speed should be doubled by Swift Swim"
    );
  ...
  it('should cause the Primordial Sea weather to fade if it switches out and no other Primordial Sea Pokemon are active', () => {
    ...
    assert.sets(
      () => battle.field.isWeather('primordialsea'),
      false,
  ...
  it('should not cause the Primordial Sea weather to fade if it switches out and another Primordial Sea Pokemon is active', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Kyogre', ability: 'primordialsea', moves: ['bulkup'] }],
    });
    assert.constant(
      () => battle.field.isWeather('primordialsea'),
  ...
  it('should cause the Primordial Sea weather to fade if its ability is suppressed and no other Primordial Sea Pokemon are active', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Lugia', ability: 'pressure', moves: ['gastroacid'] }],
    });
    assert.sets(
      () => battle.field.isWeather('primordialsea'),
      false,
  ...
  it('should not cause the Primordial Sea weather to fade if its ability is suppressed and another Primordial Sea Pokemon is active', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Kyogre', ability: 'primordialsea', moves: ['gastroacid'] }],
    });
    assert.constant(
      () => battle.field.isWeather('primordialsea'),
      () => battle.makeChoices('move helpinghand', 'move gastroacid')
  ...
  it('should cause the Primordial Sea weather to fade if its ability is changed and no other Primordial Sea Pokemon are active', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Lugia', ability: 'pressure', moves: ['entrainment'] }],
    });
    assert.sets(
      () => battle.field.isWeather('primordialsea'),
      false,
