...
describe('Protean', () => {
  ...
  it(`should change the user's type for submoves to the type of that submove, not the move calling it`, () => {
    ...
    assert(
      battle.log.every(line => !line.includes('|Normal|')),
      `It should not temporarily become Normal-type`
    );
  ...
  it(`should not change the user's type when using moves that fail earlier than Protean will activate`, () => {
      ...
      [
        {
          species: 'Kecleon',
          ability: 'protean',
          moves: ['fling', 'suckerpunch', 'steelroller', 'aurawheel'],
        },
        { species: 'Kecleon', ability: 'protean', moves: ['counter', 'metalburst'] },
        {
          species: 'Kecleon',
          ability: 'protean',
          moves: ['magnetrise', 'ingrain', 'burnup', 'auroraveil'],
        },
      ],
    ...
    assert(
      kecleon.hasType('Normal'),
      `Protean changed typing when Sucker Punch was used into a status move.`
    );

    battle.makeChoices('move steelroller', 'auto');
    assert(
      kecleon.hasType('Normal'),
      `Protean changed typing when Steel Roller was used with no Terrain active.`
    );

    battle.makeChoices('move aurawheel', 'auto');
    assert(
      kecleon.hasType('Normal'),
      `Protean changed typing when Aura Wheel was used by non-Morpeko.`
    );
    ...
    assert(
      kecleon.hasType('Normal'),
      `Protean changed typing when Counter failed to return damage.`
    );

    battle.makeChoices('move metalburst', 'auto');
    assert(
      kecleon.hasType('Normal'),
      `Protean changed typing when Metal Burst failed to return damage.`
    );
    ...
    assert(
      kecleon.hasType('Grass'),
      `Protean changed typing when Magnet Rise was used while the effect of Ingrain was active.`
    );
  ...
  it(`should not change the user's type when abilities that activate earlier than Protean will cause the user's moves to fail`, () => {
    ...
    assert(
      kecleon.hasType('Normal'),
      `Protean changed typing when a Fire-type attack was used in Primordial Sea.`
    );

    battle.makeChoices('move aquajet', 'switch 2');
    assert(
      kecleon.hasType('Normal'),
      `Protean changed typing when a Water-type attack was used in Desolate Land.`
    );

    battle.makeChoices('move mindblown', 'move powder');
    assert(
      kecleon.hasType('Normal'),
      `Protean changed typing when a Fire-type attack was used while the user was affected by Powder.`
    );

    battle.makeChoices('move aquajet', 'switch 3');
    assert(
      kecleon.hasType('Normal'),
      `Protean changed typing when a priority move was blocked by Dazzling.`
    );

    battle.makeChoices('move mindblown', 'switch 4');
    assert(
      kecleon.hasType('Normal'),
      `Protean changed typing when a exploding move was blocked by Damp.`
    );
