...
describe('Unburden', () => {
  ...
  it('should trigger when an item is consumed', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Scizor', ability: 'swarm', item: 'focussash', moves: ['swordsdance'] }],
    });
    const target = battle.p1.active[0];
    assert.sets(
      () => target.getStat('spe'),
      2 * target.getStat('spe'),
      () => battle.makeChoices('move closecombat', 'move swordsdance')
    );
  });
  ...
  it('should trigger when an item is destroyed', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Drifblim', ability: 'unburden', item: 'airballoon', moves: ['endure'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Machamp', ability: 'noguard', moves: ['stoneedge'] }],
    });
    const target = battle.p1.active[0];
    assert.sets(
      () => target.getStat('spe'),
      2 * target.getStat('spe'),
      () => battle.makeChoices('move endure', 'move stoneedge')
    );
  });
  ...
  it('should trigger when Natural Gift consumes a berry', () => {
    ...
    assert.sets(
      () => target.getStat('spe'),
      2 * target.getStat('spe'),
      () => battle.makeChoices('move naturalgift', 'move swordsdance')
    );
  });
  ...
  it('should trigger when an item is flung', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Sceptile', ability: 'unburden', item: 'whiteherb', moves: ['fling'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Scizor', ability: 'swarm', item: 'focussash', moves: ['swordsdance'] }],
    });
    ...
    assert.sets(
      () => target.getStat('spe'),
      2 * target.getStat('spe'),
      () => battle.makeChoices('move fling', 'move swordsdance')
    );
  });
  ...
  it('should trigger when an item is forcefully removed', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Sceptile', ability: 'unburden', item: 'whiteherb', moves: ['leechseed'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Scizor', ability: 'swarm', moves: ['knockoff'] }],
    });
    const target = battle.p1.active[0];
    assert.sets(
      () => target.getStat('spe'),
      2 * target.getStat('spe'),
      () => battle.makeChoices('move leechseed', 'move knockoff')
    );
  });
  ...
  it('should not be suppressed by Mold Breaker', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Scizor', ability: 'moldbreaker', moves: ['knockoff'] }],
    });
    const target = battle.p1.active[0];
    assert.sets(
      () => target.getStat('spe'),
