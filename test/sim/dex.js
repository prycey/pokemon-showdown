  ...
  it('should work fine in any order', () => {
    {
      const Dex = require('./../../dist/sim/dex').Dex;
      assert.equal(
        Dex.mod('gen2').species.getLearnsetData('nidoking').learnset.bubblebeam.join(','),
        '1M'
      );
...
describe('Dex#getSpecies', () => {
  ...
  it('should handle Pikachu forme numbering', () => {
    assert.deepEqual(Dex.forGen(6).species.get('Pikachu').formeOrder.slice(0, 7), [
      'Pikachu',
      'Pikachu-Rock-Star',
      'Pikachu-Belle',
      'Pikachu-Pop-Star',
    ...
    assert.deepEqual(Dex.forGen(7).species.get('Pikachu').formeOrder.slice(0, 9), [
      'Pikachu',
      'Pikachu-Original',
      'Pikachu-Hoenn',
      'Pikachu-Sinnoh',
