...
describe('Target Resolution', () => {
  ...
  describe(`Targetted Pokémon fainted in-turn`, () => {
    ...
    it(`should support RedirectTarget event for a fainted foe and type 'any' `, () => {
      ...
      battle.makeChoices(
        'move watergun 3, auto',
        'move swordsdance, move swordsdance, move finalgambit -2'
      );
      assert.statStage(redirector, 'spa', 1);
    });
    ...
    it(`should not redirect non-pulse/flying moves in Triples if the Pokemon is out of range`, () => {
      ...
      assert.fullHP(
        battle.p2.active[0],
        `Beartic should not be damaged by a Water Gun because it is out of range`
      );
    });
  ...
  describe(`Smart-tracking targeting effects`, () => {
    ...
    it(`should allow Stalwart to correctly target a Pokemon which switched out and back in another slot`, () => {
      ...
      assert.false.fullHP(
        regieleki,
        `Duraludon should have followed Regieleki through its switch-out.`
      );
    });
  ...
  it(`charge moves like Phantom Force should target slots turn 1 and Pokemon turn 2`, () => {
    ...
    assert.fullHP(
      battle.p1.active[1],
      'Altaria should be at full HP, because it was not targeted.'
    );
    assert.false.fullHP(
      battle.p1.active[0],
      'Aggron should not be at full HP, because it was targeted.'
    );
    ...
    assert.false.fullHP(
      battle.p1.active[1],
      'Altaria should not be at full HP, because Phantom Force was redirected and targeted it.'
    );
  });
