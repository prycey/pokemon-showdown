...
describe('Partners in Crime', () => {
  ...
  it('should activate shared abilities at the same time as other abilities', () => {
    ...
    assert.equal(
      rampardos.volatiles.quarkdrive.bestStat,
      'atk',
    ...
    assert.equal(
      ironvaliant.volatiles.quarkdrive.bestStat,
      'spa',
      `Iron Valiant should be Intimidated exactly once before Quark Drive activates`
    );
    assert.equal(
      battle.field.terrainState.source.name,
      'Incineroar',
      `Incineroar should set Electric Terrain`
    );
    ...
    assert.statStage(
      ...
      'atk',
      0,
      `Incineroar's White Herb should have activated after both Litten and Rampardos's Intimidates`
    );
  });
  ...
  it('should activate shared abilities for each ally when only the original holder switches in', () => {
    ...
    assert.statStage(
      ...
      'atk',
      -2,
      'Pincurchin should have had its innate Intimidate activate, triggering Mirror Armor'
    );
  });
  ...
  it("should not activate ally's innates if the partner faints on switch-in", () => {
    ...
    assert.statStage(
      ...
      'spa',
      1,
      'Cresselia should not have gained another Download boost after Shedinja fainted'
    );
  });
});
