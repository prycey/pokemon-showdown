...
describe('Dynamax', () => {
  ...
  it('Max Move effects should not be suppressed by Sheer Force', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Shedinja', ability: 'sturdy', item: 'ringtarget', moves: ['splash'] }],
    });
  ...
  describe(`Hacked Max Moves`, () => {
    ...
    it(`should treat Max Moves as 0 BP when used without Dynamaxing`, () => {
      ...
      assert.bounded(
        shuckle.maxhp - shuckle.hp,
        [2, 4],
        `0 BP should cause the move's damage to only be 2 after base damage calculation, resulting in 1-2 final damage for each Max Move.`
      );
    });
    ...
    it(`should prevent effects that affect regular Max Moves, like Sleep Talk and Instruct`, () => {
      ...
      assert.fullHP(
        shuckle,
        `Sleep Talk should have failed in calling a move and so not dealt damage.`
      );
    });
  });
});
