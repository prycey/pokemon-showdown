...
describe('Weather damage calculation', () => {
  ...
  it('should multiply the damage (not the basePower) in favorable weather', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Ninetales', ability: 'drought', moves: ['incinerate'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Cryogonal', ability: 'levitate', moves: ['splash'] }],
    });
  ...
  it('should reduce the damage (not the basePower) in unfavorable weather', () => {
    ...
    battle.randomizer = dmg => dmg; // max damage
    battle.setPlayer('p1', {
      team: [{ species: 'Ninetales', ability: 'drizzle', moves: ['incinerate'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Cryogonal', ability: 'levitate', moves: ['splash'] }],
    });
  ...
  it('should make Hail/Sandstorm damage some pokemon but not others', () => {
    ...
    battle.randomizer = dmg => dmg; // max damage
    battle.setPlayer('p1', {
      team: [{ species: 'Abomasnow', ability: 'snowwarning', moves: ['protect'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Sandslash', ability: 'sandveil', moves: ['protect'] }],
    });
  ...
  it(`should wear off before future attacks`, () => {
    ...
    assert(
      sandDamageIndex < futureDamageIndex,
      `Sandstorm should have dealt damage before Doom Desire`
    );
  });
  ...
  it(`should run residual weather effects in order of Speed`, () => {
    ...
    assert(
      drySkinIndex < solarPowerIndex,
      `Charizard should be damaged before Sunkern, because it is faster`
    );
  });
});
