...
describe('Hazards', () => {
      ...
      [
        {
          species: 'wynaut',
          item: 'laggingtail',
          moves: ['stealthrock', 'spikes', 'stickyweb', 'defog'],
        },
      ],
    ]);
  ...
  it(`should apply hazards in the order they were set up`, () => {
    ...
    assert(
      tSpikeIndex < websIndex,
      'Toxic Spikes should have poisoned before Sticky Web lowered speed.'
    );
  ...
  it(`should allow Berries to trigger between hazards`, () => {
    ...
    assert.false.holdsItem(
      shedinja,
      'Shedinja should have lost Lum Berry before fainting to rocks.'
    );
  });
  ...
  it(`should set up hazards to every opponents' side in a Free-for-all battle`, () => {
    ...
    battle.makeChoices();
    assert.deepEqual(
      battle.sides.map(side => !!side.sideConditions.stealthrock),
