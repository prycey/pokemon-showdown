...
describe('Terastallization', () => {
        ...
        [
          {
            ...
            ability: 'disguise',
            item: 'laggingtail',
            moves: ['shadowclaw', 'waterfall', 'sleeptalk'],
            teraType: 'Water',
          },
        ],
  ...
  it("should give STAB correctly to the user's underlying types after changing forme", () => {
    ...
    assert.bounded(
      damage,
      [48, 57],
      'Changing underlying type via forme change while terastallized did not change STAB; actual damage: ' +
        damage
    );
  ...
  it(`should allow hacked Megas to Terastallize in Hackmons play`, () => {
    battle = common
      .gen(9)
      .createBattle({ formatid: 'gen9purehackmons@@@!teampreview' }, [
    ...
    assert(
      mewtwo.hasType('Fighting'),
      'Mega Mewtwo X should have Fighting-type before Terastallization'
    );
    assert(
      necrozma.hasType('Dragon'),
      'Ultra Necrozma should have Dragon-type before Terastallization'
    );
    ...
    assert(
      necrozma.hasType('Normal'),
      'Ultra Necrozma should be Normal-type after Terastallization'
    );
  });
});
