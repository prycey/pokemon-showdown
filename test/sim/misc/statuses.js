...
describe('Burn', () => {
  ...
  it('should inflict 1/16 of max HP at the end of the turn, rounded down', () => {
    ...
    assert.hurtsBy(target, Math.floor(target.maxhp / 16), () =>
      battle.makeChoices('move bulkup', 'move willowisp')
    );
  });
  ...
  it('should not halve damage from moves with set damage', () => {
    ...
    assert.hurtsBy(battle.p2.active[0], 100, () =>
      battle.makeChoices('move seismictoss', 'move willowisp')
    );
  });
});
...
describe('Paralysis', () => {
  ...
  it('should reduce speed to 25% of its original value in Gen 6', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Vaporeon', ability: 'waterabsorb', moves: ['aquaring'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Jolteon', ability: 'voltabsorb', moves: ['thunderwave'] }],
    });
...
describe('Burn [Gen 6]', () => {
  ...
  it('should inflict 1/8 of max HP at the end of the turn, rounded down', () => {
    ...
    const target = battle.p1.active[0];
    assert.hurtsBy(target, Math.floor(target.maxhp / 8), () =>
      battle.makeChoices('move bulkup', 'move willowisp')
    );
  });
});
...
describe('Toxic Poison [Gen 2]', () => {
  ...
  it(`should pass the damage counter to Pokemon with Baton Pass`, () => {
    ...
    assert.equal(
      hp - chansey.hp,
      Math.floor(chansey.maxhp / 16) * 4,
      `Chansey should have taken a lot more damage from burn`
    );
    ...
    assert.equal(
      hp - chansey.hp,
      Math.floor(chansey.maxhp / 8),
      `Chansey should have taken normal damage from burn`
    );
  });
