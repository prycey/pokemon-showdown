...
describe('Tera Stellar', () => {
  ...
  it(`should increase the damage of non-STAB moves by 1.2x on the first use of that move type`, () => {
        ...
        [
          {
            species: 'Wynaut',
            ability: 'noguard',
            moves: ['surf', 'hydropump', 'extrasensory', 'hyperspacehole'],
            teraType: 'Stellar',
          },
        ],
    ...
    assert.bounded(
      damage,
      [39, 47],
      `Hydro Pump should have regular damage, because Water-type was already used`
    );
    ...
    assert.bounded(
      damage,
      [43, 52],
      `Hyperspace Hole should have 1.5x damage on its first use, because Psychic-type was already used`
    );
  });
  ...
  it(`should not work with Adapatability`, () => {
        ...
        [
          {
            ...
            moves: ['hyperspacehole', 'terablast'],
            teraType: 'Stellar',
          },
        ],
        [{ species: 'Happiny', ability: 'shellarmor', moves: ['softboiled'] }],
    ...
    assert.bounded(
      damage,
      [43, 52],
      `Hyperspace Hole should only have 1.5x damage on its second use`
    );
    ...
    assert.bounded(
      damage,
      [24, 29],
      `Tera Blast should not have any boosted damage on its second use`
    );
  });
  ...
  it(`should increase the damage of all hits of a multi-hit move`, () => {
    ...
    assert.bounded(
      damage,
      [144, 174],
      `Surging Strikes should have ~1.2x damage on its first use for all 3 hits`
    );
    ...
    assert.bounded(
      damage,
      [63, 75],
      `Flip Turn should have regular damage on its first use, because Water-type was already used`
    );
  });
});
