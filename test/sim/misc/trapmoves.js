...
describe('Trapping Moves', () => {
  ...
  for (const move of trappers) {
    it('should prevent Pokemon from switching out normally', () => {
      ...
      battle.setPlayer('p1', {
        team: [{ species: 'Smeargle', ability: 'prankster', moves: [toID(move)] }],
      });
      battle.setPlayer('p2', {
        team: [
    ...
    it('should free all trapped Pokemon if the user is no longer active', () => {
      ...
      if (move !== 'Thousand Waves') {
        battle.makeChoices(
          'move ' + toID(move) + ' 1, move swordsdance',
          'move synthesis, move recover'
        );
        battle.makeChoices(
          'move ' + toID(move) + ' 2, move closecombat -1',
          'move synthesis, move recover'
        );
      } else {
        battle.makeChoices(
          'move ' + toID(move) + ', move closecombat -1',
          'move synthesis, move recover'
        );
      }
...
describe('Partial Trapping Moves', () => {
  ...
  for (const move of partialtrappers) {
    it('should deal 1/8 HP per turn', () => {
      battle = common.createBattle();
      battle.setPlayer('p1', {
        team: [{ species: 'Smeargle', ability: 'noguard', moves: [toID(move), 'rest'] }],
      });
      battle.setPlayer('p2', {
        team: [{ species: 'Blissey', ability: 'naturalcure', moves: ['healbell'] }],
      });
    ...
    it('should prevent Pokemon from switching out normally', () => {
      battle = common.createBattle();
      battle.setPlayer('p1', {
        team: [{ species: 'Smeargle', ability: 'noguard', moves: [toID(move)] }],
      });
      battle.setPlayer('p2', {
        team: [
...
describe('Partial Trapping Moves [Gen 1]', () => {
  ...
  it('Wrap ends when wrapped Pokemon switches to a Pokemon that dies of residual damage', () => {
    battle = common.gen(1).createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Dragonite', moves: ['wrap', 'seismictoss', 'toxic'], evs: { hp: 255 } }],
    });
    battle.setPlayer('p2', {
      team: [
