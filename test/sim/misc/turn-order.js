...
describe('Mega Evolution', () => {
  ...
  it('should cause mega ability to affect the order of the turn in which it happens', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Banette', ability: 'frisk', item: 'banettite', moves: ['psychup'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Deoxys-Speed', ability: 'pressure', moves: ['calmmind'] }],
    });
  ...
  it('should cause base ability to not affect the order of the turn in which it happens', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Sableye', ability: 'prankster', item: 'sablenite', moves: ['psychup'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Deoxys-Speed', ability: 'pressure', moves: ['calmmind'] }],
    });
  ...
  it('should cause mega forme speed to decide turn order', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Beedrill', ability: 'swarm', item: 'beedrillite', moves: ['xscissor'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Hoopa-Unbound', ability: 'magician', moves: ['psyshock'] }],
    });
  ...
  it('should cause ultra forme speed to decide turn order', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Hoopa-Unbound', ability: 'magician', moves: ['darkpulse'] }],
    });
    const fastBase = battle.p2.active[0];
...
describe('Switching in', () => {
  it(`should trigger events in an order determined by what each Pokemon's speed was when they switched in`, () => {
    ...
    assert.equal(
      ...
      'desolateland',
      'Groudon should have reverted after Kyogre in spite of Sticky Web because it was slower before the SwitchIn event started'
    );
  });
});
