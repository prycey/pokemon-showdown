...
describe('Wonder Room', () => {
  ...
  it(`should swap the raw Defense and Special Defense stats, but not stat stage changes or other defense modifiers`, () => {
      ...
      [
        {
          species: 'Blissey',
          ability: 'shellarmor',
          moves: ['wonderroom', 'defensecurl', 'roost'],
        },
    ...
    assert.bounded(
      damage,
      [30, 36],
      `Wonder Room should cause Blissey to use its Special Defense for Brick Break's damage calculation`
    );
    ...
    assert.bounded(
      damage,
      [22, 26],
      `Wonder Room should still use Defense Curl's +1 Def boost for Brick Break's damage calculation`
    );
    ...
    assert.bounded(
      damage,
      [38, 46],
      `Wonder Room should not use Assault Vest for Brick Break's damage calculation`
    );
  });
