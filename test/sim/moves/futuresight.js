...
describe('Future Sight', () => {
  ...
  it(`should only cause the user to take Life Orb recoil on its damaging turn`, () => {
    ...
    assert.equal(
      wynaut.hp,
      wynaut.maxhp - Math.floor(wynaut.maxhp / 10),
      `Wynaut should take Life Orb recoil on Future Sight's damaging turn`
    );
  ...
  it(`should not cause the user to change typing on either its starting or damaging turn`, () => {
    ...
    battle.makeChoices();
    assert.false(
      roggenrola.hasType('Psychic'),
      `Protean Roggenrola should not change type on Future Sight's starting turn`
    );
    battle.makeChoices();
    battle.makeChoices();
    assert.false(
      roggenrola.hasType('Psychic'),
      `Protean Roggenrola should not change type on Future Sight's damaging turn`
    );
  });
  ...
  it.skip(`should use the user's most recent Special Attack stat, even if the user is not on the field`, () => {
    ...
    assert.bounded(
      damage,
      [79, 94],
      `Future Sight should deal Blade Forme damage, even though Aegislash switched out in Blade Forme`
    );
    ...
    assert.bounded(
      damage,
      [79, 94],
      `Future Sight should deal Blade Forme damage, even though Aegislash was KOed in Blade Forme.`
    );
  });
