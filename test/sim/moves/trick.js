...
describe('Trick', () => {
  ...
  it('should exchange the items of the user and target', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Mew', ability: 'synchronize', item: 'leftovers', moves: ['trick'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Purugly', ability: 'defiant', item: 'sitrusberry', moves: ['rest'] }],
    });
  ...
  it('should not take plates from Arceus', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Mew', ability: 'synchronize', moves: ['trick'] }],
    });
    battle.setPlayer('p2', {
      team: [
  ...
  it('should not cause Arceus to gain a plate', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Mew', ability: 'synchronize', item: 'fistplate', moves: ['trick'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Arceus', ability: 'download', moves: ['swordsdance'] }],
    });
    battle.makeChoices('move trick', 'move swordsdance');
  ...
  it('should not cause Genesect to gain a drive', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Mew', ability: 'synchronize', item: 'shockdrive', moves: ['trick'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Genesect', ability: 'download', moves: ['shiftgear'] }],
    });
    battle.makeChoices('move trick', 'move shiftgear');
    assert.equal(battle.p2.active[0].item, '');
  });
...
describe('Z-Trick', () => {
  ...
  it("boost the user's Speed by 2 stages, but should fail to exchange the items", () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Mew', ability: 'synchronize', item: 'psychiumz', moves: ['trick'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Rattata', ability: 'guts', item: 'leftovers', moves: ['quickattack'] }],
    });
