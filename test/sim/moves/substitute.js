...
describe('Substitute', () => {
  ...
  it('should deduct 25% of max HP, rounded down', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Mewtwo', ability: 'pressure', moves: ['substitute'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Mewtwo', ability: 'pressure', moves: ['recover'] }],
    });
  ...
  it("should not block the user's own moves from targetting itself", () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Mewtwo', ability: 'pressure', moves: ['substitute', 'calmmind'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Mewtwo', ability: 'pressure', moves: ['recover'] }],
    });
  ...
  it('should block damage from most moves', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Mewtwo', ability: 'pressure', item: 'laggingtail', moves: ['psystrike'] }],
    });
  ...
  it('should not block recoil damage', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Mewtwo', ability: 'pressure', moves: ['substitute', 'doubleedge'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Mewtwo', ability: 'pressure', moves: ['nastyplot'] }],
    });
  ...
  it("should cause recoil damage from an opponent's moves to be based on damage dealt to the substitute", () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Mewtwo', ability: 'noguard', moves: ['nastyplot', 'lightofruin'] }],
    });
    battle.makeChoices('move substitute', 'move nastyplot');
    ...
    assert.equal(
      ...
      Math.ceil(Math.floor(battle.p1.active[0].maxhp / 4) / 2)
    );
  });
  ...
  it("should cause recovery from an opponent's draining moves to be based on damage dealt to the substitute", () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Zangoose', ability: 'pressure', moves: ['substitute'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Zangoose', ability: 'noguard', moves: ['bellydrum', 'drainpunch'] }],
    });
  ...
  it('should block most status moves targetting the user', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Mewtwo', ability: 'noguard', moves: ['substitute'] }],
    });
    battle.setPlayer('p2', {
      team: [
        {
          ...
          ability: 'pressure',
          item: 'laggingtail',
          moves: ['hypnosis', 'toxic', 'poisongas', 'thunderwave', 'willowisp'],
        },
      ],
    });
    for (let i = 1; i <= 5; i++) {
  ...
  it(`[Gen 1] Substitute should not block secondary effect confusion if it is unbroken`, () => {
    battle = common
      .gen(1)
      .createBattle({ forceRandomChance: true }, [
  ...
  it(`[Gen 1] if a Pokemon with a Substitute hurts itself due to confusion and the target does not have a Substitute, there is no damage dealt.`, () => {
    ...
    assert(
      battle.log.includes(
        '|-hint|In Gen 1, if a Pokemon with a Substitute hurts itself due to confusion or Jump Kick/Hi Jump Kick recoil and the target does not have a Substitute there is no damage dealt.'
      )
    );
  ...
  it(`[Gen 1] if a Pokemon with a Substitute hurts itself due to confusion and the target has a Substitute, the target's Substitute takes the damage.`, () => {
    ...
    assert(
      battle.log.includes(
        "|-hint|In Gen 1, if a Pokemon with a Substitute hurts itself due to confusion or Jump Kick/Hi Jump Kick recoil and the target has a Substitute, the target's Substitute takes the damage."
      )
    );
    assert.equal(magikarp.hp, magikarpHp);
    assert.equal(magikarp.volatiles['substitute'].hp, magikarpSubstituteHp);
