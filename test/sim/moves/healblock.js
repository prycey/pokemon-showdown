...
describe('Heal Block', () => {
  ...
  it('should prevent Pokemon from gaining HP from residual recovery items', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Hippowdon', ability: 'sandstream', moves: ['healblock'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Spiritomb', ability: 'pressure', item: 'leftovers', moves: ['calmmind'] }],
    });
  ...
  it('should prevent Pokemon from consuming HP recovery items', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Sableye', ability: 'prankster', moves: ['healblock'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Pansage', ability: 'gluttony', item: 'berryjuice', moves: ['bellydrum'] }],
    });
  ...
  it('should disable the use of healing moves', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Spiritomb', ability: 'pressure', moves: ['healblock'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Cresselia', ability: 'levitate', moves: ['recover'] }],
    });
  ...
  it('should prevent Pokemon from using draining moves', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Venusaur', ability: 'overgrow', moves: ['gigadrain'] }],
    });
  ...
  it('should prevent abilities from recovering HP', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Sableye', ability: 'prankster', moves: ['healblock', 'surf'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Quagsire', ability: 'waterabsorb', moves: ['bellydrum', 'calmmind'] }],
    });
    battle.makeChoices('move healblock', 'move bellydrum');
  ...
  it('should prevent Leech Seed from healing HP', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Starmie', ability: 'noguard', moves: ['healblock'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Venusaur', ability: 'overgrow', moves: ['substitute', 'leechseed'] }],
    });
      ...
      team: [
        {
          ...
          ability: 'telepathy',
          item: 'normaliumz',
          moves: ['psychic', 'healblock', 'recover'],
        },
      ],
    });
    battle.setPlayer('p2', {
      team: [
        {
          ...
          ability: 'telepathy',
          item: 'psychiumz',
          moves: ['psychic', 'healblock', 'teleport'],
        },
      ],
    });
...
describe('Heal Block [Gen 4]', () => {
  ...
  it('should fail indepedently on each target', () => {
    ...
    battle.makeChoices('move sleeptalk, move sleeptalk', 'move sleeptalk, move healblock');
    assert.equal(
      battle.p2.active[1].moveLastTurnResult,
      false,
      'should fail when fails on all targets'
    );
    ...
    assert.equal(
      battle.p2.active[1].moveLastTurnResult,
      true,
      'should succeed if succeeds on at least one target'
    );
  });
});
