...
describe('Transform', () => {
  ...
  it("should copy the Pokemon's species", () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Ditto', ability: 'limber', moves: ['transform'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Hoopa-Unbound', ability: 'magician', moves: ['rest'] }],
    });
      ...
      team: [
        {
          ...
          ability: 'synchronize',
          item: 'laggingtail',
          moves: ['calmmind', 'agility', 'transform'],
        },
      ],
    });
    battle.setPlayer('p2', {
      team: [
        {
          species: 'Scolipede',
          ability: 'swarm',
          moves: ['honeclaws', 'irondefense', 'doubleteam'],
        },
      ],
    });
    for (let i = 1; i <= 3; i++) {
  ...
  it("should copy the target's focus energy status", () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Sawk', ability: 'sturdy', moves: ['focusenergy'] }],
    });
      ...
      team: [
        {
          species: 'Mew',
          ability: 'synchronize',
          moves: ['rest', 'psychic', 'energyball', 'hyperbeam'],
        },
      ],
    });
    const p1poke = battle.p1.active[0];
  ...
  it('should not copy speed boosts from Unburden', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Hitmonlee', ability: 'unburden', item: 'normalgem', moves: ['feint'] }],
    });
  ...
  it(`should not announce that its ability was suppressed after Transforming`, () => {
    ...
    assert.equal(
      abilityAnnounceIndex,
      -1,
      `It should not announce the user's ability was suppressed.`
    );
  });
});
...
describe('Transform [Gen 9]', () => {
  ...
  it("should not allow Pokemon transformed into Ogerpon to Terastallize later if they couldn't before transforming", () => {
    battle = common
      .gen(9)
      .createBattle({ formatid: 'gen9customgame@@@!teampreview,terastalclause' }, [
        [
          { species: 'Ditto', ability: 'limber', moves: ['transform'], teraType: 'Fire' },
...
describe('Transform [Gen 5]', () => {
  ...
  it("should not copy the target's focus energy status", () => {
    ...
    assert.constant(
      () => battle.p1.active[0].volatiles['focusenergy'],
      () => battle.makeChoices('move transform', 'move focusenergy')
    );
  });
});
