...
describe('Imprison', () => {
  ...
  it(`should not prevent foes from using Z-Powered Status moves`, () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Sableye', ability: 'prankster', moves: ['imprison', 'sunnyday'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Charmander', ability: 'blaze', item: 'firiumz', moves: ['sunnyday'] }],
    });
  ...
  it(`should not prevent the user from using moves that a foe knows`, () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Abra', ability: 'synchronize', moves: ['calmmind', 'gravity'] }],
    });
