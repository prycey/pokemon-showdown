...
describe('Rage Fist', () => {
  ...
  it(`should not increase BP when healed by an ally's Pollen Puff`, () => {
    ...
    assert.equal(
      annihilape.timesAttacked,
      0,
      `timesAttacked should not have incremented after a full HP Pollen Puff`
    );
    ...
    assert.equal(
      annihilape.timesAttacked,
      0,
      `timesAttacked should not have incremented after a not-full HP Pollen Puff`
    );
  });
  ...
  it(`should increase BP when hit by Dragon Darts`, () => {
    ...
    assert.equal(
      primeape.timesAttacked,
      1,
      `timesAttacked should have been increased by Dragon Darts targeting the left slot`
    );
    ...
    assert.equal(
      primeape.timesAttacked,
      2,
      `timesAttacked should have been increased by Dragon Darts targeting the right slot`
    );
  });
});
