...
describe('Grassy Terrain', () => {
  ...
  it(`should halve the base power of Earthquake, Bulldoze, and Magnitude against grounded targets`, () => {
    ...
    assert.equal(
      battle.runEvent('BasePower', aerodactyl, shaymin, Dex.moves.get('earthquake'), 100, true),
      50
    );
    assert.equal(
      battle.runEvent('BasePower', shaymin, aerodactyl, Dex.moves.get('earthquake'), 100, true),
      100
    );
    assert.equal(
      battle.runEvent('BasePower', aerodactyl, shaymin, Dex.moves.get('bulldoze'), 60, true),
      30
    );
    assert.equal(
      battle.runEvent('BasePower', shaymin, aerodactyl, Dex.moves.get('bulldoze'), 60, true),
      60
    );
  });
  ...
  it(`should heal by Speed order in the same block as Leftovers`, () => {
    ...
    assert(
      zamGrassyIndex < rillaGrassyIndex,
      'Alakazam should heal from Grassy Terrain before Rillaboom'
    );
    assert(
      rillaGrassyIndex < rillaLeftoversIndex,
      'Rillaboom should heal from Grassy Terrain before Leftovers'
    );
  });
  ...
  it(`should only decrement turn count when being set before it would decrement in the end-of-turn effects`, () => {
    ...
    assert(
      battle.field.isTerrain('grassyterrain'),
      `Grassy Terrain should still be active turn 5, ending turn 6.`
    );
  });
  ...
  it.skip(`should not skip healing Pokemon if it was set during the block it would heal Pokemon`, () => {
    ...
    assert.equal(
      coalossal.hp,
      coalossal.maxhp - 40 + Math.floor(coalossal.maxhp / 2 / 16),
      `Coalossal should have recovered HP from Grassy Terrain.`
    );
    battle.makeChoices();
    ...
    assert.false(
      battle.field.isTerrain('grassyterrain'),
      `Grassy Terrain should have ended turn 5.`
    );
  });
});
