...
describe('Electric Terrain', () => {
  ...
  it('should change the current terrain to Electric Terrain for five turns', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Florges', ability: 'symbiosis', moves: ['mist', 'electricterrain'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Florges', ability: 'symbiosis', moves: ['mist'] }],
    });
  ...
  it('should increase the base power of Electric-type attacks used by grounded Pokemon', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Jolteon', ability: 'voltabsorb', moves: ['electricterrain'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Thundurus', ability: 'defiant', moves: ['thunderwave'] }],
    });
    battle.makeChoices('move electricterrain', 'move thunderwave');
    let basePower;
    const move = Dex.moves.get('thunderbolt');
    basePower = battle.runEvent(
      'BasePower',
      battle.p1.active[0],
      battle.p2.active[0],
      move,
  ...
  it('should prevent moves from putting grounded Pokemon to sleep', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Jolteon', ability: 'voltabsorb', moves: ['electricterrain', 'spore'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Abra', ability: 'magicguard', moves: ['telekinesis', 'spore'] }],
    });
  ...
  it('should not remove active non-volatile statuses from grounded Pokemon', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Whimsicott', ability: 'prankster', moves: ['spore'] }],
    });
  ...
  it('should prevent Yawn from putting grounded Pokemon to sleep, and cause Yawn to fail', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Jolteon', ability: 'voltabsorb', moves: ['electricterrain', 'yawn'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Sableye', ability: 'prankster', moves: ['yawn'] }],
    });
  ...
  it('should cause Rest to fail on grounded Pokemon', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Jolteon', ability: 'shellarmor', moves: ['electricterrain', 'rest'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Pidgeot', ability: 'keeneye', moves: ['doubleedge', 'rest'] }],
    });
  ...
  it('should not affect Pokemon in a semi-invulnerable state', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Smeargle', ability: 'owntempo', moves: ['yawn', 'skydrop'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Sableye', ability: 'prankster', moves: ['yawn', 'electricterrain'] }],
    });
    battle.makeChoices('move yawn', 'move yawn');
  ...
  it('should cause Nature Power to become Thunderbolt', () => {
    ...
    battle.setPlayer('p2', {
      team: [{ species: 'Shuckle', ability: 'sturdy', moves: ['naturepower'] }],
    });
  ...
  it.skip(`should block Sleep before the move would have missed`, () => {
    ...
    battle.makeChoices();
    assert(
      battle.log.some(line => line.includes('|-activate|p1a: Tapu Koko|move: Electric Terrain'))
    );
  });
});
