...
describe('Nightmare [Gen 2]', () => {
  ...
  it('should not deal damage to the affected if the opponent is KOed', () => {
    ...
    battle.setPlayer('p2', { team: [{ species: 'Forretress', moves: ['spore', 'nightmare'] }] });
  ...
  it('should continue dealing damage to the affected if it falls asleep while asleep', () => {
    battle = common.gen(2).createBattle();
    battle.setPlayer('p1', { team: [{ species: 'Magcargo', moves: ['sleeptalk', 'rest'] }] });
    battle.setPlayer('p2', { team: [{ species: 'Forretress', moves: ['nightmare', 'pound'] }] });
