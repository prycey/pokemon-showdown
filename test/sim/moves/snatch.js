...
describe('Snatch', () => {
  ...
  it('should Snatch moves and run Throat Chop and Heal Block checks', () => {
      ...
      [
        {
          species: 'mew',
          item: 'powerherb',
          moves: ['throatchop', 'toxicthread', 'sleeptalk', 'healblock'],
        },
        {
          species: 'skitty',
          ability: 'intrepidsword',
          moves: ['throatchop', 'healbell', 'recover'],
        },
      ],
    ]);

    battle.makeChoices(
      'move sleeptalk, move toxicthread 2',
      'move throatchop 1, move throatchop 2'
    );
    battle.makeChoices('move sleeptalk, move snatch', 'move toxicthread 1, move healbell');
    assert.equal(
      battle.p1.active[0].status,
      'psn',
      'should not allow Heal Bell called from Snatch'
    );
    assert.equal(battle.p2.active[1].status, 'psn', 'should not allow the Snatchee to move');
    assert(
      battle.log[battle.lastMoveLine + 2].startsWith('|cant'),
      'should log that Heal Bell failed'
    );
    ...
    assert.atMost(
      battle.p1.active[1].hp,
      weavileHp,
      'should not allow Snatch to bypass Heal Block'
    );
    assert(
      battle.log[battle.lastMoveLine + 2].startsWith('|cant'),
      'should log that Recover failed'
    );
  });
  ...
  it('should not snatch Swallow if the Swallow user has no Stockpiles', () => {
    ...
    for (const line of battle.log) {
      assert(
        ...
        `Snatch should not have activated, but found -activate message ${line}`
      );
    }
  });
  ...
  it('Snatched Swallow should heal the snatcher by 25% if the snatcher has no Stockpiles', () => {
    ...
    assert.hurtsBy(battle.p1.active[0], -targetHP, () =>
      battle.makeChoices('move snatch', 'move swallow')
    );
  });
});
...
describe('Snatch [Gen 4]', () => {
  ...
  it.skip(`should only deduct additional PP from Snatch if the Snatch was successful`, () => {
    ...
    assert.equal(
      move.pp,
      move.maxpp - 1,
      `Snatch should only lose 1 PP because it was not successful.`
    );
    ...
    assert.equal(
      move.pp,
      move.maxpp - 3,
      `Snatch should be at 13 PP after losing 1 PP earlier and 2 PP this turn`
    );
  });
});
