...
describe('Gravity', () => {
  ...
  it('should ground Flying-type Pokemon and remove their Ground immunity', () => {
    ...
    battle.setPlayer('p1', {
      team: [{ species: 'Aerodactyl', ability: 'pressure', moves: ['gravity'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Aron', ability: 'sturdy', moves: ['earthpower'] }],
    });
  ...
  it('should ground Pokemon with Levitate and remove their Ground immunity', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Rotom', ability: 'levitate', moves: ['gravity'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Aron', ability: 'sturdy', moves: ['earthpower'] }],
    });
  ...
  it('should interrupt and disable the use of airborne moves', () => {
    battle = common.createBattle();
    battle.setPlayer('p1', {
      team: [{ species: 'Spiritomb', ability: 'pressure', moves: ['gravity'] }],
    });
    battle.setPlayer('p2', {
      team: [{ species: 'Aerodactyl', ability: 'pressure', moves: ['fly'] }],
    });
  ...
  it('should allow the use of Z-moves of Gravity-blocked moves, but only apply their Z-effects', () => {
    ...
    assert(
      battle.p1.active[0].hasType('Water'),
      "Z-Splash with Protean changed the user's type when it should not have."
    );
  });
});
