...
describe('Curse', () => {
  ...
  it(`should curse a non-Ghost user with Protean`, () => {
    ...
    assert.equal(
      greninja.hp,
      greninja.maxhp - Math.floor(greninja.maxhp / 2) - curseResidual,
      `Greninja should have Cursed itself`
    );
    ...
    assert.equal(
      greninja.hp,
      greninja.maxhp - Math.floor(greninja.maxhp / 2) - curseResidual * 2,
      `Greninja should have taken Curse damage again`
    );
    assert.fullHP(caterpie);
  });
  ...
  it(`should target either random opponent if the target is an ally`, () => {
    ...
    assert(
      caterpie.maxhp !== caterpie.hp || metapod.maxhp !== metapod.hp,
      `Either Caterpie or Metapod should have lost HP from Curse`
    );
  });
...
describe('XY/ORAS Curse targetting when becoming Ghost the same turn', () => {
  ...
  const triplesTeams = [
    doublesTeams[0].concat({
      species: 'Metapod',
      ability: 'shedskin',
      item: '',
      moves: ['harden', 'stringshot'],
    }),
    doublesTeams[1].concat({
      species: 'Kakuna',
      ability: 'shedskin',
      item: '',
      moves: ['harden', 'stringshot'],
