...
describe('G-Max Volcalith', () => {
  ...
  it(`should not damage Rock-types`, () => {
    ...
    battle.makeChoices(
      'move rockthrow 1 dynamax, move sleeptalk',
  ...
  it(`should deal damage for four turns, including the fourth turn`, () => {
    ...
    battle.makeChoices(
      'move rockthrow 2 dynamax, move sleeptalk',
      'move sleeptalk, move sleeptalk'
    );
    for (let i = 0; i < 3; i++) {
      battle.makeChoices();
  ...
  it.skip(`should deal damage alongside Sea of Fire or G-Max Wildfire in the order those field effects were set`, () => {
    ...
    assert(
      volcalithDamageIndex < pledgeDamageIndex,
      'G-Max Volcalith should damage before Sea of Fire, because Volcalith was set up first.'
    );
  });
  ...
  it(`should damage Pokemon in order of Speed`, () => {
    ...
    assert(
      wynautDamagedIndex < mewtwoDamagedIndex,
      'Mewtwo should be damaged before Wynaut in normal circumstances.'
    );

    const mewtwoDamagedTRIndex = log.lastIndexOf('|-damage|p1b: Wynaut');
    const wynautDamagedTRIndex = log.lastIndexOf('|-damage|p2b: Mewtwo');
    assert(
      mewtwoDamagedTRIndex < wynautDamagedTRIndex,
      'Wynaut should be damaged before Mewtwo in Trick Room.'
    );
  });
  ...
  it(`should deal damage before Black Sludge recovery/damage`, () => {
    ...
    assert.equal(
      toxicroak.hp,
      toxicroak.maxhp - Math.floor(toxicroak.maxhp / 6) + Math.floor(toxicroak.maxhp / 16)
    );
  });
  ...
  it(`should deal damage before Grassy Terrain recovery`, () => {
    ...
    assert.equal(
      rillaboom.hp,
      rillaboom.maxhp - Math.floor(rillaboom.maxhp / 6) + Math.floor(rillaboom.maxhp / 16)
    );
  });
});
