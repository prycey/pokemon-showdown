    ...
    battle.onEvent('ModifyCritRatio', battle.format, (critRatio, target, source, move) => {
      if (move.id === 'tackle') {
        assert.equal(
          critRatio,
          4,
          `Wynaut's crit rate should be boosted by both G-Max Chi Strike and Focus Energy`
        );
      }
    });
    ...
    battle.onEvent('ModifyCritRatio', battle.format, (critRatio, target, source, move) => {
      if (move.id === 'tackle') {
        assert.equal(
          critRatio,
          2,
          `Wynaut's crit rate should be boosted by Psych Up'd G-Max Chi Strike`
        );
      }
    });
    ...
    battle.onEvent('ModifyCritRatio', battle.format, (critRatio, target, source, move) => {
      if (move.id === 'tackle') {
        assert.equal(
          critRatio,
          1,
          `Magikarp's crit rate should not be boosted by Baton Passed G-Max Chi Strike`
        );
      }
    });
