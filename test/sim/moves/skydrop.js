...
describe('Sky Drop', () => {
  ...
  it(`should not claim to have dropped a Pokemon if it is already fainted`, () => {
    ...
    battle.makeChoices();
    assert(
      battle.log.indexOf('|-end|p1a: Wynaut|Sky Drop|[interrupt]') < 0,
      `Sky Drop should only announce that it failed, not that any Pokemon was freed.`
    );
  });
});
      ...
      battle = common.gen(5).createBattle({ gameType: 'doubles' }, [
        ...
        [
          { species: 'Magikarp', moves: ['sleeptalk', 'tackle'] },
          {
            ...
            ability: 'sturdy',
            moves: ['nastyplot', 'thunderbolt', 'roar'],
          },
