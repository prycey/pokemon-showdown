...
describe('Rage Powder', () => {
  ...
  it('should redirect single-target moves towards it if it is a valid target', function () {
    ...
    battle.makeChoices(
      'move ragepowder, move growth, move growth',
      'move absorb 2, move absorb 2, move absorb 2'
    );
  ...
  it('should not affect Pokemon with Powder immunities', () => {
    ...
    battle.makeChoices(
      'move growth, move ragepowder, move growth',
      'move absorb 3, move absorb 1, move absorb 1'
    );
    assert.equal(hitCount[0], 2);
    assert.equal(hitCount[1], 1);
    assert.equal(hitCount[2], 0);
