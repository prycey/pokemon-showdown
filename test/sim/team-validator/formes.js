...
describe('Team Validator', () => {
  ...
  it('should correctly validate USUM Rockruff', () => {
    team = [{ species: 'rockruff', ability: 'owntempo', moves: ['happyhour'], evs: { hp: 1 } }];
    assert.legalTeam(team, 'gen7anythinggoes');
    team = [
    ...
    assert.legalTeam(team, 'gen7anythinggoes');
    team = [{ species: 'lycanroc', ability: 'steadfast', moves: ['happyhour'], evs: { hp: 1 } }];
    assert.false.legalTeam(team, 'gen7anythinggoes');
  });
    ...
    team = [
      {
        species: 'toxtricity',
        ability: 'punkrock',
        moves: ['venomdrench, magneticflux'],
        evs: { hp: 1 },
      },
      {
        species: 'toxtricity-low-key',
        ability: 'punkrock',
        moves: ['venoshock, shiftgear'],
        evs: { hp: 1 },
      },
    ];
    assert.false.legalTeam(team, 'gen8anythinggoes');

    team = [{ species: 'rotom-wash', ability: 'levitate', moves: ['overheat'], evs: { hp: 1 } }];
    assert.false.legalTeam(team, 'gen8anythinggoes');

    team = [{ species: 'kyurem-black', ability: 'teravolt', moves: ['glaciate'], evs: { hp: 1 } }];
    assert.false.legalTeam(team, 'gen8anythinggoes');
    ...
    team = [
      {
        species: 'unowng',
        moves: ['hiddenpower'],
        ivs: { hp: 12, atk: 20, def: 18, spa: 28, spd: 28, spe: 2 },
      },
    ];
    ...
    team = [
      {
        species: 'unown',
        moves: ['hiddenpower'],
        ivs: { hp: 0, atk: 4, def: 4, spa: 4, spd: 4, spe: 4 },
      },
    ];
