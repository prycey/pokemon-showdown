...
describe('Team Validator', () => {
    ...
    team = [
      {
        species: 'mamoswine',
        ability: 'oblivious',
        moves: ['tackle', 'iceshard', 'amnesia', 'furyattack'],
        evs: { hp: 1 },
      },
    ];
  ...
  it('should disallow egg moves with male-only Hidden Abilities', () => {
    team = [{ species: 'combusken', ability: 'speedboost', moves: ['batonpass'], evs: { hp: 1 } }];
    assert.false.legalTeam(team, 'gen5ou');
  });

  it("should disallow Pokemon in Little Cup that can't be bred to be level 5", () => {
    team = [{ species: 'kubfu', ability: 'innerfocus', moves: ['aerialace'], evs: { hp: 1 } }];
  ...
  it('should allow chain breeding', () => {
    ...
    // via snubbull
    team = [{ species: 'blissey', moves: ['present', 'healbell'] }];
  ...
  it('should allow trading back Gen 2 egg moves if compatible with Gen 1', () => {
    ...
    team = [{ species: 'hitmonchan', moves: ['highjumpkick', 'bodyslam'] }];
    assert.legalTeam(team, 'gen2ou');

    team = [{ species: 'marowak', moves: ['swordsdance', 'rockslide', 'bodyslam'] }];
    assert.legalTeam(team, 'gen2ou');
  ...
  it('should disallow trading back an egg move not in gen 1', () => {
    team = [{ species: 'marowak', moves: ['swordsdance', 'ancientpower', 'bodyslam'] }];
    assert.false.legalTeam(team, 'gen2ou');
  });
  ...
  it('should allow Nidoqueen to have egg moves', () => {
    team = [{ species: 'nidoqueen', ability: 'poisonpoint', moves: ['charm'], evs: { hp: 1 } }];
  ...
  it("should disallow illegal (level up) egg move combinations involving moves that can't be tradebacked", () => {
    team = [{ species: 'chansey', level: 5, moves: ['healbell', 'softboiled'], evs: { hp: 1 } }];
    assert.false.legalTeam(team, 'gen2ou');
  });
    ...
    team = [
      {
        species: 'salamence',
        ability: 'intimidate',
        moves: ['defensecurl', 'thrash', 'dragonrage', 'dragonrush'],
        evs: { hp: 1 },
      },
    ];
    assert.false.legalTeam(team, 'gen5ou');
    ...
    team = [
      {
        species: 'toxicroak',
        ability: 'dryskin',
        moves: ['bulletpunch', 'crosschop', 'fakeout'],
        evs: { hp: 4 },
      },
    ];
    assert.legalTeam(team, 'gen5ou');
    ...
    team = [
      {
        ...
        ability: 'mindseye',
        moves: ['yawn', 'bellydrum'],
        evs: { hp: 1 },
      },
      {
        species: 'greninjabond',
        ability: 'battlebond',
        moves: ['counter', 'switcheroo'],
        evs: { hp: 1 },
      },
  ...
  it('should not allow egg Pokemon below level 5 in Gens 2-3', () => {
    team = [{ species: 'totodile', level: 1, ability: 'torrent', moves: ['ancientpower'] }];
    assert.false.legalTeam(team, 'gen3ou');
  });
});
