...
describe('Custom Rules', () => {
  it('should support legality tags', () => {
    let team = [{ species: 'kitsunoh', ability: 'frisk', moves: ['shadowstrike'], evs: { hp: 1 } }];
    assert.false.legalTeam(team, 'gen7anythinggoes');
    assert.legalTeam(team, 'gen7anythinggoes@@@+cap');

    team = [{ species: 'pikachu', ability: 'airlock', moves: ['thunderbolt'], evs: { hp: 1 } }];
    assert.false.legalTeam(team, 'gen7anythinggoes');
    assert.legalTeam(team, 'gen7ou@@@!obtainableabilities');

    team = [{ species: 'pikachu', ability: 'airlock', moves: ['dragondance'], evs: { hp: 1 } }];
    ...
    let team = [
      {
        species: 'pikachu',
        ability: 'static',
        moves: ['agility', 'protect', 'thunder', 'thunderbolt'],
        evs: { hp: 1 },
      },
    ];
    assert.false.legalTeam(team, 'gen7anythinggoes@@@-Pikachu');

    team = [{ species: 'greninjaash', ability: 'battlebond', moves: ['surf'], evs: { hp: 1 } }];
    assert.false.legalTeam(team, 'gen7anythinggoes@@@-Greninja-Bond');

    team = [{ species: 'greninjabond', ability: 'battlebond', moves: ['surf'], evs: { hp: 1 } }];
  ...
  it('should allow Pokemon to be whitelisted', () => {
    let team = [{ species: 'giratina', ability: 'pressure', moves: ['protect'], evs: { hp: 1 } }];
    assert.legalTeam(team, 'gen7ubers@@@-allpokemon,+giratinaaltered');

    team = [{ species: 'giratinaorigin', ability: 'levitate', moves: ['protect'], evs: { hp: 1 } }];
    assert.false.legalTeam(team, 'gen7ubers@@@-allpokemon,+giratinaaltered');

    // -allpokemon should override +past
    team = [{ species: 'tyrantrum', ability: 'strongjaw', moves: ['protect'], evs: { hp: 1 } }];
  ...
  it('should support banning/unbanning tag combinations', () => {
    ...
    assert.false.legalTeam(
      team,
      'gen8customgame@@@-nonexistent,+mega',
    ...
    assert.false.legalTeam(
      team,
      'gen8customgame@@@+mega,-nonexistent',
      "Nonexistent should override all tags that aren't existence-related"
    );
    ...
    assert.legalTeam(
      team,
      'gen8customgame@@@-nonexistent,+crucibellemega',
      "Nonexistent should override all tags that aren't existence-related"
    );

    team = [{ species: 'Moltres-Galar', ability: 'Berserk', moves: ['protect'], evs: { hp: 1 } }];
    assert.false.legalTeam(team, 'gen8customgame@@@-sublegendary');
  });

  it('should support restrictions', () => {
    let team = [{ species: 'Yveltal', ability: 'No Ability', moves: ['protect'], evs: { hp: 1 } }];
  ...
  it('should allow moves to be unbanned', () => {
    const team = [{ species: 'absol', ability: 'pressure', moves: ['batonpass'], evs: { hp: 1 } }];
