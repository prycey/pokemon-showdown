...
describe('value rule support (slow)', () => {
  it('should generate teams of the proper length for the format (i.e. support Max Team Size)', () => {
    ...
    testTeam({ format: 'gen8multirandombattle', rounds: 100 }, team =>
        ...
        it(`${format.name} should support Max Move Count = ${count}`, () => {
          testTeam({ format, rounds: 50 }, team => {
            for (const set of team) {
              ...
              if (dex.species.get(species).otherFormes?.length || dex.species.get(species).forme)
                continue;
              ...
              assert(
                expectedMoves.includes(set.moves.length),
                `${species} should have ${expectedMoves.toString()} moves (moves=${set.moves})`
              );
            }
          });
        });
          ...
          testTeam({ format, rounds: 50 }, team => {
            for (const set of team) {
              ...
              assert.equal(
                set.moves.length,
                expected,
                `${species} should have ${expected} moves (moves=${set.moves})`
              );
            }
          });
        });
    ...
    it(`${filename}.json should have valid set data`, () => {
      ...
      for (const [id, sets] of Object.entries(setsJSON)) {
        ...
        for (const set of sets.sets) {
          assert(
            validRoles.includes(set.role),
            `In ${format}, set for ${species.name} has invalid role: ${set.role}`
          );
          for (const move of set.movepool) {
            ...
            if (genNum === 9) {
              assert.equal(
                move,
                dexMove.name,
                `In ${format}, ${species.name} has misformatted move: ${move}`
              );
            } else {
              assert(
                move === dexMove.id || move.startsWith('hiddenpower'),
                `In ${format}, ${species.name} has misformatted move: ${move}`
              );
            }
            assert(
              validateLearnset(dexMove, { species }, 'ubers', `gen${genNum}`),
              `In ${format}, ${species.name} can't learn ${move}`
            );
          }
          for (let i = 0; i < set.movepool.length - 1; i++) {
            assert(
              set.movepool[i + 1] > set.movepool[i],
              `In ${format}, ${species.name} movepool should be sorted alphabetically`
            );
          }
          if (genNum >= 3) {
            ...
            for (const ability of set.abilities) {
              ...
              assert(
                dexAbility.exists,
                `In ${format}, ${species.name} has invalid ability: ${ability}`
              );
              // Mega/Primal Pokemon have abilities from their base formes
              const allowedAbilities = new Set(
                Object.values(
                  species.battleOnly && !species.requiredAbility
                    ? dex.species.get(species.battleOnly).abilities
                    : species.abilities
                )
              );
              if (species.unreleasedHidden) allowedAbilities.delete(species.abilities.H);
              assert(
                allowedAbilities.has(ability),
                `In ${format}, ${species.name} can't have ${ability}`
              );
            }
            for (let i = 0; i < set.abilities.length - 1; i++) {
              assert(
                set.abilities[i + 1] > set.abilities[i],
                `In ${format}, ${species.name} abilities should be sorted alphabetically`
              );
            }
          }
          if (genNum === 9) {
            ...
            for (const type of set.teraTypes) {
              ...
              assert(
                dexType.exists,
                `In ${format}, ${species.name} has invalid Tera Type: ${type}`
              );
              assert.equal(
                type,
                dexType.name,
                `In ${format}, ${species.name} has misformatted Tera Type: ${type}`
              );
            }
            for (let i = 0; i < set.teraTypes.length - 1; i++) {
              assert(
                set.teraTypes[i + 1] > set.teraTypes[i],
                `In ${format}, ${species.name} teraTypes should be sorted alphabetically`
              );
            }
          }
          if (set.preferredTypes) {
            for (const type of set.preferredTypes) {
              ...
              assert(
                dexType.exists,
                `In ${format}, ${species.name} has invalid Preferred Type: ${type}`
              );
              assert.equal(
                type,
                dexType.name,
                `In ${format}, ${species.name} has misformatted Preferred Type: ${type}`
              );
            }
            for (let i = 0; i < set.preferredTypes.length - 1; i++) {
              assert(
                set.preferredTypes[i + 1] > set.preferredTypes[i],
                `In ${format}, ${species.name} preferredTypes should be sorted alphabetically`
              );
            }
          }
        }
    ...
    it('all Pokemon should have 4 moves, except for Ditto and Unown', () => {
      testTeam({ format, rounds }, team => {
        for (const pokemon of team)
          assert(
            pokemon.name === 'Ditto' || pokemon.name === 'Unown' || pokemon.moves.length === 4,
            `In ${format}, ${pokemon.name} can generate with ${pokemon.moves.length} moves`
          );
      });
    });
    it('all moves on all sets should exist and be obtainable', () => {
      ...
      for (const pokemon of Object.keys(setsJSON)) {
        ...
        for (const set of sets) {
          assert(
            set.movepool.every(m => dex.moves.get(m).exists),
            `In ${format}, for Pokemon ${species}, one of ${set.movepool} does not exist.`
          );
          const role = set.role;
          const moves = new Set(
            set.movepool.map(m => (m.startsWith('hiddenpower') ? m : dex.moves.get(m).id))
          );
          ...
          for (let j = 0; j < rounds; j++) {
            ...
            const specialType = specialTypes
              ? genNum > 3
                ? specialTypes[j % specialTypes.length]
                : specialTypes.join()
              : '';
            ...
            for (let i = 0; i < 16; i++) {
              ...
              const movePool = set.movepool.map(m =>
                m.startsWith('hiddenpower') ? m : dex.moves.get(m).id
              );
              let moveSet;
              if (genNum === 9) {
                moveSet = generator.randomMoveset(
                  types,
                  abilities,
                  teamDetails,
                  species,
          ...
          assert.false(
            moves.size,
            `In ${format}, the following moves on ${species.name} are unused: ${[...moves].join(', ')}`
          );
        }
      }
    });
    ...
    it(`should generate valid ${format} teams`, function () {
      ...
      testTeam({ format: format.id }, team => {
        assert.equal(
          team.length,
          targetTeamSize,
          `Team of incorrect size (should have ${targetTeamSize} Pokémon but actually has ${team.length} Pokémon): ${JSON.stringify(team)}`
        );
...
describe('Battle Factory and BSS Factory data should be valid (slow)', () => {
  for (const filename of [
    'gen9/factory-sets',
    'gen8/factory-sets',
    'gen8/bss-factory-sets',
    'gen7/bss-factory-sets',
    'gen7/factory-sets',
    'gen6/factory-sets',
  ]) {
        ...
        for (const species in typeTable) {
          ...
          for (const set of speciesData.sets) {
            ...
            assert.equal(
              species.name,
              set.species,
              `miscapitalized species "${set.species}" of ${species}`
            );

            assert(
              species.id.startsWith(toID(species.baseSpecies)),
              `non-matching species "${set.species}" of ${species}`
            );
            ...
            for (const abilityName of [].concat(set.ability)) {
              ...
              assert.equal(
                ability.name,
                abilityName,
                `miscapitalized ability "${abilityName}" of ${species}`
              );
              const allowedAbilities = new Set(
                Object.values(
                  species.battleOnly && !species.requiredAbility
              ...
              if (species.unreleasedHidden) allowedAbilities.delete(species.abilities.H);
              assert(
                allowedAbilities.has(abilityName),
                `${species.name} can't have ${abilityName}`
              );
            }
            ...
            for (const natureName of [].concat(set.nature)) {
              ...
              assert.equal(
                nature.name,
                natureName,
                `miscapitalized nature "${natureName}" of ${species}`
              );
            }
            ...
            for (const moveSpec of set.moves) {
              for (const moveName of [].concat(moveSpec)) {
                ...
                assert.equal(
                  move.name,
                  moveName,
                  `miscapitalized move "${moveName}" ≠ "${move.name}" of ${species}`
                );
                if (
                  species.id === 'zacian' &&
                  [].concat(set.item).includes('Rusted Sword') &&
                  moveName === 'Behemoth Blade'
                )
                  continue;
                assert(
                  validateLearnset(move, set, vType, mod),
                  `illegal move "${moveName}" of ${species}`
                );
              }
            }
            ...
            assert.equal(
              set.moves.flat(1).length,
              new Set(set.moves.flat(1)).size,
              `${species} has repeat moves`
            );
            ...
            assert.deepEqual(
              keys,
              sortedKeys,
              `EVs out of order on set of ${species}, possibly because one of them is for the wrong stat`
            );
            ...
            if (genNum === 9) {
              ...
              for (const type of set.teraType) {
                ...
                assert.equal(
                  type,
                  dexType.name,
                  `${species.name} has misformatted Tera Type: ${type}`
                );
              }
            }
          }
          if (filename === 'gen9/factory-sets') {
            ...
            assert.equal(
              totalWeight,
              100,
              `Total set weight for ${species} is ${totalWeight < 100 ? 'less' : 'greater'} than 100%`
            );
          }
        }
      }
    ...
    for (const speciesid in setsJSON) {
      ...
      for (const set of setsJSON[speciesid].sets) {
        ...
        assert.equal(
          species.name,
          set.species,
          `miscapitalized species "${set.species}" of ${speciesid}`
        );

        assert(
          species.id.startsWith(toID(species.baseSpecies)),
          `non-matching species "${set.species}" of ${speciesid}`
        );
        ...
        for (const abilityName of [].concat(set.ability)) {
          ...
          assert.equal(
            ability.name,
            abilityName,
            `miscapitalized ability "${abilityName}" of ${speciesid}`
          );
        }
        ...
        for (const natureName of [].concat(set.nature)) {
          ...
          assert.equal(
            nature.name,
            natureName,
            `miscapitalized nature "${natureName}" of ${speciesid}`
          );
        }
        ...
        for (const moveSpec of set.moves) {
          for (const moveName of [].concat(moveSpec)) {
            ...
            assert.equal(
              move.name,
              moveName,
              `miscapitalized move "${moveName}" ≠ "${move.name}" of ${speciesid}`
            );
            assert(
              validateLearnset(move, set, vType, mod),
              `illegal move "${moveName}" of ${speciesid}`
            );
          }
        }
        ...
        assert.deepEqual(
          keys,
          sortedKeys,
          `EVs out of order on set of ${speciesid}, possibly because one of them is for the wrong stat`
        );
      ...
      assert.equal(
        totalWeight,
        100,
        `Total set weight for ${speciesid} is ${totalWeight < 100 ? 'less' : 'greater'} than 100%`
      );
    }
  });
});
