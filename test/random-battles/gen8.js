...
describe('[Gen 8] Random Battle (slow)', () => {
  ...
  it('should not generate 3-attack Alcremie-Gmax', () => {
    testSet('alcremiegmax', options, set =>
      assert(
  ...
  it('Shiinotic should always have Moonblast', () =>
    testAlwaysHasMove('shiinotic', options, 'moonblast'));
  ...
  it('Rapidash with Swords Dance should have at least two attacks', () => {
    testSet('rapidash', options, set => {
      ...
      assert(
        set.moves.filter(m => dex.moves.get(m).category !== 'Status').length > 1,
        `got ${JSON.stringify(set.moves)}`
      );
    });
  });
  ...
  it('should guarantee Poison STAB on all Grass/Poison types', function () {
    ...
    const pokemon = Object.keys(dataJSON).filter(
      pkmn =>
        dataJSON[pkmn].moves &&
        dex.species.get(pkmn).types.includes('Grass') &&
        dex.species.get(pkmn).types.includes('Poison')
    );
  ...
  it(`should minimize Chansey's attack stat`, () => {
    testSet('chansey', options, set => {
      ...
      assert(
        atkIV === 0 && atkEV === 0,
        `Chansey should have minimum attack (Atk IV: ${atkIV}, Atk EV: ${atkEV})`
      );
    });
  });

  it('should always give Palossand Shore Up', () =>
    testAlwaysHasMove('palossand', options, 'shoreup'));
  it('should always give Azumarill Aqua Jet', () =>
    testAlwaysHasMove('azumarill', options, 'aquajet'));
...
describe('[Gen 8 BDSP] Random Battle (slow)', () => {
  ...
  for (const pokemon of Object.keys(dataJSON)) {
      ...
      it(`should not give ${species} a Choice Item if it has Fake Out`, () => {
        testSet(species, options, set => {
          ...
          assert.doesNotMatch(
            set.item,
            /Choice /,
            `got Choice Item '${set.item}' on a Fake Out set (${set.moves})`
          );
        });
      });
    }
    ...
    if (species.id !== 'ditto') {
      // Ditto always wants Choice Scarf
      ...
      it(`should not generate Choice items on ${species.name} sets with status moves, unless an item-switching move or Healing Wish is generated`, () => {
        testSet(species.id, { ...options, rounds: 500 }, set => {
          if (
            set.item.startsWith('Choice') &&
            !okToHaveChoiceMoves.some(okMove => set.moves.includes(okMove))
          ) {
            assert(
              set.moves.every(m => dex.moves.get(m).category !== 'Status'),
              `Choice item and status moves on set ${JSON.stringify(set)}`
            );
          }
        });
      });
    ...
    for (const species of ['ninetales', 'torkoal']) {
      testSet(species, options, set => {
        ...
        assert.notEqual(
          set.item,
          'Power Herb',
          `${species} should not get Power Herb with Solar Beam + Drought`
        );
      });
    }
  });
  ...
  it('should not give Shell Smash Blastoise Roar', () => {
    testSet('blastoise', options, set => {
      if (set.moves.includes('roar')) {
        assert(
          !set.moves.includes('shellsmash'),
          `Blastoise has Roar and Shell Smash (${set.moves})`
        );
      }
    });
  });
  ...
  it('should always give Shaymin-Sky Air Slash', () =>
    testAlwaysHasMove('shayminsky', options, 'airslash'));
  ...
  it('should not give Breloom Focus Punch without Substitute', () => {
    testSet('breloom', options, set => {
      if (set.moves.includes('focuspunch'))
        assert(
          set.moves.includes('substitute'),
          `Breloom has Focus Punch and no Substitute (${set.moves})`
        );
    });
  });
  ...
  it('should never give Flygon Defog and Dragon Dance', () => {
    testSet('flygon', options, set => {
      if (set.moves.includes('defog')) {
        assert(
          !set.moves.includes('dragondance'),
          `Flygon has Defog and Dragon Dance (${set.moves})`
        );
      }
    });
  });
      ...
      testSet(pokemon, options, set => {
        if (set.moves.includes('defog')) {
          assert(
            ...
            `${pokemon} has Defog and Stealth Rock (${set.moves})`
          );
        }
      });
    });
  ...
  it('should not give Magcargo Fire Blast and Lava Plume', () => {
    testSet('magcargo', options, set => {
      if (set.moves.includes('fireblast')) {
        assert(
          !set.moves.includes('lavaplume'),
          `Magcargo has Fire Blast and Lava Plume (${set.moves})`
        );
      }
    });
  });
    ...
    testSet('yanmega', options, set => {
      if (set.moves.includes('protect')) {
        assert.notEqual(
          set.ability,
          'Tinted Lens',
          `Yanmega has Protect and Tinted Lens (set=${JSON.stringify(set)})`
        );
      }
    });
  });
  ...
  it('Ambipom should only get Switcheroo if it has a Choice item', () => {
    testSet('ambipom', options, set => {
      ...
      assert(
        set.item.startsWith('Choice'),
        `Ambipom has Switcheroo and no Choice item (set=${JSON.stringify(set)})`
      );
    });
  });
  ...
  it('should give Yanmega Tinted Lens when it has Choice Specs', () => {
    testSet('yanmega', options, set => {
      ...
      assert.equal(
        set.ability,
        'Tinted Lens',
        `Yanmega has Protect and no Tinted Lens (set=${JSON.stringify(set)})`
      );
    });
  });
  ...
  it('should give Yanmega Speed Boost if it has Protect', () => {
    testSet('yanmega', options, set => {
      ...
      assert.equal(
        set.ability,
        'Speed Boost',
        `Yanmega has Protect and no Speed Boost (set=${JSON.stringify(set)})`
      );
    });
  });
});
