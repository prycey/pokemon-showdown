...
describe('Punishments', () => {
  ...
  it('Should prevent a user from having two punishments of the same type', () => {
    Punishments.userids.add('banmeplease', {
      type: 'BAN',
  ...
  it('Should properly filter out expiring punishments', () => {
    const punishments = [
      { type: 'BAN', expireTime: Date.now() - 1000, id: 'banmeplease', reason: '' },
    ];
...
describe('broader, more integrated Punishments tests', function () {
  ...
  describe('room bans', () => {
    ...
    beforeEach(async () =>
      Punishments.roomBan(
        this.room,
        this.user,
        Date.now() + TEST_PUNISHMENT_DURATION,
        this.user.id,
        false,
        'test'
      )
    ...
    it('should force the user to leave the room and prevent them from rejoining', async () => {
      ...
      assert.equal(
        joinAttempt,
        true,
        `user should be able to join the room once they are unbanned`
      );
    });

    it.skip('should expire on its own', done => {
      assert(
        Punishments.hasRoomPunishType(this.room, 'roombanmeplease', 'ROOMBAN'),
        `should be in effect`
      );
  ...
  describe('locks (network) (slow)', () => {
    ...
    it('should prevent users from chatting in rooms while they are locked', async () => {
      ...
      assert.equal(
        this.room.log.log.length,
        initialLogLength,
        `user should be unable to sucessfully chat while locked`
      );
      ...
      assert(
        lastMessage.endsWith(` Lock Me Please|Hi! I'm no longer locked!`),
        `user should have sucessfuly sent a message after being locked`
      );
    });
    ...
    it('should prevent users from sending PMs other than to staff while they are locked', async () => {
      ...
      assert.notEqual(
        result,
        false,
        `user should be able to send PMs to global staff while locked`
      );
  ...
  describe('global bans (network) (slow)', () => {
    ...
    beforeEach(async () =>
      Punishments.ban(this.user, Date.now() + TEST_PUNISHMENT_DURATION, this.user.id, false, 'test')
    );
    ...
    it.skip('should expire on its own', done => {
      ...
      setTimeout(() => {
        assert(
          !Punishments.checkIpBanned(makeConnection('127.0.0.7')),
