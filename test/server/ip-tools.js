...
describe('IP tools', () => {
  ...
  it('should handle wildcards in string ranges', () => {
    assert.deepEqual(IPTools.stringToRange('1.*'), {
      minIP: IPTools.ipToNumber('1.0.0.0'),
      maxIP: IPTools.ipToNumber('1.255.255.255'),
    });
    assert.deepEqual(IPTools.stringToRange('1.1.*'), {
      minIP: IPTools.ipToNumber('1.1.0.0'),
      maxIP: IPTools.ipToNumber('1.1.255.255'),
    });
    assert.deepEqual(IPTools.stringToRange('1.1.1.*'), {
      minIP: IPTools.ipToNumber('1.1.1.0'),
      maxIP: IPTools.ipToNumber('1.1.1.255'),
    });
  });

  it('should handle single IPs as string ranges', () => {
    assert.deepEqual(IPTools.stringToRange('1.1.1.1'), {
      minIP: IPTools.ipToNumber('1.1.1.1'),
      maxIP: IPTools.ipToNumber('1.1.1.1'),
    });
  });
});
...
describe('IP tools helper functions', () => {
  ...
  it('should produce sortable IP numbers', () => {
    ...
    assert.deepEqual(
      sortedIPNumbers.map(ipnum => IPTools.numberToIP(ipnum)),
      sortedIPs
    );
  });
...
describe('IP range conflict checker', () => {
  ...
  it('should respect the widen parameter', () => {
    ...
    assert.doesNotThrow(() =>
      IPTools.checkRangeConflicts({ minIP: 100, maxIP: 201 }, ranges, true)
    );
