...
describe('Trivia', () => {
  ...
  (Config.usesqlite ? context : context.skip)('alt merging', () => {
    ...
    it('should correctly merge alts', async () => {
      ...
      assert.deepEqual(await trivia.database.getLeaderboardEntry('annika', 'alltime'), {
        score: 4,
        totalCorrectAnswers: 4,
        totalPoints: 4,
      });
      assert.deepEqual(await trivia.database.getLeaderboardEntry('annika', 'nonAlltime'), {
        score: 6,
        totalCorrectAnswers: 6,
        totalPoints: 6,
      });
      assert.deepEqual(await trivia.database.getLeaderboardEntry('annika', 'cycle'), {
        score: 2,
