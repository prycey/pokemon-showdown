...
describe('Chat', () => {
  it('should run formatText correctly', () => {
    assert.equal(
      Chat.formatText(
        `hi **__bold italics__** ^^superscript^^ \\\\subscript\\\\ normal ~~strikethrough~~ bye`
      ),
      `hi <b><i>bold italics</i></b> <sup>superscript</sup> <sub>subscript</sub> normal <s>strikethrough</s> bye`
    );
    assert.equal(Chat.formatText(`__**reverse nesting**__`), `<i><b>reverse nesting</b></i>`);
    assert.equal(Chat.formatText(`__**bad nesting__**`), `<i>**bad nesting</i>**`);
    assert.equal(
      Chat.formatText(`spaced ** out ** no __also no __ ~~ also no~~ ok`),
      `spaced ** out ** no __also no __ ~~ also no~~ ok`
    );
    assert.equal(Chat.formatText(`hi \`\` \` \`\` bye`), `hi <code>\`</code> bye`);
    assert.equal(
    ...
    assert.equal(
      Chat.formatText(
        `hi __spoiler: bye__ hi again (parenthetical spoiler: bye again (or not!!!!)) that was fun`
      ),
    ...
    assert.equal(
      Chat.formatText(
        `hi __||bye||__ hi again (parenthetical ||bye again (or not!!!!)||) that was fun`
      ),
    ...
    assert.equal(Chat.formatText(`>greentext`), `<span class="greentext">&gt;greentext</span>`);
    assert.equal(
    ...
    assert.equal(Chat.formatText(`:)`, true), `:)`);
    assert.equal(Chat.formatText(`a\nb\nc`), `a\nb\nc`);
    assert.equal(Chat.formatText(`a\nb\nc`, true), `a<br />b<br />c`);
    assert.equal(Chat.formatText(`a\nb\nc`, false, true), `a<br />b<br />c`);
  });
