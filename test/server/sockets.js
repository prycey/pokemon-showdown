...
describe('ServerStream', () => {
  ...
  const createSplit = (player, secret, shared) => [`|split|p${player}`, secret, shared];
  ...
  describe('extractChannel', () => {
    ...
    it('should return player-privileged messages for each player', () => {
      ...
      for (const player of [1, 2, 3, 4]) {
        ...
        const expectedPlayerMessages = [`|-start|p${player}a: Aerodactyl|ability: Pressure`].join(
          '\n'
        );
    ...
    it('should return privileged messages with non-privileged messages', () => {
      ...
      assert.equal(
        actualChannelMessages[omniscientPlayer].join('\n'),
