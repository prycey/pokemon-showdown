...
const DATASET_A = [
  ...
  },
  {
    ...
    ip: '127.0.0.1',
    loggedBy: 'annika',
    note: 'this entry has many parts',
    time: 4,
  },
  {
    action: 'ROOMBAN',
...
(Config.usesqlite ? describe : describe.skip)('Modlog', () => {
  ...
  describe('Modlog#prepareSQLSearch', () => {
    it('should respect the maxLines parameter', async () => {
      ...
      const noMaxLines = modlog.prepareSQLSearch(['lobby'], 0, false, {
        note: [],
        user: [],
        ip: [],
        action: [],
  ...
  describe('Modlog#write', () => {
    it('should write messages serially to the modlog', async () => {
      await modlog.write('development', {
        note: 'This message is logged first',
        action: 'UNITTEST',
      });
      await modlog.write('development', {
        note: 'This message is logged second',
        action: 'UNITTEST',
      });
      const lines = await modlog.database.all(
        await modlog.database.prepare(
    ...
    it('should use overrideID if specified', async () => {
      await modlog.write(
        'battle-gen8randombattle-1337',
        { note: "I'm testing overrideID", action: 'UNITTEST' },
        'heyadora'
      );
  ...
  describe('Modlog#search', () => {
    ...
    it('should be capable of reading the entire modlog file', async () => {
      const results = await modlog.search(
        'readingtest2',
        { note: [], user: [], ip: [], action: [], actionTaker: [] },
        10000
      );
    ...
    it('user searches should be case-insensitive', async () => {
      const notExactUpper = await modlog.search('readingtest', {
        user: [{ search: 'sOmETRoll', isExact: false }],
        note: [],
        ip: [],
        action: [],
        actionTaker: [],
      });
      const notExactLower = await modlog.search('readingtest', {
        user: [{ search: 'sometroll', isExact: false }],
        note: [],
        ip: [],
        action: [],
      ...
      const exactUpper = await modlog.search('readingtest', {
        user: [{ search: 'sOMEtroLL', isExact: true }],
        note: [],
        ip: [],
        action: [],
        actionTaker: [],
      });
      const exactLower = await modlog.search('readingtest', {
        user: [{ search: 'sometroll', isExact: true }],
        note: [],
        ip: [],
        action: [],
    ...
    it('note searches should respect isExact', async () => {
      const notExact = await modlog.search('readingtest', {
        note: [{ search: 'has man', isExact: false }],
        user: [],
        ip: [],
        action: [],
        actionTaker: [],
      });
      const exact = await modlog.search('readingtest', {
        note: [{ search: 'has man', isExact: true }],
        user: [],
        ip: [],
        action: [],
        actionTaker: [],
    ...
    it('should support limiting the number of responses', async () => {
      ...
      const limited = await modlog.search(
        'readingtest',
        { note: [], user: [], ip: [], action: [], actionTaker: [] },
        5
      );
