...
assert.legalTeam = function (team, formatName, message) {
  ...
  throw new AssertionError({
    message:
      message || 'Expected team to be valid, but it was rejected because:\n' + actual.join('\n'),
...
assert.fullHP = function (pokemon, message) {
  ...
  throw new AssertionError({
    message:
      message || `Expected ${pokemon} to be fully healed, not at ${pokemon.hp}/${pokemon.maxhp}.`,
...
assert.trapped = function (fn, unavailable, message) {
  assert.throws(
    fn,
    new RegExp(
      `\\[${unavailable ? 'Unavailable' : 'Invalid'} choice\\] Can't switch: The active Pokémon is trapped`
    ),
...
assert.cantMove = function (fn, pokemon, move, unavailable, message) {
  ...
  if (pokemon && move) {
    try {
      ...
      assert(
        e.message.includes(choiceErrorTag),
        `${message}Error "${e.message}" should contain "${choiceErrorTag}"`
      );
      assert(
        lcMessage.includes(pokemon.toLowerCase()),
        `${message}Error "${e.message}" should contain "${pokemon}"`
      );
      assert(
        lcMessage.includes(move.toLowerCase()),
        `${message}Error "${e.message}" should contain "${move}"`
      );
      return;
    }
  } else {
...
assert.cantUndo = function (fn, message) {
  assert.throws(
    fn,
    /\[Invalid choice\] Can't undo:/,
    message || 'Expected to be unable to undo choice.'
  );
};
...
assert.cantTarget = function (fn, move, message) {
  assert.cantMove(
    ...
    move,
    false,
    message || `Expected not to be able to choose a target for ${move}.`
  );
};
...
assert.constant = function (getter, fn, message) {
  ...
  throw new AssertionError({
    message:
      message || `Expected value to remain as ${initialValue}, not to change to ${finalValue}.`,
...
const assertMethods = Object.getOwnPropertyNames(assert).filter(
  methodName =>
    methodName !== 'constructor' &&
    methodName !== 'AssertionError' &&
    typeof assert[methodName] === 'function'
);
    ...
    throw new AssertionError({
      message:
        lastArgIndex < args.length
          ? args[lastArgIndex]
          : `Expected '${methodName}' assertion to fail.`,
