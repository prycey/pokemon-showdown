...
export abstract class Auth extends Map<ID, GroupSymbol | ''> {
  ...
  static hasPermission(
    ...
    if (!target) {
      ...
      // empty ID -> target is a group symbol
  ...
  static supportedRoomPermissions(room: Room | null = null) {
    ...
    return [...ROOM_PERMISSIONS, ...commands];
  }
  static hasJurisdiction(
    ...
    if (
      jurisdiction.includes('u') &&
      Auth.getGroup(symbol).rank > Auth.getGroup(targetSymbol).rank
    ) {
      return true;
    }
    return false;
...
export class RoomAuth extends Auth {
  ...
  override get(userOrID: ID | User): GroupSymbol {
    ...
    const parentAuth: Auth | null = this.room.parent
      ? this.room.parent.auth
