...
export class LadderStore {
  ...
  async updateRating(
    ...
    p1score: number,
    room: AnyObject
  ): Promise<[number, AnyObject | undefined | null, AnyObject | undefined | null]> {
    if (Ladders.disabled) {
    ...
    const [p1OldElo, p2OldElo] = (
      await Promise.all([this.getRating(p1id), this.getRating(p2id)])
    ).map(Math.round);
    ...
    room.addRaw(
      Utils.html`${p1name}'s rating: ${p1OldElo} &rarr; <strong>${p1NewElo}</strong><br />(${p1Reasons})`
    );
    ...
    room.addRaw(
      Utils.html`${p2name}'s rating: ${p2OldElo} &rarr; <strong>${p2NewElo}</strong><br />(${p2Reasons})`
    );
    ...
    if (error) {
      if (error.message !== 'stream interrupt') {
        room.add(
          `||Ladder isn't responding, score probably updated but might not have (${error.message}).`
        );
        problem = true;
      }
    } else if (!room.battle) {
      ...
      room.add(
        `|error|You should be using ladders.js and not ladders-remote.js for ladder tracking.`
      );
      room.update();
      problem = true;
    }
