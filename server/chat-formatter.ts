...
export const linkRegex =
  /(?:(?:https?:\/\/[a-z0-9-]+(?:\.[a-z0-9-]+)*|www\.[a-z0-9-]+(?:\.[a-z0-9-]+)+|\b[a-z0-9-]+(?:\.[a-z0-9-]+)*\.(?:(?:com?|org|net|edu|info|us|jp)\b|[a-z]{2,3}(?=:[0-9]|\/)))(?::[0-9]+)?(?:\/(?:(?:[^\s()&<>[\]]|&amp;|&quot;|\((?:[^\s()<>&[\]]|&amp;)*\)|\[(?:[^\s()<>&[\]]|&amp;)*])*(?:[^\s()[\]{}".,!?;:&<>*`^~\\]|\((?:[^\s()<>&[\]]|&amp;)*\)))?)?|[a-z0-9.]+@[a-z0-9-]+(?:\.[a-z0-9-]+)*\.[a-z]{2,})(?![^ ]*&gt;)/gi;
  ...
  constructor(str: string, isTrusted = false, replaceLinebreaks = false, showSyntax = false) {
    ...
    str = str.replace(linkRegex, uri => {
      ...
      if (/^[a-z0-9.]+@/gi.test(uri)) {
        ...
        if (
          uri.substr(0, 24) === 'https://docs.google.com/' ||
          uri.substr(0, 16) === 'docs.google.com/'
        ) {
          ...
          if (slashIndex - 4 > 19 + 3) {
            uri =
              `${uri.slice(0, 19)}<small class="message-overflow">${uri.slice(19, slashIndex - 4)}</small>` +
          ...
          if (colonPos > 0) {
            ...
            switch (key) {
              ...
              case 'wiki':
                ...
                term = term.slice(
                  term.charAt(key.length + 1) === ' ' ? key.length + 2 : key.length + 1
                );
    ...
    for (let i = beginningOfLine; i < this.str.length; i++) {
      ...
      switch (char) {
        ...
        case ':':
          ...
          if (i < 7) break;
          if (
            this.slice(i - 7, i + 1).toLowerCase() === 'spoiler:' ||
        ...
        case '&': // escaped '<' or '>'
          ...
          if (i === beginningOfLine && this.slice(i, i + 4) === '&gt;') {
            ...
            if (
              !'._/=:;'.includes(this.at(i + 4)) &&
              !['w&lt;', 'w&gt;'].includes(this.slice(i + 4, i + 9))
            ) {
              this.pushSpan('>', i, i);
            }
          } else {
