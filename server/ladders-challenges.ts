...
export class Challenges extends Map<ID, Challenge[]> {
  ...
  add(challenge: Challenge): true {
    ...
    if (oldChallenge) {
      throw new Chat.ErrorMessage(
        `There is already a challenge (${challenge.format}) between ${challenge.to} and ${challenge.from}!`
      );
    }
  ...
  resolveAcceptCommand(context: Chat.CommandContext) {
    ...
    if (!chall || chall.to !== context.user.id || chall.acceptCommand !== context.message) {
      throw new Chat.ErrorMessage(
        `Challenge not found. You are using the wrong command. Challenges should be accepted with /accept`
      );
    }
    return chall;
  }
