...
export class BestOfPlayer extends RoomGamePlayer<BestOfGame> {
  ...
  avatar() {
    ...
    const url =
      Chat.plugins.avatars?.Avatars.src(avatar) ||
  ...
  updateReadyButton() {
    ...
    const button =
      `|c|~|/uhtml controls,<div class="infobox"><p style="margin:6px">Are you ready for game ${gameNum}, ${this.name}?</p><p style="margin:6px">` +
      (this.ready
...
export class BestOfGame extends RoomGame<BestOfPlayer> {
  ...
  checkPrivacySettings(options: RoomBattleOptions & Partial<RoomSettings>) {
    ...
    if (privacySetter.size) {
      ...
      if (this.forcedSettings.privacy) {
        ...
        room.add(
          `|raw|<div class="broadcast-blue"><strong>This best-of set is required to be public due to a player having a name starting with '${this.forcedSettings.privacy}'.</div>`
        );
        ...
        if (inviteOnly) {
          ...
          room.add(
            `|raw|<div class="broadcast-red"><strong>This best-of set is invite-only!</strong><br />Users must be invited with <code>/invite</code> (or be staff) to join</div>`
          );
        }
      }
    }
  ...
  setPrivacyOfGames(privacy: PrivacySetting) {
    for (let i = 0; i < this.games.length; i++) {
      ...
      room
        .add(
          `|uhtmlchange|bestof|<h2><strong>Game ${gameNum}</strong> of <a href="/${this.roomid}">a best-of-${this.bestOf}</a></h2>`
        )
        .update();
      if (prevRoom) {
        prevRoom
          .add(
            `|uhtmlchange|next|Next: <a href="/${room.roomid}"><strong>Game ${gameNum} of ${this.bestOf}</strong></a>`
          )
          .update();
      }
    }
  ...
  nextGame() {
    ...
    this.room.add(
      Utils.html`|uhtml|game${gameNum}|<a href="/${battleRoom.roomid}">${battleRoom.title}</a>`
    );
    this.updateDisplay();
  ...
  override onBattleWin(room: Room, winnerid: ID) {
    ...
    if (winner) {
      ...
      if (loserPlayer && loserPlayer.dcSecondsLeft <= 0) {
        // disconnection means opp wins the set
