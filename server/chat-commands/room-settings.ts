...
export const RoomSections: {
  sectionNames: { [k in RoomSection]: string };
...
export const commands: Chat.ChatCommands = {
  ...
  roomsettings(target, room, user, connection) {
    ...
    for (const handler of Chat.roomSettings) {
      ...
      for (const option of setting.options) {
        ...
        if (option[1] === true) {
          output +=
            Utils.html`<button class="button disabled" style="font-weight:bold;color:#575757;` +
  ...
  roomsettingshelp: [
    `/roomsettings - Shows current room settings with buttons to change them (if you can).`,
  ],
  ...
  modchat(target, room, user) {
    ...
    if (user.locked) {
      // would put this below but it behaves weird if there's no modchat set
    ...
    if (
      ...
      return this.errorReply(
        `/modchat - Access denied for changing a setting currently at ${room.settings.modchat}.`
      );
    }
    ...
    switch (target) {
      ...
      default:
        ...
        const modchatLevelHigherThanUserRank =
          !room.auth.atLeast(user, target) && !user.can('bypassall');
        if (
          modchatLevelHigherThanUserRank ||
          !Users.Auth.hasPermission(user, 'modchat', target as GroupSymbol, room)
        ) {
    ...
    if (!room.settings.modchat) {
      this.add(
        '|raw|<div class="broadcast-blue"><strong>Moderated chat was disabled!</strong><br />Anyone may talk now.</div>'
      );
    } else {
      const modchatSetting = Utils.escapeHTML(room.settings.modchat);
      this.add(
        `|raw|<div class="broadcast-red"><strong>Moderated chat was set to ${modchatSetting}!</strong><br />Only users of rank ${modchatSetting} and higher can talk.</div>`
      );
    }
    if ((room as GameRoom).requestModchat && !room.settings.modchat)
      (room as GameRoom).requestModchat(null);
  ...
  automodchat(target, room, user) {
    ...
    if (!target) {
      ...
      return this.sendReply(
        `Automodchat is currently set to set modchat to ${curRank} after ${curTime} minutes.`
      );
    }
    ...
    if (isNaN(time) || time > 480 || time < 5) {
      return this.errorReply(
        'Invalid duration. Choose a number under 480 (in minutes) and over 5 minutes.'
      );
    }
    ...
    this.privateModAction(
      `${user.name} set automodchat to rank ${rank} and timeout ${time} minutes.`
    );
  ...
  inviteonlynext(target, room, user) {
    ...
    if (this.meansNo(target)) {
      ...
      user.update();
      this.sendReply(
        `Your next battle will be invite-only${Rooms.RoomBattle.battleForcedSetting(user, 'privacy') ? `, unless it is rated` : ``}.`
      );
    }
  },
  inviteonlynexthelp: [
  ...
  modjoin(target, room, user) {
    ...
    if (!target) {
      const modjoinSetting =
        room.settings.modjoin === true ? 'SYNC' : room.settings.modjoin || 'OFF';
    ...
    if (room.battle) {
      ...
      if (
        room.battle.inviteOnlySetter &&
        !user.can('mute', null, room) &&
        room.battle.inviteOnlySetter !== user.id
      ) {
        return this.errorReply(
          `Only the person who set this battle to be invite-only can turn it off.`
        );
      }
      room.battle.inviteOnlySetter = user.id;
    ...
    if (this.meansNo(target)) {
      ...
      this.add(
        `|raw|<div class="broadcast-blue"><strong>This room is no longer invite only!</strong><br />Anyone may now join.</div>`
      );
      ...
      this.add(
        `|raw|<div class="broadcast-red"><strong>Moderated join is set to sync with modchat!</strong><br />Only users who can speak in modchat can join.</div>`
      );
      ...
      this.add(
        `|raw|<div class="broadcast-red"><strong>Moderated join is set to autoconfirmed!</strong><br />Users must be rank autoconfirmed or invited with <code>/invite</code> to join</div>`
      );
      ...
      if (room.settings.isPersonal && !user.can('makeroom') && !'+%'.includes(target)) {
        return this.errorReply(
          `/modjoin - Access denied from setting modjoin past % in group chats.`
        );
      }
      ...
      this.add(
        `|raw|<div class="broadcast-red"><strong>This room is now invite only!</strong><br />Users must be rank ${target} or invited with <code>/invite</code> to join</div>`
      );
  ...
  roomlanguage(target, room, user) {
    ...
    if (!target) {
      return this.sendReply(
        `This room's primary language is ${Chat.languages.get(room.settings.language || '') || 'English'}`
      );
    }
    this.checkCan('editroom', null, room);
  ...
  slowchat(target, room, user) {
    ...
    if (this.meansNo(target)) {
      ...
      if (!user.can('bypassall') && room.userCount < SLOWCHAT_USER_REQUIREMENT) {
        return this.errorReply(
          `This room must have at least ${SLOWCHAT_USER_REQUIREMENT} users to set slowchat; it only has ${room.userCount} right now.`
        );
      }
      if (room.settings.slowchat === targetInt) {
        return this.errorReply(
          `Slow chat is already set to ${room.settings.slowchat} seconds in this room.`
        );
      }
  ...
  permissions: {
    ...
    set(target, room, user) {
      ...
      if (
        rank &&
        rank !== 'whitelist' &&
        !Config.groupsranking.includes(rank as EffectiveGroupSymbol)
      ) {
      ...
      if (!room.auth.atLeast(user, '#')) {
        return this.errorReply(
          `/permissions set - You must be at least a Room Owner to set permissions.`
        );
      }
      if (
        ...
        return this.errorReply(
          `/permissions set - You can't set the permission "${perm}" because you don't have it.`
        );
      }
      ...
      return this.privateModAction(
        `${user.name} set the required rank for ${perm} to ${displayRank}.`
      );
    },
    sethelp: [
    ...
    ''(target, room, user) {
      ...
      const permissionGroups = allPermissions.filter(
        perm => !perm.startsWith('/') && !perm.startsWith('!')
      );
  ...
  capsfilterhelp: [
    `/capsfilter [on/off] - Toggles filtering messages in the room for EXCESSIVE CAPS. Requires # ~`,
  ],
  ...
  emojifilterhelp: [
    `/emojifilter [on/off] - Toggles filtering messages in the room for emojis. Requires # ~`,
  ],
  ...
  linkfilterhelp: [
    `/linkfilter [on/off] - Toggles filtering messages in the room for links. Requires # ~`,
  ],
  ...
  banword: {
    ...
    add(target, room, user, connection, cmd) {
      ...
      if (!regex) {
        words = words.map(word => {
          if (/[\\^$*+?()|{}[\]]/.test(word) && user.can('rangeban')) {
            this.errorReply(
              `"${word}" might be a regular expression, did you mean "/banword addregex"?`
            );
          }
      ...
      let banwordRegexLen =
        room.banwordRegex instanceof RegExp ? room.banwordRegex.source.length : 32;
      ...
      if (words.length > 1) {
        this.privateModAction(
          `The banwords ${words.map(w => `'${w}'`).join(', ')} were added by ${user.name}.`
        );
    ...
    delete(target, room, user) {
      ...
      if (words.length > 1) {
        this.privateModAction(
          `The banwords ${words.map(w => `'${w}'`).join(', ')} were removed by ${user.name}.`
        );
    ...
    list(target, room, user) {
      ...
      return this.sendReply(
        `Banned phrases in room ${room.roomid}: ${room.settings.banwords.join(', ')}`
      );
    },
  ...
  showapprovals(target, room, user) {
    ...
    if (!target) {
      return this.sendReply(
        `Approvals are currently ${room.settings.requestShowEnabled ? `ENABLED` : `DISABLED`} for ${room}.`
      );
    }
    if (this.meansNo(target)) {
  ...
  hightraffic(target, room, user) {
    ...
    this.addModAction(
      `This room was marked as ${room.settings.highTraffic ? 'high' : 'low'} traffic by ${user.name}.`
    );
  },
  hightraffichelp: [
  ...
  makechatroom(target, room, user, connection, cmd) {
    ...
    if (!Rooms.global.addChatRoom(target)) {
      return this.errorReply(
        `The room '${target}' already exists or it is using an invalid title.`
      );
    }
    ...
    if (cmd === 'makeprivatechatroom') {
      ...
      if (upperStaffRoom) {
        upperStaffRoom
          .add(
            `|raw|<div class="broadcast-green">Private chat room created: <b>${Utils.escapeHTML(target)}</b></div>`
          )
          .update();
      }
      ...
      if (staffRoom) {
        staffRoom
          .add(
            `|raw|<div class="broadcast-green">Public chat room created: <b>${Utils.escapeHTML(target)}</b></div>`
          )
          .update();
      }
      const upperStaffRoom = Rooms.get('upperstaff');
  ...
  makegroupchat(target, room, user, connection, cmd) {
    ...
    if (!user.trusted) {
      return this.errorReply(
        'You must be trusted (public room driver or global voice) to make a groupchat.'
      );
    }
    ...
    const titleMsg =
      Utils.html`Welcome to ${parent ? room.title : user.name}'s` +
    ...
    if (!targetRoom) {
      return this.errorReply(
        `An unknown error occurred while trying to create the room '${title}'.`
      );
    }
    ...
    user.popup(
      `You've just made a groupchat; it is now your responsibility, regardless of whether or not you actively partake in the room. For more info, read your groupchat's staff intro.`
    );
    ...
    if (Rooms.global.deregisterChatRoom(id)) {
      ...
      if (isPrivate) {
        if (upperStaffRoom) {
          upperStaffRoom
            .add(
              `|raw|<div class="broadcast-red">Private chat room deregistered by ${user.id}: <b>${target}</b></div>`
            )
            .update();
        }
      } else {
        if (staffRoom) {
          staffRoom
            .add(
              `|raw|<div class="broadcast-red">Public chat room deregistered: <b>${target}</b></div>`
            )
            .update();
        }
        if (upperStaffRoom) {
          upperStaffRoom
            .add(
              `|raw|<div class="broadcast-red">Public chat room deregistered by ${user.id}: <b>${target}</b></div>`
            )
            .update();
        }
      }
    ...
    if (room.persist) {
      if (room.settings.isPrivate) {
        ...
        if (staffRoom) {
          staffRoom
            .add(
              Utils.html`|raw|<div class="broadcast-red">Public chat room deleted: <b>${title}</b></div>`
            )
            .update();
        }
        const upperStaffRoom = Rooms.get('upperstaff');
  ...
  renameroom(target, room, user, connection, cmd) {
    ...
    if (isGroupchat) {
      ...
      if (existingRoom && !existingRoom.settings.modjoin) {
        return this.errorReply(
          `Your groupchat name is too similar to existing chat room '${existingRoom.title}'.`
        );
      }
    ...
    room
      .add(
        Utils.html`|raw|<div class="broadcast-green">The room has been renamed to <b>${target}</b></div>`
      )
      .update();
  },
  renameroomhelp: [
    `/renameroom [new title] - Renames the current room to [new title]. Case-sensitive. Requires ~`,
  ],
  ...
  privateroom(target, room, user, connection, cmd) {
    ...
    if (battle) {
      ...
      if (battle.forcedSettings.privacy) {
        return this.errorReply(
          `This battle is required to be public because a player has a name prefixed by '${battle.forcedSettings.privacy}'.`
        );
      }
      if (room.tour?.forcePublic) {
        return this.errorReply(
          `This battle can't be hidden, because the tournament is set to be forced public.`
        );
      }
    } else if (room.settings.isPersonal) {
      this.checkCan('editroom', null, room);
    ...
    switch (cmd) {
      ...
      default:
        if (room.settings.isPrivate === true && target !== 'force') {
          return this.sendReply(
            `This room is a secret room. Use "/publicroom" to make it public, or "/hiddenroom force" to force it hidden.`
          );
        }
        setting = 'hidden';
        break;
    ...
    if (!setting) {
      ...
      if (room.parent?.settings.isPrivate) {
        return this.errorReply(
          `This room's parent ${room.parent.title} must be public for this room to be public.`
        );
      }
      ...
      if (room.privacySetter && user.can('nooverride', null, room) && !user.can('makeroom')) {
        if (!room.privacySetter.has(user.id)) {
          ...
          return this.errorReply(
            `You can't make the room public since you didn't make it private - only ${privacySetters} can.`
          );
        }
        ...
        if (room.privacySetter.size) {
          const privacySetters = [...room.privacySetter].join(', ');
          return this.sendReply(
            `You are no longer forcing the room to stay private, but ${privacySetters} also need${Chat.plural(room.privacySetter, '', 's')} to use /publicroom to make the room public.`
          );
        }
      }
      ...
      if (room.subRooms && !room.bestOf) {
        ...
        for (const subRoom of room.subRooms.values()) {
          if (!subRoom.settings.isPrivate) {
            return this.errorReply(
              `Subroom ${subRoom.title} must be private to make this room private.`
            );
          }
        }
      }
      if (room.settings.isPrivate === setting) {
        if (room.privacySetter && !room.privacySetter.has(user.id)) {
          ...
          return this.sendReply(
            `This room is already ${settingName}, but is now forced to stay that way until you use /publicroom.`
          );
        }
  ...
  hidenext(target, room, user) {
    ...
    if (this.meansNo(target)) {
      ...
      user.update();
      this.sendReply(
        `Your next battle will be hidden${Rooms.RoomBattle.battleForcedSetting(user, 'privacy') ? `, unless it is rated` : ``}.`
      );
    }
  },
  hidenexthelp: [
  ...
  subroom(target, room, user) {
    ...
    if (room.parent) {
      return this.errorReply(
        `This room is already a subroom. To change which room this subroom belongs to, remove the subroom first.`
      );
    }
    if (room.subRooms) {
      return this.errorReply(
        `This room is already a parent room, and a parent room cannot be made as a subroom.`
      );
    }
    ...
    if (parentIndex > index) {
      [settingsList[parentIndex], settingsList[index]] = [
        settingsList[index],
        settingsList[parentIndex],
      ];
    }
  ...
  subrooms(target, room, user, connection, cmd) {
    ...
    return this.sendReplyBox(
      `<p style="font-weight:bold;">${Utils.escapeHTML(room.title)}'s subroom${Chat.plural(subRooms)}:</p><ul><li>${subRoomText.join('</li><br/><li>')}</li></ul></strong>`
    );
  },
  ...
  roomdesc(target, room, user) {
    ...
    if (target.length > 80) {
      return this.errorReply(
        `Error: Room description is too long (must be at most 80 characters).`
      );
    }
    ...
    if (
      normalizedTarget.startsWith(' talk about ') ||
      normalizedTarget.startsWith(' talk here about ')
    ) {
      return this.errorReply(
        `Error: Room description must not start with the phrase "talk about".`
      );
    }
  ...
  roomdeschelp: [
    `/roomdesc [description] - Sets the [description] of the current room. Requires: ~`,
  ],
  ...
  roomintro(target, room, user, connection, cmd) {
    ...
    if (!target) {
      ...
      if (!this.broadcasting && user.can('declare', null, room, 'roomintro') && cmd !== 'topic') {
        ...
        this.sendReplyBox(
          `<details open><summary>Source:</summary><code style="white-space: pre-wrap; display: table; tab-size: 3">/roomintro ${code}</code></details>`
        );
      }
      return;
    }
    ...
    this.sendReply(
      `|raw|<div class="infobox infobox-limited">${room.settings.introMessage.replace(/\n/g, '')}</div>`
    );
  ...
  deleteroomintrohelp: [
    `/deleteroomintro - Deletes the current room's introduction. Requires: # ~`,
  ],
  ...
  staffintro(target, room, user, connection, cmd) {
    ...
    if (!target) {
      ...
      this.sendReply(
        `|raw|<div class="infobox">${room.settings.staffMessage.replace(/\n/g, ``)}</div>`
      );
      if (user.can('ban', null, room, 'staffintro') && cmd !== 'stafftopic') {
        const code = Utils.escapeHTML(room.settings.staffMessage).replace(/\n/g, '<br />');
        this.sendReplyBox(
          `<details open><summary>Source:</summary><code style="white-space: pre-wrap; display: table; tab-size: 3">/staffintro ${code}</code></details>`
        );
      }
      return;
    }
  ...
  staffintrohelp: [
    `/staffintro [content] - Set an introduction for staff members. Requires: @ # ~`,
  ],
  ...
  deletestaffintrohelp: [
    `/deletestaffintro - Deletes the current room's staff introduction. Requires: @ # ~`,
  ],
  ...
  roomalias(target, room, user) {
    ...
    if (!target) {
      ...
      return this.sendReplyBox(
        `This room has the following aliases: ${room.settings.aliases.join(', ')}`
      );
    }
    this.checkCan('makeroom');
    if (target.includes(',')) {
  ...
  roomtierdisplay(target, room, user, connection, cmd) {
    ...
    if (!resetTier) {
      ...
      this.privateModAction(
        `${user.name} changed the room's tier display to: ${displayIDToName[toID(target)]}.`
      );
  ...
  roomsection(target, room, user) {
    ...
    if (!target) {
      if (!this.runBroadcast()) return;
      this.sendReplyBox(
        Utils.html`This room is ${room.settings.section ? `in the ${sectionNames[room.settings.section]} section` : `not in a section`}.`
      );
      return;
    }
    ...
    this.privateGlobalModAction(
      `${user.name} changed the room section of ${room.title} to ${section ? sectionNames[section] : 'none'}.`
    );
...
export const roomSettings: Chat.SettingsHandler[] = [
  ...
  room => ({
    ...
    options: [...Chat.languages].map(([id, name]) => [
      name,
      id === (room.settings.language || 'english') || `roomlanguage ${id || 'off'}`,
    ]),
  }),
  room => ({
  ...
  room => ({
    ...
    options: ['off', 5, 10, 20, 30, 60].map(time => [
      `${time}`,
      time === (room.settings.slowchat || 'off') || `slowchat ${time || 'off'}`,
    ]),
  }),
  room => ({
    ...
    options: [
      [
        `tiers`,
        (room.settings.dataCommandTierDisplay ?? 'tiers') === 'tiers' || `roomtierdisplay tiers`,
      ],
      [
        `doubles tiers`,
        room.settings.dataCommandTierDisplay === 'doubles tiers' || `roomtierdisplay doubles tiers`,
      ],
      [
        `National Dex tiers`,
        room.settings.dataCommandTierDisplay === 'National Dex tiers' ||
          `roomtierdisplay National Dex tiers`,
      ],
