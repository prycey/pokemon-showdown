...
function keysToCopy(obj: object) {
  return keysIncludingNonEnumerable(obj).filter(
    ...
    prop =>
      !(
        prop.includes('__') ||
        prop.toLowerCase().includes('prop') ||
        ['valueOf', 'constructor'].includes(prop)
      )
  );
}
...
export const commands: Chat.ChatCommands = {
  ...
  potdhelp: [`/potd [pokemon] - Set the Pokemon of the Day to the given [pokemon]. Requires: ~`],
  ...
  async setnamecolor(target, room, user, connection, cmd) {
    ...
    if (!userid || userid.length > 18) {
      return this.errorReply(
        `Specify a valid name to set a new color for. Names must be <19 characters.`
      );
    }
  ...
  pmuhtmlhelp: [
    `/pmuhtml [user], [name], [html] - PMs [html] that can change to [user]. Requires * # ~`,
  ],
  pmuhtmlchangehelp: [
  ...
  sendhtmlpage(target, room, user, connection, cmd) {
    ...
    if (closeHtmlPage) {
      ...
      if (successes.length) {
        this.sendReply(
          `Sent ${Chat.toListString(successes)}${selector ? ` the selector ${selector} on` : ''} the bot page ${pageid}.`
        );
      }
      if (errors.length) {
  ...
  highlighthtmlpage(target, room, user) {
    ...
    if (targetUser.locked && !this.user.can('lock')) {
      throw new Chat.ErrorMessage(
        'This user is currently locked, so you cannot send them highlights.'
      );
    }
  ...
  botmsghelp: [
    `/botmsg [username], [message] - Send a private message to a bot without feedback. For room bots, must use in the room the bot is auth in.`,
  ],

  nick() {
    this.sendReply(
      `||New to the Pokémon Showdown protocol? Your client needs to get a signed assertion from the login server and send /trn`
    );
    this.sendReply(
      `||https://github.com/smogon/pokemon-showdown/blob/master/PROTOCOL.md#global-messages`
    );
  ...
  async hotpatch(target, room, user, connection, cmd) {
    ...
    try {
      ...
      if (target === 'all') {
        if (lock['all']) {
          return this.errorReply(
            `Hot-patching all has been disabled by ${lock['all'].by} (${lock['all'].reason})`
          );
        }
        ...
        if (lock['tournaments']) {
          return this.errorReply(
            `Hot-patching tournaments has been disabled by ${lock['tournaments'].by} (${lock['tournaments'].reason})`
          );
        }
        if (lock['chat']) {
          return this.errorReply(
            `Hot-patching chat has been disabled by ${lock['chat'].by} (${lock['chat'].reason})`
          );
        }

        this.sendReply('Hotpatching chat commands...');

        const disabledCommands = Chat.allCommands()
          .filter(c => c.disabled)
          .map(c => `/${c.fullCmd}`);
        ...
        if (lock[target]) {
          return this.errorReply(
            `Hot-patching ${target} has been disabled by ${lock[target].by} (${lock[target].reason})`
          );
        }
        ...
        for (const key of newKeys) {
          if (!oldProto[key]) {
            ...
            // compare source code
            typeof oldProto[key] !== 'function' ||
            oldProto[key].toString() !== newProto[key].toString()
          ) {
            counts.updated++;
          }
        ...
        this.sendReply(
          `Updated ${Chat.count(counts.updated, 'methods')}` +
            (counts.added
              ? `, added ${Chat.count(counts.added, 'new methods')} to ${message}`
              : '') +
        ...
        if (lock['formats']) {
          return this.errorReply(
            `Hot-patching formats has been disabled by ${lock['formats'].by} (${lock['formats'].reason})`
          );
        }
        if (lock['battles']) {
          return this.errorReply(
            `Hot-patching battles has been disabled by ${lock['battles'].by} (${lock['battles'].reason})`
          );
        }
        if (lock['validator']) {
          return this.errorReply(
            `Hot-patching the validator has been disabled by ${lock['validator'].by} (${lock['validator'].reason})`
          );
        }
        ...
        global.Teams = require('../../sim/teams').Teams;
        this.sendReply(
          'DONE. Any battles started after now will have teams be validated according to the new code.'
        );
      } else if (target === 'punishments') {
        if (lock['punishments']) {
          return this.errorReply(
            `Hot-patching punishments has been disabled by ${lock['punishments'].by} (${lock['punishments'].reason})`
          );
        }
        ...
        if (lock['modlog']) {
          return this.errorReply(
            `Hot-patching modlogs has been disabled by ${lock['modlog'].by} (${lock['modlog'].reason})`
          );
        }
  ...
  nohotpatch(target, room, user, connection, cmd) {
    ...
    if (enable) {
      ...
      if (lock[hotpatch]) {
        return this.errorReply(
          `Hot-patching ${hotpatch} has already been disabled by ${lock[hotpatch].by} (${lock[hotpatch].reason})`
        );
      }
      lock[hotpatch] = {
        by: user.name,
    ...
    for (const manager of ProcessManager.processManagers) {
      for (const [i, process] of manager.processes.entries()) {
        ...
        if (managerName.startsWith('index.')) {
          // doesn't actually tell us anything abt the process
  ...
  async savelearnsets(target, room, user, connection) {
    ...
    await FS('data/learnsets.js').write(
      `'use strict';\n\nexports.Learnsets = {\n` +
        Object.entries(Dex.data.Learnsets)
          .map(
            ([id, entry]) =>
              `\t${id}: {learnset: {\n` +
              Utils.sortBy(
  ...
  toggleripgrep(target, room, user) {
    ...
    this.addGlobalModAction(
      `${user.name} ${Config.disableripgrep ? 'disabled' : 'enabled'} Ripgrep-related functionality.`
    );
  },
  toggleripgrephelp: [
    `/toggleripgrep - Disable/enable all functionality depending on Ripgrep. Requires: ~`,
  ],
  ...
  disableladderhelp: [
    `/disableladder - Stops all rated battles from updating the ladder. Requires: ~`,
  ],
  ...
  enableladder(target, room, user) {
    ...
    const innerHTML =
      `<b>The ladder is now back.</b><br />` + `Rated games will update the ladder now..`;
    ...
    for (const u of Users.users.values()) {
      if (u.connected)
        u.send(
          `|pm|~|${u.tempGroup}${u.name}|/raw <div class="broadcast-green">${innerHTML}</div>`
        );
    }
  },
  ...
  autolockdownkill(target, room, user) {
    ...
    if (this.meansYes(target)) {
      if (Config.autolockdown) {
        return this.errorReply(
          'The server is already set to automatically kill itself upon the final battle finishing.'
        );
      }
      Config.autolockdown = true;
      this.privateGlobalModAction(
        `${user.name} used /autolockdownkill on (autokill on final battle finishing)`
      );
    } else if (this.meansNo(target)) {
      if (!Config.autolockdown) {
        return this.errorReply(
          'The server is already set to not automatically kill itself upon the final battle finishing.'
        );
      }
      Config.autolockdown = false;
      this.privateGlobalModAction(
        `${user.name} used /autolockdownkill off (no autokill on final battle finishing)`
      );
    } else {
  ...
  prelockdown(target, room, user) {
    ...
    this.privateGlobalModAction(
      `${user.name} used /prelockdown (disabled tournaments in preparation for server restart)`
    );
  },
  prelockdownhelp: [
    `/prelockdown - Prevents new tournaments from starting so that the server can be restarted. Requires: ~`,
  ],
  ...
  emergency(target, room, user) {
    ...
    for (const curRoom of Rooms.rooms.values()) {
      curRoom
        .addRaw(
          `<div class="broadcast-red">The server has entered emergency mode. Some features might be disabled or limited.</div>`
        )
        .update();
    }
  ...
  emergencyhelp: [`/emergency - Turns on emergency mode and enables extra logging. Requires: ~`],
  ...
  endemergency(target, room, user) {
    ...
    for (const curRoom of Rooms.rooms.values()) {
      curRoom
        .addRaw(
          `<div class="broadcast-green"><b>The server is no longer in emergency mode.</b></div>`
        )
        .update();
    }
  ...
  endemergencyhelp: [`/endemergency - Turns off emergency mode. Requires: ~`],
  ...
  async kill(target, room, user) {
    ...
    if (Rooms.global.lockdown !== true && noSave) {
      return this.errorReply(
        'For safety reasons, using /kill without saving battles can only be done during lockdown.'
      );
    }
  ...
  refreshpagehelp: [`/refreshpage - refreshes the page for every user online. Requires: ~`],

  async updateserver(target, room, user, connection) {
    this.canUseConsole();
    if (Monitor.updateServerLock) {
      return this.errorReply(
        `/updateserver - Another update is already in progress (or a previous update crashed).`
      );
    }
    ...
    if (target === 'private') {
      if (!validPrivateCodePath) {
        Monitor.updateServerLock = false;
        throw new Chat.ErrorMessage(
          '`Config.privatecodepath` must be set to an absolute path before using /updateserver private.'
        );
      }
      ...
      this.addGlobalModAction(
        `${user.name} used /updateserver${target === 'public' ? ' public' : ''}`
      );
    }
        ...
        .map(
          item =>
            `<td>${Object.values(item)
              .map(val => Utils.escapeHTML(val as string))
              .join('</td><td>')}</td>`
        )
        .join('</tr><tr>');
      buffer += `</tr></table></div>`;
