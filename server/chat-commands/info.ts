...
export function findFormats(targetId: string, isOMSearch = false) {
  ...
  for (const format of formatList) {
    ...
    if (
      !targetId ||
      format[(targetId + 'Show') as 'searchShow'] ||
      sectionId === targetId ||
      formatId.startsWith(targetId) ||
      exactFormat.exists
    ) {
      if (isOMSearch) {
...
export const commands: Chat.ChatCommands = {
  ...
  whois(target, room, user, connection, cmd) {
    ...
    const canViewPunishments =
      ...
      (room &&
        ...
        user.can('mute', targetUser, room) &&
        targetUser.id in room.users);
    const canViewSecretRooms =
      user === targetUser || (canViewAlts && targetUser.locked) || user.can('makeroom');
    buf += `<br />`;

    if (canViewAlts) {
      let prevNames = targetUser.previousIDs
        .map(userid => {
      ...
      for (const targetAlt of targetUser.getAltUsers(true)) {
        ...
        prevNames = targetAlt.previousIDs
          .map(userid => {
            ...
            return p
              .map(
                cur =>
                  `${userid} (${Punishments.punishmentTypes.get(cur.type)?.desc || 'punished'}` +
                  `${cur.id !== targetAlt.id ? ` as ${cur.id}` : ``})`
              )
              .join(' | ');
          })
          .join(', ');
    ...
    for (const curRoom of Rooms.rooms.values()) {
      ...
      const inPlayerTable =
        targetUser.id in curRoom.game.playerTable && !targetUser.inRooms.has(curRoom.roomid);
  ...
  offlinewhois(target, room, user) {
    ...
    if (!user.can('alts') && !atLeastOne) {
      const hasJurisdiction =
        room &&
        user.can('mute', null, room) &&
        Punishments.roomUserids.nestedHas(room.roomid, userid);
      if (!hasJurisdiction) {
  ...
  sharedbattles(target, room) {
    ...
    this.sendReplyBox(
      Utils.html`Common battles between ${targetUsername1} and ${targetUsername2}:<br />` +
        battles
          .map(id => {
            const shortId = id.startsWith('battle-') ? id.slice(7) : id;
            return Utils.html`<a href="/${id}">${shortId}</a>`;
  ...
  sharedbattleshelp: [
    `/sharedbattles [user1], [user2] - Finds recent battles common to [user1] and [user2]. Requires % @ ~`,
  ],
  ...
  showpunishmentshelp: [
    `/showpunishments - Shows the current punishments in the room. Requires: % @ # ~`,
  ],
  ...
  showglobalpunishmentshelp: [
    `/showpunishments - Shows the current global punishments. Requires: % @ # ~`,
  ],
  ...
  ipsearch(target, room, user, connection, cmd) {
    ...
    if (/[a-z]/.test(ipOrHost)) {
      // host
      this.sendReply(
        `Users with host ${ipOrHost}${targetRoom ? ` in the room ${targetRoom.title}` : ``}:`
      );
      ...
      // ip
      this.sendReply(
        `Users with IP ${ipOrHost}${targetRoom ? ` in the room ${targetRoom.title}` : ``}:`
      );
      for (const curUser of Users.users.values()) {
      ...
      // range
      this.sendReply(
        `Users in IP range ${ipOrHost}${targetRoom ? ` in the room ${targetRoom.title}` : ``}:`
      );
  ...
  ipsearchhelp: [
    `/ipsearch [ip|range|host], (room) - Find all users with specified IP, IP range, or host. If a room is provided only users in the room will be shown. Requires: ~`,
  ],
  ...
  checkchallenges(target, room, user) {
    ...
    if (!chall) {
      return this.sendReplyBox(
        Utils.html`${user1.name} and ${user2.name} are not challenging each other.`
      );
    }
    const [from, to] = user1.id === chall.from ? [user1, user2] : [user2, user1];
    this.sendReplyBox(
      Utils.html`${from.name} is challenging ${to.name} in ${Dex.formats.get(chall.format).name}.`
    );
  },
  checkchallengeshelp: [
    `!checkchallenges [user1], [user2] - Check if the specified users are challenging each other. Requires: * @ # ~`,
  ],
  ...
  ignore(target, room, user) {
    if (!room) {
      this.errorReply(
        `In PMs, this command can only be used by itself to ignore the person you're talking to: "/${this.cmd}", not "/${this.cmd} ${target}"`
      );
    }
  ...
  data(target, room, user, connection, cmd) {
    ...
    if (!newTargets?.length) {
      return this.errorReply(
        `No Pok\u00e9mon, item, move, ability or nature named '${target}' was found${Dex.gen > dex.gen ? ` in Gen ${dex.gen}` : ''}. (Check your spelling?)`
      );
    }
    ...
    for (const [i, newTarget] of newTargets.entries()) {
      ...
      switch (newTarget.searchType) {
        ...
        case 'pokemon':
          ...
          if (format?.onModifySpecies) {
            pokemon =
              format.onModifySpecies.call(
                { dex, clampIntRange: Utils.clampIntRange, toID } as Battle,
                pokemon
              ) || pokemon;
          }
          ...
          if (showDetails) {
            ...
            const gmaxMove =
              pokemon.canGigantamax || dex.species.get(pokemon.changesFrom).canGigantamax;
            ...
            for (const evoName of pokemon.evos) {
              ...
              if (evo.gen <= dex.gen) {
                ...
                switch (evo.evoType) {
                  ...
                  case 'trade':
                    evos.push(
                      `${evo.name} (trade${evo.evoItem ? ` holding ${evo.evoItem}` : condition})`
                    );
                    break;
                  default:
            ...
            if (dex.gen >= 4) {
              if (item.fling) {
                ...
                if (item.id === 'mentalherb') {
                  const flingEffect =
                    'Removes the effects of Attract, Disable, Encore, Heal Block, Taunt, and Torment from the target.';
        ...
        case 'move':
          ...
          if (showDetails) {
            ...
            const pastGensOnly =
              (move.isNonstandard === 'Past' && dex.gen >= 8) ||
            ...
            if (move.id === 'snatch' && dex.gen >= 3) {
              details[
                `<a href="https://${Config.routes.dex}/tags/nonsnatchable">Non-Snatchable Moves</a>`
              ] = '';
            }
            if (move.id === 'mirrormove') {
              details[
                `<a href="https://${Config.routes.dex}/tags/nonmirror">Non-Mirrorable Moves</a>`
              ] = '';
            }
      ...
      if (showDetails) {
        buffer += `|raw|<font size="1">${Object.entries(details)
          .map(([detail, value]) =>
  ...
  weakness(target, room, user) {
    ...
    if (species.name !== '' && !species.exists && type1.name !== '' && !type1.exists) {
      ...
      if (typeSearchResults && typeSearchResults[0].name !== '') {
        ...
        return this.sendReplyBox(
          Utils.html`${originalSearch} isn't a recognized type or Pokemon${Dex.gen > dex.gen ? ` in Gen ${dex.gen}` : ''}.`
        );
      }
    }
    ...
    if (imperfectMatch) {
      this.sendReply(
        `No Pok\u00e9mon or type named '${originalSearch}' was found${Dex.gen > dex.gen ? ` in Gen ${dex.gen}` : ''}. Searching for '${target}' instead.`
      );
    }
    const weaknesses = [];
    const resistances = [];
  ...
  effectiveness(target, room, user) {
    ...
    let factor = 0;
    if (
      dex.getImmunity(source, defender) ||
      (source.ignoreImmunity &&
        (source.ignoreImmunity === true || source.ignoreImmunity[source.type]))
    ) {
      let totalTypeMod = 0;
      if (
        source.effectType !== 'Move' ||
        (source.category !== 'Status' && (source.basePower || source.basePowerCallback))
      ) {
        for (const type of defender.types) {
          const baseMod = dex.getEffectiveness(source, type);
          const moveMod = source.onEffectiveness?.call(
            { dex: Dex } as Battle,
            baseMod,
            null,
            type,
    ...
    const additionalInfo = hasThousandArrows
      ? '<br />However, Thousand Arrows will be 1x effective on the first hit.'
      : '';
  ...
  coverage(target, room, user) {
    ...
    if (!dispTable) {
      ...
      buffer.push(
        `<b><font color=#559955>Super Effective</font></b>: ${superEff.join(', ') || '<font color=#999999>None</font>'}`
      );
      buffer.push(
        `<span class="message-effect-resist">Neutral</span>: ${neutral.join(', ') || '<font color=#999999>None</font>'}`
      );
      buffer.push(
        `<span class="message-effect-weak">Resists</span>: ${resists.join(', ') || '<font color=#999999>None</font>'}`
      );
      buffer.push(
        `<span class="message-effect-immune">Immunities</span>: ${immune.join(', ') || '<font color=#999999>None</font>'}`
      );
      ...
      for (const type of dex.types.names()) {
        icon[type] =
          `<img src="https://${Config.routes.client}/sprites/types/${type}.png" width="32" height="14">`;
        // row of icons at top
          ...
          if (type1 === type2) {
            ...
            for (const move of sources) {
              ...
              if (typeof move === 'string') {
                ...
                if (
                  (!dex.getImmunity(move.type, type1) || !dex.getImmunity(move.type, type2)) &&
                  !move.ignoreImmunity
                ) {
                  continue;
                }
                let baseMod = dex.getEffectiveness(move.type, type1);
                let moveMod = move.onEffectiveness?.call(
                  { dex } as Battle,
                  baseMod,
                  null,
                  type1,
                ...
                moveMod = move.onEffectiveness?.call(
                  { dex } as Battle,
                  baseMod,
                  null,
                  type2,
      ...
      if (hasThousandArrows) {
        buffer +=
          '<br /><b>Thousand Arrows has neutral type effectiveness on Flying-type Pok\u00e9mon if not already smacked down.';
      }
  ...
  statcalc(target, room, user) {
    ...
    for (const arg of targets) {
      ...
      if (!evSet) {
        if (lowercase === 'invested' || lowercase === 'max') {
          ...
          lowercase.endsWith('ev') ||
          lowercase.endsWith('evs') ||
          lowercase.endsWith('+') ||
          lowercase.endsWith('-')
        ) {
          ev = parseInt(arg);
      ...
      if (!modSet) {
        ...
        if (isNaN(modifier)) {
          return this.sendReplyBox(
            'Invalid value for modifier: ' + Utils.escapeHTML(String(modifier))
          );
        }
        if (modifier > 6) {
      ...
      if (!realSet) {
        if (lowercase.endsWith('real')) {
          ...
          if (isNaN(realStat)) {
            return this.sendReplyBox(
              'Invalid value for target real stat: ' + Utils.escapeHTML(arg)
            );
          }
          if (realStat < 0) {
    ...
    if (realSet) {
      if (!baseSet) {
        if (calcHP) {
          baseStat = Math.ceil(
            (100 * realStat - 10 - level * (Math.floor(ev / 4) + iv + 100)) / (2 * level)
          );
        } else {
          if (!positiveMod) {
        ...
        if (calcHP) {
          ...
          ev = Math.ceil(
            (-1 * (2 * (nature * (baseStat * level + 250) - 50 * Math.ceil(realStat)))) /
              (level * nature)
          );
        }
        ev -= 31;
        ...
        if (iv < 0 || ev > 255) {
          return this.sendReplyBox(
            `No valid EV/IV combination possible with given parameters. Maybe try a different nature?${ev}`
          );
        }
      } else {
    ...
    if (calcHP) {
      ...
      output = Math.floor(
        nature * Math.floor(((iv + 2 * baseStat + Math.floor(ev / 4)) * level) / 100 + 5)
      );
      if (positiveMod) {
        output *= (2 + modifier) / 2;
      } else {
    ...
    return this.sendReplyBox(
      `Base ${baseStat} ${calcHP ? ' HP ' : ' '}at level ${level} with ${iv} IVs, ${ev}${nature === 1.1 ? '+' : nature === 0.9 ? '-' : ''} EVs${modifier > 0 && !calcHP ? ` at ${positiveMod ? '+' : '-'}${modifier}` : ''}: <b>${Math.floor(output)}</b>.`
    );
  },
  statcalchelp: [
  ...
  punishments(target, room, user) {
    ...
    const indefinitePunishments = [
      ...
      this
        .tr`<strong>permalock</strong> - Issued for repeated instances of bad behavior and is rarely the result of a single action. ` +
        this
          .tr`These can be appealed in the <a href="https://www.smogon.com/forums/threads/discipline-appeal-rules.3583479/">Discipline Appeal</a>` +
        this.tr` forum after at least 3 months without incident.`,
      this
        .tr`<strong>permaban</strong> - Unappealable global ban typically issued for the most severe cases of offensive/inappropriate behavior.`,
    ];
  ...
  staff(target, room, user) {
    if (!this.runBroadcast()) return;
    this.sendReplyBox(
      `<a href="https://www.smogon.com/sim/staff_list">Pok&eacute;mon Showdown Staff List</a>`
    );
  },
  ...
  forums(target, room, user) {
    if (!this.runBroadcast()) return;
    this.sendReplyBox(
      `<a href="https://www.smogon.com/forums/forums/209/">Pok&eacute;mon Showdown Forums</a>`
    );
  },
  ...
  privacypolicy(target, room, user) {
    ...
    this.sendReplyBox(
      [
        this
          .tr`- We log PMs so you can report them - staff can't look at them without permission unless there's a law enforcement reason.`,
        this.tr`- We log IPs to enforce bans and mutes.`,
        this
          .tr`- We use cookies to save your login info and teams, and for Google Analytics and AdSense.`,
        this
          .tr`- For more information, you can read our <a href="https://${Config.routes.root}/privacy">full privacy policy.</a>`,
      ].join(`<br />`)
    );
  },
  ...
  suggestions(target, room, user) {
    if (!this.runBroadcast()) return;
    this.sendReplyBox(
      `<a href="https://play.pokemonshowdown.com/suggestions">Make a suggestion for Pok&eacute;mon Showdown</a>`
    );
  },
  ...
  bugs(target, room, user) {
    ...
    if (room?.battle) {
      this.sendReplyBox(
        `<center><button name="saveReplay"><i class="fa fa-upload"></i> Save Replay</button> &mdash; <a href="https://www.smogon.com/forums/threads/3520646/">Questions</a> &mdash; <a href="https://play.pokemonshowdown.com/bugs">Bug Reports</a></center>`
      );
    } else {
      this.sendReplyBox(
  ...
  optionsbutton(target, room, user) {
    if (!this.runBroadcast()) return;
    this.sendReplyBox(
      `<button name="openOptions" class="button"><i style="font-size: 16px; vertical-align: -1px" class="fa fa-cog"></i> Options</button> (The Sound and Options buttons are at the top right, next to your username)`
    );
  },
  ...
  soundbutton(target, room, user) {
    if (!this.runBroadcast()) return;
    this.sendReplyBox(
      `<button name="openSounds" class="button"><i style="font-size: 16px; vertical-align: -1px" class="fa fa-volume-up"></i> Sound</button> (The Sound and Options buttons are at the top right, next to your username)`
    );
  },
  ...
  calc(target, room, user, connection, cmd) {
    ...
    const isRandomBattle =
      room?.battle &&
      (room.battle.format.endsWith('randombattle') ||
        room.battle.format.endsWith('randomdoublesbattle'));
    const isBattleSpotBattle =
      room?.battle &&
      (SUPPORTED_BATTLESPOT_FORMATS.includes(room.battle.format) ||
        room.battle.format.includes('battlespotspecial'));
    const { dex } = this.extractFormat(room?.battle?.format);

    if (
      RANDOMS_CALC_COMMANDS.includes(cmd) ||
      (isRandomBattle &&
        !DEFAULT_CALC_COMMANDS.includes(cmd) &&
        !BATTLESPOT_CALC_COMMANDS.includes(cmd))
    ) {
      return this.sendReplyBox(
    ...
    if (
      BATTLESPOT_CALC_COMMANDS.includes(cmd) ||
      (isBattleSpotBattle && !DEFAULT_CALC_COMMANDS.includes(cmd))
    ) {
      return this.sendReplyBox(
  ...
  async formathelp(target, room, user, connection, cmd) {
    ...
    const format =
      totalMatches === 1 ? Dex.formats.get(Object.values(sections)[0].formats[0]) : null;
    ...
    if (format) {
      ...
      if (data) {
        for (const { resource_name, url } of data.resources) {
          ...
          rn = rn
            .split(' ')
            .map((x: string) => x[0].toUpperCase() + x.substr(1))
            .join(' ');
        ...
        descHtml.push(
          `This format has no resources linked on its <a href="https://www.smogon.com/dex/${genID[format.gen - 1] || 'sv'}/formats/">Smogon Dex page</a>. ` +
      ...
      return this.sendReplyBox(
        `<h2>${format.name}</h2><hr />${formatDesc ? formatDesc + '<hr />' : ''}${descHtml.join('<br />')}${rulesetHtml ? `<br />${rulesetHtml}` : ''}`
      );
    }
    ...
    for (const sectionId in sections) {
      buf.push(
        Utils.html`<th style="border:1px solid gray" colspan="2">${sections[sectionId].name}</th>`
      );
      for (const section of sections[sectionId].formats) {
        ...
        buf.push(
          `<tr><td style="border:1px solid gray">${nameHTML}</td><td style="border: 1px solid gray; margin-left:10px">${descHTML}</td></tr>`
        );
      }
    }
    buf.push(`</table>`);
  ...
  rules(target, room, user, connection, cmd) {
    ...
    if (target.length > 150) {
      return this.errorReply(
        `Error: Room rules link is too long (must be under 150 characters). You can use a URL shortener to shorten the link.`
      );
    }
  ...
  faq(target, room, user) {
    ...
    if (!target || showAll) {
      buffer.unshift(
        `<a href="https://pokemonshowdown.com/${this.tr`pages/faq`}">${this.tr`Frequently Asked Questions`}</a>`
      );
    }
    this.sendReplyBox(buffer.join(`<br />`));
  },
  ...
  smogdex(target, room, user) {
    ...
    if (pokemon.exists) {
      ...
      if (
        (pokemon.battleOnly && pokemon.baseSpecies !== 'Greninja') ||
      ...
      if (formatId === 'anythinggoes') {
        ...
        if (genNumber > 6) {
          return this.sendReplyBox(
            `Triples formats are not an available format in Pok&eacute;mon generation ${generation.toUpperCase()}.`
          );
        }
      ...
      if (
        ['ou', 'uu'].includes(formatId) &&
        generation === 'sm' &&
        room?.settings.language &&
        room.settings.language in supportedLanguages
      ) {
        ...
        this.sendReplyBox(
          Utils.html`<a href="https://www.smogon.com/dex/${generation}/pokemon/${id}${formatId ? '/' + formatId : ''}">${generation.toUpperCase()} ${formatName} ${pokemon.name} analysis</a>, brought to you by <a href="https://www.smogon.com">Smogon University</a>`
        );
      }
    }
    ...
    if (item.exists && genNumber > 1 && item.gen <= genNumber) {
      atLeastOne = true;
      this.sendReplyBox(
        `<a href="https://www.smogon.com/dex/${generation}/items/${item.id}">${generation.toUpperCase()} ${item.name} item analysis</a>, brought to you by <a href="https://www.smogon.com">Smogon University</a>`
      );
    }
    ...
    if (ability.exists && genNumber > 2 && ability.gen <= genNumber) {
      atLeastOne = true;
      this.sendReplyBox(
        `<a href="https://www.smogon.com/dex/${generation}/abilities/${ability.id}">${generation.toUpperCase()} ${ability.name} ability analysis</a>, brought to you by <a href="https://www.smogon.com">Smogon University</a>`
      );
    }
    ...
    if (move.exists && move.gen <= genNumber) {
      atLeastOne = true;
      this.sendReplyBox(
        `<a href="https://www.smogon.com/dex/${generation}/moves/${toID(move.name)}">${generation.toUpperCase()} ${move.name} move analysis</a>, brought to you by <a href="https://www.smogon.com">Smogon University</a>`
      );
    }
    ...
    if (format.id) {
      ...
      if (formatName) {
        atLeastOne = true;
        this.sendReplyBox(
          Utils.html`<a href="https://www.smogon.com/dex/${generation}/formats/${formatId}">${generation.toUpperCase()} ${formatName} format analysis</a>, brought to you by <a href="https://www.smogon.com">Smogon University</a>`
        );
      }
    }
    ...
    if (!atLeastOne) {
      return this.sendReplyBox(
        `Pok&eacute;mon, item, move, ability, or format not found for generation ${generation.toUpperCase()}.`
      );
    }
  },
  smogdexhelp: [
  ...
  bulbapedia(target, broadcast, user) {
    ...
    if (pokemon.exists) {
      ...
      this.sendReplyBox(
        Utils.html`<a href="${link}">${pokemon.name} in-game information</a>, provided by Bulbapedia`
      );
    }
    ...
    if (item.exists) {
      ...
      this.sendReplyBox(
        Utils.html`<a href="${link}">${item.name} item description</a>, provided by Bulbapedia`
      );
    }
    ...
    if (ability.exists) {
      ...
      this.sendReplyBox(
        `<a href="${link}">${ability.name} ability description</a>, provided by Bulbapedia`
      );
    }
    ...
    if (move.exists) {
      ...
      this.sendReplyBox(
        `<a href="${link}">${move.name} move description</a>, provided by Bulbapedia`
      );
    }
  ...
  register() {
    if (!this.runBroadcast()) return;
    this.sendReplyBox(
      `You will be prompted to register upon winning a rated battle. Alternatively, there is a register button in the <button name="openOptions"><i class="fa fa-cog"></i> Options</button> menu in the upper right.`
    );
  },
  ...
  dice(target, room, user) {
    ...
    if (diceDataStart >= 0) {
      ...
      if (!Number.isInteger(diceQuantity) || diceQuantity <= 0 || diceQuantity > maxDice) {
        return this.sendReply(
          `The amount of dice rolled should be a natural number up to ${maxDice}.`
        );
      }
    }
    let offset = 0;
    ...
    if (target.length) {
      ...
      if (!Number.isSafeInteger(diceFaces) || diceFaces <= 0) {
        return this.errorReply(
          `Rolled dice must have a natural amount of faces up to ${Number.MAX_SAFE_INTEGER}.`
        );
      }
    }
    ...
    if (diceQuantity > 1) {
      ...
      if (
        !Number.isSafeInteger(
          offset < 0 ? diceQuantity * diceFaces : diceQuantity * diceFaces + offset
        )
      ) {
        return this.errorReply(
    ...
    const outlierFragment = removeOutlier
      ? ` except ${removeOutlier > 0 ? 'highest' : 'lowest'}`
      : ``;
  ...
  async requestshow(target, room, user) {
    ...
    if (!/^(https?:\/\/)?(www\.)?(youtube\.com|youtu\.be)(\/|$)/i.test(link)) {
      if (link.includes('data:image/png;base64')) {
        throw new Chat.ErrorMessage(
          'Please provide an actual link (you probably copied the URL wrong?).'
        );
      }
      try {
    ...
    this.sendReply(
      `You have requested to show the link: ${link}${comment ? ` (with the comment ${comment})` : ''}.`
    );
    const message =
      `|tempnotify|pendingapprovals|Pending media request!` +
  ...
  requestshowhelp: [
    `/requestshow [link], [comment] - Requests permission to show media in the room.`,
  ],
  ...
  async approveshow(target, room, user) {
    ...
    if (request.dimensions) {
      ...
      if (resized)
        buf += Utils.html`<br /><a href="${request.link}" target="_blank">full-size image</a>`;
    } else {
  ...
  approveshowhelp: [
    `/approveshow [user] - Approves the media display request of [user]. Requires: % @ # ~`,
  ],
  ...
  denyshow(target, room, user) {
    ...
    room.sendUser(
      targetUser,
      `|raw|<div class="broadcast-red">Your media request was denied.</div>`
    );
    room.sendUser(targetUser, `|notify|Media request denied`);
  },
  denyshowhelp: [
    `/denyshow [user] - Denies the media display request of [user]. Requires: % @ # ~`,
  ],
  ...
  approvalloghelp: [
    `/approvallog - View a log of past media approvals in the current room. Requires: ~`,
  ],
  ...
  async show(target, room, user, connection) {
    ...
    if (YouTube.linkRegex.test(link)) {
      ...
      if (link.includes('data:image/png;base64')) {
        throw new Chat.ErrorMessage(
          'Please provide an actual link (you probably copied it wrong?).'
        );
      }
      if (!buf) {
        // fall back on image
        try {
          ...
          if (resized)
            buf += Utils.html`<br /><a href="${link}" target="_blank">full-size image</a>`;
        } catch {
  ...
  async registertime(target, room, user, connection) {
    ...
    const regTimeAgo =
      duration > 24 * 60 * 60 * 1000
        ...
        : 'less than a day';
    this.sendReplyBox(
      Utils.html`The user '${target}' registered ${regTimeAgo} ago, on the date ${date.toDateString()}.`
    );
  },
  ...
  code(target, room, user, connection) {
    ...
    target = this.message
      .substr(this.cmdToken.length + this.cmd.length + (this.message.includes(' ') ? 1 : 0))
      .trimRight();
    ...
    const canExecute = (perm: string) =>
      !(
        ...
        Object.values(Config.groups)
          .slice(1)
          .some(f => (f as any)[perm])
      );
    cmds = cmds.filter(
      f =>
        f.requiredPermission &&
        canExecute(f.requiredPermission) &&
        f.fullCmd !== this.handler?.fullCmd
    );
  ...
  adminhelphelp: [
    `/adminhelp - Programmatically generates a list of all administrator commands. Requires: ~`,
  ],
  ...
  randtopic(target, room, user) {
    ...
    this.runBroadcast();
    this.sendReply(
      Utils.html`|html|<div class="broadcast-blue">${Utils.randomElement(room.settings.topics)}</div>`
    );
  },
  randtopichelp: [
...
export const handlers: Chat.Handlers = {
  onRename(user, oldID, newID) {
    if (oldID === newID || !Config.usesqlite || [oldID, newID].some(f => f.startsWith('guest')))
      return;
    void Chat.database.run(`REPLACE INTO alts_log (to_id, from_id, ip) VALUES (?, ?, ?)`, [
      newID,
      oldID,
      user.latestIp,
    ]);
...
export const pages: Chat.PageTable = {
  async altslog(query, user) {
    ...
    if (num > 3000) {
      return this.errorReply(
        `3000 is the maximum number of results from the alternate account log.`
      );
    }
    if (isNaN(num) || num < 1) {
      return this.errorReply(
        `The max results must be a real number that is at least one (received "${rawLimit}")`
      );
    }
    ...
    for (const id of userids) {
      ...
      const ipStr = ips
        .map(f => `<a href="https://whatismyipaddress.com/ip/${f}">${f}</a>`)
        .join(', ');
  ...
  punishments(query, user) {
    ...
    this.checkCan('mute', null, room);
    // Ascending order
    const sortedPunishments = Utils.sortBy(
      [...Punishments.getPunishments(room.roomid)],
