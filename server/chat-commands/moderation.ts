...
export function runPromote(
  ...
  if (!targetUser && !Users.isUsernameKnown(userid) && !force) {
    throw new Chat.ErrorMessage(
      `User '${username}' is offline and unrecognized, and so can't be promoted.`
    );
  }
  ...
  if (currentSymbol === symbol) {
    throw new Chat.ErrorMessage(
      `User '${username}' is already a ${nextGroup?.name || symbol || 'regular user'} in this room.`
    );
  }
  if (!promoter.can('makeroom')) {
    if (
      currentGroup.id &&
      !promoter.can(`room${currentGroup.id || 'voice'}` as 'roomvoice', null, room)
    ) {
      throw new Chat.ErrorMessage(
        `Access denied for promoting/demoting ${username} from ${currentGroupName}.`
      );
    }
    if (
      symbol !== ' ' &&
      !promoter.can(`room${nextGroup.id || 'voice'}` as 'roomvoice', null, room)
    ) {
      throw new Chat.ErrorMessage(
        `Access denied for promoting/demoting ${username} to ${nextGroup.name}.`
      );
    }
  }
  if (
...
export const commands: Chat.ChatCommands = {
  roomowner(target, room, user) {
    ...
    if (!Users.isUsernameKnown(userid)) {
      return this.errorReply(
        `User '${targetUsername}' is offline and unrecognized, and so can't be promoted.`
      );
    }
  ...
  roompromote(target, room, user, connection, cmd) {
    ...
    if (!Config.groups[nextSymbol]) {
      if (!force || !user.can('bypassall')) {
        ...
        return this.errorReply(
          `Admins can forcepromote to nonexistent groups only if they are one character long`
        );
      }
    }
    ...
    if (!force && (nextGroup.globalonly || (nextGroup.battleonly && !room.battle))) {
      return this.errorReply(
        `Group 'room${nextGroup.id || nextSymbol}' does not exist as a room rank.`
      );
    }
    ...
    for (const toPromote of users) {
      ...
      if (this.pmTarget && targetUser) {
        ...
        shouldPopup?.popup(
          `You were demoted to Room ${nextGroupName} by ${user.name} in ${room.roomid}.`
        );
      } else if (nextSymbol === '#') {
        ...
        logRoom?.addByUser(
          user,
          `<<${room.roomid}>> ${name} was appointed Room Owner by ${user.name}`
        );
        this.modlog('ROOM OWNER', userid);
        shouldPopup?.popup(
          `You were promoted to ${nextGroupName} by ${user.name} in ${room.roomid}.`
        );
      } else {
        this.addModAction(`${name} was promoted to Room ${nextGroupName} by ${user.name}.`);
        this.modlog(`ROOM${nextGroupName.toUpperCase()}`, userid);
        shouldPopup?.popup(
          `You were promoted to Room ${nextGroupName} by ${user.name} in ${room.roomid}.`
        );
      }
  ...
  roomauth(target, room, user, connection, cmd) {
    ...
    while (curRoom.parent) {
      const modjoinSetting =
        curRoom.settings.modjoin === true ? curRoom.settings.modchat : curRoom.settings.modjoin;
      ...
      if (curRoom.parent) {
        ...
        buffer.push(
          `${curRoom.title} is a ${roomType}subroom of ${curRoom.parent.title}, so ${curRoom.parent.title} users${inheritedUserType}${also} have authority in this room.`
        );
      }
    ...
    if (!curRoom.settings.isPrivate) {
      buffer.push(
        `${curRoom.title} is a public room, so global auth with no relevant roomauth will have authority in this room.`
      );
    } else if (curRoom.settings.isPrivate === 'hidden' || curRoom.settings.isPrivate === 'voice') {
      buffer.push(
        `${curRoom.title} is a hidden room, so global auth with no relevant roomauth will have authority in this room.`
      );
    }
  ...
  sectionleaders(target, room, user, connection) {
    ...
    for (sectionid in sections) {
      ...
      buffer.push(
        `**${RoomSections.sectionNames[sectionid]}**:\n` +
  ...
  async autojoin(target, room, user, connection) {
    ...
    if (targets.length > 16 || connection.inRooms.size > 1) {
      return connection.popup(
        "To prevent DoS attacks, you can only use /autojoin for 16 or fewer rooms, when you haven't joined any rooms yet. Please use /join for each room separately."
      );
    }
    Rooms.global.autojoinRooms(user, connection);
    const autojoins: string[] = [];

    const promises = targets.map(roomid =>
      user.tryJoinRoom(roomid as RoomID, connection).then(ret => {
  ...
  warn(target, room, user) {
    ...
    const globalWarn =
      !room ||
      ['staff', 'adminlog'].includes(room.roomid) ||
      room.roomid.startsWith('help-') ||
      (room.battle && (!room.parent || room.parent.type !== 'chat'));
    ...
    if (publicReason.length > MAX_REASON_LENGTH) {
      return this.errorReply(
        `The reason is too long. It cannot exceed ${MAX_REASON_LENGTH} characters.`
      );
    }
    if (room) {
    ...
    if (timeout < 15 * 1000) {
      ...
      return this.errorReply(
        `You must wait ${remainder} more seconds before you can warn ${targetUser.name} again.`
      );
    }
  ...
  redir(target, room, user, connection) {
    ...
    if (!user.can('rangeban', targetUser)) {
      this.errorReply(
        `Redirects have been deprecated. Instead of /redirect, use <<room links>> or /invite to guide users to the correct room, and punish if users don't cooperate.`
      );
      return;
    }
    ...
    this.addModAction(
      `${targetUser.name} was redirected to room ${targetRoom.title} by ${user.name}.`
    );
  ...
  mute(target, room, user, connection, cmd) {
    ...
    if (
      ...
      const alreadyPunishment = targetUser.locked
        ? 'locked'
        : room.isMuted(targetUser)
          ? 'muted'
          : 'room banned';
      const problem = ` but was already ${alreadyPunishment}`;
      if (!reason) {
        return this.privateModAction(
          `${targetUser.name} would be muted by ${user.name} ${problem}.`
        );
      }
      return this.addModAction(
        `${targetUser.name} would be muted by ${user.name} ${problem}. (${publicReason})`
      );
    }

    if (targetUser.id in room.users) {
      targetUser.popup(
        `|modal|${user.name} has muted you in ${room.roomid} for ${Chat.toDurationString(muteDuration)}. ${publicReason}`
      );
    }
    this.addModAction(
      `${targetUser.name} was muted by ${user.name} for ${Chat.toDurationString(muteDuration)}.${publicReason ? ` (${publicReason})` : ``}`
    );
  ...
  unmute(target, room, user) {
    ...
    if (successfullyUnmuted) {
      this.addModAction(
        `${targetUser ? targetUser.name : successfullyUnmuted} was unmuted by ${user.name}.`
      );
  ...
  ban(target, room, user, connection, cmd) {
    ...
    if (targetUser.trusted && room.settings.isPrivate !== true && !room.settings.isPersonal) {
      Monitor.log(
        `[CrisisMonitor] Trusted user ${targetUser.name} ${targetUser.trusted !== targetUser.id ? ` (${targetUser.trusted})` : ``} was roombanned from ${room.roomid} by ${user.name}, and should probably be demoted.`
      );
    }
    ...
    if (targetUser.id in room.users || user.can('lock')) {
      targetUser.popup(
        `|modal||html|<p>${Utils.escapeHTML(user.name)} has banned you from the room ${room.roomid} ` +
          ...
          `<p>To appeal the ban, PM the staff member that banned you${
            room.persist
              ? ` or a room owner. ` +
                `</p><p><button name="send" value="/roomauth ${room.roomid}">List Room Staff</button></p>`
              : `.</p>`
          }`
      );
    }
    ...
    this.addModAction(
      `${name} was banned${week ? ' for a week' : ''} from ${room.title} by ${user.name}.${publicReason ? ` (${publicReason})` : ``}`
    );
    ...
    if (!room.settings.isPrivate && room.persist) {
      ...
      if (affected.length > 1) {
        displayMessage = `${name}'s ${acAccount ? ` ac account: ${acAccount}, ` : ``} banned alts: ${affected
          .slice(1)
    ...
    room.hideText([...affected.map(u => u.id), toID(inputUsername)]);
  ...
  async lock(target, room, user, connection, cmd) {
    ...
    if (targetUser) {
      ...
      if (targetUser.locked && !targetUser.locked.startsWith('#') && !week && !month) {
        return this.privateModAction(
          `${name} would be locked by ${user.name} but was already locked.`
        );
      }
    } else {
      name = targetUsername;
    ...
    if (Users.isTrusted(userid)) {
      if (force) {
        ...
        Monitor.log(
          `[CrisisMonitor] ${name} was locked by ${user.name} and demoted from ${from.join(', ')}.`
        );
        this.globalModlog('CRISISDEMOTE', targetUser, ` from ${from.join(', ')}`);
      } else {
        return this.sendReply(
          `${name} is a trusted user. If you are sure you would like to lock them use /force${month ? 'month' : week ? 'week' : ''}lock.`
        );
      }
    ...
    this.addGlobalModAction(
      `${name} was locked from talking${durationMsg} by ${user.name}.` +
    ...
    let displayMessage = '';
    if (affected.length > 1) {
      displayMessage = `${name}'s ${acAccount ? ` ac account: ${acAccount}, ` : ''} locked alts: ${affected
        .slice(1)
        .map((curUser: User) => curUser.getLastName())
        .join(', ')}`;
      this.privateModAction(displayMessage);
    } else if (acAccount) {
      displayMessage = `${name}'s ac account: ${acAccount}`;
    ...
    if (targetUser) {
      ...
      if (roomauth.length) {
        Monitor.log(
          `[CrisisMonitor] Locked user ${name} has public roomauth (${roomauth.join(', ')}), and should probably be demoted.`
        );
      }
    }
  ...
  unlock(target, room, user) {
    ...
    if (targetUser?.namelocked) {
      return this.errorReply(
        `User ${targetUser.name} is namelocked, not locked. Use /unnamelock to unnamelock them.`
      );
    }
    let reason = '';
    ...
    if (unlocked) {
      this.addGlobalModAction(
        `${unlocked.join(', ')} ${unlocked.length > 1 ? 'were' : 'was'} unlocked by ${user.name}.${reason}`
      );
  ...
  unlockname(target, room, user) {
    ...
    if (userid.startsWith('guest')) {
      return this.errorReply(
        `You cannot unlock the guest userid - provide their original username instead.`
      );
    }
    const punishment =
      Punishments.userids.getByType(userid, 'LOCK') ||
      Punishments.userids.getByType(userid, 'NAMELOCK');
    ...
    if (punishment.id === userid || Users.get(userid)?.previousIDs.includes(punishment.id as ID)) {
      return this.errorReply(
        `"${userid}" was specifically locked by a staff member (check the global modlog). Use /unlock if you really want to unlock this name.`
      );
    }
  ...
  unlockip(target, room, user) {
    ...
    this.privateGlobalModAction(
      `${user.name} unlocked the ${range ? 'IP range' : 'IP'}: ${target}`
    );
  ...
  unlocknamehelp: [
    `/unlockname [name] - Unlocks a punished alt, leaving the original lock intact. Requires: % @ ~`,
  ],
  unlockhelp: [
  ...
  async globalban(target, room, user, connection, cmd) {
    ...
    if (!targetUser && !force) {
      return this.errorReply(
        `User '${targetUsername}' not found. Use /forceglobalban to ban them anyway.`
      );
    }
    if (reason.length > MAX_REASON_LENGTH) {
      return this.errorReply(
    ...
    if (Users.isTrusted(userid)) {
      if (force) {
        const from = runCrisisDemote(userid);
        Monitor.log(
          `[CrisisMonitor] ${name} was globally banned by ${user.name} and demoted from ${from?.join(', ')}.`
        );
        this.globalModlog('CRISISDEMOTE', targetUser, ` from ${from?.join(', ')}`);
      } else {
        return this.sendReply(
          `${name} is a trusted user. If you are sure you would like to ban them use /forceglobalban.`
        );
      }
    }
    ...
    if (roomauth.length) {
      Monitor.log(
        `[CrisisMonitor] Globally banned user ${name} has public roomauth (${roomauth.join(', ')}), and should probably be demoted.`
      );
    }
    const { privateReason, publicReason } = this.parseSpoiler(reason);
    targetUser?.popup(
    ...
    this.addGlobalModAction(
      `${name} was globally banned by ${user.name}.${publicReason ? ` (${publicReason})` : ``}`
    );
  ...
  unbanip(target, room, user) {
    ...
    this.addGlobalModAction(
      `${user.name} unbanned the ${target.endsWith('*') ? 'IP range' : 'IP'}: ${target}`
    );
  ...
  yearlockname(target, room, user) {
    ...
    if (targetUser?.registered && !force) {
      return this.errorReply(
        `That user is registered. Either permalock them normally or use /forceyearlockname.`
      );
    }
    const punishment = {
      type: 'YEARLOCK',
    ...
    Punishments.savePunishments();
    this.addGlobalModAction(
      `${user.name} locked the userid '${targetUserid}' for a year${rest ? ` (${rest})` : ''}.`
    );
  ...
  lockip(target, room, user, connection, cmd) {
    ...
    this.addGlobalModAction(
      `${user.name} ${year ? 'year' : 'hour'}-${type.toLowerCase()}ed the ${ipDesc}: ${reason}`
    );
    this.globalModlog(
  ...
  modnote(target, room, user, connection) {
    ...
    if (target.length > MAX_REASON_LENGTH) {
      return this.errorReply(
        `The note is too long. It cannot exceed ${MAX_REASON_LENGTH} characters.`
      );
    }
    ...
    if (!targeted) {
      ...
      if (
        ...
        !(
          Users.get(targeted) ||
          Punishments.search(target).length ||
          IPTools.ipRegex.test(targeted)
        ) ||
  ...
  promote(target, room, user, connection, cmd) {
    ...
    const {
      ...
      targetUsername,
      rest: nextGroupName,
    } = this.splitUser(target, { exactName: true });
    const userid = toID(targetUsername);
    ...
    if (!Users.isUsernameKnown(userid)) {
      return this.errorReply(
        `/globalpromote - WARNING: '${name}' is offline and unrecognized. The username might be misspelled (either by you or the person who told you) or unregistered. Use /forcepromote if you're sure you want to risk it.`
      );
    }
    if (targetUser && !targetUser.registered) {
  ...
  promotehelp: [
    `/promote [username], [group] - Promotes the user to the specified group. Requires: ~`,
  ],
  ...
  trustuser(target, room, user, connection, cmd) {
    ...
    if (untrust) {
      if (currentGroup !== Users.Auth.defaultSymbol()) {
        return this.errorReply(
          `User '${name}' is trusted indirectly through global rank ${currentGroup}. Demote them from that rank to remove trusted status.`
        );
      }
      const trustedSourceRooms = Rooms.global.chatRooms
        .filter(
          authRoom =>
            authRoom.persist &&
            authRoom.settings.isPrivate !== true &&
            authRoom.auth.isStaff(userid)
        )
        .map(authRoom => authRoom.auth.get(userid) + authRoom.roomid)
        .join(' ');
      if (trustedSourceRooms.length && !Users.globalAuth.has(userid)) {
        return this.errorReply(
          `User '${name}' is trusted indirectly through room ranks ${trustedSourceRooms}. Demote them from those ranks to remove trusted status.`
        );
      }
      ...
      this.privateGlobalModAction(
        `${name} was set to no longer be a trusted user by ${user.name}.`
      );
  ...
  sectionleader(target, room, user, connection, cmd) {
    ...
    const userid = toID(targetUsername);
    const section = demoting
      ? Users.globalAuth.sectionLeaders.get(userid)!
      : room.validateSection(sectionid);
    const name = targetUser ? targetUser.name : targetUsername;
    if (Users.globalAuth.sectionLeaders.has(targetUser?.id || userid) && !demoting) {
      throw new Chat.ErrorMessage(
        `${name} is already a Section Leader of ${RoomSections.sectionNames[section]}.`
      );
    ...
    if (!demoting) {
      ...
      this.addGlobalModAction(
        `${name} was appointed Section Leader of ${RoomSections.sectionNames[section]} by ${user.name}.`
      );
      this.globalModlog(`SECTION LEADER`, userid, section);
      targetUser?.popup(
        `You were appointed Section Leader of ${RoomSections.sectionNames[section]} by ${user.name}.`
      );
    } else {
      const group = Users.globalAuth.get(userid);
      Users.globalAuth.deleteSection(userid);
      this.privateGlobalModAction(
        `${name} was demoted from Section Leader of ${RoomSections.sectionNames[section]} by ${user.name}.`
      );
      if (group === ' ')
        this.sendReply(
          `They are also no longer manually trusted. If they should be, use '/trustuser'.`
        );
      this.globalModlog(`DESECTION LEADER`, userid, section);
      targetUser?.popup(
        `You were demoted from Section Leader of ${RoomSections.sectionNames[section]} by ${user.name}.`
      );
    }
  ...
  demotehelp: [
    `/demote [username], [group] - Demotes the user to the specified group. Requires: ~`,
  ],
  ...
  forcepromote(target, room, user, connection) {
    ...
    this.addGlobalModAction(
      `${name} was promoted to Global ${Config.groups[nextGroup].name || 'regular user'} by ${user.name}.`
    );
    this.globalModlog(
      `GLOBAL${(Config.groups[nextGroup].name || 'regular').toUpperCase()}`,
      toID(name)
    );
  },
  ...
  htmldeclare(target, room, user) {
    ...
    for (const u in room.users) {
      Users.get(u)?.sendTo(room, `|notify|${room.title} announcement!|${Utils.stripHTML(target)}`);
    }
    this.add(`|raw|<div class="broadcast-blue"><b>${target}</b></div>`);
    this.modlog(`HTMLDECLARE`, null, target);
  },
  htmldeclarehelp: [
    `/htmldeclare [message] - Anonymously announces a message using safe HTML. Requires: # * ~`,
  ],
  ...
  globaldeclare(target, room, user) {
    ...
    for (const u of Users.users.values()) {
      if (u.connected)
        u.send(
          `|pm|~|${u.tempGroup}${u.name}|/raw <div class="broadcast-blue"><b>${target}</b></div>`
        );
    }
    this.globalModlog(`GLOBALDECLARE`, null, target);
  },
  globaldeclarehelp: [
    `/globaldeclare [message] - Anonymously sends a private message to all the users on the site. Requires: ~`,
  ],
  ...
  chatdeclarehelp: [
    `/cdeclare [message] - Anonymously announces a message to all chatrooms on the server. Requires: ~`,
  ],
  ...
  forcerename(target, room, user) {
    ...
    if (!targetUser && !offline) {
      ...
      if (targetUserInexact) {
        return this.errorReply(
          `User has already changed their name to '${targetUserInexact.name}'.`
        );
      }
      return this.errorReply(
        `User '${inputUsername}' not found. (use /offlineforcerename to rename anyway.)`
      );
    }
    if (Punishments.namefilterwhitelist.has(targetID)) {
      ...
      if (user.can('bypassall')) {
        this.errorReply(
          `Use /noforcerename remove to remove it from the list if you wish to rename it.`
        );
      }
      return false;
    }
    ...
    if (targetUser?.connected) {
      ...
      targetUser.send(
        `|nametaken||${user.name} considers your name inappropriate${publicReason ? `: ${publicReason}` : ``}`
      );
    } else {
    ...
    const roomMessage = this.pmTarget
      ? `<PM:${this.pmTarget.id}>`
      : room && room.roomid !== 'staff'
        ? `«<a href="/${room.roomid}" target="_blank">${room.roomid}</a>» `
        : '';
    const rankMessage = targetUser?.getAccountStatusString() || '';
    Rooms.global.notifyRooms(
      ['staff'],
      `|html|${roomMessage}` +
        Utils.html`<span class="username">${targetUser?.name || targetID}</span> ${rankMessage} ${forceRenameMessage}`
    );
  ...
  noforcerename: {
    add(target, room, user) {
      ...
      this.addGlobalModAction(
        `${user.name} added the name ${targetId} to the no forcerename list.${rest ? ` (${rest})` : ''}`
      );
    ...
    remove(target, room, user) {
      ...
      this.addGlobalModAction(
        `${user.name} removed ${targetId} from the no forcerename list.${rest ? ` (${rest})` : ''}`
      );
  ...
  forceclearstatus(target, room, user) {
    ...
    this.privateGlobalModAction(
      this
        .tr`${targetUser.name}'s status "${targetUser.userMessage}" was cleared by ${user.name}${displayReason}.`
    );
    this.globalModlog(
      'CLEARSTATUS',
      targetUser,
      ` from "${targetUser.userMessage}"${displayReason}`
    );
    targetUser.clearStatus();
    targetUser.popup(
      `${user.name} has cleared your status message for being inappropriate${displayReason || '.'}`
    );
  },
  ...
  async namelock(target, room, user, connection, cmd) {
    ...
    if (targetUser && targetUser.id !== toID(inputUsername) && !force) {
      return this.errorReply(
        `${inputUsername} has already changed their name to ${targetUser.name}. To namelock anyway, use /force${week ? 'week' : ''}namelock.`
      );
    }
    ...
    this.globalModlog(
      `${force ? `FORCE` : ``}${week ? 'WEEK' : ''}NAMELOCK`,
      targetUser || userid,
      privateReason
    );
    ...
    if (roomauth.length) {
      Monitor.log(
        `[CrisisMonitor] Namelocked user ${userid} has public roomauth (${roomauth.join(', ')}), and should probably be demoted.`
      );
    }
    if (targetUser) {
      Ladders.cancelSearches(targetUser);
  ...
  namelockhelp: [
    `/namelock OR /nl [user], [reason] - Name locks a [user] and shows the [reason]. Requires: % @ ~`,
  ],
  ...
  hidetext(target, room, user, connection, cmd) {
    ...
    if (/^[0-9]+\s*(,|$)/.test(reason)) {
      if (hasLineCount) {
        ...
        return this.errorReply(
          `Your reason was a number; use /hidelines if you wanted to clear a specific number of lines, or /forcehidetext if you really wanted your reason to be a number.`
        );
      }
    }
    ...
    if (!lineCount && hasLineCount) {
      return this.errorReply(
        `You must specify a number of messages to clear. To clear all messages, use /hidetext.`
      );
    }
    if (reason.length > MAX_REASON_LENGTH) {
      return this.errorReply(
  ...
  blacklist(target, room, user, connection, cmd) {
    ...
    if (!room.persist) {
      return this.errorReply(
        `This room is not going to last long enough for a blacklist to matter - just ban the user`
      );
    }
    ...
    if (targetUser.trusted && room.settings.isPrivate !== true) {
      Monitor.log(
        `[CrisisMonitor] Trusted user ${targetUser.name}${targetUser.trusted !== targetUser.id ? ` (${targetUser.trusted})` : ''} was blacklisted from ${room.roomid} by ${user.name}, and should probably be demoted.`
      );
    }
    ...
    if (!room.settings.isPrivate && room.persist) {
      ...
      let displayMessage = '';
      if (affected.length > 1) {
        displayMessage = `${name}'s ${acAccount ? ` ac account: ${acAccount},` : ''} blacklisted alts: ${affected
          .slice(1)
          .map(curUser => curUser.getLastName())
          .join(', ')}`;
        this.privateModAction(displayMessage);
  ...
  async battleban(target, room, user, connection, cmd) {
    ...
    if (!room.battle && !includesUrl && cmd !== 'forcebattleban') {
      return this.errorReply(
        `Battle bans require a battle replay if used outside of a battle; if the battle has expired, use /forcebattleban.`
      );
    }
    if (!user.can('rangeban', targetUser)) {
    ...
    this.privateGlobalModAction(
      `${targetUser.name} was banned from starting new battles by ${user.name} (${reason})`
    );

    if (targetUser.trusted) {
      Monitor.log(
        `[CrisisMonitor] Trusted user ${targetUser.name} was banned from battling by ${user.name}, and should probably be demoted.`
      );
    }
    ...
    targetUser.popup(
      `|modal|${user.name} has prevented you from starting new battles for 2 days (${reason})`
    );
  ...
  unbattlebanhelp: [
    `/unbattleban [username] - [DEPRECATED] Allows a user to battle again. Requires: % @ ~`,
  ],
  ...
  async groupchatban(target, room, user, connection, cmd) {
    ...
    this.privateGlobalModAction(
      `${targetUser.name} was banned from using groupchats for a ${isMonth ? 'month' : 'week'} by ${user.name}${reasonText}.`
    );

    if (targetUser.trusted) {
      Monitor.log(
        `[CrisisMonitor] Trusted user ${targetUser.name} was banned from using groupchats by ${user.name}, and should probably be demoted.`
      );
    }
    ...
    targetUser.popup(
      `|modal|${user.name} has banned you from using groupchats for a ${isMonth ? 'month' : 'week'}${reasonText}`
    );
  ...
  ungroupchatbanhelp: [
    `/ungroupchatban [user] - Allows a groupchatbanned user to use groupchats again. Requires: % @ ~`,
  ],
  ...
  blacklistname(target, room, user) {
    ...
    if (duplicates.length) {
      return this.errorReply(
        `[${duplicates.join(', ')}] ${Chat.plural(duplicates, 'are', 'is')} already blacklisted.`
      );
    }
    const expireTime = this.cmd.includes('perma')
    ...
    for (const userid of targets) {
      ...
      if (!Users.Auth.hasPermission(user, 'ban', room.auth.get(userid), room)) {
        return this.errorReply(
          `/blacklistname - Access denied: ${userid} is of equal or higher authority than you.`
        );
      }
      ...
      if (trusted && room.settings.isPrivate !== true) {
        Monitor.log(
          `[CrisisMonitor] Trusted user ${userid}${trusted !== userid ? ` (${trusted})` : ``} was nameblacklisted from ${room.roomid} by ${user.name}, and should probably be demoted.`
        );
      }
      if (!room.settings.isPrivate && room.persist) {
  ...
  unblacklistallhelp: [
    `/unblacklistall - Unblacklists all blacklisted users in the current room. Requires: # ~`,
  ],
