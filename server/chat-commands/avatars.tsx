...
const AVATAR_FORMATS_MESSAGE =
  Config.serverid === 'showdown'
...
if ((Config.customavatars && Object.keys(Config.customavatars).length) || Config.allowedavatars) {
  Monitor.crashlog(
    "Please remove 'customavatars' and 'allowedavatars' from Config (config/config.js). Your avatars have been migrated to the new '/addavatar' system."
  );
}
function saveCustomAvatars(instant?: boolean) {
  FS(AVATARS_FILE).writeUpdate(() => JSON.stringify(customAvatars), {
    throttle: instant ? null : 60_000,
  });
}
...
export const Avatars = new (class {
  ...
  async validate(avatar: string, options?: { rejectOfficial?: boolean }) {
    ...
    if (!/^#?[a-z0-9-]+$/.test(avatar) && !/^[a-z0-9.-]+$/.test(avatar)) {
      throw new Chat.ErrorMessage(
        `Avatar "${avatar}" is not in a valid format. ${AVATAR_FORMATS_MESSAGE}`
      );
    }
    ...
    if (options?.rejectOfficial && /^[a-z0-9-]+$/.test(avatar)) {
      throw new Chat.ErrorMessage(
        `Avatar "${avatar}" is an official avatar that all users already have access to.`
      );
    }
    return avatar;
  }
  ...
  getDefault(userid: ID) {
    ...
    if (
      new Date().getMonth() === DECEMBER &&
      entry.allowed.some(avatar => avatar?.endsWith('xmas'))
    ) {
    ...
    if (entry?.notNotified) {
      user.send(
        `|pm|~|${user.getIdentity()}|/raw ` +
          Chat.html`${(
            <>
              <p>You have a new custom avatar!</p>
              <p>{entry.allowed.map(avatar => avatar && [Avatars.img(avatar), ' '])}</p>
              Use{' '}
              <button class="button" name="send" value="/avatars">
                <code>/avatars</code>
              </button>{' '}
...
function listUsers(users: string[]) {
  return users.flatMap((userid, i) => [
    i ? ', ' : null,
    <username class="username">{userid}</username>,
  ]);
}
...
const OFFICIAL_AVATARS_HYOOPPA = new Set(['brendan', 'brendan-e', 'maxie-gen6', 'may']);
...
const OFFICIAL_AVATARS_FIFTY = new Set(['rose-zerosuit']);

const OFFICIAL_AVATARS_HORO = new Set(['florian-bb', 'juliana-bb', 'red-lgpe', 'liko', 'roy']);

const OFFICIAL_AVATARS_SELENA = new Set(['kris']);
...
export const commands: Chat.ChatCommands = {
  avatar(target, room, user) {
    ...
    if (!silent) {
      ...
      if (OFFICIAL_AVATARS_BELIOT419.has(avatar)) {
        this.sendReply(
          `|raw|(${this.tr`Artist: `}<a href="https://www.deviantart.com/beliot419">Beliot419</a>)`
        );
      }
      ...
      if (OFFICIAL_AVATARS_BRUMIRAGE.has(avatar)) {
        this.sendReply(
          `|raw|(${this.tr`Artist: `}<a href="https://twitter.com/Brumirage">Brumirage</a>)`
        );
      }
      ...
      if (OFFICIAL_AVATARS_KYLEDOVE.has(avatar)) {
        this.sendReply(
          `|raw|(${this.tr`Artist: `}<a href="https://twitter.com/DoveKyle">kyledove</a>)`
        );
      }
      if (OFFICIAL_AVATARS_HYOOPPA.has(avatar)) {
        this.sendReply(
          `|raw|(${this.tr`Artist: `}<a href="https://twitter.com/hyo_oppa">hyo-oppa</a>)`
        );
      }
      if (OFFICIAL_AVATARS_GRAPO.has(avatar)) {
        this.sendReply(
          `|raw|(${this.tr`Artist: `}<a href="https://twitter.com/Grapo_Sprites">Grapo</a>)`
        );
      }
      ...
      if (OFFICIAL_AVATARS_SELENA.has(avatar)) {
        this.sendReply(
          `|raw|(${this.tr`Artist: `}<a href="https://twitter.com/SelenaStar00">Selena</a>)`
        );
      }
    }
  },
  ...
  avatars(target, room, user) {
    ...
    const targetUserids = targetUser
      ? new Set([targetUser.id, ...targetUser.previousIDs])
      : target
        ? new Set([toID(target)])
        : null;
    ...
    if (targetUserids) {
      ...
      for (const id of targetUserids) {
        ...
        if (allowed) {
          out.push(
            <p>
              Custom avatars from account <strong>{id}</strong>:
            </p>,
            allowed.filter(Boolean).map(avatar => (
              <p>
                {hasButton ? (
                  <button name="send" value={`/avatar ${avatar}`} class="button">
                    {Avatars.img(avatar!)}
                  </button>
                ) : (
                  Avatars.img(avatar!)
      ...
      if (!out.length && target) {
        out.push(
          <p>
            User <strong>{toID(target)}</strong> doesn't have any custom avatars.
          </p>
        );
      }
    }
    if (!out.length) {
      out.push(
        <p>Custom avatars require you to be a contributor/staff or win a tournament prize.</p>
      );
    }
    ...
    this.sendReplyBox(
      <>
        {!target && [
          <p>
            You can{' '}
            <button name="avatars" class="button">
              change your avatar
            </button>{' '}
            by clicking on it in the {}
            <button name="openOptions" class="button" aria-label="Options">
              <i class="fa fa-cog"></i>
            </button>{' '}
            menu in the upper {}
            right.
          ...
          <p>
            ...
            <a href="https://play.pokemonshowdown.com/sprites/trainers/">
              <strong>full list of avatars</strong>
            </a>
            . {}
            You can use them by typing{' '}
            <code>
              /avatar <i>[avatar's name]</i>
            </code>{' '}
  ...
  removeavatar(target, room, user) {
    ...
    if (avatar) {
      ...
      this.sendReplyBox(
        <div>
          {allowed.map(curAvatar => [Avatars.img(curAvatar!), ' '])}
          <br />
          Removed from <username class="username">{inputUsername}</username>
        </div>
      );
  ...
  async masspavatar(target, room, user) {
    ...
    this.sendReplyBox(
      <div>
        {userids.map(userid => Avatars.img('#' + userid))}
        <br />
  ...
  async massxmasavatar(target, room, user) {
    ...
    this.sendReplyBox(
      <div>
        {userids.map(userid => Avatars.img(`#${userid}xmas`))}
        <br />
        Added {userids.length} avatars
      </div>
    );
  ...
  async massgavatar(target, room, user) {
    ...
    for (const avatar in toUpdate) {
      ...
      if (added.length)
        out.push(
          <div>
            {oldUsers.size ? 'Added' : 'New'}: {listUsers(added)}
          </div>
        );
