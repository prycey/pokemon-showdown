...
export const crqHandlers: { [k: string]: Chat.CRQHandler } = {
  userdetails(target, user, trustable) {
    ...
    for (const roomid of targetUser.inRooms) {
      ...
      if (targetRoom.battle) {
        if (
          targetUser.settings.hideBattlesFromTrainerCard &&
  ...
  roominfo(target, user, trustable) {
    ...
    if (
      !targetRoom ||
      (targetRoom.settings.isPrivate &&
        !user.inRooms.has(targetRoom.roomid) &&
        !user.games.has(targetRoom.roomid))
    ) {
  ...
  cmdsearch(target, user, trustable) {
    ...
    for (const command of Chat.allCommands()) {
      ...
      if (
        !!req &&
        !(command.hasRoomPermissions
          ? !!this.room && user.can(req as RoomPermission, null, this.room)
          : user.can(req))
      ) {
        continue;
      }
...
export const commands: Chat.ChatCommands = {
  ...
  versionhelp: [`/version - Get the current server version.`],
  ...
  tablefliphelp: [
    '/tableflip [message] - Sends the given message, if any, appended with (╯°□°）╯︵ ┻━┻',
  ],
  ...
  tableunfliphelp: [
    '/tableunflip [message] - Sends the given message, if any, appended with ┬──┬◡ﾉ(° -°ﾉ)',
  ],
  ...
  battle(target, room, user, connection, cmd) {
    if (cmd === 'battle') {
      return this.sendReply(
        this
          .tr`What?! How are you not more excited to battle?! Try /battle! to show me you're ready.`
      );
    }
  ...
  async linksmogon(target, room, user) {
    ...
    if (response[1] || !name) {
      throw new Chat.ErrorMessage(
        'Error while verifying username: ' + (response[1]?.message || 'malformed name received')
      );
    }
    const link = `https://www.smogon.com/tools/connect-ps-account/${user.id}/${name}`;
    user.send(`|openpage|${link}`);
    this.sendReply(
      `|html|If the page failed to open, you may link your Smogon and PS accounts by clicking <a href="${link}">this link</a>.`
    );
  },
  ...
  replyhelp: [
    `/reply OR /r [message] - Send a message to the last user you got a message from, or sent a message to.`,
  ],
    ...
    if (targetUsername === '~') {
      ...
      if (Chat.PrivateMessages.offlineIsEnabled) {
        ...
        return this.errorReply(
          this
            .tr`User ${targetUsername} is offline. Send the message again to confirm. If you are using /msg, use /offlinemsg instead.`
        );
      }
  ...
  offlinepmshelp: [`/offlinepms - View your recently received offline PMs.`],
  ...
  invite(target, room, user) {
    ...
    if (invitesBlocked) {
      if (
        invitesBlocked === true
          ? !user.can('lock')
          : !Users.globalAuth.atLeast(user, invitesBlocked as GroupSymbol)
      ) {
    ...
    if (!targetRoom.checkModjoin(pmTarget)) {
      ...
      if (!targetRoom.checkModjoin(pmTarget)) {
        return this.errorReply(
          this.tr`You do not have permission to invite people into this room.`
        );
      }
    }
  ...
  async blockpms(target, room, user, connection, cmd) {
    ...
    if (!isOffline && user.settings.blockPMs === (target || true)) {
      return this.errorReply(
        this.tr`You are already blocking ${msg}private messages! To unblock, use /unblockpms`
      );
    }
    if (Users.Auth.isAuthLevel(target)) {
      if (!isOffline) user.settings.blockPMs = target;
      this.sendReply(
        this.tr`You are now blocking ${msg}private messages, except from staff and ${target}.`
      );
    } else if (target === 'autoconfirmed' || target === 'trusted' || target === 'unlocked') {
      if (!isOffline) user.settings.blockPMs = target;
      target = this.tr(target);
      this.sendReply(
        this.tr`You are now blocking ${msg}private messages, except from staff and ${target} users.`
      );
    } else if (target === 'friends') {
      if (!isOffline) user.settings.blockPMs = target;
      this.sendReply(
        this.tr`You are now blocking ${msg}private messages, except from staff and friends.`
      );
    } else {
  ...
  async unblockpms(target, room, user, connection, cmd) {
    ...
    if (isOffline ? !(await Chat.PrivateMessages.getSettings(user.id)) : !user.settings.blockPMs) {
      return this.errorReply(
        this.tr`You are not blocking ${msg}private messages! To block, use /blockpms`
      );
    }
    if (isOffline) {
  ...
  blockinvites(target, room, user, connection, cmd) {
    ...
    if (unblock) {
      ...
      if (user.settings.blockInvites === (target || true)) {
        return this.errorReply(
          'You are already blocking room invites - to unblock, use /unblockinvites'
        );
      }
      if (target in Config.groups) {
        user.settings.blockInvites = target as GroupSymbol;
        this.sendReply(
          this.tr`You are now blocking room invites, except from staff and ${target}.`
        );
      } else if (target === 'autoconfirmed' || target === 'trusted' || target === 'unlocked') {
        user.settings.blockInvites = target;
        this.sendReply(
          this.tr`You are now blocking room invites, except from staff and ${target} users.`
        );
      } else {
  ...
  status(target, room, user, connection, cmd) {
    if (user.locked || user.semilocked) {
      return this.errorReply(
        this.tr`Your status cannot be updated while you are locked or semilocked.`
      );
    }
    ...
    if (target.length > maxLength) {
      return this.errorReply(
        this.tr`Your status is too long; it must be under ${maxLength} characters.`
      );
    }
  ...
  busy(target, room, user, connection, cmd) {
    if (target) {
      this.errorReply(
        this
          .tr`Setting status messages in /busy is no longer supported. Set a status using /status.`
      );
    }
  ...
  away(target, room, user, connection, cmd) {
    if (target) {
      this.errorReply(
        this
          .tr`Setting status messages in /away is no longer supported. Set a status using /status.`
      );
    }
    user.setStatusType('idle');
    this.sendReply(
      this.tr`You are now marked as away. Send a message or use /back to indicate you are back.`
    );
  },
  ...
  hiderank(target, room, user, connection, cmd) {
    ...
    const group = isShow
      ? Users.globalAuth.get(user.id)
      : ((target.trim() || Users.Auth.defaultSymbol()) as GroupSymbol);
    ...
    user.updateIdentity();
    this.sendReply(
      `|c|~|${this.tr`Your temporary group symbol is now`} \`\`${user.tempGroup}\`\`.`
    );
  },
  ...
  language(target, room, user) {
    ...
    return this.sendReply(
      `|html|` +
        this.tr`Pokémon Showdown will now be displayed in ${language} (except in language rooms).`
    );
  },
  languagehelp: [
  ...
  allowexportinputlog(target, room, user) {
    ...
    if (battle.allowExtraction[targetUser.id].has(user.id)) {
      return this.errorReply(
        this.tr`You have already consented to extraction with ${targetUser.name}.`
      );
    }
    battle.allowExtraction[targetUser.id].add(user.id);
    this.addModAction(
      room.tr`${user.name} consents to sharing battle team and choices with ${targetUser.name}.`
    );
  ...
  exportinputlog(target, room, user) {
    ...
    if (!battle.inputLog) {
      this.errorReply(
        this
          .tr`This command only works when the battle has ended - if the battle has stalled, use /offertie.`
      );
  ...
  exportinputloghelp: [
    `/exportinputlog - Asks players in a battle for permission to export an inputlog. Requires: ~`,
  ],
  ...
  importinputlog(target, room, user, connection) {
    ...
    if ((`\n` + target).includes(`\n>eval `) && !user.hasConsoleAccess(connection)) {
      return this.errorReply(
        this.tr`Your input log contains untrusted code - you must have console access to use it.`
      );
    }
    ...
    setTimeout(() => {
      ...
      battleRoom.add(
        `|uhtmlchange|invites|<div class="broadcast broadcast-blue"><strong>This is an imported replay</strong><br />Players will need to be manually added with <code>/invitebattle</code> or <code>/restoreplayers</code></div>`
      );
  ...
  async showset(target, room, user, connection, cmd) {
    ...
    if (!showAll) {
      ...
      if (isNaN(parsed)) {
        ...
        if (!matchedSet)
          return this.errorReply(
            this.tr`You don't have a Pokémon matching "${target}" in your team.`
          );
        team = [matchedSet];
      } else {
        ...
        if (!indexedSet) {
          return this.errorReply(
            this
              .tr`You don't have a Pokémon #${parsed} on your team - your team only has ${team.length} Pokémon.`
          );
        }
        team = [indexedSet];
      }
  ...
  acceptopenteamsheets(target, room, user, connection, cmd) {
    ...
    if (!Dex.formats.getRuleTable(format).has('openteamsheets')) {
      return this.errorReply(
        this
          .tr`This format does not allow requesting open team sheets. You can both manually agree to it by using !showteam hidestats.`
      );
    }
    if (battle.turn > 0) {
      return this.errorReply(
        this
          .tr`You cannot agree to open team sheets after Team Preview. Each player can still show their own sheet by using this command: !showteam hidestats`
      );
    }
    ...
    if (player.wantsOpenTeamSheets !== null) {
      return this.errorReply(
        this.tr`You have already made your decision about agreeing to open team sheets.`
      );
    }
  ...
  acceptopenteamsheetshelp: [
    `/acceptopenteamsheets - Agrees to an open team sheet opportunity during Team Preview, where all information on a team except stats is shared with the opponent. Requires: \u2606`,
  ],
  ...
  rejectopenteamsheetshelp: [
    `/rejectopenteamsheetshelp - Rejects an open team sheet opportunity during Team Preview, where all information on a team except stats is shared with the opponent. Requires: \u2606`,
  ],
  ...
  offertie(target, room, user, connection, cmd) {
    ...
    if (cmd === 'accepttie' && !battle.players.some(player => player.wantsTie)) {
      return this.errorReply(
        this.tr`No other player is requesting a tie right now. It was probably canceled.`
      );
    }
    const player = battle.playerTable[user.id];
  ...
  offertiehelp: [
    `/offertie - Offers a tie to all players in a battle; if all accept, it ties. Can only be used after 100+ turns have passed. Requires: \u2606 @ # ~`,
  ],
  ...
  forfeithelp: [`/forfeit - Forfeits your currently active game, if it supports that.`],
  ...
  choosehelp: [`/choose [text] - Make a choice for the currently active game.`],
  ...
  movehelp: [`/move [move] - Make a move for the current game.`],

  sw: 'switch',
  switch(target, room, user) {
    this.parse(`/choose switch ${target}`);
  },
  switchhelp: [`/switch [pokemon] - Make a switch for the current game.`],

  team(target, room, user) {
    this.parse(`/choose team ${target}`);
  },
  teamhelp: [`/team [pokemon] - Change your team for the current game.`],
  ...
  undohelp: [`/undo - Reverts the last move of the player in the current game, if it supports it.`],
  ...
  hidereplay(target, room, user, connection) {
    ...
    if (room.tour?.forcePublic) {
      return this.errorReply(
        this
          .tr`This battle can't have hidden replays, because the tournament is set to be forced public.`
      );
    }
    if (room.hideReplay)
  ...
  hidereplayhelp: [
    `/hidereplay - Hides the replay of the current battle. Requires: ${Users.PLAYER_SYMBOL} ~`,
  ],
  ...
  invitebattle(target, room, user, connection) {
    ...
    if (player.invite) {
      battle.sendInviteForm(connection);
      return this.errorReply(
        `Someone else (${player.invite}) has already been invited to be ${slot}!`
      );
    }
    ...
    if (targetUser.settings.blockChallenges && !user.can('bypassblocks', targetUser)) {
      ...
      return this.errorReply(
        this.tr`The user '${targetUser.name}' is not accepting challenges right now.`
      );
    }
    ...
    if (!targetUser.inRooms.has(room.roomid) || !player.hasTeam) {
      ...
      const playerNames = battle.players
        .map(p => p.id && p.name)
        .filter(Boolean)
        .join(', ');
      const ready = player.hasTeam
        ? battle.format
        : new Ladders.BattleReady(user.id, battle.format, user.battleSettings);
  ...
  async acceptbattle(target, room, user, connection) {
    ...
    if (player.id) {
      throw new Error(
        `Player ${player.slot} in ${chall.roomid} should not have both 'id' and 'invite'`
      );
    }
  ...
  uninvitebattle(target, room, user, connection) {
    ...
    for (const challenge of challenges) {
      if (challenge.to === target && challenge.roomid === room.roomid) {
        ...
        Ladders.challenges.send(
          challenge.from,
          target,
          `/text The battle invite was changed to someone else; sorry!`
        );
      }
    }
  ...
  restoreplayers(target, room, user) {
    ...
    if (!didSomething) {
      return this.errorReply(
        this.tr`Players could not be restored (maybe this battle already has two players?).`
      );
    }
  },
  restoreplayershelp: [`/restoreplayers - Restore previous players in an imported input log.`],
  ...
  kickgame(target, room, user) {
    ...
    if (room.battle.leaveGame(targetUser)) {
      ...
      this.addModAction(
        room.tr`${targetUser.name} was kicked from a battle by ${user.name}.${displayReason}`
      );
  ...
  kickinactivehelp: [`/kickinactive - Activates the inactive timer, if the game supports it.`],
  ...
  timer(target, room, user) {
    ...
    if (this.meansNo(target) || target === 'stop') {
      if (timer.timerRequesters.size) {
        ...
        if (force) {
          room.send(
            `|inactiveoff|${room.tr`Timer was turned off by staff. Please do not turn it back on until our staff say it's okay.`}`
          );
        }
      } else {
  ...
  forcetimer(target, room, user) {
    ...
    if (this.meansNo(target) || target === 'stop') {
      ...
      this.addModAction(
        room.tr`Forcetimer is now OFF: The timer is now opt-in. (set by ${user.name})`
      );
    } else if (this.meansYes(target) || target === 'start' || !target) {
      Config.forcetimer = true;
      this.addModAction(
        room.tr`Forcetimer is now ON: All battles will be timed. (set by ${user.name})`
      );
    } else {
  ...
  forcewin(target, room, user) {
    ...
    if (
      ...
      !(
        room.game &&
        typeof (room.game as any).tie === 'function' &&
        typeof (room.game as any).win === 'function'
      )
    ) {
  ...
  async search(target, room, user, connection) {
    if (target) {
      if (Config.laddermodchat && !Users.globalAuth.atLeast(user, Config.laddermodchat)) {
        ...
        this.popupReply(
          this
            .tr`This server requires you to be rank ${groupName} or higher to search for a battle.`
        );
        return false;
      }
      ...
      if (
        !user.registered &&
        Config.forceregisterelo &&
        (await ladder.getRating(user.id)) >= Config.forceregisterelo
      ) {
        user.send(
  ...
  searchhelp: [`/search [format] - Searches for a battle in the specified format.`],
  ...
  challenge(target, room, user, connection) {
    ...
    if (user.locked && !targetUser.locked) {
      return this.popupReply(
        this
          .tr`You are locked and cannot challenge unlocked users. If this user is your friend, ask them to challenge you instead.`
      );
    }
    ...
    if (
      Config.pmmodchat &&
      !user.hasSysopAccess() &&
      !Users.globalAuth.atLeast(user, Config.pmmodchat as GroupSymbol)
    ) {
      const groupName = Config.groups[Config.pmmodchat].name || Config.pmmodchat;
      this.popupReply(
        this.tr`This server requires you to be rank ${groupName} or higher to challenge users.`
      );
      return false;
    }
  ...
  blockchallenges(target, room, user) {
    ...
    if (Users.Auth.isAuthLevel(target)) {
      ...
      target = this.tr(target);
      this.sendReply(
        this.tr`You are now blocking challenges, except from staff and ${target} users.`
      );
    } else {
  ...
  cancelchallenge(target, room, user, connection) {
    ...
    if (!chall || chall.from !== user.id) {
      connection.popup(
        `You are not challenging ${this.pmTarget.name}. Maybe they accepted/rejected before you cancelled?`
      );
      return false;
    }
  ...
  async accept(target, room, user, connection) {
    ...
    if (!chall || chall.to !== user.id) {
      connection.popup(
        `${this.pmTarget.id} is not challenging you. Maybe they cancelled before you accepted?`
      );
      return false;
    }
    ...
    this.sendChatMessage(
      Utils.html`/nonotify ${user.name} accepted the challenge, starting &laquo;<a href="/${gameRoom.roomid}">${gameRoom.roomid}</a>&raquo;`
    );
    return true;
  },
  ...
  reject(target, room, user, connection) {
    ...
    if (!chall || chall.to !== user.id) {
      connection.popup(
        `${this.pmTarget.id} is not challenging you. Maybe they cancelled before you rejected?`
      );
      return false;
    }
  ...
  useteamhelp: [
    `/useteam [packed team] - Sets your team for your next battles to the given [team].`,
  ],
  ...
  vtm(target, room, user, connection) {
    ...
    const format =
      originalFormat.effectType === 'Format' ? originalFormat : Dex.formats.get('Anything Goes');
    if (format.effectType !== 'Format')
      return this.popupReply(this.tr`Please provide a valid format.`);

    return TeamValidatorAsync.get(format.id)
      .validateTeam(user.battleSettings.team, { user: user.id })
      .then(result => {
        const matchMessage =
          originalFormat === format
            ? ''
            : this.tr`The format '${originalFormat.name}' was not found.`;
  ...
  help(target, room, user) {
    ...
    if (!target) {
      const broadcastMsg = this
        .tr`(replace / with ! to broadcast. Broadcasting requires: + % @ # ~)`;

      this.sendReply(
        `${this.tr`COMMANDS`}: /report, /msg, /reply, /logout, /challenge, /search, /rating, /whois, /user, /join, /leave, /userauth, /roomauth`
      );
      this.sendReply(
        `${this.tr`BATTLE ROOM COMMANDS`}: /savereplay, /hideroom, /inviteonly, /invite, /timer, /forfeit`
      );
      this.sendReply(
        `${this.tr`OPTION COMMANDS`}: /nick, /avatar, /ignore, /status, /away, /busy, /back, /timestamps, /highlight, /showjoins, /hidejoins, /blockchallenges, /blockpms`
      );
      this.sendReply(
        `${this.tr`INFORMATIONAL/RESOURCE COMMANDS`}: /groups, /faq, /rules, /intro, /formatshelp, /othermetas, /analysis, /punishments, /calc, /git, /cap, /roomhelp, /roomfaq ${broadcastMsg}`
      );
      this.sendReply(
        `${this.tr`DATA COMMANDS`}: /data, /dexsearch, /movesearch, /itemsearch, /learn, /statcalc, /effectiveness, /weakness, /coverage, /randommove, /randompokemon ${broadcastMsg}`
      );
      if (user.tempGroup !== Users.Auth.defaultSymbol()) {
        this.sendReply(
          `${this.tr`DRIVER COMMANDS`}: /warn, /mute, /hourmute, /unmute, /alts, /forcerename, /modlog, /modnote, /modchat, /lock, /weeklock, /unlock, /announce`
        );
        this.sendReply(
          `${this.tr`MODERATOR COMMANDS`}: /globalban, /unglobalban, /ip, /markshared, /unlockip`
        );
        this.sendReply(
          `${this.tr`ADMIN COMMANDS`}: /declare, /forcetie, /forcewin, /promote, /demote, /banip, /host, /ipsearch`
        );
      }
    ...
    if (!getHelp(Chat.commands, target.split(' '))) {
      throw new Chat.ErrorMessage(
        this.tr`Could not find help for '/${target}'. Try /help for general help.`
      );
    }
  },
  helphelp: [
