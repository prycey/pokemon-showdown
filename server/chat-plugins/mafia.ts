...
class MafiaPlayer extends Rooms.RoomGamePlayer<Mafia> {
  ...
  isTreestump() {
    return (
      this.eliminated === MafiaEliminateType.TREESTUMP ||
  ...
  isSpirit() {
    return (
      this.eliminated === MafiaEliminateType.SPIRIT ||
      this.eliminated === MafiaEliminateType.SPIRITSTUMP
    );
  }
  ...
  tryUpdateHtmlRoom() {
    if (
      ...
      ].includes(this.eliminated as any)
    ) {
      this.updateHtmlRoom();
    }
  }
...
class Mafia extends Rooms.RoomGame<MafiaPlayer> {
  ...
  join(user: User, staffAdd: User | null = null, force = false) {
    ...
    if (this.started) {
      player.role = {
        ...
        image: '',
        memo: [
          `You were added to the game after it had started. To learn about your role, PM the host (${this.host}).`,
        ],
      };
  ...
  static isHostBanned(room: Room, user: User) {
    return (
      Mafia.isGameBanned(room, user) ||
      Punishments.hasRoomPunishType(room, toID(user), 'MAFIAGAMEBAN')
    );
  }
  ...
  getPlayer(userid: ID) {
    ...
    if (matches.length > 1) {
      // Should never happen
      throw new Error(
        `Duplicate player IDs in Mafia game! Matches: ${matches.map(p => p.id).join(', ')}`
      );
    }
  ...
  setRoles(user: User, roleString: string, force = false, reset = false) {
    ...
    if (force) {
      ...
      this.originalRoleString = this.originalRoles
        .map(
          r =>
            `<span style="font-weight:bold;color:${MafiaData.alignments[r.alignment].color || '#FFF'}">${r.safeName}</span>`
        )
        .join(', ');
    ...
    if (problems.length) {
      ...
      return this.sendUser(
        user,
        `|error|To forcibly set the roles, use /mafia force${reset ? 're' : ''}setroles`
      );
    }
  ...
  resetGame() {
    ...
    if (this.takeIdles) {
      this.sendDeclare(
        `Night ${this.dayNum}. Submit whether you are using an action or idle. If you are using an action, DM your action to the host.`
      );
    } else {
  ...
  static parseRole(roleString: string) {
      ...
      while (currentWord.length) {
        ...
        if (currentSearch in MafiaData.roles) {
          ...
          if (role.alignment && role.alignment !== currentSearch) {
            problems.push(
              `The role ${roleString} has multiple possible alignments (${role.alignment} and ${currentSearch})`
            );
          }
    ...
    if (problems.length) {
      ...
      if (alignment) {
        ...
        problems.push(
          `Alignment desync: role ${role.name}'s alignment ${role.alignment} doesn't exist in data. Please report this to a mod.`
        );
      }
    }
  ...
  start(user: User, day = false) {
    ...
    if (this.IDEA.data && !this.IDEA.discardsHidden) {
      this.room
        .add(
          `|html|<div class="infobox"><details><summary>IDEA discards:</summary>${this.IDEA.discardsHTML}</details></div>`
        )
        .update();
    }
  }
  ...
  vote(voter: MafiaPlayer, targetId: ID) {
    ...
    if (targetId === voter.id && !hammering && this.selfEnabled === 'hammer') {
      return this.sendUser(
        voter,
        `|error|You may only vote yourself when placing the hammer vote.`
      );
    }
    ...
    if (previousVote) {
      this.sendTimestamp(
        `${voter.name} has shifted their vote from ${previousVote === 'novote' ? 'No Vote' : this.getPlayer(previousVote)?.name} to ${name}`
      );
    } else {
      this.sendTimestamp(
        name === 'No Vote'
    ...
    if (this.getHammerValue(targetId) <= vote.trueCount) {
      // HAMMER
      this.sendDeclare(
        `Hammer! ${targetId === 'novote' ? 'Nobody' : Utils.escapeHTML(name!)} was voted out!`
      );
  ...
  unvote(voter: MafiaPlayer, force = false) {
    ...
    if (!target && voter.voting !== 'novote') {
      throw new Error(
        `Unable to find target when unvoting. Voter: ${voter.id}, Target: ${voter.voting}`
      );
    }
  ...
  voteBoxFor(userid: ID) {
    ...
    for (const key of this.getRemainingPlayers()
      .map(p => p.id)
      .concat(this.enableNV ? ['novote' as ID] : [])) {
  ...
  applyVoteModifier(requester: User, targetPlayer: MafiaPlayer, mod: number) {
    ...
    if (mod === oldMod || ((isNaN(mod) || mod === 1) && oldMod === undefined)) {
      ...
      return this.sendUser(
        requester,
        `|error|${targetPlayer} already has a vote modifier of ${mod}`
      );
    }
    ...
    if (targetPlayer.voting) {
      ...
      if (this.getHammerValue(targetPlayer.voting) <= this.votes[targetPlayer.voting].trueCount) {
        this.sendRoom(
          `${targetPlayer.voting} has been voted out due to a modifier change! They have not been eliminated.`
        );
        this.night(true);
      }
    }
    if (newMod === 1) {
      ...
      return this.sendUser(
        requester,
        `${targetPlayer.name} has been given a vote modifier of ${newMod}`
      );
    }
  }
  ...
  applyHammerModifier(user: User, target: MafiaPlayer, mod: number) {
    ...
    if (this.votes[target.id]) {
      ...
      if (this.hammerCount + newMod <= this.votes[target.id].trueCount) {
        ...
        this.sendRoom(
          `${target} has been voted due to a modifier change! They have not been eliminated.`
        );
        this.night(true);
      }
    }
  ...
  setHammer(count: number) {
    ...
    if (isNaN(count)) {
      ...
      this.sendDeclare(
        `The hammer count has been set at ${this.hammerCount}, and votes have been reset.`
      );
    }
    this.clearVotes();
  }
  ...
  shiftHammer(count: number) {
    ...
    if (isNaN(count)) {
      ...
      this.sendDeclare(
        `The hammer count has been shifted to ${this.hammerCount}. Votes have not been reset.`
      );
    }
    const hammered = [];
    ...
    if (hammered.length) {
      this.sendDeclare(
        `${Chat.count(hammered, 'players have')} been hammered: ${hammered.join(', ')}. They have not been removed from the game.`
      );
      this.night(true);
    }
  }
  ...
  eliminate(toEliminate: MafiaPlayer, ability: MafiaEliminateType) {
    ...
    toEliminate.eliminationOrder =
      this.getEliminatedPlayers() // Before eliminating, get other eliminated players
    ...
    this.sendDeclare(
      `${toEliminate.safeName} ${ability}! ${!this.noReveal && ability === MafiaEliminateType.ELIMINATE ? `${toEliminate.safeName}'s role was ${toEliminate.getStylizedRole()}.` : ''}`
    );
  ...
  revealRole(user: User, toReveal: MafiaPlayer, revealAs: string) {
    ...
    this.sendDeclare(
      `${toReveal.safeName}'s role ${toReveal.isEliminated() ? `was` : `is`} ${revealAs}.`
    );
    this.updatePlayers();
  }
  ...
  revive(user: User, toRevive: MafiaPlayer) {
    if (this.phase === 'IDEApicking') {
      return this.sendUser(
        user,
        `|error|You cannot add or remove players while IDEA roles are being picked.`
      );
    }
  ...
  setDeadline(minutes: number, silent = false) {
    ...
    this.sendTimestamp(
      `**The deadline has been set for ${minutes} minute${minutes === 1 ? '' : 's'}.**`
    );
  }
  ...
  nextSub(userid: ID | null = null) {
    ...
    const toSubOut = this.getPlayer(
      userid || this.hostRequestedSub.shift() || this.requestedSub.shift() || ''
    );
    if (!toSubOut) {
      // Should never happen
  ...
  ideaDistributeRoles(user: User) {
    ...
    if (this.phase !== 'locked' && this.phase !== 'IDEAlocked') {
      return this.sendUser(
        user,
        `|error|The game must be in a locked state to distribute IDEA roles.`
      );
    }
    ...
    this.sendDeclare(
      `${this.IDEA.data.name} roles have been distributed. You will have ${IDEA_TIMER / 1000} seconds to make your picks.`
    );
  ...
  ideaPick(user: User, selection: string[]) {
    ...
    if (!this.IDEA?.data) {
      return this.sendRoom(
        `Trying to pick an IDEA role with no module running, target: ${JSON.stringify(selection)}. Please report this to a mod.`
      );
    }
    ...
    if (!player?.IDEA) {
      return this.sendRoom(
        `Trying to pick an IDEA role with no player IDEA object, user: ${user.id}. Please report this to a mod.`
      );
    }
    ...
    if (selection[1]) {
      ...
      if (roleIndex === -1) {
        return this.sendUser(
          user,
          `|error|${selection[1]} is not an available role, perhaps it is already selected?`
        );
      }
  ...
  ideaFinalizePicks() {
    if (!this.IDEA?.data) {
      return this.sendRoom(
        `Tried to finalize IDEA picks with no IDEA module running, please report this to a mod.`
      );
    }
    const randed = [];
    for (const player of this.players) {
      if (!player.IDEA) {
        return this.sendRoom(
          `Trying to pick an IDEA role with no player IDEA object, user: ${player.id}. Please report this to a mod.`
        );
      }
      ...
      if (this.IDEA.data.picks.length === 1) {
        ...
        if (parsedRole.problems.length) {
          this.sendRoom(
            `Problems found when parsing IDEA role ${player.IDEA.picks[this.IDEA.data.picks[0]]}. Please report this to a mod.`
          );
        }
        ...
        if (!this.IDEA.data.untrusted) {
          for (const pick of role) {
            if (pick.substr(0, 10) === 'alignment:') {
              ...
              if (parsedRole.problems.length) {
                this.sendRoom(
                  `Problems found when parsing IDEA role ${pick}. Please report this to a mod.`
                );
              }
    ...
    for (const player of this.players.sort((a, b) => a.id.localeCompare(b.id))) {
      ...
      if (!IDEA) {
        return this.sendRoom(
          `No IDEA data for player ${player} when finalising IDEAs. Please report this to a mod.`
        );
      }
    ...
    if (randed.length) {
      this.sendDeclare(
        `${randed.join(', ')} did not pick a role in time and were randomly assigned one.`
      );
    }
  ...
  sendPlayerList() {
    this.room
      .add(
        `|c:|${Math.floor(Date.now() / 1000)}|~|**Players (${this.getRemainingPlayers().length})**: ${this.getRemainingPlayers()
          .map(p => p.name)
          .sort()
          .join(', ')}`
      )
    ...
    for (const alt of user.getAltUsers(true)) {
      if (!force && (this.getPlayer(alt.id) || this.played.includes(alt.id))) {
        throw new Chat.ErrorMessage(
          `${self ? `You already have` : `${user.id} already has`} an alt in the game.`
        );
      }
      if (this.hostid === alt.id || this.cohostids.includes(alt.id)) {
        throw new Chat.ErrorMessage(
          `${self ? `You have` : `${user.id} has`} an alt as a game host.`
        );
      }
    }
  }
  ...
  setSelfVote(user: User, setting: boolean | 'hammer') {
    ...
    if (from) {
      this.sendDeclare(
        `Self${from === 'hammer' ? 'hammering' : 'voting'} has been ${setting ? `changed to Self${setting === 'hammer' ? 'hammering' : 'voting'}` : 'disabled'}.`
      );
    } else {
      this.sendDeclare(
        `Self${setting === 'hammer' ? 'hammering' : 'voting'} has been ${setting ? 'enabled' : 'disabled'}.`
      );
    }
  ...
  setVotelock(user: User, setting: boolean) {
    ...
    if (this.voteLock === setting) {
      return this.sendUser(
        user,
        `|error|Votes are already ${setting ? 'set to lock' : 'set to not lock'}.`
      );
    }
...
export const pages: Chat.PageTable = {
  mafia(query, user) {
    ...
    buf += `<p style="font-weight:bold;">Players (${players.length}): ${players
      .map(p => p.safeName)
      .sort()
      .join(', ')}</p>`;
    ...
    if (isPlayer && game.phase === 'IDEApicking') {
      ...
      if (!IDEA) {
        return game.sendRoom(
          `IDEA picking phase but no IDEA object for user: ${user.id}. Please report this to a mod.`
        );
      }
    ...
    if (isHost) {
      ...
      for (const player of game.getRemainingPlayers()) {
        ...
        buf +=
          game.voteModifiers[player.id] !== undefined
            ? `(votes worth ${game.getVoteValue(player)})`
            : '';
        buf +=
          player.hammerRestriction !== null
            ? `(${player.hammerRestriction ? 'actor' : 'priest'})`
            : '';
      ...
      for (const eliminated of game.getEliminatedPlayers()) {
        ...
        buf +=
          eliminated.hammerRestriction !== null
            ? `(${eliminated.hammerRestriction ? 'actor' : 'priest'})`
            : '';
      ...
      buf += `<p style="font-weight:bold">${Object.values(MafiaData.alignments)
        .map(a => `<span style="color:${a.color || '#FFF'}">${a.name}</span>`)
        .join(', ')}</p>`;
    ...
    if (!isHost) {
      if (game.phase === 'signups') {
        ...
        if (
          (!isPlayer && game.subs.includes(user.id)) ||
          (isPlayer && !game.requestedSub.includes(user.id))
        ) {
  ...
  mafialadder(query, user) {
    ...
    const entries = Utils.sortBy(Object.entries(logs[section][month]), ([key, value]) => -value);
  ...
  mafia: {
    ...
    host(target, room, user, connection, cmd) {
      ...
      if (room.game)
        return this.errorReply(
          `There is already a game of ${room.game.title} in progress in this room.`
        );
      ...
      if (nextHost) {
        ...
        while ((hostid = hostQueue.shift())) {
          ...
          if (
            ...
            !room.users[targetUser.id] ||
            Mafia.isHostBanned(room, targetUser)
          ) {
            skipped.push(hostid);
            targetUser = null;
          } else {
        ...
        if (skipped.length) {
          this.sendReply(
            `${skipped.join(', ')} ${Chat.plural(skipped.length, 'were', 'was')} not online, not in the room, or are host banned and were removed from the host queue.`
          );
        }
        ...
        if (room.roomid === 'mafia' && hostQueue.length && toID(targetUsername) !== hostQueue[0]) {
          if (!cmd.includes('force')) {
            return this.errorReply(
              `${targetUsername} isn't the next host on the queue. Use /mafia forcehost if you're sure.`
            );
          }
        }
      }
      ...
      switch (command) {
        ...
        case 'add':
          ...
          if (!targetUser?.connected && !command.includes('force')) {
            return this.errorReply(
              `User ${targetUserID} not found. To forcefully add the user to the queue, use /mafia queue forceadd, ${targetUserID}`
            );
          }
          if (hostQueue.includes(targetUserID))
            return this.errorReply(`User ${targetUserID} is already on the host queue.`);
          ...
          room
            .add(`User ${targetUserID} has been added to the host queue by ${user.name}.`)
            .update();
          break;
        case 'del':
        case 'delete':
        case 'remove':
          ...
          if (index === -1)
            return this.errorReply(`User ${targetUserID} is not on the host queue.`);
          hostQueue.splice(index, 1);
          room
            .add(`User ${targetUserID} has been removed from the host queue by ${user.name}.`)
            .update();
          break;
        case '':
        case 'show':
    ...
    playercap(target, room, user) {
      ...
      if (num < game.playerCount) {
        return this.errorReply(
          `Player cap has to be equal or more than the amount of players in game.`
        );
      }
    ...
    closedsetup(target, room, user) {
      ...
      if (game.started) {
        return this.errorReply(
          `You can't ${action === 'on' ? 'enable' : 'disable'} closed setup because the game has already started.`
        );
      }
      if ((action === 'on' && game.closedSetup) || (action === 'off' && !game.closedSetup)) {
        return this.errorReply(
          `Closed setup is already ${game.closedSetup ? 'enabled' : 'disabled'}.`
        );
      }
    ...
    takeidles(target, room, user) {
      ...
      if ((action === 'off' && !game.takeIdles) || (action === 'on' && game.takeIdles)) {
        return this.errorReply(
          `Actions and idles are already ${game.takeIdles ? '' : 'not '}being accepted.`
        );
      }
      game.takeIdles = action === 'on';
      game.sendDeclare(
        `Actions and idles are ${game.takeIdles ? 'now' : 'no longer'} being accepted.`
      );
      game.updatePlayers();
    },
    takeidleshelp: [
    ...
    setroles(target, room, user, connection, cmd) {
      ...
      if (reset) {
        ...
        if (game.phase !== 'locked' && game.phase !== 'IDEAlocked') {
          return this.errorReply(
            game.phase === 'signups'
              ? `You need to close signups first.`
    ...
    idea(target, room, user) {
      ...
      if (
        ...
        game.hostid !== user.id &&
        !game.cohostids.includes(user.id)
      ) {
    ...
    idearerollhelp: [
      `/mafia ideareroll - rerolls the roles for the current IDEA module. Requires host % @ # ~`,
    ],
    ...
    ideadiscards(target, room, user) {
      ...
      if (target) {
        ...
        return this.sendReply(
          `IDEA discards are now ${game.IDEA.discardsHidden ? 'hidden' : 'visible'}.`
        );
      }
      ...
      if (!this.runBroadcast()) return;
      this.sendReplyBox(
        `<details><summary>IDEA discards:</summary>${game.IDEA.discardsHTML}</details>`
      );
    },
    ideadiscardshelp: [
      `/mafia ideadiscards - shows the discarded roles`,
    ...
    kill(target, room, user, connection, cmd) {
      ...
      if (game.phase === 'IDEApicking') {
        return this.errorReply(
          `You cannot add or remove players while IDEA roles are being picked.`
        ); // needs to be here since eliminate doesn't pass the user
      }
    ...
    idle(target, room, user, connection, cmd) {
      ...
      if (player.isEliminated()) {
        return this.errorReply(
          `You have been eliminated from the game and cannot take any actions.`
        );
      }
      if (game.phase !== 'night') {
      ...
      if (!game.takeIdles) {
        return this.errorReply(
          `The host is not accepting idles through the script. Send your action or idle to the host.`
        );
      }
        ...
        case 'action':
          ...
          if (target) {
            ...
            try {
              ...
              throw new Chat.ErrorMessage(
                `Your action submission contained a word banned by this room.`
              );
            }
            user.sendTo(
              room,
              `You have decided to use an action, with the following details: ${target}`
            );
          } else {
            user.sendTo(
              room,
              `You have decided to use an action. Please submit details about your action.`
            );
          }
    ...
    addhelp: [`/mafia add [player] - Add a new player to the game. Requires host % @ # ~`],
      ...
      if (target === 'off') {
        ...
        if (isNaN(num)) {
          ...
          if (game.dlAt - Date.now() > 0) {
            return this.sendReply(
              `|raw|<strong>The deadline is in ${Chat.toDurationString(game.dlAt - Date.now()) || '0 seconds'}.</strong>`
            );
          } else {
    ...
    silence(target, room, user, connection, cmd) {
      ...
      if (silence === targetPlayer.silenced) {
        return this.errorReply(
          `${targetPlayer.name} is already ${!silence ? 'not' : ''} silenced.`
        );
      }
    ...
    nighttalk(target, room, user, connection, cmd) {
      ...
      if (nighttalk === targetPlayer.nighttalk) {
        return this.errorReply(
          `${targetPlayer.name} is already ${!nighttalk ? 'not' : ''} able to talk during the night.`
        );
      }
      targetPlayer.nighttalk = nighttalk;
      this.sendReply(
        `${targetPlayer.name} can ${!nighttalk ? 'no longer' : 'now'} talk during the night.`
      );
    ...
    priest(target, room, user, connection, cmd) {
      ...
      if (remove) {
        ...
        if (actor !== targetPlayer.hammerRestriction) {
          return this.errorReply(
            `${targetPlayer.name} is ${targetPlayer.hammerRestriction ? 'an actor' : 'a priest'}.`
          );
        }
      ...
      if (actor === targetPlayer.hammerRestriction) {
        return this.errorReply(
          `${targetPlayer.name} is already ${targetPlayer.hammerRestriction ? 'an actor' : 'a priest'}.`
        );
      }
      targetPlayer.hammerRestriction = actor;
      this.sendReply(
        `${targetPlayer.name} is now ${targetPlayer.hammerRestriction ? 'an actor (can only hammer)' : "a priest (can't hammer)"}.`
      );
      if (actor) {
    ...
    forcevote(target, room, user) {
      ...
      if (this.meansYes(target)) {
        ...
        game.sendDeclare(
          `Forcevoting has been enabled. Your vote will start on yourself, and you cannot unvote!`
        );
    ...
    rolelist(target, room, user, connection, cmd) {
      ...
      const roleString = Utils.sortBy(
        showOrl ? game.originalRoles : game.roles,
        role => role.alignment
      )
        .map(role => role.safeName)
        .join(', ');
    ...
    playerroles(target, room, user) {
      ...
      this.sendReplyBox(
        game.players
          .map(
            p =>
              `${p.safeName}: ${p.role ? (p.role.alignment === 'solo' ? 'Solo ' : '') + p.role.safeName : 'No role'}`
          )
          .join('<br/>')
      );
    },
    ...
    view(target, room, user, connection) {
      ...
      if (this.broadcasting) {
        return this.sendReplyBox(
          `<button name="joinRoom" value="view-mafia-${room.roomid}" class="button"><strong>Spectate the game</strong></button>`
        );
      }
      ...
      switch (action) {
        case 'in':
          if (player) {
            ...
            if (game.subs.includes(user.id))
              return this.errorReply(`You are already on the sub list.`);
            if (game.played.includes(user.id))
              return this.errorReply(`You cannot sub back into the game.`);
        ...
        case 'out':
          if (player) {
            ...
            if (!game.subs.includes(user.id))
              return this.errorReply(`You are not on the sub list.`);
    ...
    autosub(target, room, user) {
      ...
      if (this.meansYes(toID(target))) {
        if (game.autoSub)
          return this.errorReply(`Automatic subbing of players is already enabled.`);
        game.autoSub = true;
        user.sendTo(room, `Automatic subbing of players has been enabled.`);
        game.nextSub();
      } else if (this.meansNo(toID(target))) {
        if (!game.autoSub)
          return this.errorReply(`Automatic subbing of players is already disabled.`);
        game.autoSub = false;
    ...
    subhost(target, room, user, connection, cmd) {
      ...
      if (cmd.includes('cohost')) {
        ...
        game.sendDeclare(
          Utils.html`${targetUser.name} has been substituted as the new host, replacing ${oldHostid}.`
        );
        this.modlog('MAFIASUBHOST', targetUser, `replacing ${oldHostid}`, {
          noalts: true,
          noip: true,
        });
      }
    ...
    removecohost(target, room, user) {
      ...
      if (cohostIndex < 0) {
        if (game.hostid === targetID) {
          return this.errorReply(
            `${target} is the host, not a cohost. Use /mafia subhost to replace them.`
          );
        }
    ...
    data(target, room, user, connection, cmd) {
      ...
      if (cmd === 'role' && !target && room) {
        ...
        if (!game) {
          return this.errorReply(
            `There is no game of mafia running in this room. If you meant to display information about a role, use /mafia role [role name]`
          );
        }
      ...
      if (!result)
        return this.errorReply(`"${target}" is not a valid mafia alignment, role, theme, or IDEA.`);
      ...
      if (dataType === 'theme') {
        ...
        for (const i in result) {
          ...
          const roles = [];
          for (const role of (result as MafiaDataTheme)[num]
            .split(',')
            .map((x: string) => x.trim())) {
    ...
    win(target, room, user, connection, cmd) {
      ...
      this.modlog(
        `MAFIAPOINTS`,
        null,
        `${points < 0 ? points * -1 : points} points were ${points < 0 ? 'taken from' : 'awarded to'} ${Chat.toListString(toGiveTo)}`
      );
      room.add(buf).update();
    },
    winhelp: [
    ...
    mvp(target, room, user, connection, cmd) {
      ...
      writeFile(LOGS_FILE, logs);
      this.modlog(
        `MAFIA${cmd.toUpperCase()}`,
        null,
        `MVP and 10 points were ${cmd === 'unmvp' ? 'taken from' : 'awarded to'} ${Chat.toListString(args)}`
      );
      room
        .add(
          `MVP and 10 points were ${cmd === 'unmvp' ? 'taken from' : 'awarded to'}: ${Chat.toListString(args)}`
    ...
    leaderboard(target, room, user, connection, cmd) {
      ...
      if (this.broadcasting) {
        return this.sendReplyBox(
          `<button name="joinRoom" value="view-mafialadder-${cmd}" class="button"><strong>${cmd}</strong></button>`
        );
      }
    ...
    hostban(target, room, user, connection, cmd) {
      ...
      if (Punishments.hasRoomPunishType(room, userid, `MAFIA${this.cmd.toUpperCase()}`)) {
        ...
        user.sendTo(
          room,
          `User '${targetUser.name}' is already hostbanned in this room, but they will now be gamebanned.`
        );
      ...
      this.privateModAction(
        `${targetUser.name} was banned from ${cmd === 'hostban' ? 'hosting' : 'playing'} mafia games by ${user.name}.`
      );
    },
    hostbanhelp: [
    ...
    unhostban(target, room, user, connection, cmd) {
      ...
      this.privateModAction(
        `${targetUser.name} was unbanned from ${cmd === 'unhostban' ? 'hosting' : 'playing'} mafia games by ${user.name}.`
      );
    ...
    addrole(target, room, user, connection, cmd) {
      ...
      if (alignment && !(alignment in MafiaData.alignments))
        return this.errorReply(`${alignment} is not a valid alignment.`);
      if (image && !VALID_IMAGES.includes(image))
        return this.errorReply(`${image} is not a valid image.`);
      ...
      if (id in MafiaData.aliases) {
        return this.errorReply(
          `${name} is already an alias (pointing to ${MafiaData.aliases[id]}).`
        );
      }
    ...
    addalignment(target, room, user, connection, cmd) {
      ...
      const [name, plural, color, buttonColor, image, ...memo] = target
        .split('|')
        .map(e => e.trim());
      const id = toID(name);
      ...
      if (!overwrite && id in MafiaData.alignments) {
        return this.errorReply(
          `${name} is already an alignment. Use /mafia overwritealignment to overwrite.`
        );
      }
    ...
    addtheme(target, room, user, connection, cmd) {
      ...
      if (!overwrite && id in MafiaData.themes) {
        return this.errorReply(
          `${name} is already a theme. Use /mafia overwritetheme to overwrite.`
        );
      }
      ...
      if (problems.length)
        return this.errorReply(`Problems found when parsing roles:\n${problems.join('\n')}`);
    ...
    addidea(target, room, user, connection, cmd) {
      ...
      if (!overwrite && id in MafiaData.IDEAs) {
        return this.errorReply(
          `${name} is already an IDEA. Use /mafia overwriteidea to overwrite.`
        );
      }
    ...
    addtermhelp: [
      `/mafia addterm name|memo1|memo2... - Adds a term to the database. Requires % @ # ~`,
    ],
    ...
    addalias(target, room, user, connection, cmd) {
      ...
      if (from in MafiaData.aliases) {
        return this.errorReply(
          `${from} is already an alias (pointing to ${MafiaData.aliases[from]})`
        );
      }
    ...
    deletedata(target, room, user) {
      ...
      if (!(source in MafiaData)) {
        return this.errorReply(
          `Invalid source. Valid sources are ${Object.keys(MafiaData).join(', ')}`
        );
      }
      ...
      if (dataSource === MafiaData.alignments) {
        if (entry === 'solo' || entry === 'town')
          return this.errorReply(`You cannot delete the solo or town alignments.`);
    ...
    deletedatahelp: [
      `/mafia deletedata source,entry - Removes an entry from the database. Requires % @ # ~`,
    ],
    listdata(target, room, user) {
      ...
      if (dataSource === MafiaData.aliases) {
        ...
        const entries = Object.entries(dataSource)
          .map(
            ([key, data]) =>
              `<button class="button" name="send" value="/mafia dt ${key}">${data.name}</button>`
          )
          .join('');
