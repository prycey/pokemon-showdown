...
abstract class Giveaway extends Rooms.SimpleRoomGame {
  ...
  send(content: string | Chat.VNode, isStart = false) {
    this.room.add(
      Chat.html`|uhtml|giveaway${this.gaNumber}${this.phase}|${(<div {...this.getStyle()}>{content}</div>)}`
    );
    if (isStart)
  ...
  changeUhtml(content: string | Chat.VNode) {
    this.room.uhtmlchange(
      `giveaway${this.gaNumber}${this.phase}`,
      Chat.html`${(<div {...this.getStyle()}>{content}</div>)}`
    );
    this.room.update();
  }
  ...
  static getSprite(set: PokemonSet): [ID, Chat.VNode] {
    ...
    return [species.id, <img src={`/sprites/ani${shiny}/${spriteid}.gif`} />];
  }
  ...
  static convertIVs(setObj: PokemonSet, ivs: string[]) {
    ...
    if (ivs.length) {
      ...
      const array = Object.keys(convertedIVs).map(
        (x, i) => `${convertedIVs[x as StatID]} ${statNames[i]}`
      );
    ...
    return (
      <center>
        ...
        <table style={{ marginLeft: 'auto', marginRight: 'auto' }}>
          <tr>
            <td colSpan={2} style={{ textAlign: 'center' }}>
              <strong>Giver:</strong> {this.giver.name}
              <br />
          ...
          {!!this.extraInfo?.trim().length && (
            <tr>
              <td colSpan={2} style={{ textAlign: 'center' }}>
                ...
                <br />
                <Chat.JSX.FormatText isTrusted>
                  {this.extraInfo.trim().replace(/<br \/>/g, '\n')}
                </Chat.JSX.FormatText>
              </td>
            </tr>
          )}
        ...
        <p style={{ textAlign: 'center', fontSize: '7pt', fontWeight: 'bold' }}>
          ...
          Do not join if you are currently unable to trade. Do not enter if you have already won
          this exact Pok&eacute;mon, {}
...
export class QuestionGiveaway extends Giveaway {
  ...
  constructor(
    ...
    this.winner = null;
    this.send(
      this.generateWindow('The question will be displayed in one minute! Use /guess to answer.'),
      true
    );
  ...
  static splitTarget(
    ...
    user: User,
    type: 'create' | 'store' | 'submit'
  ) {
    let [giver, ot, tid, game, question, answers, ivs, ball, extraInfo, ...prize] = target
      .split(sep)
    ...
    if (!targetUser.autoconfirmed) {
      throw new Chat.ErrorMessage(
        `User '${targetUser.name}' needs to be autoconfirmed to give something away.`
      );
    }
    ...
    return this.generateWindow(
      <>
        <p style={{ textAlign: 'center', fontSize: '13pt' }}>
          Giveaway Question: <b>{this.question}</b>
        </p>
  ...
  start() {
    this.changeUhtml(
      <p style={{ textAlign: 'center', fontSize: '13pt', fontWeight: 'bold' }}>
  ...
  change(value: string, user: User, answer = false) {
    ...
    if (this.phase !== 'pending') {
      return user.sendTo(
        this.room,
        'You cannot change the question or answer once the giveaway has started.'
      );
    }
    if (!answer) {
      this.question = value;
    ...
    if (!ans.length) {
      return user.sendTo(
        this.room,
        'You must specify at least one answer and it must not contain any special characters.'
      );
    }
    this.answers = ans;
    user.sendTo(
      this.room,
      `The answer${Chat.plural(ans, 's have', 'has')} been changed to ${ans.join(', ')}.`
    );
  }
    ...
    if (force) {
      ...
      if (!this.winner) {
        ...
        this.changeUhtml(
          <p style={style}>The giveaway has ended! Scroll down to see the answer.</p>
        );
        this.phase = 'ended';
        this.clearTimer();
        this.room.modlog({
        ...
        if (this.winner.connected) {
          this.winner.popup(
            `You have won the giveaway. PM **${this.giver.name}** to claim your prize!`
          );
        }
  ...
  static sanitizeAnswers(answers: string[]) {
    return answers
      .map(val => QuestionGiveaway.sanitize(val))
      .filter((val, index, array) => toID(val).length && array.indexOf(val) === index);
  }
...
export class LotteryGiveaway extends Giveaway {
  ...
  static splitTarget(
    ...
    let [giver, ot, tid, game, winners, ivs, ball, extraInfo, ...prize] = target
      .split(sep)
      .map(param => param.trim());
    ...
    if (winners) {
      ...
      if (isNaN(numWinners) || numWinners < 1 || numWinners > 5) {
        throw new Chat.ErrorMessage(
          'The lottery giveaway can have a minimum of 1 and a maximum of 5 winners.'
        );
      }
    }
    return {
  ...
  override generateReminder(joined = false) {
    ...
    return this.generateWindow(
      <>
        ...
        {Chat.count(this.maxWinners, 'winners')}!<br />
        <button class="button" name="send" value={`/giveaway ${toID(cmd)}lottery`}>
          <strong>{cmd}</strong>
        </button>
      </>
    );
  ...
  drawLottery() {
    ...
    if (force) {
      ...
      this.send(
        this.generateWindow(
          <>
            <p style={{ textAlign: 'center', fontSize: '10pt', fontWeight: 'bold' }}>
              Lottery Draw
            </p>
            <p style={{ textAlign: 'center' }}>
              {Chat.count(this.joined.size, 'users')} joined the giveaway.
              <br />
              Our lucky winner{Chat.plural(this.winners)}: <b>{winnerNames}</b>!<br />
              Congratulations!
            </p>
          </>
        )
      ...
      for (const winner of this.winners) {
        ...
        if (winner.connected) {
          winner.popup(
            `You have won the lottery giveaway! PM **${this.giver.name}** to claim your prize!`
          );
        }
      }
      if (this.giver.connected)
...
export class GTS extends Rooms.SimpleRoomGame {
  ...
  send(content: string) {
    this.room.add(
      Chat.html`|uhtml|gtsga${this.gtsNumber}|${(<div class="broadcast-blue">{content}</div>)}`
    );
    this.room.update();
  }
  ...
  changeUhtml(content: string) {
    this.room.uhtmlchange(
      ...
      Chat.html`${(<div class="broadcast-blue">{content}</div>)}`
    );
    this.room.update();
  }
  ...
  generateWindow() {
    ...
    const rightSide = this.noDeposits ? (
      <strong>
        More Pok&eacute;mon have been deposited than there are prizes in this giveaway and new
        deposits will not be accepted. {}
        If you have already deposited a Pok&eacute;mon, please be patient, and do not withdraw your
        Pok&eacute;mon.
      </strong>
    ) : (
      <>
        To participate, deposit <strong>{this.deposit}</strong> into the GTS and look for{' '}
        <strong>{this.lookfor}</strong>
      </>
    );
    return (
      <>
        <p
          style={{ textAlign: 'center', fontSize: '14pt', fontWeight: 'bold', marginBottom: '2px' }}
        >
    ...
    this.room.send(
      Chat.html`|html|${(
        <p style={{ textAlign: 'center', fontSize: '11pt' }}>
          More Pok&eacute;mon have been deposited than there are prizes in this giveaway and new
          deposits will not be accepted. {}
          If you have already deposited a Pok&eacute;mon, please be patient, and do not withdraw
          your Pok&eacute;mon.
        </p>
      )}`
    );
...
export const commands: Chat.ChatCommands = {
  gts: {
    ...
    full(target, room, user) {
      ...
      this.privateModAction(
        `The giveaway was forcibly ended by ${user.name} with ${amount} left${target}`
      );
    },
  },
  gtshelp: [
  ...
  giveaway: {
    ...
    ban(target, room, user, connection, cmd) {
      ...
      const durationMsg =
        cmd === 'monthban' ? ' for a month' : cmd === 'permaban' ? ' permanently' : '';
      this.privateModAction(
        `${targetUser.name} was banned from entering giveaways${durationMsg} by ${user.name}.${reasonMessage}`
      );
    },
    unban(target, room, user) {
      ...
      this.privateModAction(
        `${targetUser.name} was unbanned from entering giveaways by ${user.name}.`
      );
    ...
    create: {
      ...
      question(target, room, user) {
        ...
        if (room.game) {
          throw new Chat.ErrorMessage(
            `There is already a room game (${room.game.constructor.name}) going on.`
          );
        }
        // Syntax: giver|ot|tid|game|question|answer1,answer2,etc|ivs/format/like/this|pokeball|packed set
        const { targetUser, ot, tid, game, question, answers, ivs, ball, extraInfo, prize } =
          QuestionGiveaway.splitTarget(target, '|', this, user, 'create');
        const set = Teams.import(prize)?.[0];
        if (!set)
          throw new Chat.ErrorMessage(
            `Please submit the prize in the form of a PS set importable.`
          );

        room.game = new QuestionGiveaway(
          user,
          targetUser,
          room,
          ot,
        ...
        this.modlog(
          'QUESTION GIVEAWAY',
          null,
          `for ${targetUser.getLastId()} (OT: ${ot} TID: ${tid} Nature: ${(room.game as LotteryGiveaway).prize.nature} Ball: ${ball}${extraInfo ? ` Other box info: ${extraInfo}` : ''})`
        );
      },
      lottery(target, room, user) {
        ...
        const { targetUser, ot, tid, game, winners, ivs, ball, prize, extraInfo } =
    ...
    store: {
      ...
      question(target, room, user) {
        ...
        const { targetUser, ot, tid, game, prize, question, answers, ball, extraInfo, ivs } =
      ...
      lottery(target, room, user) {
        ...
        const { targetUser, ot, tid, game, prize, winners, ball, extraInfo, ivs } =
    ...
    submit: {
      ...
      question(target, room, user) {
        ...
        this.sendReply(
          `You have submitted a question giveaway for ${set.species}. If you log out or go offline, the giveaway won't go through.`
        );
        const message =
          `|tempnotify|pendingapprovals|Pending question giveaway request!` +
        ...
        room.sendMods(
          Chat.html`|uhtml|giveaway-request-${user.id}|${(
            <div class="infobox">
              ...
              <br />
              <button class="button" name="send" value="/j view-giveaways-submitted">
                View pending giveaways
              </button>
            </div>
          )}`
        );
      ...
      lottery(target, room, user) {
        ...
        this.sendReply(
          `You have submitted a lottery giveaway for ${set.species}. If you log out or go offline, the giveaway won't go through.`
        );
        const message =
          `|tempnotify|pendingapprovals|Pending lottery giveaway request!` +
    ...
    approve(target, room, user) {
      ...
      if (!targetUser?.connected) {
        ...
        throw new Chat.ErrorMessage(
          `${targetUser?.name || toID(target)} is offline, so their giveaway can't be run.`
        );
      }
      ...
      if (!hasGiveaway) {
        this.refreshPage('giveaways-submitted');
        throw new Chat.ErrorMessage(
          `${targetUser?.name || toID(target)} doesn't have any submitted giveaways.`
        );
      }
      ...
      if (hasGiveaway.type === 'question') {
        ...
        this.parse(
          `/giveaway create question ${data.targetUserID}|${data.ot}|${data.tid}|${data.game}|${data.question}|${data.answers.join(',')}|${data.ivs.join('/')}|${data.ball}|${data.extraInfo}|${Teams.pack([data.prize])}`
        );
      } else {
        const data = giveaway as LotteryGiveawayData;
        this.parse(
          `/giveaway create lottery ${data.targetUserID}|${data.ot}|${data.tid}|${data.game}|${data.winners}|${data.ivs.join('/')}|${data.ball}|${data.extraInfo}|${Teams.pack([data.prize])}`
        );
      }
      ...
      this.privateModAction(
        `${user.name} approved a ${hasGiveaway.type} giveaway by ${targetUser.name}.`
      );
      this.modlog(`GIVEAWAY APPROVE ${hasGiveaway.type.toUpperCase()}`, targetUser, null, {
        noalts: true,
        noip: true,
      });
    },
    ...
    delete(target, room, user, connection, cmd) {
      ...
      if (del) {
        ...
        if (
          ...
          !indexStr ||
          index <= -1 ||
          !['question', 'lottery'].includes(toID(type)) ||
          isNaN(index)
        ) {
        ...
        saveData();
        this.privateModAction(
          `${user.name} deleted a ${typedType} giveaway by ${giveaway.targetUserID}.`
        );
        ...
        this.privateModAction(
          `${user.name} denied a ${hasGiveaway.type} giveaway by ${targetUser.name}.`
        );
        this.modlog(`GIVEAWAY DENY ${hasGiveaway.type.toUpperCase()}`, targetUser, reason || null, {
          noalts: true,
          noip: true,
        });
      }
    ...
    count(target, room, user) {
      ...
      if (!Dex.species.get(target).exists) {
        throw new Chat.ErrorMessage(
          `No Pok\u00e9mon entered. Proper syntax: /giveaway count pokemon`
        );
      }
      ...
      this.sendReplyBox(
        `This Pokémon has been given away ${Chat.count(count, 'times')}, a total of ${Chat.count(recent, 'times')} in the past month.`
      );
    },
  },
  giveawayhelp(target, room, user) {
    ...
    if (user.can('show', null, room)) {
      buf.push(
        <details>
          ...
          <br />
          <code>
            /giveaway create question Giver | OT | TID | Game | Question | Answer 1, Answer 2,
            Answer 3 | IV/IV/IV/IV/IV/IV | Pok&eacute; Ball | Extra Info | Prize
          </code>{' '}
          - Start a new question giveaway (voices can only host their own). Requires: + % @ # ~
          <br />
          <code>
            /giveaway create lottery Giver | OT | TID | Game | # of Winners | IV/IV/IV/IV/IV/IV |
            Pok&eacute; Ball | Extra Info | Prize
          </code>{' '}
          - Start a new lottery giveaway (voices can only host their own). Requires: + % @ # ~<br />
          <code>/giveaway changequestion/changeanswer</code> - Changes the question/answer of a
          question giveaway. Requires: Being giveaway host
          <br />
          <code>/giveaway viewanswer</code> - Shows the answer of a question giveaway. Requires:
          Being giveaway host/giver
          <br />
          <code>/giveaway ban [user], [reason]</code> - Temporarily bans [user] from entering
          giveaways. Requires: % @ # ~<br />
          <code>/giveaway end</code> - Forcibly ends the current giveaway. Requires: % @ # ~<br />
          <code>/giveaway count [pokemon]</code> - Shows how frequently a certain Pok&eacute;mon has
          been given away.
          <br />
          <code>/giveaway whitelist [user]</code> - Allow the given [user] to make giveaways.
          Requires: % @ # ~<br />
          <code>/giveaway unwhitelist [user]</code> - Remove the given user from the giveaway
          whitelist. Requires: % @ # ~
        </details>
      );
    }
    ...
    buf.push(
      <details>
        <summary>Giveaway participation commands</summary>
        <code>/guess [target]</code> - Guesses an answer for a question giveaway.
        <br />
        <code>/giveaway submit</code> - Allows users to submit giveaways. They must remain online
        after submitting for it to go through.
        <br />
        <code>/giveaway viewanswer</code> - Guesses an answer for a question giveaway. Requires:
        Giveaway host/giver
        <br />
        <code>/giveaway remind</code> - Shows the details of the current giveaway.
        <br />
  ...
  for (const i in titles) {
    ...
    if (pageid === i) {
      ...
      urls.push(
        <>
          {icon}{' '}
          <a href={`/view-giveaways-${i}`} target="replace">
            {title}
          </a>
        </>
      );
...
function formatFakeButton(url: string, text: Chat.VNode): Chat.VNode {
  return (
    <a class="button" style={{ textDecoration: 'inherit' }} target="replace" href={url}>
      {text}
    </a>
  );
}
...
function generatePokeballDropdown() {
  const pokeballs = Dex.items
    .all()
    .filter(item => item.isPokeball)
  ...
  return (
    <>
      <label for="ball">Pok&eacute; Ball type: </label>
      <select name="ball">{pokeballsObj}</select>
    </>
  );
}
            ...
            if (!type || !['lottery', 'question'].includes(type)) {
              return (
                <center>
                  ...
                  {formatFakeButton(
                    `/view-giveaways-create-lottery`,
                    <>
                      <i class="fa fa-random"></i> Lottery
                    </>
                  )}{' '}
                  |{' '}
                  {formatFakeButton(
                    `/view-giveaways-create-question`,
                    <>
                ...
                return (
                  <>
                    ...
                    <form data-submitsend="/giveaway create lottery {giver}|{ot}|{tid}|{game}|{winners}|{ivs}|{ball}|{info}|{set}">
                      ...
                      Game:{' '}
                      <div>
                        <input type="radio" id="bdsp" name="game" value="bdsp" />
                        <label for="bdsp">BDSP</label>
                        <input type="radio" id="swsh" name="game" value="swsh" />
                        <label for="swsh">SwSh</label>
                        <input type="radio" id="sv" name="game" value="sv" checked />
                      ...
                      <br />
                      <br />
                      <label for="ivs">IVs (Formatted like "1/30/31/X/HT/30"): </label>
                      <input name="ivs" />
                      <br />
                      <br />
                      <label for="set">Prize:</label>
                      <br />
                      <textarea
                        style={{ width: '70%', height: '300px' }}
                        placeholder="Paste set importable"
                        name="set"
                      ></textarea>
                      <br />
                      <br />
              ...
              case 'question':
                return (
                  <>
                    ...
                    <form data-submitsend="/giveaway create question {giver}|{ot}|{tid}|{game}|{question}|{answers}|{ivs}|{ball}|{info}|{set}">
                      <label for="giver">Giver:</label>
                      <input name="giver" />
                      <br />
                      <br />
                      ...
                      <br />
                      <br />
                      <label for="answers">Answers (separated by comma):</label>
                      <input name="answers" />
                      <br />
                      <br />
                      {generatePokeballDropdown()}
                      ...
                      <textarea
                        style={{ width: '70%', height: '300px' }}
                        placeholder="Paste set importable here"
                        name="set"
                      ></textarea>
                      <br />
                      <br />
                ...
                if (wifiData.storedGiveaways.lottery.includes(giveaway as LotteryGiveawayData)) {
                  ...
                  buf.push(
                    <div class="infobox">
                      ...
                      <details>
                        <summary>
                          <psicon pokemon={giveaway.prize.species} /> Prize
                        </summary>
                        <Chat.JSX.FormatText isTrusted>
                  ...
                  buf.push(
                    <div class="infobox">
                      ...
                      <strong>OT:</strong> {giveaway.ot}, <strong>TID:</strong> {giveaway.tid}
                      <br />
                      <strong>Question:</strong> {giveaway.question}
                      <br />
                      <strong>Answer{Chat.plural(giveaway.answers.length, 's')}:</strong>{' '}
                ...
                <>
                  ...
                  {(() => {
                    if (!type || !['question', 'lottery'].includes(type)) {
                      return (
                        <center>
                          ...
                          {formatFakeButton(
                            `/view-giveaways-stored-add-lottery`,
                            <>
                              <i class="fa fa-random"></i> Lottery
                            </>
                          )}{' '}
                          |{' '}
                          {formatFakeButton(
                            `/view-giveaways-stored-add-question`,
                            <>
                              <i class="fa fa-question"></i> Question
                            </>
                          )}
                    ...
                    switch (type) {
                      case 'lottery':
                        return (
                          <form data-submitsend="/giveaway store lottery {giver}|{ot}|{tid}|{game}|{winners}|{ivs}|{ball}|{info}|{set}">
                            <label for="giver">Giver: </label>
                            <input name="giver" />
                            <br />
                            <br />
                            ...
                            <br />
                            <button class="button" type="submit">
                              Store Lottery Giveaway
                            </button>
                          </form>
                        );
                      case 'question':
                        return (
                          <form data-submitsend="/giveaway store question {giver}|{ot}|{tid}|{game}|{question}|{answers}|{ivs}|{ball}|{info}|{set}">
                            <label for="giver">Giver:</label>
                            <input name="giver" />
                            <br />
                            <br />
              ...
              for (let giveaway of giveaways) {
                ...
                const claimedTitle =
                  giveaway.claimed === user.id
                    ? 'Unclaim'
                    : giveaway.claimed
                      ? `Claimed by ${giveaway.claimed}`
                      : `Claim`;
                const disabled =
                  giveaway.claimed && giveaway.claimed !== user.id ? ' disabled' : '';
                buf.push(
                  <div class="infobox">
                    {(() => {
                      if (
                        wifiData.submittedGiveaways.lottery.includes(
                          giveaway as LotteryGiveawayData
                        )
                      ) {
                        giveaway = giveaway as LotteryGiveawayData;
                        return (
                          <>
                            ...
                            {!!giveaway.claimed && (
                              <>
                                <br />
                                <strong>Claimed:</strong> {giveaway.claimed}
                              </>
                            )}
                            <br />
                            <strong>Pok&eacute; Ball:</strong> <psicon item={giveaway.ball} />
                    ...
                    {!Users.get(giveaway.targetUserID)?.connected ? (
                      ...
                      <>
                        <button
                          class="button"
                          name="send"
                          value={`/giveaway deny ${giveaway.targetUserID}`}
                        >
                          <i class="fa fa-times-circle"></i> Deny giveaway
                        </button>
                        <button
                        ...
                        <button
                          ...
                          style={{ float: 'right' }}
                          name="send"
                          value={`/giveaway approve ${giveaway.targetUserID}`}
                        >
                          Create giveaway
                        </button>
                      </>
                ...
                <>
                  ...
                  {(() => {
                    if (!type || !['question', 'lottery'].includes(type)) {
                      return (
                        <center>
                          <h3>Pick a giveaway type</h3>
                          {formatFakeButton(
                            `/view-giveaways-submitted-add-lottery`,
                            <>
                              <i class="fa fa-random"></i> Lottery
                            </>
                          )}{' '}
                          |{' '}
                          {formatFakeButton(
                            `/view-giveaways-submitted-add-question`,
                            <>
                              <i class="fa fa-question"></i> Question
                            </>
                          )}
                    ...
                    switch (type) {
                      case 'lottery':
                        return (
                          <form data-submitsend="/giveaway submit lottery {giver}|{ot}|{tid}|{game}|{winners}|{ivs}|{ball}|{info}|{set}">
                            <label for="giver">Giver: </label>
                            <input name="giver" />
                            <br />
                            <br />
                            ...
                            <br />
                            <label for="info">
                              Additional information (provide a link of proof here):
                            </label>
                            <br />
                            <textarea
                              style={{ width: '50%', height: '100px' }}
                            ...
                            <br />
                            <button class="button" type="submit">
                              Submit Lottery Giveaway
                            </button>
                          </form>
                        );
                      case 'question':
                        return (
                          <form data-submitsend="/giveaway submit question {giver}|{ot}|{tid}|{game}|{question}|{answers}|{ivs}|{ball}|{info}|{set}">
                            <label for="giver">Giver:</label>
                            <input name="giver" />
                            <br />
                            <br />
...
Chat.multiLinePattern.register(
  `/giveaway (create|new|start|store|submit|save) (question|lottery) `
);
