...
export const commands: Chat.ChatCommands = {
  ...
  mixandmega(target, room, user) {
    ...
    if (mod) {
      ...
      if (dex === Dex.dexes['gen9ssb']) {
        throw new Chat.ErrorMessage(
          `The SSB mod supports custom elements for Mega Stones that have the capability of crashing the server.`
        );
      }
    }
    ...
    if (!stone) {
      throw new Chat.ErrorMessage(
        `Error: Mega Stone/Primal Orb/Rusted Item/Origin Item/Mask not found.`
      );
    }
    ...
    for (statName in species.baseStats) {
      // Add the changed stats and weight
  ...
  stone(target) {
    ...
    if (!stone) {
      const formeIdRegex = new RegExp(
        `(?:mega[xy]?|primal|origin|crowned|epilogue|cornerstone|wellspring|hearthflame|douse|shock|chill|burn|${dex.types
          .all()
          .map(x => x.id)
          .filter(x => x !== 'normal')
          .join('|')})$`
      ...
      for (const poke of species.otherFormes) {
        const formeRegex = new RegExp(
          `(?:-Douse|-Shock|-Chill|-Burn|-Cornerstone|-Wellspring|-Hearthflame|-Crowned|-Epilogue|-Origin|-Primal|-Mega(?:-[XY])?|${dex.types
            .names()
            .filter(x => x !== 'Normal')
            .map(x => '-' + x)
            .join('|')})$`
        ...
        const flag =
          megaPoke.requiredMove === 'Dragon Ascent' ? megaPoke.requiredMove : megaPoke.requiredItem;
    ...
    for (const aStone of toDisplay) {
      ...
      buf += `</li>`;
      this.sendReply(
        `|raw|<div class="message"><ul class="utilichart">${buf}<li style="clear:both"></li></ul></div>`
      );
      this.sendReply(
        `|raw|<font size="1">${Object.entries(details)
          .map(([detail, value]) => `<font color="#686868">${detail}:</font> ${value}`)
          .join('&nbsp;|&ThickSpace;')}</font>`
      );
    }
  },
  stonehelp: [
    `/stone <mega stone or other>[, generation] - Shows the changes that a mega stone/orb applies to a Pok\u00e9mon.`,
  ],
  ...
  tiershift(target, room, user, connection, cmd) {
    ...
    for (const statName in species.baseStats) {
      ...
      species.baseStats[statName] = Utils.clampIntRange(
        species.baseStats[statName] + boost,
        1,
        255
      );
  ...
  franticfusions(target, room, user, connection, cmd) {
    ...
    if (fusion.name.length) {
      if (!fusion.exists || fusion.gen > dex.gen) {
        ...
        throw new Chat.ErrorMessage(
          `Error: Pok\u00e9mon '${monName}' not found${additionalReason}.`
        );
      }
    ...
    for (const statName in species.baseStats) {
      ...
      if (!fusion.name.length) {
        ...
        species.baseStats[statName] = Utils.clampIntRange(
          species.baseStats[statName] + addition,
          1,
          255
        );
        species.bst += species.baseStats[statName];
    ...
    const abilities = new Set<string>([
      ...
      ...Object.values(fusion.abilities),
    ]);
  ...
  scalemons(target, room, user, connection, cmd) {
    ...
    for (const stat in species.baseStats) {
      ...
      species.baseStats[stat] = Utils.clampIntRange(
        (species.baseStats[stat] * scale) / bstNoHP,
        1,
        255
      );
  ...
  crossevolve(target, user, room) {
    ...
    if (setStage + 1 !== crossStage) {
      throw new Chat.ErrorMessage(
        `Error: Cross evolution must follow evolutionary stages. (${species.name} is Stage ${setStage} and can only cross evolve to Stage ${setStage + 1})`
      );
    }
    const mixedSpecies = Utils.deepClone(species);
    ...
    for (statName in species.baseStats) {
      ...
      mixedSpecies.baseStats[statName] = Utils.clampIntRange(
        mixedSpecies.baseStats[statName] + statChange,
        1,
        255
      );
      mixedSpecies.bst += mixedSpecies.baseStats[statName];
  ...
  showevo(target, room, user, connection, cmd) {
    ...
    if (!evo.prevo) {
      const evoBaseSpecies = Dex.species.get(
        (Array.isArray(evo.battleOnly) ? evo.battleOnly[0] : evo.battleOnly) ||
          evo.changesFrom ||
          evo.name
      );
      ...
      for (i in evo.baseStats) {
        ...
        if (!isReEvo) {
          ...
          deltas.baseStats[i] = Utils.clampIntRange(
            evoBaseSpecies.baseStats[i] + statChange,
            1,
            255
          );
      ...
      if (!isReEvo) {
        this.sendReply(
          `|raw|<font size="1"><font color="#686868">Gen:</font> ${details['Gen']}&nbsp;|&ThickSpace;<font color="#686868">Weight:</font> ${details['Weight']}&nbsp;|&ThickSpace;<font color="#686868">Stage:</font> ${details['Stage']}</font>`
        );
      }
    } else {
  ...
  pokemovehelp: [`/pokemove <Pok\u00e9mon> - Shows the Pokemove data for <Pok\u00e9mon>.`],
};
