...
export class LastFMInterface {
  ...
  validateAccountName(accountName: string) {
    ...
    if (
      !(
        ...
        /^[a-zA-Z]/.test(sanitizedName) &&
        sanitizedName.length > 1 &&
        sanitizedName.length < 16
      )
    ) {
      throw new Chat.ErrorMessage(
        `The provided account name (${sanitizedName}) is invalid. Valid last.fm usernames are between 2-15 characters, start with a letter, and only contain letters, numbers, hyphens, and underscores.`
      );
    }
    return true;
  }
  ...
  async tryGetTrackData(track: string, artist?: string) {
    ...
    if (!buf) {
      throw new Chat.ErrorMessage(
        `No results for '${artist ? `${artist} - ` : ``}${track}' found. Check spelling?`
      );
    }
    return buf;
  }
  checkHasKey() {
    if (!Config.lastfmkey) {
      throw new Chat.ErrorMessage(
        `This server does not support last.fm commands. If you're the owner, you can enable them by setting up Config.lastfmkey.`
      );
    }
  }
  getScrobbleBadge() {
...
class RecommendationsInterface {
  ...
  async add(
    ...
    if (this.get(artist, title)) {
      throw new Chat.ErrorMessage(
        `The song titled '${title}' by ${artist} is already recommended.`
      );
    }
  ...
  approveSuggestion(submitter: string, artist: string, title: string) {
    ...
    if (!rec) {
      throw new Chat.ErrorMessage(
        `There is no song titled '${title}' by ${artist} suggested from ${submitter.trim()}.`
      );
    }
    if (!recommendations.saved) recommendations.saved = [];
    recommendations.saved.push(rec);
  ...
  likeRecommendation(artist: string, title: string, liker: User) {
    ...
    if (
      (!Config.noipchecks && rec.liked.ips.includes(liker.latestIp)) ||
      rec.liked.userids.includes(liker.id)
    ) {
...
export const commands: Chat.ChatCommands = {
  ...
  approvesuggestion(target, room, user) {
    ...
    this.privateModAction(
      `${user.name} approved a suggested recommendation from ${submitter} for '${title}' by ${artist}.`
    );
    this.modlog(
      `RECOMMENDATION`,
      null,
      `approve: '${toID(title)}' by ${toID(artist)} from ${submitter}`
    );
  },
  ...
  denysuggestion(target, room, user) {
    ...
    this.privateModAction(
      `${user.name} denied a suggested recommendation from ${submitter} for '${title}' by ${artist}.`
    );
    this.modlog(
      `RECOMMENDATION`,
      null,
      `deny: '${toID(title)}' by ${toID(artist)} from ${submitter}`
    );
  },
  denysuggestionhelp: [
  ...
  viewrecommendationshelp: [`/viewrecommendations OR /viewrecs - View all recommended songs.`],
