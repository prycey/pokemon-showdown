...
export class AutoResponder {
  ...
  static logMessage(roomid: RoomID, entry: LoggedMessage) {
    return this.logStream.writeLine(
      JSON.stringify({
        ...entry,
        room: roomid,
          ...
          .map(term => {
            if (term.length > 100 && !raw) {
              throw new Chat.ErrorMessage(
                `One or more of your arguments is too long. Use less than 100 characters.`
              );
            }
            if (item.startsWith('|') || item.endsWith('|')) {
              throw new Chat.ErrorMessage(
                `Invalid use of |. Make sure you have an option on either side.`
              );
            }
            if (term.startsWith('!')) {
              return `^(?!.*${term.slice(1)})`;
  ...
  tryAddRegex(inputString: string, raw?: boolean) {
    let [args, faq] = inputString.split('=>').map(item => item.trim()) as [
      string,
      string | undefined,
    ];
  ...
  static canOverride(user: User, room: Room) {
    ...
    return (
      (devAuth?.atLeast(user, '%') && devAuth?.has(user.id) && room.auth.atLeast(user, '@')) ||
      user.can('rangeban')
    );
  }
  destroy() {
  ...
  autoresponder: {
    ...
    toggle(target, room, user) {
      ...
      this.privateModAction(
        `${user.name} ${!room.responder ? 'disabled' : 'enabled'} the auto-response filter.`
      );
    ...
    add(target, room, user, connection, cmd) {
      ...
      this.privateModAction(
        `${user.name} added regex for "${target.split('=>')[0]}" to the autoresponder.`
      );
    ...
    remove(target, room, user) {
      ...
      this.privateModAction(
        `${user.name} removed regex ${num} from the usable regexes for ${faq}.`
      );
    ...
    unignore(target, room, user) {
      ...
      this.privateModAction(
        `${user.name} removed ${Chat.count(targets.length, 'terms')} from the autoresponder ignore list.`
      );
    ...
    switch (args[0]) {
      case 'stats':
        ...
        for (const [dateKey, total] of dayKeys) {
          totalCount += total;
          days.push(
            `- <a roomid="view-autoresponder-${room.roomid}-stats-${dateKey}">${dateKey}</a> (${total})`
          );
        }
      ...
      case 'keys':
        ...
        buf += Object.entries(roomData.pairs)
          .map(([item, regexes]) => {
