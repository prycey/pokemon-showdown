...
export const Filters = new (class {
  ...
  constructor() {
    for (const letter in EVASION_DETECTION_SUBSTITUTIONS) {
      this.EVASION_DETECTION_SUB_STRINGS[letter] =
        `[${EVASION_DETECTION_SUBSTITUTIONS[letter].join('')}]`;
    }
    this.load();
  }
  constructEvasionRegex(str: string) {
    const buf =
      '\\b' +
      [...str]
        .map(letter => (this.EVASION_DETECTION_SUB_STRINGS[letter] || letter) + '+')
        .join('\\.?') +
      '\\b';
  ...
  add(filterWord: Partial<Chat.FilterWord> & { list: string; word: string }) {
    ...
    if (filterWords[filterWord.list].some(val => String(val.regex) === String(filterWord.regex))) {
      throw new Chat.ErrorMessage(
        `${filterWord.word} is already added to the ${filterWord.list} list.`
      );
    }
  ...
  loadLegacy() {
    ...
    loop: for (const line of lines) {
      ...
      const [location, word, punishment, reason, times, ...rest] = line
        .split('\t')
      ...
      for (const key in Chat.monitors) {
        if (
          Chat.monitors[key].location === location &&
          Chat.monitors[key].punishment === punishment
        ) {
          ...
          if (punishment === 'EVASION') {
            ...
            regex = new RegExp(
              punishment === 'SHORTENER' ? `\\b${word}` : word,
              replacement ? 'igu' : 'iu'
            );
          }
      ...
      Monitor.crashlog(
        new Error("Couldn't find [location, punishment] pair for a filter word"),
        'The main process',
        {
          location,
          word,
  ...
  monitor(line, room, user, message, lcMessage, isStaff) {
    ...
    if (match) {
      ...
      if (room) {
        ...
        this.errorReply(
          `Please do not say '${match[0]}'${publicReason ? ` ${publicReason}` : ``}.`
        );
      }
      return false;
    }
...
Chat.registerMonitor('shorteners', {
  ...
  monitor(line, room, user, message, lcMessage, isStaff) {
    ...
    if (regex.test(lcMessage)) {
      ...
      this.errorReply(
        `Please do not use URL shorteners such as '${word}'${publicReason ? ` ${publicReason}` : ``}.`
      );
      return false;
    }
  },
...
export const chatfilter: Chat.ChatFilter = function (message, user, room) {
  ...
  const isStaffRoom =
    room && ((room.persist && room.roomid.endsWith('staff')) || room.roomid.startsWith('help-'));
...
export const loginfilter: Chat.LoginFilter = user => {
  ...
  if (typeof offlineWarn !== 'undefined') {
    user.send(
      `|c|~|/warn You were warned while offline${offlineWarn.length ? `: ${offlineWarn}` : '.'}`
    );
      ...
      if (match) {
        if (Chat.monitors[list].punishment === 'AUTOLOCK') {
          void Punishments.autolock(
            ...
            'staff',
            `NameMonitor`,
            `inappropriate Pokémon nickname: ${name}`,
            `${user.name} - using an inappropriate Pokémon nickname: ||${name}||`,
            true
          );
          ...
          void Punishments.autolock(
            ...
            'staff',
            'FilterEvasionMonitor',
            `Evading filter in Pokémon nickname (${name} => ${word})`,
            `${user.name}: Pokémon nicknamed ||\`\`${name} => ${word}\`\`||`,
            true
          );
        }
        line.hits++;
...
export const statusfilter: Chat.StatusFilter = (status, user) => {
  ...
  const impersonationRegex =
    /\b(?:global|room|upper|senior)?\s*(?:staff|admin|administrator|leader|owner|founder|mod|moderator|driver|voice|operator|sysop|creator)\b/gi;
  ...
  filters(query, user, connection) {
    ...
    for (const key in Chat.monitors) {
      ...
      if (filterWords[key].length) {
        content += filterWords[key]
          .map(({ regex, word, reason, publicReason, replacement, hits }) => {
...
export const commands: Chat.ChatCommands = {
  ...
  filter: {
    add(target, room, user) {
      ...
      if (!list || !rest.length) {
        return this.errorReply(
          `Syntax: /filter add list ${separator} word ${separator} reason [${separator} optional public reason]`
        );
      }

      if (!(list in filterWords)) {
        return this.errorReply(
          `Invalid list: ${list}. Possible options: ${Object.keys(filterWords).join(', ')}`
        );
      }
      ...
      if (Chat.monitors[list].punishment === 'FILTERTO') {
        [filterWord.word, filterWord.replacement, filterWord.reason, filterWord.publicReason] =
          rest;
      ...
      if (Chat.monitors[list].punishment === 'FILTERTO') {
        this.globalModlog(
          `ADDFILTER`,
          null,
          `'${String(filterWord.regex)} => ${filterWord.replacement}' to ${list} list${reason}`
        );
      } else {
    ...
    remove(target, room, user) {
      ...
      const notFound = words.filter(
        val => !filterWords[list].filter(entry => entry.word === val).length
      );
      if (notFound.length) {
        return this.errorReply(
          `${notFound.join(', ')} ${Chat.plural(notFound, 'are', 'is')} not on the ${list} list.`
        );
      }
    ...
    test(target, room, user) {
      ...
      const lcMessage = Chat.stripFormatting(
        target
          .replace(/\u039d/g, 'N')
          .toLowerCase()
          // eslint-disable-next-line no-misleading-character-class
