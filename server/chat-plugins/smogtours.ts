...
export const commands: Chat.ChatCommands = {
  smogtours: {
    ...
    async add(target, room, user, connection, cmd) {
      ...
      console.log(targets);
      const [title, rawSection, url, rawEnds, rawImg, rawCredit, rawArtistName, rawShort, rawDesc] =
      ...
      if (image && !(toID(rawCredit) && toID(rawArtistName))) {
        return this.popupReply(
          `All images must have the artist named and a link to the profile of the user who created them.`
        );
      }
      if (rawCredit || rawArtistName) {
        // if one exists, both should, as verified above
    ...
    whitelist(target, room, user) {
      ...
      if (!section) {
        return this.errorReply(
          `Invalid section ID: "${sectionID}". Valid IDs: ${Object.keys(tours).join(', ')}`
        );
      }
...
export function renderPageChooser(curPage: string, buffer: string, user?: User) {
  ...
  if (user && (checkWhitelisted('', user) || user?.can('rangeban'))) {
    ...
    buf += renderTab(`<strong>Manage</strong>`, true, curPage === 'manage');
    buf += renderTab(
...
export const pages: Chat.PageTable = {
  tournaments: {
    ...
    start(query, user) {
      ...
      const keys = Utils.sortBy(Object.keys(tours), k => [k === possibleCategory, k]).filter(
        cat => checkWhitelisted(toID(cat), user) || user.can('rangeban')
      );
    ...
    manage(query, user) {
      ...
      buf += Object.keys(tours)
        .map(cat => {
          ...
          innerBuf +=
            section.tours
              .map(t => `&bull; <a href="/view-tournaments-edit-${cat}-${t.id}">${t.title}</a>`)
              .join('<br />') || 'None active.';
          return innerBuf;
        })
        .filter(Boolean)
