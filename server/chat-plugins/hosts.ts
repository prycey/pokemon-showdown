...
function checkCanPerform(
  context: Chat.PageContext | Chat.CommandContext,
  user: User,
  permission: GlobalPermission = 'lockdown'
) {
...
function getHostType(type: string) {
  ...
  throw new Chat.ErrorMessage(
    `'${type}' is not a valid host type. Please specify one of ${HOST_SUFFIXES.join(', ')}.`
  );
}
...
export const pages: Chat.PageTable = {
    ...
    Utils.sortBy(openProxies, ip => {
      ...
      if (number === null) {
        Rooms.get('upperstaff')?.add(
          `|error|Invalid IP address in IPTools.singleIPOpenProxies: '${ip}'`
        );
        return -1;
      }
      return number;
  ...
  hosts(query, user) {
    ...
    const residentialHosts = ['all', 'residential', 'res'].includes(type)
      ? [...IPTools.residentialHosts]
      : [];
...
export const commands: Chat.ChatCommands = {
  ...
  ipranges: {
    ...
    view(target, room, user) {
      ...
      if (!types.includes(type)) {
        return this.errorReply(
          `'${type}' isn't a valid host type. Specify one of ${types.join(', ')}.`
        );
      }
    ...
    rename(target, room, user) {
      ...
      this.globalModlog(
        'IPRANGE RENAME',
        null,
        `IP range ${formatRange(toRename, true)} to ${range.host}`
      );
    },
    renamehelp: [
  ...
  addhosts(target, room, user, connection, cmd) {
    ...
    switch (type) {
      case 'openproxy':
        for (const host of hosts) {
          ...
          if (removing !== IPTools.singleIPOpenProxies.has(host)) {
            return this.errorReply(
              `'${host}' is ${removing ? 'not' : 'already'} in the list of proxy IPs.`
            );
          }
        }
        if (removing) {
      ...
      case 'proxy':
        for (const host of hosts) {
          ...
          if (removing !== IPTools.proxyHosts.has(host)) {
            return this.errorReply(
              `'${host}' is ${removing ? 'not' : 'already'} in the list of proxy hosts.`
            );
          }
        }
        if (removing) {
      ...
      case 'mobile':
        for (const host of hosts) {
          ...
          if (removing !== IPTools.mobileHosts.has(host)) {
            return this.errorReply(
              `'${host}' is ${removing ? 'not' : 'already'} in the list of mobile hosts.`
            );
          }
        }
        if (removing) {
      ...
      default:
        return this.errorReply(
          `'${type}' isn't one of 'openproxy', 'proxy', 'residential', or 'mobile'.`
        );
    }
    this.privateGlobalModAction(
  ...
  nomarkshared: {
    add(target, room, user) {
      ...
      this.privateGlobalModAction(
        `The IP '${ip}' was blacklisted from being marked as shared by ${user.name}.`
      );
    ...
    remove(target, room, user) {
      ...
      this.privateGlobalModAction(
        `The IP '${target}' was unblacklisted from being marked as shared by ${user.name}.`
      );
