...
const ACCIDENTAL_LEAKS =
  /^((?:\s)?(?:\/{2,}|[^\w/]+)|\s\/)?(?:\s)?(?:s\W?cavenge|s\W?cav(?:engers)? guess|d\W?t|d\W?ata|d\W?etails|g\W?(?:uess)?|v)\b/i;
...
const scavengersData = databaseContentsJSON
  ? JSON.parse(databaseContentsJSON)
  : { recycledHunts: [] };
...
const HostLeaderboard =
  LeaderboardRoom?.scavLeaderboard?.scavsHostLeaderboard || new PlayerLadder(HOST_DATA_FILE);
const PlayerLeaderboard =
  LeaderboardRoom?.scavLeaderboard?.scavsPlayerLeaderboard || new PlayerLadder(PLAYER_DATA_FILE);
...
function formatQueue(
  ...
  if (queue?.length) {
    buffer = queue
      .map((item, index) => {
        ...
        const queuedBy = item.hosts.every(h => h.id !== item.staffHostId)
          ? ` / ${item.staffHostId}`
          : '';
        let questions;
        if (
          !broadcasting &&
          (item.hosts.some(h => h.id === viewer.id) || viewer.id === item.staffHostId)
        ) {
  ...
  if (showStaff) {
    template += `<table style="width: 100%"><tr><td style="text-align: left;">Auto Timer Duration: ${timerDuration} minutes</td><td>Auto Dequeue: <button class="button${
      !queueDisabled
        ? '" name="send" value="/scav disablequeue"'
        : ' disabled" style="font-weight:bold; color:#575757; font-weight:bold; background-color:#d3d3d3;"'
    }>OFF</button>&nbsp;<button class="button${
      queueDisabled
        ? '" name="send" value="/scav enablequeue"'
        : ' disabled" style="font-weight:bold; color:#575757; font-weight:bold; background-color:#d3d3d3;"'
    }>ON</button></td><td style="text-align: right;"><button class="button" name="send" value="/scav next 0">Start the next hunt</button></td></tr></table>`;
  }
  return template;
}
...
class ScavengerHuntDatabase {
  static getRecycledHuntFromDatabase() {
    ...
    return scavengersData.recycledHunts[
      Math.floor(Math.random() * scavengersData.recycledHunts.length)
    ];
  }
  ...
  static removeHintToRecycledHunt(huntNumber: number, questionNumber: number, hintNumber: number) {
    scavengersData.recycledHunts[huntNumber - 1].questions[questionNumber - 1].hints.splice(
      hintNumber - 1
    );
    this.updateDatabaseOnDisk();
  }
  ...
  static hasHunt(hunt_number: number) {
    return (
      !isNaN(hunt_number) && hunt_number > 0 && hunt_number <= scavengersData.recycledHunts.length
    );
  }

  static getFullTextOfHunt(hunt: {
    hosts: FakeUser[];
    questions: { text: string; answers: string[]; hints?: string[] }[];
  }) {
...
export class ScavengerHunt extends Rooms.RoomGame<ScavengerHuntPlayer> {
  ...
  getCreationMessage(newHunt?: boolean): string {
    ...
    return (
      `|raw|<div class="broadcast-blue"><strong>${huntType} scavenger hunt by <em>${hosts}</em> has been started${staffHost}.</strong>` +
  ...
  setTimer(minutes: number) {
    ...
    if (minutes > 24 * 60) {
      ...
      throw new Chat.ErrorMessage(
        `Time limit must be under 24 hours (you asked for ${Chat.toDurationString(minutes * 60000)}).`
      );
    }
  ...
  getQuestion(question: number, showHints?: boolean) {
    ...
    return (
      `|raw|<div class="ladder"><table style="width:100%;table-layout:fixed;"><tr>` +
      ...
      `<td><div style="overflow:auto; max-width: 100vw; max-height: 50vh">${
        ...
        (showHints && current.question.spoilers.length
          ? `<details><summary>Extra Hints:</summary>${current.question.spoilers
              .map(p => `- ${p}`)
              .join('<br />')}</details>`
          : ``)
      }</div></td>` +
          ...
          .map(
            (q, i) =>
              `<tr><td>${
                ...
                (q.spoilers.length
                  ? `<details><summary>Extra Hints:</summary>${q.spoilers
                      .map(s => `- ${s}`)
                      .join('<br />')}</details>`
                  : ``)
              }</div></td><td>${
                i + 1 >= qLimit ? `` : Utils.escapeHTMLForceWrap(q.answer.join(' ; '))
              }</td></tr>`
          )
          .join('') +
        `</table><div>`
  ...
  onComplete(player: ScavengerHuntPlayer) {
    ...
    const completionMessage = this.runEvent(
      'ConfirmCompletion',
      player,
      time,
      blitz,
    ...
    this.announce(
      `The ${this.gameType ? `${this.gameType} ` : ''}scavenger hunt by ${hosts} was ended ${endedBy ? 'by ' + Utils.escapeHTML(endedBy.name) : 'automatically'}.<br />` +
        `${this.completed
          ...
          .map(
            (p, i) =>
              `${Utils.formatOrder(i + 1)} place: <em>${Utils.escapeHTML(p.name)}</em> <span style="color: lightgreen;">[${p.time}]</span>.<br />`
          )
          .join('')}` +
        `${
          this.completed.length > sliceIndex
            ? `Consolation Prize: ${this.completed
                ...
                .map(
                  e =>
                    `<em>${Utils.escapeHTML(e.name)}</em> <span style="color: lightgreen;">[${e.time}]</span>`
                )
                .join(', ')}<br />`
            : ''
        }<br />` +
  ...
  onEnd(reset?: boolean, endedBy?: User) {
    ...
    if (
      !ScavengerHuntDatabase.isEmpty() &&
      this.room.settings.scavSettings?.addRecycledHuntsToQueueAutomatically
    ) {
      if (!this.room.settings.scavQueue) this.room.settings.scavQueue = [];

      const next = ScavengerHuntDatabase.getRecycledHuntFromDatabase();
      const correctlyFormattedQuestions = next.questions.flatMap((question: AnyObject) => [
        question.text,
        question.answers,
      ]);
    ...
    if (!reset) {
      ...
      if (!this.runEvent('GivePoints')) {
        const winPoints =
          this.room.settings.scavSettings?.winPoints?.[this.gameType] ||
    ...
    if (this.room.settings.scavQueue?.length) {
      setTimeout(() => {
        ...
        if (
          !room ||
          room.game ||
          !room.settings.scavQueue?.length ||
          room.settings.scavSettings?.scavQueueDisabled
        )
          return;
        ...
        if (game) {
          ...
          room
            .add(
              `|c|~|[ScavengerManager] A scavenger hunt by ${Chat.toListString(next.hosts.map(h => h.name))} has been automatically started. It will automatically end in ${duration} minutes.`
            )
            .update(); // highlight the users with "hunt by"
          room.modlog({
            action: 'SCAV NEW',
            note: `${game.gameType.toUpperCase()}: creators - ${game.hosts.map(h => h.id)}`,
          });
        }
  ...
  validatePlayer(player: ScavengerHuntPlayer) {
    ...
    if (uniqueConnections > 1 && this.room.settings.scavSettings?.scavmod?.ipcheck) {
      ...
      player.sendRoom(
        'You have been caught for attempting a hunt with multiple connections on your account.  Staff has been notified.'
      );
...
export class ScavengerHuntPlayer extends Rooms.RoomGamePlayer<ScavengerHunt> {
  ...
  onNotifyChange(num: number) {
    ...
    if (num === this.currentQuestion) {
      this.sendRoom(
        `|raw|<div style="overflow:scroll; max-height: 50vh"><strong>The hint has been changed to:</strong> ${this.game.formatOutput(this.game.questions[num].hint)}</div>`
      );
    }
  }
...
const ScavengerCommands: Chat.ChatCommands = {
  ...
  games: {
    ...
    start(target, room, user) {
      ...
      if (room.getGame(ScavengerHunt)) {
        return this.errorReply(
          'You cannot start a scavenger game where there is already a scavenger hunt in the room.'
        );
      }
    ...
    async rank(target, room, user) {
      ...
      if (!rank) {
        this.sendReplyBox(
          `User '${targetId}' does not have any points on the scavenger games leaderboard.`
        );
      } else {
        this.sendReplyBox(
          Utils.html`User '${rank.name}' is #${rank.rank} on the scavenger games leaderboard with ${rank.points} points.`
        );
      }
    },
  },
  teamscavs: {
    ...
    createteam(target, room, user) {
      ...
      game.teams[teamName] = {
        name: teamName,
        answers: [],
        players: [leaderUser.id],
        question: 1,
        completed: false,
      };
      game.announce(
        Utils.html`A new team "${teamName}" has been created with ${leaderUser.name} as the leader.`
      );
    },
    ...
    addplayer(target, room, user) {
      ...
      const targetUsers = target
        .split(',')
        .map(id => Users.getExact(id))
        .filter(u => u?.connected) as User[];
      ...
      game.announce(
        Utils.html`${Chat.toListString(targetUsers.map(u => u.name))} ${targetUsers.length > 1 ? 'have' : 'has'} been added into ${userTeam.name}.`
      );
    },
      ...
      for (const entry of playerchanges) {
        ...
        if (entry.trim().startsWith('-')) {
          ...
          if (team.players.includes(userid)) {
            ...
            this.errorReply(
              `User "${userid}" is already part of another team - "${targetUserTeam.name}".`
            );
            continue;
          }
    ...
    teams(target, room, user) {
      ...
      for (const teamID in game.teams) {
        const team = game.teams[teamID];
        display.push(
          Utils.html`<strong>${team.name}</strong> - <strong>${team.players[0]}</strong>${team.players.length > 1 ? ', ' + team.players.slice(1).join(', ') : ''}`
        );
      }
    ...
    guesses(target, room, user) {
      ...
      this.sendReplyBox(
        Utils.html`<strong>Question #${team.question} guesses:</strong> ${team.answers.sort().join(', ')}`
      );
    },
  ...
  create(target, room, user, connection, cmd) {
    ...
    if (
      !cmd.includes('force') &&
      ['regular', 'unrated', 'recycled'].includes(gameType) &&
      !mod &&
      room.settings.scavQueue?.length &&
      !room.scavgame
    ) {
      return this.errorReply(
        `There are currently hunts in the queue! If you would like to start the hunt anyways, use /forcestart${gameType === 'regular' ? 'hunt' : gameType}.`
      );
    }
    ...
    if (!hosts.length) {
      return this.errorReply(
        'The user(s) you specified as the host is not online, or is not in the room.'
      );
    }
  ...
  status(target, room, user) {
    ...
    if (game.hosts.some(h => h.id === user.id) || game.staffHostId === user.id) {
      ...
      for (let i = 0; i < game.questions.length; i++) {
        ...
        const players = Object.values(game.playerTable).filter(
          player => player.currentQuestion === i && !player.completed
        );
        if (!players.length) {
          ...
          str += players
            .map(pl =>
              pl.lastGuess > Date.now() - 1000 * 300
  ...
  forceend: 'end',
    ...
    if (!this.cmd.includes('force')) {
      if (!completed.length) {
        return this.errorReply(
          'No one has finished the hunt yet.  Use /forceendhunt if you want to end the hunt and reveal the answers.'
        );
      }
  ...
  edithunt(target, room, user) {
    ...
    if (!game.onEditQuestion(parseInt(question), toID(type), value.join(',').trim())) {
      return this.sendReply(
        '/scavengers edithunt [question number], [hint | answer], [value] - edits the current scavenger hunt.'
      );
    }
  },
  ...
  spoiler(target, room, user) {
    ...
    const playersOnQ = game.players.filter(
      player => player.currentQuestion === question && !player.completed
    );
  ...
  kick(target, room, user) {
    ...
    if (success) {
      ...
      return this.privateModAction(
        `${user.name} has kicked '${targetId}' from the scavenger hunt.`
      );
    }
  ...
  dequeue(target, room, user) {
    ...
    this.privateModAction(
      `${user.name} has removed a scavenger hunt created by [${removed.hosts.map(u => u.id).join(', ')}] from the queue.`
    );
  ...
  viewqueue(target, room, user) {
    ...
    this.sendReply(
      `|uhtml|scav-queue|${formatQueue(room.settings.scavQueue, user, room, this.broadcasting)}`
    );
  },
  ...
  defaulttimer(target, room, user) {
    ...
    if (!target) {
      ...
      return this.sendReply(
        `The default scavenger timer is currently set at: ${duration_string} minutes.`
      );
    }
    ...
    room.saveSettings();
    this.privateModAction(
      `The default scavenger timer has been set to ${duration} minutes by ${user.name}.`
    );
  ...
  addpoints(target, room, user) {
    ...
    this.privateModAction(
      `${targetId} was given ${points} points on the current scavengers ladder by ${user.name}.`
    );
  ...
  removepoints(target, room, user) {
    ...
    this.privateModAction(
      `${user.name} has taken ${points} points from ${targetId} on the current scavengers ladder.`
    );
  ...
  async ladder(target, room, user) {
    ...
    this.sendReply(
      `|uhtml${isChange ? 'change' : ''}|scavladder|<div class="ladder" style="overflow-y: scroll; max-height: 300px;"><table style="width: 100%"><tr><th>Rank</th><th>Name</th><th>Points</th></tr>${ladder
        .map(entry => {
        ...
        .join('')}</table></div>` +
        `<div style="text-align: center"><button class="button" name="send" value="/scav top ${
          hideStaff ? 'yes' : 'no'
        }">${hideStaff ? 'Show' : 'Hide'} Auth</button></div>`
    );
  },
  ...
  async rank(target, room, user) {
    ...
    if (!rank) {
      this.sendReplyBox(
        `User '${targetId}' does not have any points on the scavengers leaderboard.`
      );
    } else {
      this.sendReplyBox(
        Utils.html`User '${rank.name}' is #${rank.rank} on the scavengers leaderboard with ${rank.points} points.`
      );
    }
  },
  ...
  setblitz(target, room, user) {
    ...
    if (!target) {
      ...
      const source = Object.entries(
        Object.assign(DEFAULT_BLITZ_POINTS, room.settings.scavSettings.blitzPoints || {})
      );
      for (const entry of source) {
        points.push(`${entry[0]}: ${entry[1]}`);
      }
      return this.sendReplyBox(
        `The points rewarded for winning hunts within a minute is:<br />${points.join('<br />')}`
      );
    }
    ...
    if (isNaN(blitzPoints) || blitzPoints < 0 || blitzPoints > 1000) {
      return this.errorReply(
        'The points value awarded for blitz must be an integer bewteen 0 and 1000.'
      );
    }
    ...
    room.saveSettings();
    this.privateModAction(
      `${user.name} has set the points awarded for blitz for ${gameType} hunts to ${blitzPoints}.`
    );
    ...
    if (room.parent && !room.persist && scavsRoom) {
      ...
      scavsRoom.sendMods(
        `(${user.name} has set the points awarded for blitz for ${gameType} hunts to ${blitzPoints} in <<${room.roomid}>>.)`
      );
      scavsRoom.roomlog(
        `(${user.name} has set the points awarded for blitz for ${gameType} hunts to ${blitzPoints} in <<${room.roomid}>>.)`
      );
  ...
  sethostpoints(target, room, user) {
    ...
    if (!target) {
      const pointSetting = Object.hasOwnProperty.call(room.settings.scavSettings, 'hostPoints')
        ? room.settings.scavSettings.hostPoints
        : DEFAULT_HOST_POINTS;
    ...
    room.saveSettings();
    this.privateModAction(
      `${user.name} has set the points awarded for hosting regular scavenger hunts to ${points}`
    );
    ...
    if (room.parent && !room.persist) {
      ...
      scavsRoom.sendMods(
        `(${user.name} has set the points awarded for hosting regular scavenger hunts to - ${points} in <<${room.roomid}>>)`
      );
      scavsRoom.roomlog(
        `(${user.name} has set the points awarded for hosting regular scavenger hunts to - ${points} in <<${room.roomid}>>)`
      );
  ...
  setpoints(target, room, user) {
    ...
    if (!target) {
      ...
      for (const entry of source) {
        points.push(
          `${entry[0]}: ${entry[1].map((p: number, i: number) => `(${i + 1}) ${p}`).join(', ')}`
        );
      }
      return this.sendReplyBox(
        `The points rewarded for winning hunts is:<br />${points.join('<br />')}`
      );
    }
    ...
    if (winPoints.some(p => isNaN(p) || p < 0 || p > 1000) || !winPoints.length) {
      return this.errorReply(
        'The points value awarded for winning a scavenger hunt must be an integer between 0 and 1000.'
      );
    }
    ...
    this.privateModAction(
      `${user.name} has set the points awarded for winning ${type} scavenger hunts to - ${pointsDisplay}`
    );
    ...
    if (room.parent && !room.persist) {
      ...
      scavsRoom.sendMods(
        `(${user.name} has set the points awarded for winning ${type} scavenger hunts to - ${pointsDisplay} in <<${room.roomid}>>)`
      );
      scavsRoom.roomlog(
        `(${user.name} has set the points awarded for winning ${type} scavenger hunts to - ${pointsDisplay} in <<${room.roomid}>>)`
      );
  ...
  settwist(target, room, user) {
    ...
    if (target === 'RESET') {
      ...
      if (!ScavMods.twists[twist] || twist === 'constructor')
    ...
    if (room.settings.scavSettings.officialtwist) {
      this.privateModAction(
        `${user.name} has set the official twist to ${room.settings.scavSettings.officialtwist}`
      );
    } else {
    ...
    if (room.parent && !room.persist) {
      if (room.settings.scavSettings.officialtwist) {
        ...
        scavsRoom.sendMods(
          `(${user.name} has set the official twist to - ${room.settings.scavSettings.officialtwist} in <<${room.roomid}>>)`
        );
        scavsRoom.roomlog(
          `(${user.name} has set the official twist to  - ${room.settings.scavSettings.officialtwist} in <<${room.roomid}>>)`
        );
  ...
  twists(target, room, user) {
    ...
    buffer += Object.values(ScavMods.twists)
      .map(
        twist =>
  ...
  async huntlogs(target, room, user) {
    ...
    this.sendReply(
      `|${isUhtmlChange ? 'uhtmlchange' : 'uhtml'}|scav-huntlogs|<div class="ladder" style="overflow-y: scroll; max-height: 300px;"><table style="width: 100%"><tr><th>Rank</th><th>Name</th><th>Hunts Created</th><th>Total Hunts Created</th><th>History</th></tr>${data
        .map(entry => {
          const auth = room.auth.get(toID(entry.name)).trim();
          const color = auth ? 'inherit' : 'gray';
          return (
            `<tr><td>${entry.rank}</td><td><span style="color: ${color}">${auth || '&nbsp;'}</span>${Utils.escapeHTML(entry.name)}</td>` +
        ...
        .join('')}</table></div><div style="text-align: center">${sortingFields
        .map(
          f =>
            `<button class="button${f === sortMethod ? ' disabled' : ''}" name="send" value="/scav huntlogs ${f}, 1">${f}</button>`
        )
        .join(' ')}</div>`
    );
  },
  ...
  async playlogs(target, room, user) {
    ...
    if (target === 'RESET') {
      ...
      PlayerLeaderboard.write();
      this.privateModAction(
        `${user.name} has reset the player log leaderboard into the next month.`
      );
    ...
    const formattedData = data.map(d => {
      ...
      d['cumulative-ratio'] = (
        ((d['cumulative-finish'] || 0) / (d['cumulative-join'] || 1)) *
        100
      ).toFixed(2);
      return d;
    });
    ...
    this.sendReply(
      `|${isUhtmlChange ? 'uhtmlchange' : 'uhtml'}|scav-playlogs|<div class="ladder" style="overflow-y: scroll; max-height: 300px;"><table style="width: 100%"><tr><th>Rank</th><th>Name</th><th>Finished Hunts</th><th>Joined Hunts</th><th>Ratio</th><th>Infractions</th></tr>${formattedData
        .map(entry => {
          const auth = room.auth.get(toID(entry.name)).trim();
          const color = auth ? 'inherit' : 'gray';
        ...
        .map(
          f =>
            `<button class="button${f === sortMethod ? ' disabled' : ''}" name="send" value="/scav playlogs ${f}, 1">${f}</button>`
        )
        .join(' ')}</div>`
    );
  },
  ...
  infract(target, room, user) {
    ...
    this.privateModAction(
      `${user.name} has ${change > 0 ? 'given' : 'taken'} one infraction point ${change > 0 ? 'to' : 'from'} '${targetId}'.`
    );
  ...
  modsettings: {
    ...
    update(target, room, user) {
      ...
      this.sendReply(
        `|uhtml${this.cmd === 'update' ? 'change' : ''}|scav-modsettings|<div class=infobox><strong>Scavenger Moderation Settings:</strong><br /><br />` +
    ...
    ipcheck(target, room, user) {
      ...
      this.privateModAction(
        `${user.name} has set multiple connections verification to ${setting[target] ? 'ON' : 'OFF'}.`
      );
  ...
  recycledhunts(target, room, user) {
    ...
    if (!getScavsRoom(room)) {
      return this.errorReply(
        'Scavenger Hunts can only be added to the database in the scavengers room.'
      );
    }
    ...
    if (cmd === 'removehunt') {
      ...
      return this.privateModAction(
        `Hint added to Recycled hunt #${huntNumber} question #${questionNumber}: ${hintText}.`
      );
    } else if (cmd === 'removehint') {
      ScavengerHuntDatabase.removeHintToRecycledHunt(huntNumber, questionNumber, hintNumber);
      return this.privateModAction(
        `Hint #${hintNumber} was removed from Recycled hunt #${huntNumber} question #${questionNumber}.`
      );
      ...
      if (
        ...
        return this.errorReply(
          `Autostarting recycled hunts is already ${room.settings.scavSettings.addRecycledHuntsToQueueAutomatically ? 'on' : 'off'}.`
        );
      }
      room.settings.scavSettings.addRecycledHuntsToQueueAutomatically =
        !room.settings.scavSettings.addRecycledHuntsToQueueAutomatically;
      this.privateModAction(
        `Automatically adding recycled hunts to the queue is now ${room.settings.scavSettings.addRecycledHuntsToQueueAutomatically ? 'on' : 'off'}`
      );
      if (params[0] === 'on') {
  ...
  scavengershelp(target, room, user) {
    ...
    const display =
      target === 'all'
        ...
        : target === 'staff' || target === 'auth'
          ? staffCommands
          : target === 'games' || target === 'game'
            ? gamesCommands
            : userCommands;
