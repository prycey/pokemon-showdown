...
class Team {
  ...
  maxBid(credits = this.credits) {
    return (
      credits + this.auction.minBid * Math.min(0, this.players.length - this.auction.minPlayers + 1)
    );
  }
}
...
export class Auction extends Rooms.SimpleRoomGame {
  ...
  generateUsernameList(players: (string | Player)[], max = players.length, clickable = false) {
    ...
    if (players.length > max) {
      buf += ` <span title="${players
        .slice(max)
        .map(p => Utils.escapeHTML(typeof p === 'object' ? p.name : p))
        .join(', ')}">(+${players.length - max})</span>`;
    }
    buf += `</span>`;
    return buf;
  }
  ...
  setMinBid(amount: number) {
    if (this.state !== 'setup') {
      throw new Chat.ErrorMessage(
        `The minimum bid cannot be changed after the auction has started.`
      );
    }
  ...
  setMinPlayers(amount: number) {
    if (this.state !== 'setup') {
      throw new Chat.ErrorMessage(
        `The minimum number of players cannot be changed after the auction has started.`
      );
    }
    if (!amount || amount > 30) {
  ...
  setMaxPlayers(amount: number) {
    ...
    if (this.state !== 'setup') {
      throw new Chat.ErrorMessage(
        `The maximum number of players cannot be changed after the auction has started.`
      );
    }
  ...
  setNomTimeLimit(seconds: number) {
    if (this.state !== 'setup') {
      throw new Chat.ErrorMessage(
        `The nomination time limit cannot be changed after the auction has started.`
      );
    }
  ...
  setBidTimeLimit(seconds: number) {
    if (this.state !== 'setup') {
      throw new Chat.ErrorMessage(
        `The bid time limit cannot be changed after the auction has started.`
      );
    }
  ...
  setType(auctionType: string) {
    if (this.state !== 'setup') {
      throw new Chat.ErrorMessage(
        `The auction type cannot be changed after the auction has started.`
      );
    }
    if (!['auction', 'blind', 'snake'].includes(toID(auctionType))) {
      throw new Chat.ErrorMessage(
        `Invalid auction type "${auctionType}". Valid types are "auction", "blind", and "snake".`
      );
    }
  ...
  addAuctionPlayer(name: string, tiersPlayed: string[], tiersNotPlayed: string[]) {
    ...
    if (name.length > 25)
      throw new Chat.ErrorMessage(`Player names must be 25 characters or less.`);
    if (
      tiersPlayed.some(tier => tier.length > 30) ||
      tiersNotPlayed.some(tier => tier.length > 30)
    ) {
      throw new Chat.ErrorMessage(`Tier names must be 30 characters or less.`);
    }
    const player: Player = {
  ...
  assignPlayer(name: string, teamName?: string) {
    ...
    if (teamName) {
      ...
      if (!this.getUndraftedPlayers().length) {
        return this.end(
          'The auction has ended because there are no players remaining in the draft pool.'
        );
      }
    } else {
      player.team?.removePlayer(player);
  ...
  addCreditsToTeam(teamName: string, amount: number) {
    ...
    if (team.maxBid(newCredits) < this.minBid) {
      throw new Chat.ErrorMessage(
        `A team must have enough credits to draft the minimum amount of players.`
      );
    }
  ...
  start() {
    ...
    const problemTeams = [...this.teams.values()]
      .filter(t => t.maxBid() < this.minBid)
      .map(t => t.name);
    if (problemTeams.length) {
      throw new Chat.ErrorMessage(
        `The following teams do not have enough credits to draft the minimum amount of players: ${problemTeams.join(', ')}`
      );
    }
    this.next();
  }
  ...
  next() {
    ...
    if (!this.queue.filter(team => !team.isSuspended()).length) {
      return this.end(
        'The auction has ended because there are no teams remaining that can draft players.'
      );
    }
    if (!this.getUndraftedPlayers().length) {
      return this.end(
    ...
    this.sendHTMLBox(this.generateAuctionTable());
    this.sendMessage(
      `/html It is now <b>${Utils.escapeHTML(this.nominatingTeam.name)}</b>'s turn to nominate a player. Managers: ${this.nominatingTeam
        .getManagers()
        .map(m => `<username class="username">${Utils.escapeHTML(m)}</username>`)
        .join(' ')}`
    );
    this.startNomTimer();
  }
  ...
  nominate(user: User, target: string) {
    ...
    if (this.type === 'snake') {
      this.sendMessage(
        Utils.html`/html <b>${this.nominatingTeam.name}</b> drafted <username>${this.nominatedPlayer.name}</username>!`
      );
      ...
      this.sendMessage(
        Utils.html`/html <username class="username">${user.name}</username> from team <b>${this.nominatingTeam.name}</b> has nominated <username>${player.name}</username> for auction!`
      );
    ...
    if (Number(message.replace(',', '.'))) {
      ...
      try {
        ...
        if (e instanceof Chat.ErrorMessage) {
          ...
          user.sendTo(
            this.room,
            `|raw|<span class="message-error">An unexpected error occurred while placing your bid.</span>`
          );
        }
      }
    }
  ...
  finishCurrentNom() {
    ...
    this.sendMessage(
      Utils.html`/html <b>${this.highestBidder.name}</b> bought <username>${this.nominatedPlayer.name}</username> for <b>${this.highestBid}</b> credits!`
    );
  ...
  pokeNomTimer() {
    ...
    if (!this.nomTimeRemaining) {
      ...
      if (this.nomTimeRemaining % 30 === 0 || [20, 10, 5].includes(this.nomTimeRemaining)) {
        this.sendMessage(
          `/html <span class="message-error">${this.nomTimeRemaining} seconds left!</span>`
        );
      }
    }
  }
  ...
  pokeBidTimer() {
    ...
    if (!this.bidTimeRemaining) {
      ...
      if (this.bidTimeRemaining % 30 === 0 || [20, 10, 5].includes(this.bidTimeRemaining)) {
        this.sendMessage(
          `/html <span class="message-error">${this.bidTimeRemaining} seconds left!</span>`
        );
      }
    }
  }
...
export const commands: Chat.ChatCommands = {
  auction: {
    ...
    createhelp: [`/auction create [startingcredits] - Creates an auction. Requires: % @ # ~`],
    ...
    minbidhelp: [`/auction minbid [amount] - Sets the minimum bid. Requires: # ~ auction owner`],
    ...
    addowners(target, room, user) {
      ...
      this.addModAction(
        `${user.name} added ${Chat.toListString(owners.map(o => Users.getExact(o)!.name))} as auction owner${Chat.plural(owners.length)}.`
      );
    },
    addownershelp: [
    ...
    removeowners(target, room, user) {
      ...
      this.addModAction(
        `${user.name} removed ${Chat.toListString(owners.map(o => Users.getExact(o)?.name || o))} as auction owner${Chat.plural(owners.length)}.`
      );
    },
    removeownershelp: [
    ...
    addmanagers(target, room, user) {
      ...
      this.addModAction(
        `${user.name} added ${Chat.toListString(managers)} as manager${Chat.plural(managers.length)} for team ${team.name}.`
      );
    },
    addmanagershelp: [
    ...
    removemanagers(target, room, user) {
      ...
      const managers = managerNames.map(m => Users.getExact(m)?.name || toID(m));
      this.addModAction(
        `${user.name} removed ${Chat.toListString(managers)} as manager${Chat.plural(managers.length)}.`
      );
    },
    removemanagershelp: [
    ...
    addcredits(target, room, user) {
      ...
      this.addModAction(
        `${user.name} ${credits < 0 ? 'removed' : 'added'} ${Math.abs(credits)} credits ${credits < 0 ? 'from' : 'to'} team ${team.name}.`
      );
    },
    addcreditshelp: [
    ...
    nominatehelp: [`/auction nominate OR /nom [player] - Nominates a player for auction.`],
    skip: 'skipnom',
    ...
    running() {
      ...
      const runningAuctions = [...Rooms.rooms.values()]
        .filter(r => r.getGame(Auction))
        .map(r => r.title);
  ...
  bid() {
    this.errorReply(
      `/bid is no longer supported. Send the amount by itself in the chat to place your bid.`
    );
  },
  overpay() {
