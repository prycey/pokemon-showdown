...
export const pages: Chat.PageTable = {
  repeats(args, user) {
    ...
    for (const repeat of room.settings.repeats) {
      ...
      const phrase = repeat.faq
        ? visualizeFaq(roomFaqs[room.roomid][repeat.id])
        : repeat.isHTML
          ? repeat.phrase
          : Chat.formatText(repeatText, true);
...
export const commands: Chat.ChatCommands = {
  ...
  repeat(target, room, user, connection, cmd) {
    ...
    if (!id)
      throw new Chat.ErrorMessage(
        this.tr`Repeat names must include at least one alphanumeric character.`
      );
    ...
    if (
      isNaN(interval) ||
      !/[0-9]{1,}/.test(intervalString) ||
      interval < 1 ||
      interval > 24 * 60
    ) {
      throw new Chat.ErrorMessage(
        this
          .tr`You must specify an interval as a number of minutes or chat messages between 1 and 1440.`
      );
    }

    if (Repeats.hasRepeat(room, id)) {
      throw new Chat.ErrorMessage(
        this.tr`The phrase labeled with "${id}" is already being repeated in this room.`
      );
    }
    ...
    this.modlog(
      'REPEATPHRASE',
      null,
      `every ${interval} ${isByMessages ? `chat messages` : `minute`}${Chat.plural(interval)}: "${phrase.replace(/\n/g, ' ')}"`
    );
  ...
  repeatfaq(target, room, user, connection, cmd) {
    ...
    if (Repeats.hasRepeat(room, topic as ID)) {
      throw new Chat.ErrorMessage(
        this.tr`The text for the Room FAQ "${topic}" is already being repeated.`
      );
    }
    ...
    this.modlog(
      'REPEATPHRASE',
      null,
      `every ${interval} ${isByMessages ? 'chat message' : 'minute'}${Chat.plural(interval)}: the Room FAQ for "${topic}"`
    );
    this.privateModAction(
      isByMessages
  ...
  removerepeat(target, room, user) {
    ...
    if (!Repeats.hasRepeat(room, id)) {
      return this.errorReply(
        this.tr`The phrase labeled with "${id}" is not being repeated in this room.`
      );
    }
