...
export const prenoms: { [k: string]: [string, AnyObject][] } = JSON.parse(
  FS(PRENOMS_FILE).readIfExistsSync() || '{}'
);
...
class OtdHandler {
  ...
  lastPrenom: number;
  winners: AnyObject[];
  constructor(id: string, room: Room, settings: OtdSettings) {
    this.id = id;
  ...
  addNomination(user: User, nomination: string) {
    ...
    if (this.winners.slice(-30).some(entry => toNominationId(entry[this.keys[0]]) === id)) {
      return user.sendTo(
        this.room,
        `This ${this.name.toLowerCase()} has already been ${this.id} in the past month.`
      );
    }
    for (const value of this.removedNominations.values()) {
      if (
        value.userids.includes(toID(user)) ||
        (!Config.noipchecks && value.ips.includes(user.latestIp))
      ) {
        return user.sendTo(
          this.room,
    ...
    if (prevNom) {
      if (
        !(
          prevNom.userids.includes(toID(user)) ||
          (!Config.noipchecks && prevNom.ips.includes(user.latestIp))
        )
      ) {
        return user.sendTo(
          this.room,
          `This ${this.name.toLowerCase()} has already been nominated.`
        );
      }
    }
  ...
  generateNomWindow() {
    ...
    for (const value of this.nominations.values()) {
      entries.push(
        Utils.html`<li><b>${value.nomination}</b> <i>(Submitted by ${value.name})</i></li>`
      );
    }
  ...
  rollWinner() {
    ...
    this.room.add(
      Utils.html`|uhtml|otd|<div class="broadcast-blue"><p style="font-weight:bold;text-align:center;font-size:12pt;">` +
        ...
        `(Nomination by ${winner.name})</p><p style="font-weight:bold;">Thanks to today's participants:` +
        namesHTML
    );
    this.room.update();
  ...
  async generateWinnerDisplay() {
    ...
    let output =
      `<div class="broadcast-blue" style="text-align:center;">` +
    ...
    if (winner.quote)
      output += Utils.html`<br /><span style="font-style:italic;">"${winner.quote}"</span>`;
  ...
  generateWinnerList(context: Chat.PageContext) {
    ...
    for (let i = 0; i < this.keys.length; i++) {
      if (
        i === 0 ||
        (['song', 'event', 'link', 'tagline', 'sport', 'country'].includes(this.keys[i]) &&
          !(this.keys[i] === 'link' && this.keys.includes('song')))
      ) {
...
export const otdCommands: Chat.ChatCommands = {
  start(target, room, user, connection, cmd) {
    ...
    this.privateModAction(
      `${user.name} has started nominations for the ${handler.name} of the ${handler.timeLabel}.`
    );
  ...
  starthelp: [`/-otd start - Starts nominations for the Thing of the Day. Requires: % @ # ~`],
    ...
    if (!handler.voting) {
      return this.errorReply(
        `There is no ${handler.name} of the ${handler.timeLabel} nomination in progress.`
      );
    }
    ...
    this.privateModAction(
      `${user.name} has ended nominations for the ${handler.name} of the ${handler.timeLabel}.`
    );
  ...
  remove(target, room, user) {
    ...
    if (handler.removeNomination(userid)) {
      this.privateModAction(
        `${user.name} removed ${target}'s nomination for the ${handler.name} of the ${handler.timeLabel}.`
      );
      this.modlog(`${handler.id.toUpperCase()} REMOVENOM`, userid);
    } else {
      this.sendReply(
        `User '${target}' has no nomination for the ${handler.name} of the ${handler.timeLabel}.`
      );
    }
  },
  removehelp: [
  ...
  removewinner(target, room, user) {
    ...
    this.privateModAction(
      `${user.name} removed the nomination for ${removed[handler.keys[0]]} from ${removed.nominator}`
    );
    this.modlog(
      `${handler.id.toUpperCase()} REMOVEWINNER`,
      removed.nominator,
      removed[handler.keys[0]]
    );
  },
  removewinnerhelp: [
  ...
  force(target, room, user) {
    ...
    this.privateModAction(
      `${user.name} forcibly set the ${handler.name} of the ${handler.timeLabel} to ${target}.`
    );
  ...
  delay(target, room, user) {
    ...
    if (!(handler.voting && handler.timer)) {
      return this.errorReply(
        `There is no ${handler.name} of the ${handler.timeLabel} nomination to disable the timer for.`
      );
    }
    clearTimeout(handler.timer);

    this.privateModAction(
      `${user.name} disabled the ${handler.name} of the ${handler.timeLabel} timer.`
    );
  },
  delayhelp: [
  ...
  set(target, room, user) {
    ...
    for (const param of params) {
      ...
      switch (key) {
        ...
        case 'image':
          if (!/https?:\/\//.test(value)) {
            return this.errorReply(
              `Please enter a valid URL for the ${key} (starting with http:// or https://)`
            );
          }
    ...
    if (keys.length) {
      ...
      return this.privateModAction(
        `${user.name} changed the following propert${Chat.plural(keys, 'ies', 'y')} of the ${handler.name} of the ${handler.timeLabel}: ${keys.join(', ')}`
      );
    }
  },
  sethelp: [
  ...
  toggleupdate(target, room, user) {
    ...
    if (this.meansYes(target)) {
      if (otd.settings.updateOnNom) {
        throw new Chat.ErrorMessage(
          `This -OTD is already set to update automatically on nomination.`
        );
      }
    ...
    this.privateModAction(
      `${user.name} set the ${otd.name} of the ${otd.timeLabel} to ${logMessage}`
    );
...
export const commands: Chat.ChatCommands = {
  otd: {
    ...
    delete(target, room, user) {
      ...
      this.privateGlobalModAction(
        `${user.name} deleted the OTD ${otd.name} of the ${otd.timeLabel}`
      );
    },
  },
  otdhelp: [
...
for (const otd in otdData) {
  ...
  if (!room) {
    Monitor.warn(
      `Room for -otd ${settings.title} of the ${settings.timeLabel} (${settings.roomid}) not found.`
    );
    continue;
  }
