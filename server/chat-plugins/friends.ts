...
export const Friends = new (class {
  async notifyPending(user: User) {
    ...
    if (pendingCount === 1) {
      ...
      sendPM(
        Utils.html`/raw <span class="username">${senderName}</span> sent you a friend request!`,
        user.id
      );
      sendPM(buf, user.id);
      sendPM(
  ...
  checkCanUse(context: Chat.CommandContext | Chat.PageContext) {
    ...
    if (!user.autoconfirmed) {
      throw new Chat.ErrorMessage(
        context.tr`You must be autoconfirmed to use the friends feature.`
      );
    }
  ...
  friends: {
    ...
    async accept(target, room, user, connection) {
      ...
      if (user.settings.blockFriendRequests) {
        return this.errorReply(
          this.tr`You are currently blocking friend requests, and so cannot accept your own.`
        );
      }
      if (!target) return this.parse('/help friends');
    ...
    toggle(target, room, user, connection) {
      ...
      if (this.meansYes(target)) {
        ...
        this.sendReply(
          this.tr(
            setting
              ? `You are currently blocking friend requests.`
              : `You are not blocking friend requests.`
          )
        );
      }
    ...
    viewnotifications(target, room, user, connection, cmd) {
      ...
      if (!cmd.includes('hide') || (target && this.meansYes(target))) {
        ...
        this.sendReply(
          this.tr(
            setting
              ? `You are currently allowing friend notifications.`
              : `Your friend notifications are disabled.`
          )
        );
      }
      this.refreshPage('friends-settings');
    ...
    async togglelogins(target, room, user, connection, cmd) {
      ...
      if (cmd.includes('hide')) {
        ...
        if (!setting)
          return this.errorReply(
            this.tr`You are already allowing friends to see your login times.`
          );
    ...
    async listdisplay(target, room, user, connection) {
      ...
      if (this.meansYes(target)) {
        if (setting) {
          return this.errorReply(
            this.tr`You are already allowing other people to view your friends list.`
          );
        }
        await Chat.Friends.setHideList(user.id, true);
        this.refreshPage('friends-settings');
        return this.sendReply(
          this.tr`You are now allowing other people to view your friends list.`
        );
      } else if (this.meansNo(target)) {
        if (!setting) {
        ...
        if (!battles.length) {
          ...
          buf += battles
            .map(([friend, battle]) => {
              ...
              const rated = room.battle.rated
                ? `<small style="float:right">(Rated: ${room.battle.rated})</small>`
                : '';
