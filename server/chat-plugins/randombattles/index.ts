...
function getHTMLCriteriaDescription(criteria: SetCriteria) {
  const format = (list: { name: string }[]) =>
    list.map(m => Utils.html`<strong>${m.name}</strong>`);
  const parts = [];
  ...
  if (moves.mustHave.length) {
    parts.push(
      `had the move${Chat.plural(moves.mustHave)} ${Chat.toListString(format(moves.mustHave))}`
    );
  }
  if (moves.mustNotHave.length) {
    parts.push(
      `did not have the move${Chat.plural(moves.mustNotHave)} ${Chat.toListString(format(moves.mustNotHave), 'or')}`
    );
  }
  ...
  if (ability.mustNotHave.length) {
    parts.push(
      `did not have the ${Chat.plural(ability.mustNotHave, 'abilities', 'ability')} ${Chat.toListString(format(ability.mustNotHave), 'or')}`
    );
  }
  ...
  if (item.mustNotHave.length) {
    parts.push(
      `did not have the item${Chat.plural(item.mustNotHave)} ${Chat.toListString(format(item.mustNotHave), 'or')}`
    );
  }
  ...
  if (nature.mustNotHave.length) {
    parts.push(
      `did not have the nature${Chat.plural(nature.mustNotHave)} ${Chat.toListString(format(nature.mustNotHave), 'or')}`
    );
  }
  ...
  if (teraType.mustNotHave.length) {
    parts.push(
      `did not have the Tera Type${Chat.plural(teraType.mustNotHave)} ${Chat.toListString(format(teraType.mustNotHave), 'or')}`
    );
  }
...
function getSets(
  ...
  format: string | Format = 'gen9randombattle'
): {
  level: number;
  sets: any[];
} | null {
  ...
  const setsFile = JSON.parse(
    FS(
      `data/random-battles/${folderName}/${isDoubles ? 'doubles-' : ''}sets.json`
    ).readIfExistsSync() || '{}'
  );
...
function battleFactorySets(
  ...
  const statsFile = JSON.parse(
    FS(
      `data/random-battles/gen${genNum}/${isBSS ? `bss-` : ``}factory-sets.json`
    ).readIfExistsSync() || '{}'
  );
  ...
  if (!isBSS) {
    ...
    if (!(TIERS[toID(tier)] in statsFile)) {
      throw new Chat.ErrorMessage(
        `${TIERS[toID(tier)]} is not included in [Gen ${genNum}] Battle Factory.`
      );
    }
    ...
    if (!(species.id in t)) {
      ...
      throw new Chat.ErrorMessage(
        `${species.name} doesn't have any sets in ${TIERS[toID(tier)]} for ${formatName}.`
      );
    }
...
export const commands: Chat.ChatCommands = {
  ...
  randombattles(target, room, user, connection, cmd) {
    ...
    if (!searchResults?.length) {
      this.errorReply(
        `No Pok\u00e9mon named '${args[0]}' was found${Dex.gen > dex.gen ? ` in Gen ${dex.gen}` : ''}. (Check your spelling?)`
      );
      return;
    }
    ...
    if (dex.gen === 1) {
      ...
      if (!rbyMoves) {
        ...
        return this.errorReply(
          `Error: ${species.name} has no Random Battle data in ${GEN_NAMES[toID(args[1])]}`
        );
      }
      movesets.push(
        `<span style="color:#999999;">Moves for ${species.name} in ${format.name}:</span>${rbyMoves}`
      );
      setCount = 1;
    } else if (isLetsGo) {
      ...
      if (!lgpeMoves) {
        this.sendReply(inexactMsg);
        return this.errorReply(
          `Error: ${species.name} has no Random Battle data in [Gen 7 Let's Go]`
        );
      }
      movesets.push(
        `<span style="color:#999999;">Moves for ${species.name} in ${format.name}:</span><br />${lgpeMoves}`
      );
      setCount = 1;
    } else {
  ...
  cap1v1(target, room, user) {
    ...
    if (!species.exists)
      return this.errorReply(`Error: Pok\u00e9mon '${target.trim()}' not found.`);
  ...
  cap1v1help: [`/cap1v1 [pokemon] - Displays a Pok\u00e9mon's CAP 1v1 sets.`],
  ...
  randombattlesetprobabilities(target, room, user) {
    ...
    if (possibleFormat.exists) {
      if (!possibleFormat.team) {
        throw new Chat.ErrorMessage(
          `${possibleFormat.name} does not have randomly-generated teams.`
        );
      }
    ...
    if (!species.exists) {
      throw new Chat.ErrorMessage(
        `Species ${species.name} does not exist in the specified format.`
      );
    }
    ...
    if (!setExists) {
      throw new Chat.ErrorMessage(
        `${species.name} does not have random battle moves in ${format.name}.`
      );
    }
        ...
        case 'item':
          ...
          if (itemNegation) {
            ...
            if (criteria.item.mustHave) {
              throw new Chat.ErrorMessage(
                `Impossible situation: two items (${criteria.item.mustHave.name} and ${item.name}) are required.`
              );
            }
        ...
        case 'ability':
          ...
          if (abilityNegation) {
            ...
            if (criteria.ability.mustHave) {
              throw new Chat.ErrorMessage(
                `Impossible situation: two abilities (${criteria.ability.mustHave.name} and ${ability.name}) are required.`
              );
            }
        ...
        case 'nature':
          ...
          if (natureNegation) {
            ...
            if (criteria.nature.mustHave) {
              throw new Chat.ErrorMessage(
                `Impossible situation: two natures (${criteria.nature.mustHave.name} and ${nature.name}) are required.`
              );
            }
        ...
        case 'teratype':
          ...
          if (typeNegation) {
            ...
            if (criteria.teraType.mustHave) {
              throw new Chat.ErrorMessage(
                `Impossible situation: two Tera Types (${criteria.teraType.mustHave.name} and ${type.name}) are required.`
              );
            }
  ...
  generateteamhelp: [
    `/genteam [format] - Generates a team for the given format. Requires: % @ ~ or Random Battles room auth`,
  ],
};
