...
export function getSpeciesName(set: PokemonSet, format: Format) {
  ...
  const megaRayquazaPossible =
    ['gen6', 'gen7'].includes(format.mod) && !format.ruleset.includes('Mega Rayquaza Clause');
  if (species.startsWith('Pikachu-')) {
    ...
    species === 'Rayquaza' &&
    moves.includes('Dragon Ascent') &&
    !item.zMove &&
    megaRayquazaPossible
  ) {
    return 'Rayquaza-Mega';
  } else if (species === 'Poltchageist-Artisan') {
    // Babymons from here on out
    return 'Poltchageist';
...
export const commands: Chat.ChatCommands = {
  ...
  randswinrates(target, room, user) {
    ...
    return this.parse(
      `/j view-winrates-${target ? Dex.formats.get(target).id : `gen${Dex.gen}randombattle`}`
    );
  },
  randswinrateshelp: [
  ...
  async removewinrates(target, room, user) {
    ...
    this.privateGlobalModAction(
      `${user.name} removed Random Battle winrates for the month of ${target}`
    );
  },
};
...
export const pages: Chat.PageTable = {
  async winrates(query, user) {
    ...
    const prevMonth = new Date(new Date(`${month}-15`).getTime() - 30 * 24 * 60 * 60 * 1000)
      .toISOString()
      .slice(0, 7);
    let hasButton = false;
    ...
    const statData: Stats =
      month === stats.month
        ? stats
        : JSON.parse(await FS(STATS_PATH.replace('{{MONTH}}', month)).read());
