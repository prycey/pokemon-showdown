...
export const TeamsHandler = new (class {
  ...
  readyPromise: Promise<void> | null = Config.usepostgres
    ? (async () => {
        try {
  ...
  async search(search: TeamSearch, user: User, count = 10, includePrivate = false) {
    ...
    return result.filter(row => {
      ...
      if (search.pokemon?.length) {
        match = search.pokemon.some(pokemon =>
          team.some(set => toID(set.species) === toID(pokemon))
        );
      }
      if (!match) return false;
      if (search.moves?.length) {
        match = search.moves.some(move =>
          team.some(set => set.moves.some(m => toID(m) === toID(move)))
        );
      }
      if (!match) return false;
      if (search.abilities?.length) {
        match = search.abilities.some(ability =>
          team.some(set => toID(set.ability) === toID(ability))
        );
      }
      return match;
    });
  ...
  async save(
    ...
    if (count >= MAX_TEAMS) {
      connection.popup(
        `You have too many teams stored. If you wish to upload this team, delete some first.`
      );
      return null;
    }
    ...
    if (isUpdate && existing) {
      ...
      const exists = await this.query('SELECT * FROM teams WHERE ownerid = $1 AND team = $2', [
        user.id,
        rawTeam,
      ]);
      if (exists.length) {
  ...
  list(userid: ID, count: number, publicOnly = false) {
    ...
    return this.query<StoredTeam>(query, [userid, count]);
  }
  preview(teamData: StoredTeam, user?: User | null, isFull = false) {
    ...
    if (!team) {
      Monitor.crashlog(
        new Error(`Malformed team drawn from database`),
        'A teams chat page',
        teamData
      );
  ...
  renderTeam(teamData: StoredTeam, user?: User) {
    ...
    if (!team) {
      Monitor.crashlog(
        new Error('Invalid team retrieved from database'),
        'A teams database request',
        teamData
      );
      throw new Chat.ErrorMessage(
        'An error occurred with retrieving the team. Please try again later.'
      );
    }
    buf += team
      .map(set => {
        ...
        if (set.name && set.name !== set.species) {
          teamBuf = teamBuf.replace(
            set.name,
            Utils.html`<psicon pokemon="${set.species}" /> <br />${set.name}`
          );
        } else {
          teamBuf = teamBuf.replace(
            set.species,
            `<psicon pokemon="${set.species}" /> <br />${set.species}`
          );
        }
        if (set.item) {
  ...
  async count(user: string | User) {
    ...
    const result = await this.query<{ count: number }>(
      `SELECT count(*) AS count FROM teams WHERE ownerid = $1`,
      [id]
    );
  ...
  async get(teamid: number | string): Promise<StoredTeam | null> {
    ...
    const rows = await this.query(`SELECT * FROM teams WHERE teamid = $1`, [teamid]);
  ...
  async delete(id: string | number) {
    ...
    await this.query(`DELETE FROM teams WHERE teamid = $1`, [id]);
  }
})();
...
export const commands: Chat.ChatCommands = {
  teams: {
    ...
    async setprivacy(target, room, user, connection) {
      ...
      await TeamsHandler.query(`UPDATE teams SET private = $1 WHERE teamid = $2`, [
        privacy,
        teamId,
      ]);
      ...
      return this.popupReply(
        privacy ? `Team set to private. Password: ${privacy}` : `Team set to public.`
      );
    },
...
export const pages: Chat.PageTable = {
  ...
  team(query, user, connection) {
    return ((pages.teams as Chat.PageTable).view as Chat.PageHandler).call(
      this,
      query,
      user,
      connection
      ...
      switch (sorter) {
        ...
        default:
          return this.errorReply(
            `Invalid sort term '${sorter}'. Must be either 'views' or 'latest'.`
          );
      }
