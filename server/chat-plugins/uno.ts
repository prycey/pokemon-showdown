...
const textShadow =
  'text-shadow: 1px 0px black, -1px 0px black, 0px -1px black, 0px 1px black, 2px -2px black;';
...
export class UNO extends Rooms.RoomGame<UNOPlayer> {
        ...
        `|uhtml|uno-${this.gameNumber}|<div class="broadcast-blue">` +
          ...
          `${
            this.suppressMessages
              ? `<p style="font-size: 6pt; text-align: center">Game messages won't show up unless you're playing or watching.`
              : ''
          }</div>`
      );
  ...
  onStart(isAutostart?: boolean) {
    ...
    this.sendToRoom(
      `|uhtmlchange|uno-${this.gameNumber}|<div class="infobox"><p>The game of UNO has started. <button class="button" name="send" value="/uno spectate">Spectate Game</button></p>${this.suppressMessages ? `<p style="font-size: 6pt">Game messages won't show up unless you're playing or watching.` : ''}</div>`,
      true
    );
    this.state = 'play';
    ...
    this.sendToRoom(
      `|raw|The top card is <span style="font-weight:bold;color: ${textColors[this.topCard.color]}">${this.topCard.name}</span>.`
    );
  ...
  eliminate(player: UNOPlayer | null) {
    ...
    if (removingCurrentPlayer) {
      if (this.state === 'color') {
        ...
        this.sendToRoom(
          `|raw|${Utils.escapeHTML(name)} has not picked a color, the color will stay as <span style="color: ${textColors[this.topCard.changedColor]}">${this.topCard.changedColor}</span>.`
        );
      }
    }
  ...
  nextTurn(starting?: boolean) {
    void this.onAwaitUno().then(() => {
      ...
      this.timer = setTimeout(() => {
        this.sendToRoom(
          `|c:|${Math.floor(Date.now() / 1000)}|~|${player.name} has been automatically disqualified.`
        );
  ...
  onPlay(player: UNOPlayer, cardName: string) {
    ...
    this.sendToRoom(
      `|raw|${Utils.escapeHTML(player.name)} has played a <span style="font-weight:bold;color: ${textColors[card.color]}">${card.name}</span>.`
    );
  ...
  onRunEffect(value: string, initialize?: boolean) {
    ...
    switch (value) {
      case 'Reverse':
        this.direction *= -1;
        this.sendToRoom(
          `|c:|${Math.floor(Date.now() / 1000)}|~|The direction of the game has changed.`
        );
      ...
      case 'Skip':
        this.onNextPlayer();
        this.sendToRoom(
          `|c:|${Math.floor(Date.now() / 1000)}|~|${this.currentPlayer!.name}'s turn has been skipped.`
        );
        break;
      case '+2':
        this.onNextPlayer();
        this.sendToRoom(
          `|c:|${Math.floor(Date.now() / 1000)}|~|${this.currentPlayer!.name} has been forced to draw 2 cards.`
        );
      ...
      case '+4':
        ...
        this.sendToRoom(
          `|c:|${Math.floor(Date.now() / 1000)}|~|${next.name} has been forced to draw 4 cards.`
        );
        this.onDrawCard(next, 4);
        this.isPlusFour = true;
        this.timer = setTimeout(() => {
          this.sendToRoom(
            `|c:|${Math.floor(Date.now() / 1000)}|~|${this.currentPlayer!.name} has been automatically disqualified.`
          );
  ...
  onSelectColor(player: UNOPlayer, color: Color) {
    ...
    this.sendToRoom(
      `|c:|${Math.floor(Date.now() / 1000)}|~|The color has been changed to ${color}.`
    );
    if (this.timer) clearTimeout(this.timer);
  ...
  drawCard(count: number) {
    ...
    for (let i = 0; i < count; i++) {
      if (!this.deck.length) {
        ...
        this.deck = this.discards.length
          ? Utils.shuffle(this.discards)
          : Utils.shuffle(createDeck());
  ...
  onUno(player: UNOPlayer, unoId: ID) {
    ...
    this.sendToRoom(
      `|c:|${Math.floor(Date.now() / 1000)}|~|**UNO!** ${player.name} is down to their last card!`
    );
  ...
  onCheckUno() {
    ...
    if (this.awaitUnoPlayer !== this.currentPlayer) {
      this.sendToRoom(
        `|c:|${Math.floor(Date.now() / 1000)}|~|${this.awaitUnoPlayer.name} forgot to say UNO! and is forced to draw 2 cards.`
      );
  ...
  override destroy() {
    ...
    if (this.autostartTimer) clearTimeout(this.autostartTimer);
    this.sendToRoom(
      `|uhtmlchange|uno-${this.gameNumber}|<div class="infobox">The game of UNO has ended.</div>`,
      true
    );
...
class UNOPlayer extends Rooms.RoomGamePlayer<UNO> {
  ...
  buildHand() {
    return Utils.sortBy(this.hand, card => [card.color, card.value]).map((card, i) =>
      cardHTML(card, i === this.hand.length - 1)
    );
  }
  ...
  sendDisplay() {
    ...
    const draw =
      '<button class="button" style="width: 45%; background: rgba(0, 0, 255, 0.05)" name=send value="/uno draw">Draw a card!</button>';
    const pass =
      '<button class="button" style=" width: 45%; background: rgba(255, 0, 0, 0.05)" name=send value="/uno pass">Pass!</button>';
  ...
  uno: {
    ...
    create(target, room, user, connection, cmd) {
      ...
      const suppressMessages =
        cmd.includes('private') || !(cmd.includes('public') || room.roomid === 'gamecorner');
    ...
    setcap(target, room, user) {
      ...
      if (game.state !== 'signups') {
        throw new Chat.ErrorMessage(
          `There is no UNO game in the signups phase in this room, so adjusting the player cap would do nothing.`
        );
      }
      let cap = parseInt(target);
      if (isNaN(cap) || cap < 2) {
    ...
    timer(target, room, user) {
      ...
      this.addModAction(
        `${user.name} has set the UNO automatic disqualification timer to ${amount} seconds.`
      );
    ...
    suppress(target, room, user) {
      ...
      if (state === undefined) {
        return this.sendReply(
          `Suppression of UNO game messages is currently ${game.suppressMessages ? 'on' : 'off'}.`
        );
      }
      if (state === game.suppressMessages) {
        throw new Chat.ErrorMessage(
          `Suppression of UNO game messages is already ${game.suppressMessages ? 'on' : 'off'}.`
        );
      }
      ...
      this.addModAction(
        `${user.name} has turned ${state ? 'on' : 'off'} suppression of UNO game messages.`
      );
