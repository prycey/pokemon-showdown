...
export const videoDataCache: Map<string, VideoData> =
  Chat.oldPlugins.youtube?.videoDataCache || new Map();
export const searchDataCache: Map<string, string[]> =
  Chat.oldPlugins.youtube?.searchDataCache || new Map();
...
function loadData() {
  ...
  if (!(raw.channels && raw.categories)) {
    // hasn't been converted to new format
...
type GroupwatchData =
  | (VideoData & { groupwatchType: 'youtube' })
  | (TwitchChannel & { groupwatchType: 'twitch' });
export class GroupWatch extends Rooms.SimpleRoomGame {
  ...
  hints() {
    ...
    if (this.started && this.info.groupwatchType === 'youtube') {
      ...
      hints.push(
        `Video is currently at ${Chat.toDurationString(diff)} (${Math.floor(diff / 1000)} seconds)`
      );
    }
    return hints;
  }
...
export const commands: Chat.ChatCommands = {
  ...
  youtube: {
    ...
    removechannelhelp: [
      `/youtube removechannel - Delete channel data from the YouTube database. Requires: % @ #`,
    ],
    ...
    repeat(target, room, user) {
      ...
      if (!target) {
        ...
        return this.sendReply(
          `Interval is currently set to ${Chat.toDurationString(YouTube.intervalTime * 60 * 1000)}.`
        );
      }
    ...
    addcategory(target, room, user) {
      ...
      if (YouTube.data.categories.map(toID).includes(categoryID)) {
        return this.errorReply(
          `This category is already added. To change it, remove it and re-add it.`
        );
      }
    ...
    removecategory(target, room, user) {
      ...
      this.privateModAction(
        `${user.name} removed the category '${target}' from the category list.`
      );
    ...
    setcategory(target, room, user) {
      ...
      this.privateModAction(
        `${user.name} set the channel ${channel.name}'s category to '${category}'.`
      );
    },
    decategorize(target, room, user) {
      ...
      this.privateModAction(
        `${user.name} removed the channel ${channel.name} from the category ${category}.`
      );
    },
  },
  youtubehelp: [
  ...
  groupwatch: {
    async create(target, room, user) {
      ...
      if (!url || !title)
        return this.errorReply(
          `You must specify a video to watch and a title for the group watch.`
        );
    ...
    switch (toID(type)) {
      case 'categories':
        if (!YouTube.data.categories.length) {
          return this.errorReply(
            `There are currently no categories in the Youtube channel database.`
          );
        }
