...
export const database: TriviaDatabase = new TriviaSQLiteDatabase(
  'config/chat-plugins/triviadata.json'
);
...
function getTriviaGame(room: Room | null) {
  ...
  if (game.gameid !== 'trivia') {
    throw new Chat.ErrorMessage(
      room.tr`The currently running game is not Trivia, it's ${game.title}.`
    );
  }
...
function getMastermindGame(room: Room | null) {
  ...
  if (game.gameid !== 'mastermind') {
    throw new Chat.ErrorMessage(
      room.tr`The currently running game is not Mastermind, it's ${game.title}.`
    );
  }
...
export async function requestAltMerge(from: ID, to: ID) {
  ...
  if (!(await database.getLeaderboardEntry(from, 'alltime'))) {
    throw new Chat.ErrorMessage(
      `The user '${from}' does not have an entry in the Trivia leaderboard.`
    );
  }
  if (!(await database.getLeaderboardEntry(to, 'alltime'))) {
    throw new Chat.ErrorMessage(
      `The user '${to}' does not have an entry in the Trivia leaderboard.`
    );
  }
...
export async function mergeAlts(from: ID, to: ID) {
  if (pendingAltMerges.get(from) !== to) {
    throw new Chat.ErrorMessage(
      `Both '${from}' and '${to}' must use /trivia mergescore to approve the merge.`
    );
  }
...
class Ladder {
  ...
  async computeCachedLadder() {
    ...
    for (const [lb, data] of Object.entries(leaderboards) as [
      Leaderboard,
      ...
      for (const key of [
        ...
        'totalPoints',
        'totalCorrectAnswers',
      ] as (keyof TriviaLeaderboardScore)[]) {
...
export class Trivia extends Rooms.RoomGame<TriviaPlayer> {
  ...
  addTriviaPlayer(user: User) {
    ...
    if (this.kickedUsers.has(user.id)) {
      throw new Chat.ErrorMessage(
        this.room.tr`You were kicked from the game and thus cannot join it again.`
      );
    }
    for (const id of user.previousIDs) {
      if (this.playerTable[id]) {
      ...
      if (this.kickedUsers.has(id)) {
        throw new Chat.ErrorMessage(
          this.room.tr`You were kicked from the game and cannot join until the next game.`
        );
      }
    }
  ...
  init() {
    ...
    broadcast(
      ...
      this.room
        .tr`Mode: ${this.game.mode} | Category: ${this.game.category} | Cap: ${this.getDisplayableCap()}<br />` +
        `<button class="button" name="send" value="/trivia join">` +
        this.room.tr`Sign up for the Trivia game!` +
        `</button>` +
  ...
  getDescription() {
    return this.room
      .tr`Mode: ${this.game.mode} | Category: ${this.game.category} | Cap: ${this.getDisplayableCap()}`;
  }
  ...
  formatPlayerList(settings: { max: number | null; requirePoints?: boolean }) {
    return this.getTopPlayers(settings)
      .map(player => {
  ...
  kick(user: User) {
    if (!this.playerTable[user.id]) {
      if (this.kickedUsers.has(user.id)) {
        throw new Chat.ErrorMessage(
          this.room.tr`User ${user.name} has already been kicked from the game.`
        );
      }
  ...
  pause() {
    ...
    if (this.phase === QUESTION_PHASE) {
      throw new Chat.ErrorMessage(
        this.room.tr`You cannot pause the trivia game during a question.`
      );
    }
    this.isPaused = true;
  ...
  async askQuestion() {
    ...
    if (
      question.category === MOVE_QUESTIONS_AFTER_USE_FROM_CATEGORY &&
      (await database.shouldMoveEventQuestions())
    ) {
      await database.moveQuestionToCategory(
        question.question,
        MOVE_QUESTIONS_AFTER_USE_TO_CATEGORY
      );
    }
  }
  ...
  async win(buffer: string) {
    ...
    for (const i in this.playerTable) {
      ...
      user.sendTo(
        this.room.roomid,
        (this.game.givesPoints
          ? this.room.tr`You gained ${player.points} points and answered `
          : this.room.tr`You answered `) +
    ...
    const scores = Object.fromEntries(
      this.getTopPlayers({ max: null }).map(player => [player.player.id, player.player.points])
    );
    if (this.game.givesPoints) {
      await database.addHistory([
        {
          ...this.game,
          length:
            typeof this.game.length === 'number'
              ? `${this.game.length} questions`
              : this.game.length,
          scores,
        },
      ]);
  ...
  getPrizes() {
    ...
    const multiplier =
      this.game.length === 'infinite' ? Math.floor(this.questionNumber / 25) || 1 : 1;
  ...
  getTopPlayers(
    ...
    Utils.sortBy(ranks, ({ player }) => [
      -player.points,
      player.lastQuestion,
      hrtimeToNanoseconds(player.answeredAt),
    ]);
  ...
  getWinningMessage(winners: TopPlayer[]) {
    ...
    let initialPart = this.room
      .tr`${Utils.escapeHTML(p1.name)} won the game with a final score of <strong>${p1.player.points}</strong>`;
    if (!this.game.givesPoints) {
      return `${initialPart}.`;
    } else {
    ...
    switch (winners.length) {
      case 1:
        return this.room
          .tr`${initialPart}their leaderboard score has increased by <strong>${prizes[0]}</strong> points!`;
      case 2:
        return (
          this.room
            .tr`${initialPart}their leaderboard score has increased by <strong>${prizes[0]}</strong> points! ` +
          this.room
            .tr`${Utils.escapeHTML(p2.name)} was a runner-up and their leaderboard score has increased by <strong>${prizes[1]}</strong> points!`
        );
      case 3:
        return (
          initialPart +
          Utils.html`${this.room.tr`${p2.name} and ${p3.name} were runners-up. `}` +
          this.room
            .tr`Their leaderboard score has increased by ${prizes[0]}, ${prizes[1]}, and ${prizes[2]}, respectively!`
        );
    }
  }
  ...
  getStaffEndMessage(winners: TopPlayer[], mapper: (k: TopPlayer) => string) {
    ...
    if (winners.length) {
      const winnerParts: ((k: TopPlayer) => string)[] = [
        winner =>
          this.room.tr`User ${mapper(winner)} won the game of ` +
          (this.game.givesPoints ? this.room.tr`ranked ` : this.room.tr`unranked `) +
          this.room
            .tr`${this.game.mode} mode trivia under the ${this.game.category} category with ` +
      ...
      message =
        `No participants in the game of ` +
        (this.game.givesPoints ? this.room.tr`ranked ` : this.room.tr`unranked `) +
...
export class FirstModeTrivia extends Trivia {
  override answerQuestion(answer: string, user: User) {
    ...
    if (player.answer) {
      throw new Chat.ErrorMessage(
        this.room.tr`You have already attempted to answer the current question.`
      );
    }
    ...
    const buffer =
      Utils.html`${this.room.tr`Correct: ${players}`}<br />` +
      this.room.tr`Answer(s): ${this.curAnswers.join(', ')}` +
      `<br />` +
      this.room.tr`They gained <strong>5</strong> points!` +
      `<br />` +
      this.room.tr`The top 5 players are: ${this.formatPlayerList({ max: 5 })}`;

    const cap = this.getCap();
    if (
      (cap.points && player.points >= cap.points) ||
      (cap.questions && this.questionNumber >= cap.questions)
    ) {
...
export class TimerModeTrivia extends Trivia {
  ...
  override tallyAnswers() {
    ...
    for (const [pointValue, players] of innerBuffer) {
      ...
      rowAdded = true;
      const playerNames = Utils.sortBy(players, ([name, answeredAt]) => answeredAt).map(
        ([name]) => name
      );
      buffer +=
        '<tr style="background-color: #6688AA">' +
...
export class TriumvirateModeTrivia extends Trivia {
  override answerQuestion(answer: string, user: User) {
    ...
    const correctAnswers = Object.keys(this.playerTable).filter(
      id => this.playerTable[id].isCorrect
    ).length;
    if (correctAnswers === 3) {
      if (this.phaseTimeout) clearTimeout(this.phaseTimeout);
  ...
  override tallyAnswers() {
    ...
    if (playersWithPoints.length) {
      ...
      buffer =
        this.room.tr`Correct: ${players}<br />` +
...
export class Mastermind extends Rooms.SimpleRoomGame {
  ...
  init() {
    broadcast(
      ...
      this.room
        .tr`The top <strong>${this.numFinalists}</strong> players will advance to the finals!` +
        `<br />` +
  ...
  formatPlayerList() {
    return Utils.sortBy(
      ...
      .map(player => {
        const isFinalist =
          this.currentRound instanceof MastermindFinals &&
          player.id in this.currentRound.playerTable;
        const name = isFinalist
          ? Utils.html`<strong>${player.name}</strong>`
  ...
  startRound(playerID: ID, category: ID, questions: TriviaQuestion[], timeout: number) {
    if (this.currentRound) {
      throw new Chat.ErrorMessage(
        this.room.tr`There is already a round of Mastermind in progress.`
      );
    }
    ...
    if (this.leaderboard.has(playerID)) {
      throw new Chat.ErrorMessage(
        this.room.tr`The user "${playerID}" has already played their round of Mastermind.`
      );
    }
    if (this.playerCount <= this.numFinalists) {
      throw new Chat.ErrorMessage(
        this.room
          .tr`You cannot start the game of Mastermind until there are more players than finals slots.`
      );
    }
  ...
  async startFinals(timeout: number) {
    ...
    for (const player in this.playerTable) {
      if (!this.leaderboard.has(toID(player))) {
        throw new Chat.ErrorMessage(
          this.room.tr`You cannot start finals until the user '${player}' has played a round.`
        );
      }
    }
    ...
    this.currentRound = new MastermindFinals(
      this.room,
      'all' as ID,
      questions,
      this.getTopPlayers(this.numFinalists)
    );
      ...
      void (async () => {
        if (this.currentRound) {
          ...
          if (winner) {
            ...
            buf = this.room
              .tr`${winnerName} won the game of Mastermind with ${winner.player.points} points!`;
          }
  ...
  kick(toKick: User, kicker: User) {
    ...
    if (this.numFinalists > this.players.length - 1) {
      throw new Chat.ErrorMessage(
        this.room
          .tr`Kicking ${toKick.name} would leave this game of Mastermind without enough players to reach ${this.numFinalists} finalists.`
      );
    }
...
export class MastermindRound extends FirstModeTrivia {
  ...
  override start() {
    ...
    broadcast(
      this.room,
      this.room
        .tr`A Mastermind round in the ${this.game.category} category for ${name} is starting!`
    );
    player.sendRoom(
  ...
  override addTriviaPlayer(user: User): string | undefined {
    throw new Chat.ErrorMessage(
      `This is a round of Mastermind; to join the overall game of Mastermind, use /mm join`
    );
  }
...
const triviaCommands: Chat.ChatCommands = {
  ...
  async new(target, room, user, connection, cmd) {
    ...
    let categories: ID[] | 'random' = targets[1].split('+').map(cat => {
      const id = toID(cat);
    ...
    const questions = await getQuestions(
      categories,
      randomizeQuestionOrder ? 'random' : 'oldestfirst'
    );
    ...
    const questionsNecessary =
      typeof length === 'string' ? (LENGTHS[length].cap || 75) / 5 : length;
    if (questions.length < questionsNecessary) {
      if (categories === 'random') {
        return this.errorReply(
          this
            .tr`There are not enough questions in the randomly chosen category to finish a trivia game.`
        );
      }
      ...
      return this.errorReply(
        this
          .tr`There are not enough questions under the specified categories to finish a trivia game.`
      );
    }
  ...
  kickhelp: [
    `/trivia kick [username] - Kick players from a trivia game by username. Requires: % @ # ~`,
  ],
  ...
  starthelp: [
    `/trivia start - Ends the signup phase of a trivia game and begins the game. Requires: + % @ # ~`,
  ],
  ...
  async win(target, room, user) {
    ...
    if (game.game.length !== 'infinite' && !user.can('editroom', null, room)) {
      return this.errorReply(
        this
          .tr`Only Room Owners and higher can force a Trivia game to end with winners in a non-infinite length.`
      );
    }
  ...
  status(target, room, user) {
    ...
    let buffer =
      `${game.isPaused ? this.tr`There is a paused trivia game` : this.tr`There is a trivia game in progress`}, ` +
      this.tr`and it is in its ${game.phase} phase.` +
      `<br />` +
      this
        .tr`Mode: ${game.game.mode} | Category: ${game.game.category} | Cap: ${game.getDisplayableCap()}`;
    ...
    if (player) {
      ...
      return this.errorReply(
        this.tr`User ${targetUser.name} is not a player in the current trivia game.`
      );
    }
  ...
  async add(target, room, user, connection, cmd) {
    ...
    for (const param of params) {
      if (param.length !== 3) {
        this.errorReply(
          this
            .tr`Invalid arguments specified in "${param}". View /trivia help for more information.`
        );
        continue;
      }
      ...
      if (!ALL_CATEGORIES[category]) {
        this.errorReply(
          this
            .tr`'${param[0].trim()}' is not a valid category. View /trivia help for more information.`
        );
        continue;
      }
      if (cmd === 'submit' && !MAIN_CATEGORIES[category]) {
        this.errorReply(
          this.tr`You cannot submit questions in the '${ALL_CATEGORIES[category]}' category`
        );
        continue;
      }
      ...
      if (question.length > MAX_QUESTION_LENGTH) {
        this.errorReply(
          this
            .tr`Question "${param[1].trim()}" is too long! It must remain under ${MAX_QUESTION_LENGTH} characters.`
        );
        continue;
      }
      ...
      const cache = new Set();
      const answers = param[2]
        .split(',')
        .map(toID)
        .filter(answer => !cache.has(answer) && !!cache.add(answer));
      if (!answers.length) {
        this.errorReply(
          this.tr`No valid answers were specified for question '${param[1].trim()}'.`
        );
        continue;
      }
    ...
    if (cmd === 'add') {
      ...
      this.privateModAction(
        `Questions ${formattedQuestions} were added to the question database by ${user.name}.`
      );
    } else {
      ...
      this.privateModAction(
        `Questions ${formattedQuestions} were submitted to the submission database by ${user.name} for review.`
      );
    }
  },
  submithelp: [
  ...
  async accept(target, room, user, connection, cmd) {
    ...
    this.errorReply(
      this.tr`'${target}' is an invalid argument. View /trivia help questions for more information.`
    );
  },
  accepthelp: [
  ...
  async delete(target, room, user) {
    ...
    if (!question) {
      return this.errorReply(
        this
          .tr`'${target}' is not a valid argument. View /trivia help questions for more information.`
      );
    }
    ...
    return this.privateModAction(
      room.tr`${user.name} removed question '${target}' (category: ${category}) from the question database.`
    );
  },
  deletehelp: [
  ...
  async move(target, room, user) {
    ...
    for (const param of params) {
      ...
      this.modlog(
        'TRIVIAQUESTION',
        null,
        `changed category for '${param[1].trim()}' from '${oldCat}' to '${param[0]}'`
      );
      return this.privateModAction(
        this
          .tr`${user.name} changed question category from '${oldCat}' to '${param[0]}' for '${param[1].trim()}' ` +
  ...
  async migrate(target, room, user) {
    ...
    for (const category of [sourceCategory, destinationCategory]) {
      if (category in MAIN_CATEGORIES) {
        throw new Chat.ErrorMessage(
          `Main categories cannot be used with /trivia migrate. If you need to migrate to or from a main category, contact a technical Administrator.`
        );
      }
    ...
    if (user.lastCommand !== command) {
      this.sendReply(
        `Are you SURE that you want to PERMANENTLY move all questions in the category ${sourceCategoryName} to ${destinationCategoryName}?`
      );
    ...
    this.modlog(
      `TRIVIAQUESTION MIGRATE`,
      null,
      `${sourceCategoryName} to ${destinationCategoryName}`
    );
    this.privateModAction(
      `${user.name} migrated all ${numQs} questions in the category ${sourceCategoryName} to ${destinationCategoryName}.`
    );
  },
  migratehelp: [
    ...
    if (!isAnswersOnly) {
      ...
      if (!newQuestionText) {
        ...
        if (newQuestionText.length > MAX_QUESTION_LENGTH) {
          throw new Chat.ErrorMessage(
            `The new question text is too long. The limit is ${MAX_QUESTION_LENGTH} characters.`
          );
        }
      }
    }
      ...
      if (!answerArgs) {
        ...
        for (const arg of answerArgs?.split(',') || []) {
          ...
          if (answer.length > MAX_ANSWER_LENGTH) {
            throw new Chat.ErrorMessage(
              `The answer '${answer}' is too long. The limit is ${MAX_ANSWER_LENGTH} characters.`
            );
          }
  ...
  async qs(target, room, user) {
    ...
    if (!ALL_CATEGORIES[category]) {
      return this.errorReply(
        this.tr`'${target}' is not a valid category. View /help trivia for more information.`
      );
    }

    const list = await database.getQuestions([category], Number.MAX_SAFE_INTEGER, {
      order: 'oldestfirst',
    });
    if (!list.length) {
    ...
    if (user.can('ban', null, room)) {
      ...
      buffer +=
        `<tr><td colspan="3">${this.tr`There are <strong>${list.length}</strong> questions in the ${cat} category.`}</td></tr>` +
      ...
      const cat = target;
      buffer +=
        `<td colspan="2">${this.tr`There are <strong>${list.length}</strong> questions in the ${cat} category.`}</td></tr>` +
  ...
  async search(target, room, user, connection, cmd) {
    ...
    if (/^q(?:uestion)?s?$/.test(type)) {
      ...
      return this.sendReplyBox(
        this
          .tr`No valid search category was entered. Valid categories: submissions, subs, questions, qs`
      );
    }
    ...
    let buffer =
      `|raw|<div class="ladder"><table><tr><th>#</th><th>${this.tr`Category`}</th><th>${this.tr`Question`}</th></tr>` +
      `<tr><td colspan="3">${this.tr`There are <strong>${results.length}</strong> matches for your query:`}</td></tr>`;
    buffer += results
      .map(
        (q, i) =>
          this
            .tr`<tr><td><strong>${i + 1}</strong></td><td>${q.category}</td><td>${q.question}</td></tr>`
      )
      .join('');
    buffer += '</table></div>';
  ...
  async rank(target, room, user) {
    ...
    const score = (await database.getLeaderboardEntry(userid, 'nonAlltime')) || {
      score: 0,
      totalPoints: 0,
  ...
  rankhelp: [
    `/trivia rank [username] - View the rank of the specified user. If no name is given, view your own.`,
  ],
  ...
  async ladder(target, room, user, connection, cmd) {
    ...
    let buffer =
      '|raw|<div class="ladder" style="overflow-y: scroll; max-height: 300px;"><table>' +
    ...
    for (let i = Math.max(0, num - 100); i < num; i++) {
      ...
      for (const leader of leaders) {
        ...
        const leaderid = leaderObj
          ? Utils.escapeHTML(leaderObj.name)
          : (leader as unknown as string);
  ...
  async clearqs(target, room, user) {
    ...
    if (ALL_CATEGORIES[category]) {
      if (SPECIAL_CATEGORIES[category]) {
        ...
        return this.privateModAction(
          room.tr`${user.name} removed all questions of category '${category}'.`
        );
      } else {
        return this.errorReply(
          this.tr`You cannot clear the category '${ALL_CATEGORIES[category]}'.`
        );
      }
    } else {
  ...
  clearqshelp: [
    `/trivia clearqs [category] - Remove all questions of the given category. Requires: # ~`,
  ],
  ...
  async history(target, room, user) {
    ...
    if (target) {
      ...
      if (lines < 1 || lines > 100 || isNaN(lines)) {
        throw new Chat.ErrorMessage(
          `You must specify a number of games to view the history of between 1 and 100.`
        );
      }
    }
  ...
  historyhelp: [
    `/trivia history [n] - View a list of the n most recently played trivia games. Defaults to 10.`,
  ],
  ...
  async removeleaderboardentry(target, room, user) {
    ...
    await Promise.all(
      (Object.keys(LEADERBOARD_ENUM) as Leaderboard[]).map(lb =>
        database.deleteLeaderboardEntry(userid, lb)
      )
    );
    cachedLadder.invalidateCache();
...
const mastermindCommands: Chat.ChatCommands = {
  ...
  players(target, room, user) {
    ...
    let buf = this
      .tr`There is a Mastermind game in progress, and it is in its ${game.phase} phase.`;
