...
export interface TriviaDatabase {
  ...
  getQuestionCounts():
    | Promise<{ [k: string]: number; total: number }>
...
export class TriviaSQLiteDatabase implements TriviaDatabase {
  ...
  async updateLeaderboardForUser(
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't update the leaderboard for ${userid} because SQLite is not enabled.`
      );
    }
  ...
  async addHistory(history: Iterable<TriviaHistory>) {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't add a Trivia game to the history because SQLite is not enabled.`
      );
    }
  ...
  async addQuestionSubmissions(questions: Iterable<TriviaQuestion>) {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't submit a Trivia question for review because SQLite is not enabled.`
      );
    }
  ...
  async setShouldMoveEventQuestions(shouldMove: boolean) {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't enable/disable moving event questions because SQLite is not enabled.`
      );
    }
  ...
  async mergeLeaderboardEntries(from: ID, to: ID) {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't merge ${from} and ${to}'s Trivia leaderboard entries because SQLite is not enabled.`
      );
    }
    ...
    for (const lbDiscrim of Object.values(LEADERBOARD_ENUM)) {
      ...
      await Chat.database.run(this.leaderboardInsertion!, [
        to,
        toScores.score,
        toScores.totalPoints,
        toScores.totalCorrectAnswers,
        lbDiscrim,
      ]);
  ...
  async shouldMoveEventQuestions() {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't find out if we are moving event questions because SQLite is not enabled.`
      );
    }
  ...
  async acceptSubmissions(submissions: string[]) {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't accept Trivia question submissions because SQLite is not enabled.`
      );
    }
  ...
  async getQuestions(
    ...
    if (categories === 'all') {
      ...
      query = `SELECT * FROM trivia_questions WHERE category IN (${formatSQLArray(categories)}) AND is_submission = 0 ORDER BY ${options.order === 'random' ? 'RANDOM()' : `added_at ${options.order === 'oldestfirst' ? 'ASC' : 'DESC'}`} LIMIT ?`;
  ...
  async getLeaderboardEntry(
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't find out if user ${id} has a Trivia leaderboard entry because SQLite is not enabled.`
      );
    }
  ...
  async getLeaderboards(): Promise<TriviaLeaderboards> {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't get the Trivia leaderboard scores because SQLite is not enabled.`
      );
    }
  ...
  async getQuestion(questionText: string): Promise<TriviaQuestion | null> {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't check if a Trivia question already exists because SQLite is not enabled.`
      );
    }
  ...
  async ensureQuestionDoesNotExist(questionText: string) {
    if (await this.getQuestion(questionText)) {
      throw new Chat.ErrorMessage(
        `Question "${questionText}" is already in the question database.`
      );
    }
  }
  ...
  async getSubmissions(): Promise<TriviaQuestion[]> {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't retrieve the Trivia question submissions because SQLite is not enabled.`
      );
    }
  ...
  async getQuestionCounts(): Promise<{ [k: string]: number; total: number }> {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't retrieve the Trivia question counts because SQLite is not enabled.`
      );
    }

    const allCategories = (await this.categoriesQuery!.all([])).map(
      (row: AnyObject) => row.category
    );
  ...
  async searchQuestions(
    ...
    const rows = await this.questionSearchQuery!.all([
      `%${search}%`,
      Number(options.searchSubmissions),
    ]);
  ...
  async clearSubmissions() {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't clear the Trivia question submissions because SQLite is not enabled.`
      );
    }
  ...
  async clearCategory(category: string) {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't clear the Trivia questions in category "${category}" because SQLite is not enabled.`
      );
    }
  ...
  async clearCycleLeaderboard() {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't clear the cycle leaderboard because SQLite is not enabled.`
      );
    }
  ...
  async deleteQuestion(questionText: string) {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't delete the Trivia question because SQLite is not enabled.`
      );
    }
  ...
  async deleteLeaderboardEntry(userid: ID, leaderboard: Leaderboard) {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't delete leaderboard entries because SQLite is not enabled.`
      );
    }
  ...
  async deleteSubmissions(submissions: string[]) {
    ...
    if (!Config.usesqlite) {
      throw new Chat.ErrorMessage(
        `Can't delete Trivia question submissions because SQLite is not enabled.`
      );
    }
  ...
  private async prepareStatements() {
    ...
    this.historyScoresQuery = await Chat.database.prepare(
      `SELECT userid, score FROM trivia_game_scores WHERE game_id = ?`
    );
