  ...
  async spotlights(query, user, connection) {
    ...
    if (sortType && !['time', 'alphabet'].includes(sortType)) {
      return this.errorReply(
        `Invalid sorting type '${sortType}' - must be either 'time', 'alphabet', or not provided.`
      );
    }
      ...
      const sortedKeys = Utils.sortBy(Object.keys(spotlights[room.roomid]), key => {
        switch (sortType) {
          case 'time': {
            ...
            const sortedSpotlights = Utils.sortBy(
              spotlights[room.roomid][key].slice(),
              k => -k.time
            );
...
export const commands: Chat.ChatCommands = {
  removedaily(target, room, user) {
    ...
    if (rest) {
      ...
      if (queueNumber >= spotlights[room.roomid][key].length) {
        return this.errorReply(
          `Queue number needs to be between 1 and ${spotlights[room.roomid][key].length - 1}`
        );
      }
      ...
      this.privateModAction(
        `${user.name} successfully removed the daily spotlight named '${key}'.`
      );
    }
  ...
  swapdaily(target, room, user) {
    ...
    if (indexA < 1 || indexB < 1 || indexA >= queueLength || indexB >= queueLength) {
      return this.errorReply(
        `Queue numbers must between 1 and the length of the queue (${queueLength}).`
      );
    }
    ...
    this.privateModAction(
      `${user.name} swapped the queued dailies for '${key}' at queue numbers ${indexA} and ${indexB}.`
    );
    Chat.refreshPageFor(`spotlights-${room.roomid}`, room);
  },
  queuedaily: 'setdaily',
  ...
  async setdaily(target, room, user, connection, cmd) {
    ...
    if (indexString && (index < 1 || index > queueLength)) {
      return this.errorReply(
        `Queue numbers must be between 1 and the length of the queue (${queueLength}).`
      );
    }
  ...
  async daily(target, room, user) {
    ...
    if (!this.broadcasting && user.can('ban', null, room, 'setdaily')) {
      ...
      this.sendReplyBox(
        `<details><summary>Source</summary><code style="white-space: pre-wrap; display: table; tab-size: 3">/setdaily ${key},${image ? `${image},` : ''}${code}</code></details>`
      );
    }
    room.update();
  },
