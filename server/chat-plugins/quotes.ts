...
export const commands: Chat.ChatCommands = {
  randquote(target, room, user) {
    ...
    const attribution =
      toID(target) === 'showauthor' ? `<hr /><small>Added by ${userid} on ${time}</small>` : '';
  ...
  addquote(target, room, user) {
    ...
    if (room.settings.isPrivate && roomQuotes.length >= MAX_QUOTES) {
      return this.errorReply(
        `This room already has ${MAX_QUOTES} quotes, which is the maximum for private rooms.`
      );
    }
  ...
  removequote(target, room, user) {
    ...
    this.privateModAction(
      `${user.name} removed quote indexed at ${index + 1}: "${collapsedQuote}" (originally added by ${removed.userid}).`
    );
  ...
  viewquote(target, room, user) {
    ...
    const time = Chat.toTimestamp(new Date(date), { human: true });
    const attribution =
      toID(showAuthor) === 'showauthor' ? `<hr /><small>Added by ${userid} on ${time}</small>` : '';
  ...
  quote() {
    this.sendReply(
      `/quote as a method of adding quotes has been deprecated. Use /addquote instead.`
    );
