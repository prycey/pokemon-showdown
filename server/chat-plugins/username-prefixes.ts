...
export class PrefixManager {
  ...
  addPrefix(prefix: ID, type: 'privacy' | 'modchat') {
    if (!Config.forcedprefixes) Config.forcedprefixes = [];
    const entry = Config.forcedprefixes.find(
      (x: AnyObject) => x.prefix === prefix && x.type === type
    );
    if (entry) {
      throw new Chat.ErrorMessage(
        `Username prefix '${prefix}' is already configured to force ${type}.`
      );
    }
  ...
  removePrefix(prefix: ID, type: 'privacy' | 'modchat') {
    const entry = Config.forcedprefixes.findIndex(
      (x: AnyObject) => x.prefix === prefix && x.type === type
    );
    if (entry < 0) {
      throw new Chat.ErrorMessage(
        `Username prefix '${prefix}' is not configured to force ${type}!`
      );
    }
  ...
  validateType(type: string) {
    if (type !== 'privacy' && type !== 'modchat') {
      throw new Chat.ErrorMessage(
        `'${type}' is not a valid type of forced prefix. Valid types are 'privacy' and 'modchat'.`
      );
    }
    return type;
  }
...
export const commands: Chat.ChatCommands = {
  ...
  usernameprefix: {
    ...
    add(target, room, user, connection, cmd) {
      ...
      if (prefix.length > 18) {
        throw new Chat.ErrorMessage(
          `Specified prefix '${prefix}' is longer than the maximum user ID length.`
        );
      }
      ...
      this.globalModlog(
        `FORCEDPREFIX ${isAdding ? 'ADD' : 'REMOVE'}`,
        null,
        `'${prefix}' ${isAdding ? 'to' : 'from'} ${type}`
      );
      this.addGlobalModAction(
        `${user.name} set the username prefix ${prefix} to${isAdding ? '' : ' no longer'} disable ${type}.`
      );
    },
        ...
        types
          .map(type => {
            ...
            const info = prefixes.length
              ? `<code>${prefixes.join('</code>, <code>')}</code>`
              : `none`;
