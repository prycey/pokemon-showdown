...
export class LogReaderRoom {
  ...
  async listMonths() {
    if (roomlogTable) {
      const dates =
        await roomlogTable.query<any>()`SELECT DISTINCT month FROM roomlog_dates WHERE roomid = ${this.roomid}`;
  ...
  async listDays(month: string) {
    if (roomlogTable) {
      const dates =
        await roomlogTable.query<any>()`SELECT DISTINCT date FROM roomlog_dates WHERE roomid = ${this.roomid} AND month = ${month}`;
...
export const LogViewer = new (class {
  async day(roomid: RoomID, day: string, opts?: string) {
    ...
    buf +=
      `<p><a roomid="${prevRoomid}" class="blocklink" style="text-align:center">▲<br />${prevDay}</a></p>` +
    ...
    switch (cmd) {
      case 'c': {
        ...
        return (
          `<div ${getClass('chat')}>` +
          `<small>[${timeLink}]` +
          Utils.html` ${group}</small><username>${name.slice(1)}:</username> ` +
...
export abstract class Searcher {
  ...
  renderLinecountResults(
    ...
    if (!results) {
      ...
      const sortedResults = Utils.sortBy(resultKeys, userid => -totalResults[userid]).slice(
        0,
        MAX_TOPUSERS
      );
      let total = 0;
  ...
  abstract activityStats(
    room: RoomID,
    month: string
  ): Promise<{ average: RoomStats; days: RoomStats[] }>;
}
...
export class FSLogSearcher extends Searcher {
  ...
  async dayStats(room: RoomID, day: string) {
    ...
    results.deadPercent = !results.totalLines
      ? 100
      : (waitIncrements.length / results.totalLines) * 100;
  ...
  async activityStats(room: RoomID, month: string) {
    const days = (await Monitor.logPath(`chat/${room}/${month}`).readdir()).map(f =>
      f.slice(0, -4)
    );
    ...
    for (const day of days) {
      if (day === today) {
        // if the day is not over: do not count it, it'll skew the numbers
        continue;
      }
...
export class RipgrepLogSearcher extends FSLogSearcher {
  async ripgrepSearchMonth(opts: ChatlogSearch) {
    ...
    try {
      const options = ['-e', search, Monitor.logPath(`chat/${roomid}/${month}`).path, '-i'];
      if (args) {
        options.push(...args);
      }
      ...
      if (
        e.code !== 1 &&
        !e.message.includes('stdout maxBuffer') &&
...
export const pages: Chat.PageTable = {
  ...
  roomstats(args, user) {
    ...
    return LogSearcher.runLinecountSearch(
      this,
      room ? room.roomid : (args[2] as RoomID),
      date,
      toID(target)
    );
  },
...
export const commands: Chat.ChatCommands = {
  ...
  chatlog(target, room, user) {
    ...
    return this.parse(
      `/join view-chatlog-${roomid}--today${opts ? `--${opts.map(toID).join('--')}` : ''}`
    );
  },
  ...
  linecount(target, room, user) {
    ...
    if (!search.roomid) {
      if (!room) {
        return this.errorReply(
          `If you're not specifying a room, you must use this command in a room.`
        );
      }
    ...
    return this.parse(
      `/join view-roomstats-${search.roomid}--${search.date}${search.user ? `--${search.user}` : ''}`
    );
  },
  linecounthelp() {
  ...
  async getbattlechat(target, room, user) {
    ...
    if (tarRoom) {
      ...
      try {
        const raw = await Net(
          `https://${Config.routes.replays}/${roomid.slice('battle-'.length)}.json`
        ).get();
  ...
  async groupchatsearch(target, room, user) {
    ...
    return this.sendReplyBox(
      `Groupchats with a roomid matching '${target}': ` +
        (buffer.length
          ? buffer.map(id => `<a href="/view-chatlog-${id}">${id}</a>`).join('; ')
          : 'None found.')
    );
  },
  groupchatsearchhelp: [
