...
const defaults: AutoSettings = {
  punishments: [
    ...
    },
    {
      ...
      punishment: 'warn',
      severity: {
        type: ['sexual_explicit', 'severe_toxicity', 'identity_attack'],
        certainty: 0.15,
      },
    },
  ],
...
function visualizePunishment(punishment: AutoPunishment) {
  ...
  if (punishment.severity) {
    buf.push(
      `severity: ${punishment.severity.certainty} (for ${punishment.severity.type.join(', ')})`
    );
  }
...
async function lock(
  ...
  addModAction(
    `${id} was ${type} by Artemis. (${result.reason || `report from ${ticket.creator}`})`
  );
  globalModlog(
...
export const checkers: {
  ...
  ) => CheckerOutput | Promise<CheckerOutput>;
} = {
...
export async function runPunishments(
  ticket: TicketState & { text: [string, string] },
  typeId: string
) {
...
export const commands: Chat.ChatCommands = {
  ...
  autohelpticket: {
    ...
    addpunishment(target, room, user) {
      ...
      this.globalModlog(
        `AUTOHELPTICKET ADDPUNISHMENT`,
        null,
        visualizePunishment(punishment as AutoPunishment)
      );
    },
    dp: 'deletepunishment',
    ...
    viewpunishments() {
      ...
      buf += settings.punishments
        .map((curP, i) => `<strong>${i + 1}:</strong> ${visualizePunishment(curP)}`)
        .join('<br />');
      return this.sendReplyBox(buf);
    },
  ...
  autohelpticket: {
    async stats(query, user) {
      ...
      if (failed.length) {
        ...
        buf += failed
          .map(([userid, type]) => `<a href="/view-help-text-${userid}">${userid}</a> (${type})`)
          .join('<br />');
        buf += `</details>`;
      }
      return buf;
