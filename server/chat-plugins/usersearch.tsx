...
export const nameList = new Set<string>(
  JSON.parse(FS('config/chat-plugins/usersearch.json').readIfExistsSync() || '[]')
);
      ...
      buf.push(
        <span
          dangerouslySetInnerHTML={{
            __html:
              `<button class="button" name="send" value="/msgroom staff,/${toID(cmdName)} ${userid}` +
...
class SearchUsernames extends Chat.JSX.Component<{ target: string; page?: boolean }> {
  render() {
    ...
    if (!page) {
      return (
        <>
          Users with a name matching '{target}':
          <br />
                    ...
                    for (const username of results.online) {
                      online.push(
                        <tr>
                          ...
                          <td>
                            <PunishmentHTML userid={toID(username)} target={target} />
                          </td>
                        </tr>
                      );
                    }
...
export const commands: Chat.ChatCommands = {
  ...
  usersearch(target, room, user, connection, cmd) {
    ...
    target = toID(target);
    if (!target) {
      // just join directly if it's the page cmd, they're likely looking for the full list
  ...
  usersearchnames: {
    ...
    add(target, room, user) {
      ...
      for (const [i, arg] of targets.entries()) {
        ...
        if (arg.length < 3) {
          targets.splice(i, 1);
          this.errorReply(
            `Term ${arg} is too short for the usersearch term list. Must be more than 3 characters.`
          );
          continue;
        }
        nameList.add(arg);
      ...
      if (!room || room.roomid !== 'staff') {
        this.sendReply(
          `You added the ${count} "${targets.join(', ')}" to the usersearch name list.`
        );
      }
      saveNames();
    },
    remove(target, room, user) {
      ...
      if (!room || room.roomid !== 'staff') {
        this.sendReply(
          `You removed the ${count} "${targets.join(', ')}"" from the usersearch name list.`
        );
      }
      saveNames();
    },
                  ...
                  for (const k of Utils.sortBy(Object.keys(sorted), v => -sorted[v])) {
                    buf.push(
                      <tr>
                        ...
                        <td>{sorted[k]}</td>
                        <td>
                          <button class="button" name="send" value={`/uspage ${k}`}>
                            Search
                          </button>
                        </td>
                      </tr>
