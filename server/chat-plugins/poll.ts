...
export class Poll extends Rooms.MinorActivity {
  ...
  static generateResults(
    options: Rooms.MinorActivityData,
    room: Room,
    ended = false,
  ...
  updateTo(user: User, connection: Connection | null = null) {
    ...
    if (selection) {
      ...
      recipient.sendTo(
        this.room,
        `|uhtmlchange|poll${this.activityNumber}|${this.generateVotes(user)}`
      );
    }
  }
    ...
    for (const id in this.room.users) {
      ...
      if (selection) {
        ...
        if (this.multiPoll && thisUser.id in this.pendingVotes) {
          thisUser.sendTo(
            this.room,
            `|uhtml|poll${this.activityNumber}|${this.generateVotes(thisUser)}`
          );
        } else {
  ...
  displayTo(user: User, connection: Connection | null = null) {
    ...
    if (user.id in this.voters) {
      ...
      recipient.sendTo(
        this.room,
        `|uhtml|poll${this.activityNumber}|${Poll.generateResults(
          state,
          this.room,
          false,
  ...
  destroy() {
    ...
    this.room.send(
      `|uhtmlchange|poll${this.activityNumber}|<div class="infobox">(${this.room.tr`The poll has ended &ndash; scroll down to see the results`})</div>`
    );
...
export const commands: Chat.ChatCommands = {
  poll: {
    ...
    new(target, room, user, connection, cmd, message) {
      ...
      if (target !== text)
        return this.errorReply(this.tr`You are not allowed to use filtered words in polls.`);
        ...
        if (isEscapeCharacter) {
          if (nextCharacter) {
            ...
            return this.errorReply(
              this.tr`Extra escape character. To end a poll with '\\', enter it as '\\\\'`
            );
          }
          continue;
        }
      ...
      if (room.minorActivity && !queue) {
        return this.errorReply(
          this.tr`There is already a poll or announcement in progress in this room.`
        );
      }
    ...
    deletequeue(target, room, user) {
      ...
      if (isNaN(slot)) {
        return this.errorReply(
          this.tr`Can't delete poll at slot ${target} - "${target}" is not a number.`
        );
      }
      if (!queue[slot - 1])
        return this.errorReply(this.tr`There is no poll in queue at slot ${slot}.`);
    ...
    clearqueuehelp: [`/poll clearqueue - deletes the queue of polls. Requires: % @ # ~`],
    ...
    select(target, room, user, connection, cmd) {
      ...
      if (isNaN(parsed))
        return this.errorReply(this.tr`To vote, specify the number of the option.`);
    ...
    timer(target, room, user) {
      ...
      if (target) {
        ...
        room.add(
          this
            .tr`The poll timer was turned on: the poll will end in ${Chat.toDurationString(timeoutMins * MINUTES)}.`
        );
        this.modlog('POLL TIMER', null, `${timeoutMins} minutes`);
        return this.privateModAction(
          room.tr`The poll timer was set to ${timeoutMins} minute(s) by ${user.name}.`
        );
      } else {
        ...
        if (poll.timeout) {
          return this.sendReply(
            this
              .tr`The poll timer is on and will end in ${Chat.toDurationString(poll.timeoutMins * MINUTES)}.`
          );
        } else {
...
process.nextTick(() => {
  Chat.multiLinePattern.register(
    '/poll (new|create|createmulti|htmlcreate|htmlcreatemulti|queue|queuemulti|htmlqueuemulti) '
  );
});
...
for (const room of Rooms.rooms.values()) {
  ...
  if (room.settings.minorActivity) {
    ...
    if (typeof room.settings.minorActivity.activityNumber !== 'number') {
      // @ts-expect-error old format
      room.settings.minorActivity.activityNumber =
        room.settings.minorActivity.pollNumber ||
        // @ts-expect-error old format
