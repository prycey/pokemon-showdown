...
function formatEvent(
  ...
  const categories = getAllCategories(room).filter(category =>
    (room.settings.events![category] as RoomEventCategory).events.includes(eventID)
  );
...
export const commands: Chat.ChatCommands = {
  ...
  roomevents: {
    ...
    add(target, room, user) {
      ...
      if (!(eventName && date && desc)) {
        return this.errorReply(
          "You're missing a command parameter - to see this command's syntax, use /help roomevents."
        );
      }
      ...
      this.privateModAction(
        `${user.name} ${oldEvent ? 'edited the' : 'added a'} roomevent titled "${eventNameActual}".`
      );
    ...
    rename(target, room, user) {
      ...
      const oldID = getAliases(room).includes(toID(oldName))
        ? getEventID(oldName, room)
        : toID(oldName);
      if (newID === oldID)
      ...
      this.privateModAction(
        `${user.name} renamed the roomevent titled "${originalName}" to "${newName}".`
      );
    ...
    start(target, room, user) {
      ...
      if (!room.settings.events || !Object.keys(room.settings.events).length) {
        return this.errorReply(
          'There are currently no planned upcoming events for this room to start.'
        );
      }
    ...
    remove(target, room, user) {
      ...
      if (!room.settings.events || Object.keys(room.settings.events).length === 0) {
        return this.errorReply(
          'There are currently no planned upcoming events for this room to remove.'
        );
      }
      ...
      for (const category of getAllCategories(room).map(
        cat => room.settings.events?.[cat] as RoomEventCategory
      )) {
    ...
    view(target, room, user) {
      ...
      for (const potentialCategory of getAllCategories(room)) {
        if (
          events
            .map(event => toID(event.eventName))
            .filter(id =>
              (room.settings.events?.[potentialCategory] as RoomEventCategory).events.includes(id)
            ).length
        )
          hasCategories = true;
        break;
      ...
      if (
        !this.broadcasting &&
        user.can('ban', null, room, 'roomevents add') &&
        events.length === 1
      ) {
        const event = events[0];
        this.sendReplyBox(
          Utils.html`<details><summary>Source</summary><code style="white-space: pre-wrap; display: table; tab-size: 3">/roomevents add ${event.eventName} | ${event.date} | ${event.desc}</code></details>`.replace(
            /\n/g,
            '<br />'
          )
        );
    ...
    addalias(target, room, user) {
      ...
      if (!room.persist) return this.errorReply('This command is unavailable in temporary rooms.');
      this.checkCan('ban', null, room);
      const [alias, eventId] = target
        .split(target.includes('|') ? '|' : ',')
        .map(argument => toID(argument));
      if (!(alias && eventId)) {
        return this.errorReply(
          'Usage: /roomevents addalias [alias], [event name]. Aliases must contain at least one alphanumeric character.'
        );
      }
      if (!room.settings.events || Object.keys(room.settings.events).length === 0) {
      ...
      this.privateModAction(
        `${user.name} added an alias "${alias}" for the roomevent "${eventId}".`
      );
    ...
    addtocategory(target, room, user) {
      ...
      if (!room.persist) return this.errorReply('This command is unavailable in temporary rooms.');
      this.checkCan('ban', null, room);
      const [eventId, categoryId] = target
        .split(target.includes('|') ? '|' : ',')
        .map(argument => toID(argument));
      ...
      if (!category) {
        return this.errorReply(
          `There is no category titled "${categoryId}". To create it, use /roomevents addcategory ${categoryId}.`
        );
      }
      ...
      this.privateModAction(
        `${user.name} added the roomevent "${eventId}" to the category "${categoryId}".`
      );
    ...
    removefromcategory(target, room, user) {
      ...
      this.privateModAction(
        `${user.name} removed the roomevent "${eventId}" from the category "${categoryId}".`
      );
    ...
    addcategory(target, room, user) {
      ...
      if (!target) {
        return this.errorReply(
          'Usage: /roomevents addcategory [category name]. Categories must contain at least one alphanumeric character.'
        );
      }
      convertAliasFormat(room);
      if (!room.settings.events) room.settings.events = Object.create(null);
    ...
    categories(target, room, user) {
      ...
      const categoryButtons = getAllCategories(room).map(
        category =>
          `<button class="button" name="send" value="/roomevents view ${category}">${category}</button>`
      );
    ...
    sortby(target, room, user) {
      ...
      const sortable = Object.values(room.settings.events).filter(
        (event): event is RoomEvent => 'eventName' in event
      );
      ...
      switch (columnName) {
        ...
        case 'eventdate':
          sortable.sort((a, b) =>
            toID(a.date) < toID(b.date) ? -multiplier : toID(b.date) < toID(a.date) ? multiplier : 0
          );
          break;
        case 'desc':
        case 'description':
        case 'eventdescription':
          sortable.sort((a, b) =>
            toID(a.desc) < toID(b.desc) ? -multiplier : toID(b.desc) < toID(a.desc) ? multiplier : 0
          );
          break;
        case 'eventname':
        case 'name':
          sortable.sort((a, b) =>
            toID(a.eventName) < toID(b.eventName)
              ? -multiplier
              : toID(b.eventName) < toID(a.eventName)
                ? multiplier
                : 0
          );
          break;
        default:
          return this.errorReply(
            `Invalid column name "${columnName}". Please use one of: date, desc, name.`
          );
      }
