...
class Leaderboard {
  ...
  async htmlLadder(): Promise<string> {
    ...
    return `<div class="ladder" style="overflow-y: scroll; max-height: 170px;"><table style="width: 100%"><tr><th>Rank</th><th>Name</th><th>Points</th></tr>${data
      .map(line => `<tr><td>${line.rank}</td><td>${line.name}</td><td>${line.points}</td></tr>`)
      .join('')}</table></div>`;
  }
}
...
const TWISTS: { [k: string]: Twist } = {
  perfectscore: {
    ...
    onAfterEnd(isReset) {
      ...
      if (perfect.length) {
        this.announce(
          Utils.html`${Chat.toListString(perfect)} ${perfect.length > 1 ? 'have' : 'has'} completed the hunt without a single wrong answer!`
        );
      }
    },
  },
  ...
  bonusround: {
    ...
    onAfterEnd(isReset) {
      ...
      if (noSkip.length) {
        this.announce(
          Utils.html`${Chat.toListString(noSkip)} ${noSkip.length > 1 ? 'have' : 'has'} completed the hunt without skipping the last question!`
        );
      }
    },
  },
    ...
    onJoin(user: User) {
      if (!Config.noipchecks) {
        ...
        if (altIp) {
          user.sendTo(
            this.room,
            `You already have started the hunt as ${this.altIps[altIp].name}.`
          );
          return true;
        }
      }
  ...
  minesweeper: {
    ...
    onAfterLoad() {
      ...
      if ((this.mines as string[][]).some(mineSet => mineSet.length === 0)) {
        this.announce(
          'This twist requires at least one mine for each question. Please reset the hunt and make it again.'
        );
        this.huntLocked = true;
      }
    },
    ...
    onAfterEnd(isReset) {
      ...
      if (noMines.length) {
        this.announce(
          Utils.html`${Chat.toListString(noMines)} ${noMines.length > 1 ? 'have' : 'has'} completed the hunt without hitting a single mine!`
        );
    ...
    mod: {
      ...
      onAfterEnd() {
        ...
        if (!game.playerlist) {
          ...
          this.announce(
            `Round ${game.round} - ${Chat.toListString(this.completed.map(p => `<em>${p.name}</em>`))} have successfully completed the last hunt and have moved on to the next round!`
          );
        } else {
          let eliminated = [];
          ...
          this.announce(
            `Round ${game.round} - ${Chat.toListString(eliminated.map(n => `<em>${n}</em>`))} ${Chat.plural(eliminated, 'have', 'has')} been eliminated! ${Chat.toListString(game.playerlist.map(p => `<em>${this.playerTable[p].name}</em>`))} have successfully completed the last hunt and have moved on to the next round!`
          );
        }
  ...
  scavengergames: {
    ...
    mod: {
      ...
      onAfterEnd() {
        ...
        this.announce(
          `Round ${game.round} - ${Chat.toListString(eliminated.map(n => `<em>${n}</em>`))} ${eliminated.length ? `${Chat.plural(eliminated, 'have', 'has')} been eliminated!` : ''} ${Chat.toListString(game.playerlist.map(p => `<em>${this.playerTable[p].name}</em>`))} have successfully completed the last hunt and have moved on to the next round!`
        );
    ...
    mod: {
      ...
      async onAfterEnd() {
        ...
        for (const [i, completed] of this.completed.map(e => e.name).entries()) {
          const points =
            game.pointDistribution[i] || game.pointDistribution[game.pointDistribution.length - 1];
  ...
  jumpstart: {
    ...
    mod: {
      ...
      onViewHunt(user) {
        if (
          this.answerLock &&
          !(this.hosts.some(h => h.id === user.id) || user.id === this.staffHostId)
        ) {
          return true;
        }
      },
      ...
      onCreateCallback() {
        if (this.answerLock) {
          return (
            `|raw|<div class="broadcast-blue"><strong>${['official', 'unrated'].includes(this.gameType) ? 'An' : 'A'} ${this.gameType} ` +
