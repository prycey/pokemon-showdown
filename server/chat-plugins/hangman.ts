...
const DIACRITICS_AFTER_UNDERSCORE =
  /_[\u0300-\u036f\u0483-\u0489\u0610-\u0615\u064B-\u065F\u0670\u06D6-\u06DC\u06DF-\u06ED\u0E31\u0E34-\u0E3A\u0E47-\u0E4E]+/g;
...
export class Hangman extends Rooms.SimpleRoomGame {
  ...
  constructor(room: Room, user: User, word: string, hint = '', gameOptions: HangmanOptions = {}) {
    super(room);
  ...
  override choose(user: User, word: string) {
    ...
    if (normalized.length < 1) {
      throw new Chat.ErrorMessage(
        `Use "/guess [letter]" to guess a letter, or "/guess [phrase]" to guess the entire Hangman phrase.`
      );
    }
    if (sanitized.length > MAX_HANGMAN_LENGTH) {
      throw new Chat.ErrorMessage(
        `Guesses must be ${MAX_HANGMAN_LENGTH} or fewer letters – "${word}" is too long.`
      );
    }
  ...
  guessWord(word: string, guesser: string) {
    ...
    const wordSoFar = this.wordSoFar
      .filter(letter => /[a-zA-Z_]/.test(letter))
      .join('')
      .toLowerCase();
    ...
    if (this.guesses.length) {
      if (this.letterGuesses.length) {
        output +=
          'Letters: ' +
          this.letterGuesses
            .map(g => `<strong${g[1] === '1' ? '' : ' style="color: #DBA"'}>${g[0]}</strong>`)
            .join(', ');
      }
      if (result === 2) {
        ...
        output +=
          Utils.html`<br />Guessed: ${this.guesses[this.guesses.length - 1]} ` +
  ...
  static validateParams(params: string[]) {
    ...
    if (phrase.length > MAX_HANGMAN_LENGTH) {
      throw new Chat.ErrorMessage(
        `Phrase must be less than ${MAX_HANGMAN_LENGTH} characters long.`
      );
    }
    if (phrase.split(' ').some(w => w.length > MAX_INDIVIDUAL_WORD_LENGTH)) {
      throw new Chat.ErrorMessage(
        `Each word in the phrase must be less than ${MAX_INDIVIDUAL_WORD_LENGTH} characters long.`
      );
    }
  ...
  hangman: {
    ...
    addrandom(target, room, user) {
      ...
      this.privateModAction(
        `${user.name} added a random hangman with ${Chat.count(args.length, 'hints')}.`
      );
    ...
    removerandom(target, room, user) {
      ...
      if (!hints.length) {
        ...
        hangmanData[room.roomid][word].hints = hangmanData[room.roomid][word].hints.filter(
          item => !hints.includes(toID(item))
        );
        if (!hangmanData[room.roomid][word].hints.length) {
          delete hangmanData[room.roomid][word];
        }
        this.privateModAction(
          `${user.name} deleted ${Chat.count(hints, 'hints')} for the hangman term '${word}'`
        );
    ...
    addtag(target, room, user) {
      ...
      Hangman.save();
      this.privateModAction(
        `${user.name} added ${Chat.count(tags, 'tags')} to the hangman term ${term}`
      );
    ...
    untag(target, room, user) {
      ...
      if (tags.length) {
        this.privateModAction(
          `${user.name} removed ${Chat.count(tags, 'tags')} from the hangman term ${term}`
        );
        this.modlog(`HANGMAN UNTAG`, null, `${term}: ${tags.map(Utils.escapeHTML).join(', ')}`);
        hangmanData[room.roomid][term].tags = hangmanData[room.roomid][term].tags?.filter(
          t => !tags.includes(t)
        );
      } else {
  ...
  hangman(args, user) {
    ...
    for (const t in roomTerms) {
      ...
      if (user.can('mute', null, room, 'hangman addrandom')) {
        ...
        buf += roomTerms[t].hints
          .map(
            hint =>
              `${hint} <button class="button" name="send" value="/msgroom ${room.roomid}, /hangman rr ${t},${hint}" aria-label="Delete"><i class="fa fa-trash"></i></button>`
          )
          .join(', ');
        ...
        if (roomTerms[t].tags) {
          ...
          buf += roomTerms[t].tags
            ?.map(
              tag =>
                `${tag} <button class="button" name="send" value="/msgroom ${room.roomid}, /hangman untag ${t},${tag}" aria-label="Delete"><i class="fa fa-trash"></i></button>`
            )
            .join(', ');
