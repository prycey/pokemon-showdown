...
export const commands: Chat.ChatCommands = {
  ...
  suspects: {
    ...
    async add(target, room, user) {
      ...
      for (const req of reqs) {
        ...
        if (!['elo', 'gxe', 'coil'].includes(k)) {
          return this.errorReply(
            `Invalid requirement type: ${k}. Must be 'coil', 'gxe', or 'elo'.`
          );
        }
        if (k === 'coil' && !reqs.some(x => toID(x).startsWith('b'))) {
          throw new Chat.ErrorMessage(
            'COIL reqs are specified, but you have not provided a B value (with the argument `b=num`)'
          );
        }
        const val = Number(v);
        if (isNaN(val) || val < 0) {
      ...
      const [out, error] = await LoginServer.request(
        suspectTests.suspects[format.id] ? 'suspects/edit' : 'suspects/add',
        {
          format: format.id,
      ...
      if (out?.actionerror || error) {
        throw new Chat.ErrorMessage(
          'Error adding suspect test: ' + (out?.actionerror || error?.message)
        );
      }

      this.privateGlobalModAction(
        `${user.name} ${suspectTests.suspects[format.id] ? 'edited the' : 'added a'} ${format.name} suspect test.`
      );
      this.globalModlog(
        'SUSPECTTEST',
        null,
        `${suspectTests.suspects[format.id] ? 'edited' : 'added'} ${format.name}`
      );
      ...
      if (reqData.coil)
        this.sendReply(
          "Remember to add a B value for your test's COIL setting with /suspects setbvalue."
        );
    },
    ...
    async remove(target, room, user) {
      ...
      this.sendReply(
        `Remember to remove COIL settings with /suspects deletecoil if you had them enabled.`
      );
    },
    ...
    whitelist(target, room, user) {
      ...
      if (!userid || userid.length > 18) {
        if (suspectTests.whitelist.length) {
          this.sendReplyBox(
            `Current users with /suspects access: <username>${suspectTests.whitelist.join('</username>, <username>')}</username>`
          );
        }
        return this.parse(`/help suspects`);
      }
    ...
    unwhitelist(target, room, user) {
      ...
      this.privateGlobalModAction(
        `${user.name} unwhitelisted ${userid} from adding suspect tests.`
      );
    ...
    async setbvalue(target, room, user, connection, cmd) {
      ...
      if (!toID(formatStr) || !format.exists) {
        throw new Chat.ErrorMessage(
          `Specify a valid format to set a COIL B value for. Check spelling?`
        );
      }
      ...
      this.globalModlog(
        `${source ? 'SET' : 'REMOVE'}BVALUE`,
        null,
        `${format.id}${bVal ? ` to ${bVal}` : ''}`
      );
