...
export function getBadges(user: User, curFormat: string) {
  ...
  userBadges = Utils.sortBy(userBadges, x => Object.keys(BADGE_THRESHOLDS).indexOf(x.type)).slice(
    0,
    2
  );
...
function getUserHTML(user: User, format: string) {
  ...
  if (badgeType) {
    ...
    return (
      `<img src="https://${Config.routes.client}/sprites/misc/${formatType}_${badgeType}.png" />` +
      buf
    );
  }
  return buf;
...
export async function getLadderTop(format: string) {
  try {
    const results = await Net(
      `https://${Config.routes.root}/ladder/?format=${toID(format)}&json`
    ).get();
...
function checkPublicPhase() {
  ...
  return (
    new Date().getDate() >= daysInCurrentMonth - PUBLIC_PHASE_LENGTH &&
...
export function rollTimer() {
  ...
  if (discussionRoom) {
    if (checkPublicPhase() && discussionRoom.settings.isPrivate) {
      ...
      discussionRoom
        .add(`|html|<div class="broadcast-blue">The public phase of the month has ended.</div>`)
        .update();
    }
  }
}
...
export const pages: Chat.PageTable = {
  ...
  seasonladder(query, user) {
    ...
    if (!Object.keys(data.badgeholders[season]).includes(format)) {
      ...
      const seasonsDesc = Utils.sortBy(Object.keys(data.badgeholders), s =>
        s.split('-').map(x => -Number(x))
      );
...
export const handlers: Chat.Handlers = {
  onBattleStart(user, room) {
    ...
    for (const badge of badges) {
      room.add(
        `|badge|${slot}|${badge.type}|${badge.format}|${BADGE_THRESHOLDS[badge.type]}-${data.current.season}`
      );
    }

    if (
      checkPublicPhase() &&
      !room.battle.forcedSettings.privacy &&
      ...
      if (seasonRoom) {
        ...
        const p2html = otherUser
          ? getUserHTML(otherUser, room.battle.format)
          : `<username>${otherPlayer.name}</username>`;
