...
export const REPLAY_REGEX = new RegExp(
  `${Utils.escapeRegex(Config.routes.replays)}/(?:[a-z0-9]-)?(?:[a-z0-9]+)-(?:[0-9]+)(?:-[a-z0-9]{31}pw)?`,
  'g'
);
...
export function writeTickets() {
  FS(TICKET_FILE).writeUpdate(() => JSON.stringify(tickets), { throttle: 5000 });
}
...
export class HelpTicket extends Rooms.SimpleRoomGame {
    ...
    if (toID(this.ticket.claimed) === user.id) {
      if (this.claimQueue.length) {
        ...
        this.room.modlog({
          action: 'TICKETCLAIM',
          isGlobal: false,
          loggedBy: toID(this.ticket.claimed),
        });
  ...
  override onLogMessage(message: string, user: User) {
    ...
    if (
      (!user.isStaff || this.ticket.userid === user.id) &&
      (message.length < 3 || blockedMessages.includes(toID(message)))
    ) {
      this.room.add(
        `|c|~Staff|${this.room.tr`Hello! The global staff team would be happy to help you, but you need to explain what's going on first.`}`
      );
      this.room.add(
        `|c|~Staff|${this.room.tr`Please post the information I requested above so a global staff member can come to help.`}`
      );
      this.room.update();
      return false;
    }
    if ((!user.isStaff || this.ticket.userid === user.id) && !this.ticket.active) {
      ...
      this.room
        .add(
          `|c|~Staff|${this.room.tr`Thank you for the information, global staff will be here shortly. Please stay in the room.`}`
        )
        .update();
  ...
  getButton() {
    ...
    const creator = this.ticket.claimed
      ? Utils.html`${this.ticket.creator}`
      : Utils.html`<strong>${this.ticket.creator}</strong>`;
  ...
  close(result: boolean | 'ticketban' | 'deleted', staff?: User) {
    ...
    this.room.modlog({
      action: 'TICKETCLOSE',
      isGlobal: false,
      loggedBy: staff?.id || ('unknown' as ID),
    });
  ...
  writeStats(result: boolean | 'ticketban' | 'deleted') {
    ...
    if (this.activationTime) {
      firstClaimWait =
        (this.firstClaimTime ? this.firstClaimTime : this.closeTime) - this.activationTime;
      involvedStaff = Array.from(this.involvedStaff.entries())
        .map(s => s[0])
        .join(',');
    }
    // Write to TSV
  ...
  static logTextResult(
    ticket: TicketState & { text: [string, string]; resolved: ResolvedTicketInfo }
  ) {
    const entry = {
      text: ticket.text,
  ...
  static notifyResolved(user: User, ticket: TicketState, userid = user.id) {
    ...
    if (seen) return;
    const timeString =
      Date.now() - time > 1000 ? `, ${Chat.toDurationString(Date.now() - time)} ago.` : '.';
    user.send(
      `|pm|~Staff|${user.getIdentity()}|Hello! Your report was resolved by ${by}${timeString}`
    );
    if (result?.trim()) {
...
const unclaimedTicketTimer: { [k: string]: NodeJS.Timeout | null } = {
  upperstaff: null,
  staff: null,
};
...
function pokeUnclaimedTicketTimer(hasUnclaimed: boolean, hasAssistRequest: boolean) {
  ...
  if (hasUnclaimed && !unclaimedTicketTimer[room.roomid]) {
    ...
    timerEnds[room.roomid] =
      Date.now() + (hasAssistRequest ? NOTIFY_ASSIST_TIMEOUT : NOTIFY_ALL_TIMEOUT);
  } else if (
    hasAssistRequest &&
...
export async function listOpponentsFrom(ticket: TicketState & { text: [string, string] }) {
...
export const textTickets: { [k: string]: TextTicketInfo } = {
  pmharassment: {
    ...
    async getReviewDisplay(ticket, staff, conn) {
      ...
      replays = replays
        .filter((url, index) => replays.indexOf(url) === index)
        .concat(sharedBattles);
  ...
  roomhelp: {
    ...
    checker(input) {
      ...
      if (!room) {
        return [`That room was not found.`, `Enter either the room name or a room alias.`];
      }
      ...
      if (punishments.length) {
        for (const [, , punishment] of punishments) {
          if (userids.includes(punishment.id as ID)) {
            return [
              'Your current punishment was explicitly given to you. Please open an Appeal ticket instead.',
            ];
          }
        }
      }
...
export const pages: Chat.PageTable = {
  help: {
    request(query, user, connection) {
      if (!user.named) {
        const buf =
          `>view-help-request${query.length ? '-' + query.join('-') : ''}\n` +
          `|init|html\n` +
          `|title|Request Help\n` +
        ...
        switch (page) {
          ...
          case 'appeal':
            ...
            if (user.locked || isStaff) {
              const hostfiltered =
                ...
                (user.latestHostType === 'proxy' && user.locked !== user.id);
              if (!hostfiltered) {
          ...
          case 'public':
            if (user.ips.some(ip => Punishments.sharedIpBlacklist.has(ip))) {
              buf +=
                '<p>The public place you are in has had frequent misbehavior. As such, we can not unlock it, to prevent the bad users on it from abusing this. We apologize for the inconvenience.</p>';
              break;
            } else {
    ...
    async list(query, user, connection) {
      ...
      if (!type) {
        ...
        buf += keys
          .map(k => `<a class="button" href="/view-help-list-${k}" target="replace">${k}</a>`)
          .join(' | ');
        return buf;
      }
      this.title += ` ${type}`;
    ...
    async text(query, user, connection) {
      ...
      if (!ticket.text) {
        return this.errorReply(
          `That is either not a text ticket, or it has not yet been submitted.`
        );
      }
    ...
    async logs(query, user, connection) {
      ...
      for (const ticket of logs) {
        ...
        if (context) {
          ...
          // Otherwise, let it be.
          const noEscape = !context.includes('<br />')
            ? Chat.stripHTML(context) !== context
            : false;
    ...
    stats(query, user, connection) {
      ...
      if (table === 'tickets') {
        if (
          !['type', 'totaltickets', 'total', 'initwait', 'wait', 'resolution', 'result'].includes(
            col
          )
        )
          col = 'type';
      ...
      const ticketStats: { [k: string]: string }[] = rawTicketStats
        .split('\n')
...
export const commands: Chat.ChatCommands = {
  report(target, room, user) {
    ...
    const meta = this.pmTarget
      ? `-user-${this.pmTarget.id}`
      : this.room
        ? `-room-${this.room.roomid}`
        : '';
    if (this.broadcasting) {
      return this.sendReplyBox(
        `<button name="joinRoom" value="view-help-request--report${meta}" class="button"><strong>${this.tr`Report someone`}</strong></button>`
      );
    }
  ...
  appeal(target, room, user) {
    ...
    if (this.broadcasting) {
      return this.sendReplyBox(
        `<button name="joinRoom" value="view-help-request--appeal${meta}" class="button"><strong>${this.tr`Appeal a punishment`}</strong></button>`
      );
    }
  ...
  helpticket: {
    ...
    create(target, room, user) {
      ...
      if (this.broadcasting) {
        return this.sendReplyBox(
          `<button name="joinRoom" value="view-help-request${meta}" class="button"><strong>${this.tr`Request help`}</strong></button>`
        );
      }
      if (user.can('lock')) {
    ...
    async submit(target, room, user, connection, cmd) {
      if (user.can('lock') && !user.can('bypassall')) {
        return this.popupReply(
          this.tr`Global staff can't make tickets. They can only use the form for reference.`
        );
      }
      if (!user.named)
      ...
      if (Monitor.countTickets(user.latestIp)) {
        ...
        return this.popupReply(
          this.tr`Due to high load, you are limited to creating ${maxTickets} tickets every hour.`
        );
      }
      let [ticketType, reportTargetType, reportTarget, text, contextString] = Utils.splitFirst(
        target,
        '|',
        4
      ).map(s => s.trim());
      ...
      if (textTicket) {
        ...
        await HelpTicket.modlog({
          ...
          loggedBy: user.id,
          note: `(${ticket.type}) ${text.replace(/<br \/>/gi, ' | ')}${contextString ? `, context: ${contextString}` : ''}`,
        });
        writeTickets();
        notifyStaff();
      ...
      if (reportTargetType === 'user' && reportTarget) {
        switch (ticket.type) {
          case 'PM Harassment':
            ...
            contexts['PM Harassment'] =
              this
                .tr`Hi! Please click the button below to give global staff permission to check PMs.` +
              this
                .tr` Or if ${reportTarget} is not the user you want to report, please tell us the name of the user who you want to report.`;
            break;
          case 'Inappropriate Username':
      ...
      const introMsg =
        Utils.html`<h2 style="margin:0">${this.tr`Help Ticket`} - ${user.name}</h2>` +
      ...
      if (reportTargetType === 'room') {
        ...
        if (targetID !== ticket.userid) {
          ...
          if (!commonBattles.length) {
            ...
            reportTargetInfo += commonBattles
              .map(roomid => Utils.html`<a href=/${roomid}>${roomid.replace(/^battle-/, '')}`)
              .join(', ');
          }
        }
      }
    ...
    async resolve(target, room, user) {
      ...
      if (!ticket.text) {
        return this.popupReply(
          `That ticket cannot be resolved with /helpticket resolve. Join it instead.`
        );
      }
      ...
      // ticketType\ttotalTime\ttimeToFirstClaim\tinactiveTime\tresolution\tresult\tstaff,userids,seperated,with,commas
      writeStats(
        `${ticket.type}\t${Date.now() - ticket.created}\t0\t0\tresolved\tvalid\t${user.id}`
      );
      ...
      HelpTicket.logTextResult(
        ticket as TicketState & { text: [string, string]; resolved: ResolvedTicketInfo }
      );
      notifyStaff();
    ...
    addnote(target, room, user) {
      ...
      this.addGlobalModAction(
        `${user.name} added the note "${note}" to ${ticket.userid}'s helpticket.`
      );
    ...
    removenote(target, room, user) {
      ...
      if (!targetId)
        return this.errorReply(
          `Specify the userid that created the ticket you want to remove a note from.`
        );
      ...
      if (!note) {
        return this.errorReply(
          `${staff === user.id ? 'you do' : `'${staff}' does`} not have a note on that ticket.`
        );
      }
      if (!room || room.roomid !== 'staff') {
        this.sendReply(
          `You removed the note '${note}' (by ${staff}) on ${ticket.userid}'s ticket.`
        );
      }
      ...
      this.room = Rooms.get('staff') || null;
      this.addModAction(
        `${user.name} removed ${staff}'s note ("${note}") from ${ticket.userid}'s helpticket.`
      );
    ...
    addresponse(target, room, user) {
      ...
      this.privateGlobalModAction(
        `${user.name} added a response button '${name}' for the ticket type ${typeId} ("${response}")`
      );
      this.globalModlog(
        `HELPTICKET ADDRESPONSE`,
        null,
        `'${response}' named ${name} for ${typeId}`
      );
    },
    addresponsehelp: [
    ...
    removeresponse(target, room, user) {
      ...
      writeSettings();
      this.privateGlobalModAction(
        `${user.name} removed the response named '${name}' from the responses for ${typeId} tickets`
      );
    ...
    listresponses(target, room, user) {
      ...
      buf += Object.keys(table)
        .map(
          type =>
            `<p>${ticketTitles[type]}<p>` +
            Object.keys(settings.responses[type])
              .map(name => Utils.html`<p>- ${name}: "${settings.responses[type][name]}"</p>`)
              .join('')
        )
        .join('<hr />');
      return this.sendReplyBox(buf);
    ...
    async ban(target, room, user) {
      ...
      if (targetUser) {
        ...
        if (punishment) {
          return this.privateModAction(
            `${username} would be ticket banned by ${user.name} but was already ticket banned.`
          );
        }
        if (targetUser.trusted) {
          Monitor.log(
            `[CrisisMonitor] Trusted user ${targetUser.name}${targetUser.trusted !== targetUser.id ? ` (${targetUser.trusted})` : ``} was ticket banned by ${user.name}, and should probably be demoted.`
          );
        }
      } else {
      ...
      if (targetUser) {
        targetUser.popup(
          `|modal|${user.name} has banned you from creating help tickets.${reason ? `\n\nReason: ${reason}` : ``}\n\nYour ban will expire in a few days.`
        );
      }

      const affected: (User | ID)[] = await HelpTicket.ban(targetUser || userid, reason);
      this.addGlobalModAction(
        `${username} was ticket banned by ${user.name}.${reason ? ` (${reason})` : ``}`
      );
      const acAccount =
        targetUser && targetUser.autoconfirmed !== userid && targetUser.autoconfirmed;

      let displayMessage = '';
      if (affected.length > 1) {
        const alts = affected
          .slice(1)
          .map(userObj => (typeof userObj === 'string' ? userObj : userObj.getLastName()))
          .join(', ');
    ...
    ignore(target, room, user) {
      ...
      if (user.settings.ignoreTickets) {
        return this.errorReply(
          this
            .tr`You are already ignoring help ticket notifications. Use /helpticket unignore to receive notifications again.`
        );
      }
    ...
    unignore(target, room, user) {
      ...
      if (!user.settings.ignoreTickets) {
        return this.errorReply(
          this
            .tr`You are not ignoring help ticket notifications. Use /helpticket ignore to stop receiving notifications.`
        );
      }
    ...
    async private(target, room, user) {
      ...
      if (!(await logPath.exists())) {
        return this.errorReply(
          `There are no logs for tickets from '${userid}' on the date '${date}'.`
        );
      }
      ...
      this.privateGlobalModAction(
        `${user.name} set the ticket logs for '${userid}' on '${date}' to be private.`
      );
    },
    privatehelp: [
    ...
    async public(target, room, user) {
      ...
      await logPath.copyFile(
        Monitor.logPath(`chat/help-${userid}/${date.slice(0, -3)}/${date}.txt`).path
      );
      await logPath.unlinkIfExists();
      this.globalModlog(`HELPTICKET PUBLICLOGS`, null, `${userid} (${date})`);
      this.privateGlobalModAction(
        `${user.name} set the ticket logs for '${userid}' on '${date}' to be public.`
      );
    },
    publichelp: [
