...
export function getIPData(ip: string) {
  try {
    return Net('https://miapi.dev/api/ip/' + ip)
      .get()
      .then(JSON.parse) as Promise<IPData>;
  } catch {
    return null;
  }
}
...
export const Nominations = new (class {
  ...
  save() {
    FS('config/chat-plugins/permas.json').writeUpdate(() =>
      JSON.stringify({ noms: this.noms, icons: this.icons })
    );
  }
  notifyStaff() {
  ...
  async add(target: string, connection: Connection) {
    ...
    const [primary, rawAlts, rawIps, type, details] = Utils.splitFirst(target, '|', 4).map(f =>
      f.trim()
    );
    ...
    if (!details) {
      this.error(
        'Details must be provided. Explain why this user should be permalocked.',
        connection
      );
    }
    if (!primaryID) {
      this.error(
        'A primary username must be provided. Use one of their alts if necessary.',
        connection
      );
    }
    ...
    Rooms.get('staff')?.addByUser(
      user,
      `${user.name} submitted a perma nomination for ${primaryID}`
    );
  }
  find(id: string) {
  ...
  close(target: string, context: Chat.CommandContext) {
    ...
    this.noms.splice(
      this.noms.findIndex(f => f.primaryID === entry.primaryID),
      1
    );
    this.save();
    this.notifyStaff();
...
export const commands: Chat.ChatCommands = {
  perma: {
    ...
    async actmain(target, room, user) {
      this.checkCan('rangeban');
      const [primaryName, standingName, postReason] = Utils.splitFirst(target, ',', 2).map(f =>
        f.trim()
      );
      ...
      if (!threadNum) {
        throw new Chat.ErrorMessage(
          'The link to the perma has not been set - the post could not be made.'
        );
      }
      ...
      const res = await Smogon.post(threadNum, postBuf);
      if (!res || res.error) {
      ...
      nom.post = url;
      this.popupReply(
        `|html|Standing successfully changed. Smogon post can be found <a href="${url}">at this link</a>.`
      );
    },
