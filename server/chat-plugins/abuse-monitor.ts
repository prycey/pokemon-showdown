...
export const cache: {
  ...
  for (const k in plugin.cache) {
    ...
    if (typeof cur.recommended?.type === 'string') {
      // would be object if it was the new entry
...
export const muted =
  Chat.oldPlugins['abuse-monitor']?.muted || new WeakMap<Room, WeakMap<User, number>>();
...
const defaults: FilterSettings = {
  ...
  replacements: {},
  recommendOnly: true,
  punishments: [{ certainty: 0.93, type: 'IDENTITY_ATTACK', punishment: 'WARN', count: 2 }],
};
...
function visualizePunishment(punishment: PunishmentSettings) {
  return Utils.sortBy(Object.keys(punishment))
...
export const punishmentCache: WeakMap<User, Record<string, number>> = Chat.oldPlugins[
  'abuse-monitor'
]?.punishmentCache || new WeakMap();

export async function searchModlog(query: {
  user: ID;
  ip?: string | string[];
  actions?: string[];
}) {
...
export async function runActions(
  ...
  for (const punishment of settings.punishments) {
    if (prevRecommend?.type) {
      // avoid making extra db queries by frontloading this check
  ...
  if (recommended.length) {
    ...
    if (roomRecord) {
      ...
      roomRecord.recommended[user.id] = {
        type: punishment,
...
function addGlobalModAction(message: string, room: GameRoom) {
  ...
  Rooms.get(`staff`)
    ?.add(`|c|~|/log ${getViewLink(room.roomid)} ${message}`)
    .update();
}
...
export async function lock(user: User, room: GameRoom, reason: string, isWeek?: boolean) {
  ...
  addGlobalModAction(
    `${user.name} was locked from talking by Artemis${isWeek ? ' for a week. ' : '. '}(${reason})`,
    room
  );
  if (affected.length > 1) {
    Rooms.get('staff')?.add(
      `|c|~|/log (${user.id}'s ` +
        `locked alts: ${affected
          .slice(1)
...
function checkAccess(
  ...
  perm: GlobalPermission = 'bypassall'
) {
...
export const commands: Chat.ChatCommands = {
  ...
  abusemonitor: {
      ...
      for (const [i, p] of settings.punishments.entries()) {
        ...
        for (const k in response) {
          ...
          if (descriptors.length) {
            // ignore modlog / flag -only based actions
    ...
    async compare(target) {
      ...
      const againstResponse =
        (await new Promise<Record<string, number> | null>(resolve => {
    ...
    toggle(target) {
      ...
      this.privateGlobalModAction(
        `${this.user.name} ${!settings.disabled ? 'enabled' : 'disabled'} the abuse monitor.`
      );
    ...
    threshold(target) {
      ...
      this.refreshPage('abusemonitor-settings');
      this.privateGlobalModAction(
        `${this.user.name} set the abuse monitor trigger threshold to ${num}.`
      );
    ...
    async resolve(target) {
      ...
      const inserted = await Chat.database.get(
        `SELECT result FROM perspective_stats WHERE roomid = ?`,
        [roomid]
      );
      if (inserted?.result === 1) {
        // (hardcode on 1 because 2 is dead)
    ...
    async nojoinpunish(target, room, user) {
      ...
      if (result) {
        // command succeeded - send followup
        this.add(
    ...
    async rescale(target, room, user) {
      ...
      for (const chunk of examples) {
        ...
        if (!(message && rawNum)) {
          return this.errorReply(
            `Invalid example: "${chunk}". Must be in \`\`message|num\`\` format.`
          );
        }
    ...
    async userclear(target, room, user) {
      ...
      user.lastCommand = '';
      const results = await Chat.database.run('DELETE FROM perspective_logs WHERE userid = ?', [
        targetId,
      ]);
      if (!results.changes) {
      ...
      this.privateGlobalModAction(
        `${user.name} cleared abuse monitor logs for ${targetUsername}${rest ? ` (${rest})` : ''}.`
      );
    ...
    editspecial(target, room, user) {
      ...
      if (!(type in types)) {
        return this.errorReply(
          `Invalid type: ${type}. Valid types: ${Object.keys(types).join(', ')}.`
        );
      }
      ...
      if (settings.specials[type]?.[percent] && !this.cmd.includes('f')) {
        return this.errorReply(
          `That special case already exists. Use /am forceeditspecial to change it.`
        );
      }
      ...
      this.refreshPage('abusemonitor-settings');
      this.privateGlobalModAction(
        `${user.name} set the abuse monitor special case for ${type} at ${percent}% to ${score}.`
      );
      this.globalModlog('ABUSEMONITOR SPECIAL', type, `${percent}% to ${score}`);
      this.sendReply(
        `|html|Remember to use <code>/am respawn</code> to deploy the settings to the child processes.`
      );
    },
    ds: 'deletespecial',
    deletespecial(target, room, user) {
      ...
      this.refreshPage('abusemonitor-settings');
      this.privateGlobalModAction(
        `${user.name} deleted the abuse monitor special case for ${type} at ${percent}%.`
      );
    ...
    changeall(target, room, user) {
      ...
      this.privateGlobalModAction(
        `${user.name} updated all abuse-monitor punishments of type ${to} to type ${from}`
      );
      saveSettings();
    ...
    addpunishment(target, room, user) {
      ...
      for (const cur of targets) {
        ...
        key = toID(key);
        if (!key || !value) {
          // sent from the page, val wasn't sent.
          continue;
        }
        switch (key) {
          case 'punishment':
          case 'p':
            ...
            if (!PUNISHMENTS.includes(value)) {
              return this.errorReply(
                `Invalid punishment: ${value}. Valid punishments: ${PUNISHMENTS.join(', ')}.`
              );
            }
          ...
          case 't':
            ...
            if (
              !Artemis.RemoteClassifier.ATTRIBUTES[
                value as keyof typeof Artemis.RemoteClassifier.ATTRIBUTES
              ]
            ) {
              return this.errorReply(
          ...
          case 'ct':
            ...
            if (isNaN(certainty) || certainty > 1 || certainty < 0) {
              return this.errorReply(
                `Invalid certainty '${value}'. Must be a number above 0 and below 1.`
              );
            }
          ...
          case 'secondary':
            ...
            if (
              !Artemis.RemoteClassifier.ATTRIBUTES[
                sType as keyof typeof Artemis.RemoteClassifier.ATTRIBUTES
              ]
            ) {
              return this.errorReply(
            ...
            if (isNaN(sCertainty) || sCertainty > 1 || sCertainty < 0) {
              return this.errorReply(
                `Invalid secondary certainty '${sValue}'. Must be a number above 0 and below 1.`
              );
            }
            if (!punishment.secondaryTypes) {
      ...
      this.refreshPage('abusemonitor-settings');
      this.privateGlobalModAction(
        `${user.name} added a ${punishment.punishment} abuse-monitor punishment.`
      );
    ...
    deletepunishment(target, room, user) {
      ...
      this.refreshPage('abusemonitor-settings');
      this.privateGlobalModAction(
        `${user.name} removed the abuse-monitor punishment indexed at ${idx + 1}.`
      );
      this.stafflog(
        `Punishment: ` +
          // eslint-disable-next-line @typescript-eslint/no-base-to-string
          `${Object.keys(punishment)
            .map(f => `${f}: ${punishment[f as keyof PunishmentSettings]}`)
            .join(', ')}`
      );
    ...
    async failures(target) {
      ...
      logs = logs.filter(
        (
          log // proofing against node's stupid date lib
    ...
    togglepunishments(target, room, user) {
      ...
      if (this.meansYes(target)) {
        ...
        return this.sendReply(
          `Automatic punishments are: ${!settings.recommendOnly ? 'ON' : 'OFF'}.`
        );
      }
    ...
    replace(target, room, user) {
      ...
      saveSettings();
      this.privateGlobalModAction(
        `${user.name} added an Artemis replacement for '${old}' to '${newWord}'.`
      );
  ...
  abusemonitor: {
    ...
    async userlogs(query, user) {
      ...
      const logs = await Chat.database.all(
        `SELECT * FROM perspective_logs WHERE userid = ? LIMIT 100`,
        [userid]
      );
      if (!logs.length) {
    ...
    async stats(query, user) {
      ...
      const prev = new Date(new Date(`${dateString}-15`).getTime() - 30 * 24 * 60 * 60 * 1000)
        .toISOString()
        .slice(0, 7);
      ...
      const dayStats: Record<
        string,
        { successes: number; failures: number; dead: number; total: number }
      > = {};
      for (const log of logs) {
      ...
      const percent = (numerator: number, denom: number) =>
        Math.floor((numerator / denom) * 100) || 0;
      ...
      for (const [i, day] of sortedDays.entries()) {
        ...
        if (cur.failures) {
          ...
          // so one cannot confuse dead tickets ~ false hit tickets
          data += ' | 0 (0%)';
        }
      ...
      if (punishmentStats.total) {
        ...
        header = '';
        data = '';
        const sortedDayStats = Utils.sortBy(Object.keys(punishmentStats.byDay), d =>
          new Date(d).getTime()
        );
    ...
    async settings() {
      ...
      if (settings.punishments.length) {
        for (const [i, p] of settings.punishments.entries()) {
          buf += `&bull; ${i + 1}: `;
          buf += Object.keys(p)
            .map(f => `${f}: ${visualizePunishmentKey(p, f as keyof PunishmentSettings)}`)
            .join(', ');
      ...
      const keys = Utils.sortBy(Object.keys(Artemis.RemoteClassifier.ATTRIBUTES), k => [
        -Object.keys(settings.specials[k] || {}).length,
        k,
      ]);
      for (const k of keys) {
