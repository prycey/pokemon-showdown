...
export const commands: Chat.ChatCommands = {
  ...
  removefaq(target, room, user) {
    ...
    Object.keys(roomFaqs[room.roomid])
      .filter(val => getAlias(room.roomid, val) === topic)
      .map(val => delete roomFaqs[room.roomid][val]);
  ...
  addalias(target, room, user) {
    ...
    if (alias === topic)
      return this.errorReply('You cannot make the alias have the same name as the topic.');
    if (roomFaqs[room.roomid][alias] && !roomFaqs[room.roomid][alias].alias) {
      return this.errorReply(
        'You cannot overwrite an existing topic with an alias; please delete the topic first.'
      );
    }
    ...
    if (getAlias(room.roomid, topic)) {
      return this.errorReply(
        `You cannot make an alias of an alias. Use /addalias ${alias}, ${getAlias(room.roomid, topic)} instead.`
      );
    }
  ...
  roomfaq(target, room, user, connection, cmd) {
    ...
    if (!this.broadcasting && user.can('ban', null, room, 'addfaq')) {
      ...
      this.sendReplyBox(
        `<details><summary>Source</summary><code style="white-space: pre-wrap; display: table; tab-size: 3">/${command} ${topic}, ${code}</code></details>`
      );
    }
  },
  roomfaqhelp: [
