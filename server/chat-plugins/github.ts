...
export const gitData: GitData = JSON.parse(
  FS('config/chat-plugins/github.json').readIfExistsSync() || '{}'
);

interface GitHookHandler {
  on(
    event: 'pull_request',
    callback: (repo: string, ref: string | undefined, result: PullRequest) => void
  ): void;
...
export const GitHub = new (class {
  ...
  handlePush(repo: string, ref: string, result: Push) {
    ...
    for (const commit of result.commits) {
      ...
      messages.staff.push(
        Utils.html`[<span style="color:#FF00FF">${repoName}</span>] <a href="${url}">${shortMessage}</a> <span style="color:#909090">(${username})</span>`
      );
    }
...
export const commands: Chat.ChatCommands = {
  ...
  github: {
    ...
    ban(target, room, user) {
      ...
      if (room.auth.has(toID(GitHub.getUsername(username)))) {
        return this.errorReply(
          'That user is Dev roomauth. If you need to do this, demote them and try again.'
        );
      }
      ...
      this.privateModAction(
        `${user.name} banned the GitHub user ${username} from having their GitHub actions reported to this server.`
      );
    ...
    unban(target, room, user) {
      ...
      GitHub.save();
      this.privateModAction(
        `${user.name} allowed the GitHub user ${target} to have their GitHub actions reported to this server.`
      );
    ...
    addusername(target, room, user) {
      ...
      GitHub.save();
      this.privateModAction(
        `${user.name} set ${gitName}'s name on reported GitHub actions to be ${username}.`
      );
