...
export async function runBattleSearch(
  userids: ID[],
  month: string,
  tierid: ID,
  turnLimit?: number
  ...
  files = files
    .filter(item => item.startsWith(month))
    .map(item => Monitor.logPath(`${month}/${tierid}/${item}`).path);
  ...
  if (useRipgrep) {
    ...
    let output;
    try {
      output = await ProcessManager.exec([
        'rg',
        '-i',
        regexString,
        '--no-line-number',
    ...
    for (const line of output.stdout.split('\n').reverse()) {
      ...
      const foe =
        userids.length > 1 ? null : userid === toID(data.p1) ? toID(data.p2) : toID(data.p1);
      if (foe) {
...
function buildResults(
  data: { [k: string]: BattleSearchResults },
  userids: ID[],
  month: string,
  tierid: ID,
  turnLimit?: number
  ...
  buf +=
    userids.length > 1
      ? `the users ${userids.join(', ')} were players`
      : `the user ${userids[0]} was a player`;
  buf += turnLimit
...
async function rustBattleSearch(
  ...
  if (days < 1 || days > 15) {
    throw new Chat.ErrorMessage(
      `Days must be between 1 and 15. To search longer ranges, use psbattletools manually on sim3.`
    );
  }
  if (!targetUser) {
  ...
  if (user.lastCommand !== '/battlesearch' && [30, 31, 1].includes(currentDayOfMonth)) {
    ...
    buf.push(
      `Battlesearch can interfere with usage stats processing due to high computational load.`
    );
  ...
  search.stdout.on('data', data => {
    buf = buf.replace(
      '</div>',
      `${Chat.formatText(data.toString()).replace(/\n/g, '<br />')}</div>`
    );
    connection.send(buf);
  });
  search.on('close', () => {
...
export const pages: Chat.PageTable = {
  async battlesearch(args, user, connection) {
    ...
    const [ids, rawLimit, month, formatid, confirmation] = Utils.splitFirst(
      this.pageid.slice(18),
      '--',
      5
    );
    ...
    if (!ids || (turnLimit && turnLimit < 1)) {
      return user.popup(
        `Some arguments are missing or invalid for battlesearch. Use /battlesearch to start over.`
      );
    }
