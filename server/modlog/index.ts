...
export class Modlog {
  ...
  async setupDatabase() {
    ...
    this.altsInsertionQuery = await this.database.prepare(
      `INSERT INTO alts (modlog_id, userid) VALUES (?, ?)`
    );
  ...
  async getGlobalPunishmentsSQL(userid: ID, days: number) {
    ...
    if (!this.globalPunishmentsSearchQuery) {
      throw new Error(
        `Modlog#globalPunishmentsSearchQuery is falsy but an SQL search function was called.`
      );
    }
  ...
  async search(
    ...
    const results = (await this.database.all(query.queryText, query.args)).map((row: any) =>
      this.dbRowToModlogEntry(row)
    );
  ...
  prepareSQLSearch(
    ...
    for (const user of search.user) {
      ...
      ors.push({
        query: `(userid ${tester} OR autoconfirmed_userid ${tester})`,
        args: [param, param],
      });
      ors.push({
