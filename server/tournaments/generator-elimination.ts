...
class ElimNode {
  ...
  toJSON() {
    ...
    if (!this.children) {
      node.team =
        this.user ||
        (this.losersBracketIndex <= 1
          ? `(loser's bracket)`
          : `(loser's bracket ${this.losersBracketIndex})`);
    } else {
...
export class Elimination {
    ...
    for (let losersBracketIndex = 1; losersBracketIndex < this.maxSubtrees; losersBracketIndex++) {
      ...
      for (const depth in matchesByDepth) {
        ...
        for (; n < matchesThisDepth.length - 1; n += 2) {
          ...
          oldLeaf.setChildren([
            new ElimNode({ losersBracketIndex, fromNode: oldLeafFromNode }),
            newBranch,
          ]);
          ...
          const newRightChild = new ElimNode({
            losersBracketIndex,
            fromNode: matchesThisDepth[n + 1],
          });
  ...
  disqualifyUser(user: TournamentPlayer) {
    ...
    if (found) {
      ...
      if (error) {
        throw new Error(
          `Unexpected ${error} from setMatchResult([${found.match.join(', ')}], ${found.result})`
        );
      }
    }
  ...
  setMatchResult(
    [p1, p2]: [TournamentPlayer, TournamentPlayer],
    result: 'win' | 'loss',
    score: number[]
  ) {
    if (!this.isBracketFrozen) return 'BracketNotFrozen';
    ...
    const targetNode = this.treeRoot.find(node => {
      if (
        node.state === 'available' &&
        node.children![0].user === p1 &&
        node.children![1].user === p2
      ) {
        return node;
      }
      return undefined;
    ...
    if (targetNode.parent) {
      ...
      if (loser.losses <= winner.losses && !loser.isDisqualified) {
        ...
        const newNode = new ElimNode({
          state: 'available',
          losersBracketNode: targetNode.losersBracketNode,
        });
