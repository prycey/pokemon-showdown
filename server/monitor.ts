...
export const Monitor = new (class {
  ...
  error(text: string) {
    (Rooms.get('development') || Rooms.get('staff') || Rooms.get('lobby'))
      ?.add(`|error|${text}`)
      .update();
  ...
  countConnection(ip: string, name = '') {
    ...
    if (count === 500) {
      this.adminlog(
        `[ResourceMonitor] IP ${ip} banned for cflooding (${count} times in ${Chat.toDurationString(duration)}${name ? ': ' + name : ''})`
      );
      return true;
    }
    ...
    if (count > 500) {
      if (count % 500 === 0) {
        ...
        if (c === 2 || c === 4 || c === 10 || c === 20 || c % 40 === 0) {
          this.adminlog(
            `[ResourceMonitor] IP ${ip} still cflooding (${count} times in ${Chat.toDurationString(duration)}${name ? ': ' + name : ''})`
          );
        }
      }
      return true;
  ...
  countBattle(ip: string, name = '') {
    ...
    if (duration < 5 * 60 * 1000 && count % 30 === 0) {
      this.adminlog(
        `[ResourceMonitor] IP ${ip} has battled ${count} times in the last ${Chat.toDurationString(duration)}${name ? ': ' + name : ''})`
      );
      return true;
    }
  ...
  countPrepBattle(ip: string, connection: Connection) {
    ...
    connection.popup(
      'Due to high load, you are limited to 12 battles and team validations every 3 minutes.'
    );
    return true;
  }
