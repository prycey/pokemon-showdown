...
const LadderStore: typeof import('./ladders-remote').LadderStore = (
  typeof Config === 'object' && Config.remoteladder
    ? require('./ladders-remote')
    : require('./ladders-local')
).LadderStore;
...
class Ladder extends LadderStore {
  async prepBattle(
    ...
    if (isRated && !Ladders.disabled) {
      ...
      if (Ladders.disabled) {
        connection.popup(
          `The ladder is temporarily disabled due to technical difficulties - you will not receive ladder rating for this game.`
        );
        rating = 1;
      }
  ...
  async makeChallenge(connection: Connection, targetUser: User) {
    ...
    if (existingChall) {
      if (
        ...
        connection.popup(
          `There's already a challenge (${existingChall.format}) between you and ${targetUser.name}!`
        );
  ...
  static updateSearch(user: User, connection: Connection | null = null) {
    ...
    for (const roomid of user.games) {
      ...
      if (!room) {
        Monitor.warn(
          `while searching, room ${roomid} expired for user ${user.id} in rooms ${[...user.inRooms]} and games ${[...user.games]}`
        );
      ...
      if (!game) {
        Monitor.warn(
          `while searching, room ${roomid} has no game for user ${user.id} in rooms ${[...user.inRooms]} and games ${[...user.games]}`
        );
        user.games.delete(roomid);
        continue;
      }
  ...
  addSearch(newSearch: BattleReady, user: User) {
    ...
    for (const search of formatTable.searches.values()) {
      ...
      const matched = this.matchmakingOK([
        [search, searcher],
        [newSearch, user],
      ]);
      if (matched) {
  ...
  static periodicMatch() {
    ...
    for (const [formatid, formatTable] of Ladders.searches) {
      ...
      for (const search of formatTable.searches.values()) {
        ...
        const matched = matchmaker.matchmakingOK([
          [search, searcher],
          [longestSearch, longestSearcher],
        ]);
        if (matched) {
  ...
  static match(readies: BattleReady[]) {
    ...
    if (missingUser) {
      for (const ready of readies) {
        Users.get(ready.userid)?.popup(
          `Sorry, your opponent ${missingUser} went offline before your battle could start.`
        );
      }
      return undefined;
    }
