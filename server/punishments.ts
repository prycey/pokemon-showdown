...
export const Punishments = new (class {
  ...
  async punish(
    ...
    if (mobileIps.size) {
      ...
      const mobilePunishment = {
        type,
        id,
        expireTime: mobileExpireTime,
        reason,
        rest,
      } as Punishment;
  ...
  async punishInner(
    ...
    userids: Set<ID>,
    ips: Set<string>,
    mobileIps: Set<string>
  ) {
    const existingPunishment = Punishments.userids.getByType(
      user.locked || toID(user.name),
      punishment.type
    );
  ...
  punishName(userid: ID, punishment: Punishment) {
    ...
    const { type, id, expireTime, reason, rest } = punishment;
    const affected = Users.findUsers([...userids], [...ips], {
      includeTrusted: PUNISH_TRUSTED,
      forPunishment: true,
    });
    userids.delete(id as ID);
    void Punishments.appendPunishment(
  ...
  async autolock(
    ...
    if (namelock) {
      ...
      await Punishments.namelock(
        user,
        expires,
        toID(namelock),
        false,
        `Autonamelock: ${name}: ${reason}`
      );
    } else {
    ...
    if (roomauth.length) {
      Monitor.log(
        `[CrisisMonitor] Autolocked user ${name} has public roomauth (${roomauth.join(', ')}), and should probably be demoted.`
      );
    }
  }
  unlock(name: string) {
  ...
  punishRange(range: string, reason: string, expireTime?: number | null, punishType?: string) {
  ...
  addSharedIp(ip: string, note: string) {
    ...
    for (const user of Users.users.values()) {
      const sharedIp = user.ips.some(curIP =>
        isRange ? IPTools.checkPattern([pattern], IPTools.ipToNumber(curIP)) : curIP === ip
      );
  ...
  removeSharedIp(ip: string) {
    ...
    if (pattern && pattern.minIP !== pattern.maxIP) {
      ...
      Punishments.sharedRanges = new Map(
        [...Punishments.sharedRanges].filter(([range]) => !isMatch(range))
      );
    } else {
  ...
  hasRoomPunishType(room: Room | RoomID, name: string, types: string | string[]) {
    ...
    return Punishments.roomUserids
      .nestedGet(room as RoomID, name)
      ?.some(p => types.includes(p.type));
  }

  sortedTypes = ['TICKETBAN', 'LOCK', 'NAMELOCK', 'BAN'];
  sortedRoomTypes: string[] = [
    ...(global.Punishments?.sortedRoomTypes || []),
    'ROOMBAN',
    'BLACKLIST',
  ];
  ...
  checkName(user: User, userid: string, registered: boolean) {
    ...
    for (const punishment of punishments) {
      ...
      const bannedUnder =
        punishUserid !== userid
          ? ` because you have the same IP as banned user: ${punishUserid}`
          : '';

      if (id === 'BATTLEBAN') {
        if (
          punishUserid !== user.id &&
          Punishments.isSharedIp(user.latestIp) &&
          user.autoconfirmed
        ) {
      ...
      if (id === 'NAMELOCK' || user.namelocked) {
        user.send(
          `|popup||html|You are namelocked and can't have a username${bannedUnder}. Your namelock will expire in a few days.${reason}${appeal}`
        );
        ...
        if (punishUserid === '#hostfilter' || punishUserid === '#ipban') {
          user.send(
            `|popup||html|Your IP (${user.latestIp}) is currently locked due to being a proxy. We automatically lock these connections since they are used to spam, hack, or otherwise attack our server. Disable any proxies you are using to connect to PS.\n\n<a href="view-help-request--appeal"><button class="button">Help me with a lock from a proxy</button></a>`
          );
        } else if (user.latestHostType === 'proxy' && user.locked !== user.id) {
          user.send(
            `|popup||html|You are locked${bannedUnder} on the IP (${user.latestIp}), which is a proxy. We automatically lock these connections since they are used to spam, hack, or otherwise attack our server. Disable any proxies you are using to connect to PS.\n\n<a href="view-help-request--appeal"><button class="button">Help me with a lock from a proxy</button></a>`
          );
        } else if (!user.notified.lock) {
          user.send(
            `|popup||html|You are locked${bannedUnder}. ${user.permalocked ? `This lock is permanent.` : `Your lock will expire in a few days.`}${reason}${appeal}`
          );
        }
  ...
  checkIpBanned(connection: Connection) {
    const ip = connection.ip;
    if (
      Punishments.cfloods.has(ip) ||
      (Monitor.countConnection(ip) && Punishments.cfloods.add(ip))
    ) {
      connection.send(
        `|popup||modal|PS is under heavy load and cannot accommodate your connection right now.`
      );
      return '#cflood';
    }
  ...
  checkNewNameInRoom(user: User, userid: string, roomid: RoomID): Punishment[] | null {
    let punishments: Punishment[] | null =
      Punishments.roomUserids.nestedGet(roomid, userid) || null;
    if (!punishments) {
      const room = Rooms.get(roomid)!;
      if (room.parent) {
    ...
    if (punishments) {
      for (const punishment of punishments) {
        ...
        if (info?.onActivate) {
          info.onActivate.call(
            this,
            user,
            punishment,
            Rooms.get(roomid)!,
            punishment.id === user.id
          );
          continue;
        }
  ...
  isGlobalBanned(user: User): Punishment | undefined {
    ...
    const punishment =
      Punishments.userids.getByType(user.id, 'BAN') ||
      Punishments.userids.getByType(user.id, 'FORCEBAN');
    if (punishment) return punishment;
  }
  ...
  getRoomPunishments(
    user: User | string,
    options: Partial<{ checkIps: any; publicOnly: any }> = {}
  ) {
    if (!user) return [];
    const userid = toID(user);
    ...
    for (const curRoom of Rooms.global.chatRooms) {
      if (
        !curRoom ||
        curRoom.settings.isPrivate === true ||
      ...
      if (typeof user !== 'string' && curRoom.muteQueue) {
        ...
        for (const entry of curRoom.muteQueue) {
          if (
            ...
            punishments.push([
              curRoom,
              { type: 'MUTE', id: entry.userid, expireTime: entry.time, reason: '' } as Punishment,
            ]);
          }
        }
      }
  ...
  getPunishments(roomid?: RoomID, ignoreMutes?: boolean) {
    ...
    // `Punishments.roomIps.get(roomid)` guaranteed to exist above
    (roomid ? Punishments.roomUserids.get(roomid)! : Punishments.userids).each(
      (punishment, userid) => {
        const { type, id, expireTime, reason, rest } = punishment;
        if (id.startsWith('#')) return;
        let entry = punishmentTable.find(
          ([curId, cur]) => id === curId && cur.punishType === type
        )?.[1];
        if (!entry) {
          entry = {
            userids: [],
  ...
  async monitorRoomPunishments(user: User | ID) {
    ...
    const minPunishments =
      typeof Config.monitorminpunishments === 'number' ? Config.monitorminpunishments : 3;
    ...
    if (punishments.length >= minPunishments) {
      ...
      const punishmentText = punishments
        .map(([room, punishment]) => {
      ...
      if (Config.punishmentautolock && points >= AUTOLOCK_POINT_THRESHOLD) {
        ...
        const globalPunishments = await Rooms.Modlog.getGlobalPunishments(
          userid,
          AUTOWEEKLOCK_DAYS_TO_SEARCH
        );
        ...
        Monitor.log(
          `[PunishmentMonitor] ${(user as User).name || userid} currently has punishments in ${punishments.length} rooms: ${punishmentText}`
        );
      }
    }
  }
