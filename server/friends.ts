...
export class FriendsDatabase {
  ...
  static setupDatabase(fileName?: string) {
    ...
    for (const k in ACTIONS) {
      try {
        ...
        throw new Error(
          `Friends DB statement crashed: ${ACTIONS[k as keyof typeof ACTIONS]} (${e.message})`
        );
      }
    }
  ...
  async request(user: User, receiverID: ID) {
    ...
    if (receiver) {
      sendPM(
        `/raw <span class="username">${user.name}</span> sent you a friend request!`,
        receiver.id,
        user.id
      );
...
const TRANSACTIONS: { [k: string]: (input: any[]) => DatabaseResult } = {
  send: requests => {
    for (const request of requests) {
      ...
      const existingFriendship = statements.findFriendship.all({
        user1: senderID,
        user2: receiverID,
      });
  ...
  query => {
    ...
    try {
      ...
      if (!e.name.endsWith('FailureMessage')) {
        result.error =
          "Sorry! The database process crashed. We've been notified and will fix this.";
...
if (require.main === module) {
  ...
  if (process.mainModule === module) {
    global.Monitor = {
      crashlog(
        error: Error,
        source = 'A friends database process',
