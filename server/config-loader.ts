...
const FLAG_PRESETS = new Map([['--no-security', ['nothrottle', 'noguestsecurity', 'noipchecks']]]);
...
export function load(invalidate = false) {
  ...
  if (config.usesqlite) {
    try {
      ...
      throw new Error(
        `better-sqlite3 is not installed or could not be loaded, but Config.usesqlite is enabled.`
      );
    }
  }
    ...
    for (const key in groupData) {
      if (isPermission(key)) {
        ...
        if (typeof jurisdiction === 'string' && jurisdiction.includes('s')) {
          reportError(
            `Outdated jurisdiction for permission "${key}" of group "${symbol}": 's' is no longer a supported jurisdiction; we now use 'ipself' and 'altsself'`
          );
