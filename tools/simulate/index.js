...
if (process.argv[2]) {
  const help = ['help', '-help', '--help', 'h', '-h', '--help', '?', '-?', '--?'].includes(
    process.argv[2]
  );
  const unknown =
    !['multi', 'random', 'exhaustive'].includes(process.argv[2]) &&
    !/^[0-9]+$/.test(process.argv[2]);
...
const RejectionTracker = new (class {
  ...
  onRejectionHandled(promise) {
    this.unhandled.splice(
      this.unhandled.findIndex(u => u.promise === promise),
      1
    );
  }
  ...
  case 'exhaustive':
    {
      ...
      const maxFailures =
        argv.maxFailures ||
        argv.failures ||
        (formats.length > 1 ? ExhaustiveRunner.MAX_FAILURES : 1);
