...
export function parseBrackets(line: string, openingBracket: '(' | '[', greedy?: boolean) {
  ...
  const bracketCloseIndex = greedy
    ? line.lastIndexOf(brackets[openingBracket])
    : line.indexOf(brackets[openingBracket]);
...
export function modernizeLog(line: string, nextLine?: string): string | undefined {
  ...
  if (line.startsWith('SCAV ')) {
    line = line.replace(
      /: (\[room: .*?\]) by (.*)/,
      (match, roominfo, rest) => `: by ${rest} ${roominfo}`
    );
  }
  line = line.replace(
  ...
  if (line.includes(':')) {
    ...
    if (possibleModernAction === possibleModernAction.toUpperCase()) {
      ...
      if (
        /\(.+\) by [a-z0-9]{1,19}$/.test(line) &&
        !['OLD MODLOG', 'NOTE'].includes(possibleModernAction)
      ) {
  ...
  const modernizerTransformations: { [k: string]: (log: string) => string } = {
    ...
    ' was blacklisted from ': log => {
      ...
      const banned = toID(
        log.slice(0, log.indexOf(` was ${isName ? 'name' : ''}blacklisted from `))
      );
    ...
    '[agameofhangman] was started by ': log =>
      `HANGMAN: by ${toID(log.slice('[agameofhangman] was started by '.length))}`,
    '[agameofunowas] created by ': log =>
      `UNO CREATE: by ${toID(log.slice('[agameofunowas] created by '.length))}`,
...
export function parseModlog(
  ...
  if (actionTakerIndex !== -1) {
    ...
    const actionTaker = line.slice(
      actionTakerIndex + 3,
      colonIndex > actionTakerIndex ? colonIndex : undefined
    );
...
export class ModlogConverterSQLite {
  ...
  constructor(
    databaseFile: string,
    textLogDir: string,
    isTesting?: DatabaseType.Database,
    ...
    if (isTesting || Config.nofswriting) {
      this.isTesting = {
        ...
        db: isTesting || new Database(':memory:'),
      };
    }
  ...
  async toTxt() {
    ...
    for (const { roomid } of roomids) {
      ...
      const insertEntries = async () => {
        ...
        if (
          !Config.nofswriting &&
          (entriesLogged % ENTRIES_TO_BUFFER === 0 || entriesLogged < ENTRIES_TO_BUFFER)
        ) {
      ...
      for (const result of results) {
        ...
        const entry: ModlogEntry = {
          ...
          ip: result.ip,
          isGlobal:
            result.roomid?.startsWith('global-') || result.roomid === 'global' || result.is_global,
...
export class ModlogConverterTxt {
  ...
  async toSQLite() {
    ...
    const files = this.isTesting
      ? [...this.isTesting.files.keys()]
      : await FS(this.textLogDir).readdir();
...
export const ModlogConverter = {
  async convert(
    ...
    if (from === 'txt' && to === 'txt' && outputLogPath) {
      ...
      const converter = new ModlogConverterSQLite(
        databasePath,
      ...
      const converter = new ModlogConverterTxt(
        databasePath,
        textLogDirectoryPath,
        undefined,
        newestAllowedTimestamp
