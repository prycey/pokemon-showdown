...
export const Formats: import('../sim/dex-formats').FormatList = [
  ...
  },
  {
    ...
    onBegin() {
      ...
      this.add(
        '-message',
        `This is a Generation 9 LC-based Pet Mod where multiple Pokemon have been reimagined in order to fit the LC metagame.`
      );
  ...
  },
  {
    ...
    onBegin() {
      ...
      this.add(
        '-message',
        `This is a Generation 2 Pet Mod that brings the Doubles game mode introduced in Generation 3 back in time to GSC.`
      );
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      ...
      let problems =
        this.dex.formats.get('Obtainable Misc').onChangeSet?.call(this, set, this.format) || null;
      if (Array.isArray(problems) && problems.length) return problems;
      const crossNonstandard =
        (!this.ruleTable.has('standardnatdex') && crossSpecies.isNonstandard === 'Past') ||
        crossSpecies.isNonstandard === 'Future';
      const crossIsCap =
        !this.ruleTable.has('+pokemontag:cap') && crossSpecies.isNonstandard === 'CAP';
      ...
      const crossIsUnreleased =
        crossSpecies.tier === 'Unreleased' &&
        crossSpecies.isNonstandard === 'Unobtainable' &&
        !this.ruleTable.has('+unobtainable');
      if (crossSpecies.battleOnly || crossIsUnreleased || !crossSpecies.prevo) {
        return [
          `${species.name} cannot cross evolve into ${crossSpecies.name} because it isn't an evolution.`,
        ];
      }
      if (this.ruleTable.isRestrictedSpecies(crossSpecies)) {
        return [
          `${species.name} cannot cross evolve into ${crossSpecies.name} because it is banned.`,
        ];
      }
      ...
      if (item.itemUser?.length) {
        if (!item.itemUser.includes(crossSpecies.name) || crossSpecies.name !== species.name) {
          return [
            `${species.name} cannot use ${item.name} because it is cross evolved into ${crossSpecies.name}.`,
          ];
        }
      }
    ...
    onModifySpecies(species, target, source, effect) {
      ...
      mixedSpecies.weightkg = Math.max(
        0.1,
        species.weightkg + crossSpecies.weightkg - crossPrevoSpecies.weightkg
      ).toFixed(1);
  ...
  },
  {
    ...
    onValidateTeam(team) {
      ...
      for (const set of team) {
        ...
        if (
          ...
          !this.ruleTable.has(`+pokemontag:${this.toID(species.isNonstandard)}`)
        ) {
          return [`${species.baseSpecies} does not exist in gen 9.`];
        }
        if (
          ((item.itemUser?.includes(species.name) || item.forcedForme === species.name) &&
            !item.megaStone &&
            !item.isPrimalOrb) ||
    ...
    onSwitchIn(pokemon) {
      ...
      if (originalSpecies.exists && pokemon.m.originalSpecies !== originalSpecies.baseSpecies) {
        ...
        if (
          oSpecies.types.length !== pokemon.species.types.length ||
          oSpecies.types[1] !== pokemon.species.types[1] ||
  ...
  },
  {
    ...
    onBeforeSwitchIn(pokemon) {
      ...
      for (const ability of format.getSharedPower!(pokemon)) {
        ...
        pokemon.volatiles[effect] = this.initEffectState({
          id: this.toID(effect),
          target: pokemon,
        });
    ...
    battle: {
      endTurn() {
        ...
        for (const pokemon of this.getAllActive(false, true)) {
          ...
          if (
            this.turn & 1 &&
            pokemon.position === (this.turn & 2 ? 0 : 1) &&
            pokemon.hp &&
            pokemon.allies().length
          ) {
            pokemon.volatiles['commanding'] = this.initEffectState({
              id: 'commanding',
              name: 'Commanding',
              target: pokemon,
            });
            pokemon.volatiles['gastroacid'] = this.initEffectState({
              id: 'gastroacid',
              name: 'Gastro Acid',
              target: pokemon,
            });
            this.add(
              '-message',
              `${pokemon.side.name}'s ${pokemon.name !== pokemon.species.name ? `${pokemon.name} (${pokemon.species.name})` : pokemon.name} will be skipped next turn.`
            );
          } else {
      ...
      getAllActive(includeFainted, includeCommanding) {
        ...
        for (const side of this.sides) {
          for (const pokemon of side.active) {
            if (
              pokemon &&
              (includeFainted || !pokemon.fainted) &&
              (includeCommanding || !pokemon.volatiles['commanding'])
            ) {
      ...
      if (move.id === pokemon.moveSlots[0].id && !move.flags['futuremove']) {
        ...
        move.onTry = function (source, t) {
          if (!t.side.addSlotCondition(t, 'futuremove')) {
            this.hint(
              'Future moves fail when the targeted slot already has a future move focused on it.'
            );
            return false;
          }
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      ...
      const accuracyLoweringMove = move.secondaries?.some(
        secondary => secondary.boosts?.accuracy && secondary.boosts?.accuracy < 0
      );
      const flinchMove = move.secondaries?.some(secondary => secondary.volatileStatus === 'flinch');
      const freezeMove =
        move.secondaries?.some(secondary => secondary.status === 'frz') || move.id === 'triattack';
      if (
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      if (!teamHas.abilityMap) {
        ...
        for (const pokemon of Dex.species.all()) {
          if (
            ...
            !this.ruleTable.has(`+pokemontag:${this.toID(pokemon.isNonstandard)}`)
          )
            continue;
      ...
      if (!validSources.length) {
        ...
        return [
          `${name} has an illegal set with an ability from ${this.dex.species.get(pokemonWithAbility[0]).name}.`,
        ];
      }
    ...
    battle: {
      spreadModify(baseStats, set) {
        ...
        for (statName in modStats) {
          ...
          modStats[statName] = tr(
            (tr(2 * stat + set.ivs[usedStat] + tr(set.evs[usedStat] / 4)) * set.level) / 100 + 5
          );
        }
        if ('hp' in baseStats) {
          const stat = baseStats['hp'];
          modStats['hp'] = tr(
            (tr(2 * stat + set.ivs['hp'] + tr(set.evs['hp'] / 4) + 100) * set.level) / 100 + 10
          );
        }
      ...
      natureModify(stats, set) {
        ...
        if (nature.plus) {
          ...
          stats[s] = this.ruleTable.has('overflowstatmod')
            ? Math.min(stats[nature.plus], 728)
            : stats[nature.plus];
    ...
    onBeforeSwitchIn(pokemon) {
      ...
      if (ally && ally.ability !== pokemon.ability) {
        if (!pokemon.m.innate && !BAD_ABILITIES.includes(this.toID(ally.ability))) {
          ...
          if (
            !ngas ||
            ally.getAbility().flags['cantsuppress'] ||
            pokemon.hasItem('Ability Shield')
          ) {
            pokemon.volatiles[pokemon.m.innate] = this.initEffectState({
              id: pokemon.m.innate,
              target: pokemon,
              pic: ally,
            });
        ...
        if (!ally.m.innate && !BAD_ABILITIES.includes(this.toID(pokemon.ability))) {
          ...
          if (
            ...
            ally.hasItem('Ability Shield')
          ) {
            ally.volatiles[ally.m.innate] = this.initEffectState({
              id: ally.m.innate,
              target: ally,
              pic: pokemon,
            });
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      ...
      if (set.moves?.length) {
        if (set.moves.length > this.ruleTable.maxMoveCount) {
          problems.push(
            `${set.name} has ${set.moves.length} moves, which is more than the limit of ${this.ruleTable.maxMoveCount}.`
          );
          return problems;
        }
        for (const [i, moveid] of set.moves.entries()) {
          ...
          if (
            ...
            problems.push(
              `${pokemove.isNonstandard} Pok\u00e9mon are not allowed to be used as Pokemoves.`
            );
          }
          if (
            this.ruleTable.isRestrictedSpecies(pokemove) ||
            this.ruleTable.isBannedSpecies(pokemove)
          ) {
    ...
    onModifyMove(move, pokemon, target) {
      ...
      if (species.exists) {
        ...
        move.category =
          species.baseStats['spa'] >= species.baseStats['atk'] ? 'Special' : 'Physical';
  ...
  },
  {
    ...
    onValidateRule() {
      if (this.format.gameType !== 'singles') {
        throw new Error(
          `Sharing is Caring currently does not support ${this.format.gameType} battles.`
        );
      }
    },
  ...
  },
  {
    ...
    onBegin() {
      for (const pokemon of this.getAllPokemon()) {
        ...
        pokemon.teraType = pokemon.teraType
          .replace(/(Ghost|Fairy)/g, 'Psychic')
          .replace(/Bug/g, 'Grass')
          .replace(/Ice/g, 'Water')
          .replace(/(Rock|Ground)/g, 'Fighting')
    ...
    battle: {
      tiebreak() {
        ...
        const notFainted = this.sides.map(
          side => side.pokemon.filter(pokemon => !pokemon.fainted).length
        );
        this.add(
          '-message',
          this.sides.map((side, i) => `${side.name}: ${notFainted[i]} Pokemon left`).join('; ')
        );
        ...
        const hpPercentage = tiedSides.map(
          side =>
            (side.pokemon.map(pokemon => pokemon.hp / pokemon.maxhp).reduce((a, b) => a + b) *
              100) /
            6
        );
        this.add(
          '-message',
          tiedSides
            .map((side, i) => `${side.name}: ${Math.round(hpPercentage[i])}% total HP left`)
            .join('; ')
        );
        ...
        this.add(
          '-message',
          tiedSides
            .map((side, i) => `${side.name}: ${Math.round(hpTotal[i])} total HP left`)
            .join('; ')
        );
  ...
  },
  {
    ...
    onValidateTeam(team, format, teamHas) {
      ...
      for (const trademark of teamHas.trademarks.keys()) {
        if (teamHas.trademarks.get(trademark) > 1) {
          problems.push(
            `You are limited to 1 of each Trademark.`,
            `(You have ${teamHas.trademarks.get(trademark)} Pok\u00e9mon with ${trademark} as a Trademark.)`
          );
        }
      }
      return problems;
    ...
    validateSet(set, teamHas) {
      ...
      if (set.moves.map(this.toID).includes(ability.id)) {
        return [
          `${set.name} may not use ${ability.name} as both a trademark and one of its moves simultaneously.`,
        ];
      }
      ...
      const validator = new TeamValidator(
        dex.formats.get(`${this.format.id}@@@${customRules.join(',')}`)
      );
  ...
  },
  {
    ...
    onValidateSet(set, format, setHas, teamHas) {
      if (set.item) {
        ...
        if (
          item.megaEvolves &&
          !(this.ruleTable.has(`+item:${item.id}`) || this.ruleTable.has(`+pokemontag:mega`))
        ) {
  ...
  },
  {
    ...
    onBegin() {
      ...
      this.add(
        `raw|<div class='broadcast-green'><b>Wondering what all these custom moves, abilities, and items do?<br />Check out the <a href="https://www.smogon.com/articles/super-staff-bros-ultimate" target="_blank">Super Staff Bros: Ultimate Guide</a> or use /ssb to find out!</b></div>`
      );
    ...
    onSwitchIn(pokemon) {
      ...
      if (
        this.dex.species.get(name).exists ||
        this.dex.moves.get(name).exists ||
        this.dex.abilities.get(name).exists ||
        name === 'blitz'
      ) {
  ...
  },
  {
    ...
    onBegin() {
      this.add(
        `raw|<div class="broadcast-blue"><b>Battle Factory Tier: ${this.teamGenerator.factoryTier}</b></div>`
      );
    },
  },
  {
    ...
    onTeamPreview() {
      ...
      for (const pokemon of this.getAllPokemon()) {
        const details = pokemon.details
          .replace(', shiny', '')
          .replace(
            /(Greninja|Gourgeist|Pumpkaboo|Xerneas|Zacian|Zamazenta|Dudunsparce)(-[a-zA-Z?-]+)?/g,
            '$1-*'
          );
  ...
  },
  {
    ...
    desc:
      `Teams generated automatically based on heuristics (rules), with levels based on previous success/failure in battle. ` +
  ...
  },
  {
    ...
    onValidateSet(set) {
      ...
      if (set.teraType === 'Steel') {
        return [
          `${species.name} has Steel as its Tera type, which is banned from Metronome Battle.`,
        ];
      }
      if (species.bst > 625) {
        return [
          `${species.name} is banned.`,
          `(Pok\u00e9mon with a BST higher than 625 are banned)`,
        ];
      }
      const item = this.dex.items.get(set.item);
    ...
    onSwitchIn(pokemon) {
      ...
      for (const i in pokemon.species.baseStats) {
        ...
        this.add(
          '-start',
          pokemon,
          `${pokemon.species.baseStats[i as keyof StatsTable]}${i === 'spa' ? 'spc' : i}`,
          '[silent]'
        );
      }
    },
