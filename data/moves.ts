...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
  ...
  crushgrip: {
    ...
    basePowerCallback(pokemon, target) {
      ...
      const bp =
        Math.floor(
          Math.floor((120 * (100 * Math.floor((hp * 4096) / maxHP)) + 2048 - 1) / 4096) / 100
        ) || 1;
  ...
  gearup: {
    ...
    onHitSide(side, source, move) {
      ...
      for (const target of targets) {
        didSomething =
          this.boost({ atk: 1, spa: 1 }, target, source, move, false, true) || didSomething;
      }
      return didSomething;
    },
    secondary: null,
  ...
  hardpress: {
    ...
    basePowerCallback(pokemon, target) {
      ...
      const bp =
        Math.floor(
          Math.floor((100 * (100 * Math.floor((hp * 4096) / maxHP)) + 2048 - 1) / 4096) / 100
        ) || 1;
      this.debug(`BP for ${hp}/${maxHP} HP: ${bp}`);
      return bp;
    },
  ...
  magneticflux: {
    ...
    onHitSide(side, source, move) {
      ...
      for (const target of targets) {
        didSomething =
          this.boost({ def: 1, spd: 1 }, target, source, move, false, true) || didSomething;
      }
      return didSomething;
    },
    secondary: null,
  ...
  wonderroom: {
    ...
    condition: {
      ...
      onModifyMove(move, source, target) {
        ...
        this.hint(
          `${move.name} uses ${statAndBoosts === 'def' ? '' : 'Sp. '}Def boosts when Wonder Room is active.`
        );
      },
      onFieldStart(field, source) {
        if (source?.hasAbility('persistent')) {
