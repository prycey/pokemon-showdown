export const Moves: import('../../../sim/dex-moves').ModdedMoveDataTable = {
  ...
  conversion: {
    ...
    flags: { metronome: 1 },
    onHit(target) {
      const possibleTypes = target.moveSlots
        .map(moveSlot => {
  ...
  encore: {
    ...
    condition: {
      ...
      onStart(target, source) {
        ...
        if (
          !target.lastMove ||
          target.lastMove.flags['failencore'] ||
  ...
  healblock: {
    ...
    condition: {
      ...
      onTryHeal(damage, pokemon, source, effect) {
        if (
          effect &&
          (effect.id === 'drain' || effect.id === 'leechseed' || effect.id === 'wish')
        ) {
          return false;
        }
      },
  ...
  highjumpkick: {
    ...
    onMoveFail(target, source, move) {
      ...
      this.damage(
        this.clampIntRange(damage / 2, 1, Math.floor(target.maxhp / 2)),
        source,
        source,
        move
      );
  ...
  skillswap: {
    ...
    onHit(target, source) {
      ...
      if (
        targetAbility === sourceAbility ||
        source.hasItem('griseousorb') ||
        target.hasItem('griseousorb')
      ) {
        return false;
      }
