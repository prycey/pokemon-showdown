export const Abilities: import('../../../sim/dex-abilities').ModdedAbilityDataTable = {
  ...
  intimidate: {
    inherit: true,
    onStart(pokemon) {
      const activated = pokemon
        .adjacentFoes()
        .some(
          target =>
            !(
              target.volatiles['substitute'] ||
              target.volatiles['substitutebroken']?.move === 'uturn'
            )
        );
  ...
  stickyhold: {
    ...
    onTakeItem(item, pokemon, source) {
      if (
        ...
        (this.activeMove && this.activeMove.id === 'knockoff')
      ) {
  ...
  wonderguard: {
    ...
    onTryHit(target, source, move) {
      if (move.id === 'firefang') {
        this.hint(
          'In Gen 4, Fire Fang is always able to hit through Wonder Guard.',
          true,
          target.side
        );
        return;
