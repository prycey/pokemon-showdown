  ...
  pokemon: {
    ...
    recalculateStats() {
      ...
      for (statName in this.storedStats) {
        ...
        stat = Math.floor(
          (Math.floor(2 * stat + this.set.ivs[statName] + Math.floor(this.set.evs[statName] / 4)) *
            this.level) /
            100 +
            5
        );
  ...
  actions: {
    ...
    runMove(moveOrMoveName, pokemon, targetLoc, options) {
      ...
      if (pokemon.moveThisTurn || !this.battle.runEvent('BeforeMove', pokemon, target, move)) {
        this.battle.debug(
          `${pokemon.fullname} move interrupted; movedThisTurn: ${pokemon.moveThisTurn}`
        );
          ...
          if (!move.multihit || move.lastHit) {
            ...
            if (
              ...
              if (
                ...
                this.battle.randomChance(effectChance + 1, 256)
              ) {
