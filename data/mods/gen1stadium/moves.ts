export const Moves: import('../../../sim/dex-moves').ModdedMoveDataTable = {
    ...
    condition: {
      ...
      onBeforeMove(pokemon, t, move) {
        if (this.effectState.duration === 1) {
          ...
          this.actions.moveHit(target, pokemon, move, {
  ...
  counter: {
    ...
    damageCallback(pokemon, target) {
      ...
      const lastMoveThisTurn =
        target.side.lastMove &&
        target.side.lastMove.id === target.side.lastSelectedMove &&
        !this.queue.willMove(target) &&
        this.dex.moves.get(target.side.lastMove.id);
      if (!lastMoveThisTurn) {
      ...
      const lastMoveThisTurnIsCounterable =
        lastMoveThisTurn &&
        lastMoveThisTurn.basePower > 0 &&
        ['Normal', 'Fighting'].includes(lastMoveThisTurn.type) &&
        lastMoveThisTurn.id !== 'counter';
    ...
    condition: {
      ...
      onTryHit(target, source, move) {
        ...
        if (target.volatiles['substitute']) {
          if (move.recoil) {
            this.damage(
              this.clampIntRange(Math.floor((damage * move.recoil[0]) / move.recoil[1]), 1),
              source,
              target,
              'recoil'
            );
