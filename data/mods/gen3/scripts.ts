  ...
  actions: {
    ...
    modifyDamage(baseDamage, pokemon, target, move, suppressMessages = false) {
      ...
      baseDamage = Math.floor(
        this.battle.runEvent('ModifyDamagePhase2', pokemon, target, move, baseDamage)
      );
      ...
      if (move.multihit) {
        ...
        for (i = 0; i < hits && target.hp && pokemon.hp; i++) {
          ...
          if (move.multiaccuracy && i > 0) {
            ...
            if (accuracy !== true) {
              if (!move.ignoreAccuracy) {
                boosts = this.battle.runEvent('ModifyBoost', pokemon, null, null, {
                  ...pokemon.boosts,
                });
                boost = this.battle.clampIntRange(boosts['accuracy'], -6, 6);
                if (boost > 0) {
              ...
              if (!move.ignoreEvasion) {
                boosts = this.battle.runEvent('ModifyBoost', target, null, null, {
                  ...target.boosts,
                });
                boost = this.battle.clampIntRange(boosts['evasion'], -6, 6);
                if (boost > 0) {
    ...
    calcRecoilDamage(damageDealt, move) {
      return this.battle.clampIntRange(
        Math.floor((damageDealt * move.recoil![0]) / move.recoil![1]),
        1
      );
    },
  },
