  ...
  sketch: {
    ...
    onHit(target, source) {
      ...
      if (
        source.transformed ||
        !lastMove ||
        disallowedMoves.includes(lastMove.id) ||
        source.moves.includes(lastMove.id) ||
        lastMove.isZ
      )
        return false;
  ...
  encore: {
    ...
    condition: {
      ...
      onStart(target) {
        ...
        if (!lastMove || target.volatiles['dynamax']) return false;
        if ((lastMove as ActiveMove).isZOrMaxPowered)
          lastMove = this.dex.moves.get(lastMove.baseMove);
        // @ts-expect-error modded
        const linkedMoves: [string, string] = target.getLinkedMoves(true);
        const moveIndex = target.moves.indexOf(lastMove.id);
        if (
          linkedMoves.includes(lastMove.id) &&
          this.dex.moves.get(linkedMoves[0]).flags['failencore'] &&
      ...
      onOverrideAction(pokemon, target, move) {
        if (!this.effectState.turnsActivated[this.turn]) {
          ...
          (Array.isArray(this.effectState.move) ? this.effectState.move.length : 1)
        ) {
      ...
      onDisableMove(pokemon) {
        if (Array.isArray(this.effectState.move)) {
          for (const moveSlot of pokemon.moveSlots) {
            if (
              moveSlot.id !== this.effectState.move[0] &&
              moveSlot.id !== this.effectState.move[1]
            ) {
              pokemon.disableMove(moveSlot.id);
            }
          }
