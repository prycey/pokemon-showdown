export const Scripts: ModdedBattleScriptsData = {
  ...
  getActionSpeed(action) {
    if (action.choice === 'move') {
      ...
      if (
        ...
        !move.isZ &&
        !move.isMax &&
        (linkIndex = linkedMoves.indexOf(this.toID(action.move))) >= 0
      ) {
        const linkedActions =
          action.linked || linkedMoves.map(moveid => this.dex.getActiveMove(moveid));
        const altMove = linkedActions[1 - linkIndex];
        const thisPriority = this.runEvent(
          'ModifyPriority',
          action.pokemon,
          null,
          linkedActions[linkIndex],
          priority
        );
        const thatPriority = this.runEvent(
          'ModifyPriority',
          action.pokemon,
          null,
          altMove,
      ...
      case 'move':
        ...
        if (action.linked) {
          ...
          for (let i = linkedMoves.length - 1; i >= 0; i--) {
            const validTarget = this.validTargetLoc(
              action.targetLoc,
              action.pokemon,
              linkedMoves[i].target
            );
    ...
    runMove(moveOrMoveName, pokemon, targetLoc, options) {
      ...
      if (!externalMove) {
        ...
        if (!lockedMove) {
          if (!pokemon.deductPP(baseMove, null, target) && move.id !== 'struggle') {
            ...
            this.battle.hint(
              `This is not a bug, this is really how it works on the ${gameConsole}; try it yourself if you don't believe us.`
            );
            this.battle.clearActiveMove(true);
            pokemon.moveThisTurnResult = false;
            return;
      ...
      if (move.flags['dance'] && moveDidSomething && !move.isExternal) {
        ...
        for (const dancer of dancers) {
          ...
          const dancersTarget =
            !target!.isAlly(dancer) && pokemon.isAlly(dancer) ? target! : pokemon;
  ...
  pokemon: {
    ...
    getLinkedMoves(ignoreDisabled) {
      ...
      if (
        this.hasItem('assaultvest') &&
        (this.battle.dex.moves.get(ret[0]).category === 'Status' ||
          this.battle.dex.moves.get(ret[1]).category === 'Status')
      ) {
        return [];
      }
      return ret;
