...
export const Scripts: ModdedBattleScriptsData = {
  ...
  pokemon: {
    ...
    getStat(statName, unboosted, unmodified, fastReturn) {
      ...
      if (
        (['Cubone', 'Marowak'].includes(this.species.name) &&
          this.item === 'thickclub' &&
          statName === 'atk') ||
  ...
  actions: {
    ...
    tryMoveHit(target, pokemon, move) {
      ...
      if (
        move.recoil &&
        move.totalDamage &&
        (pokemon.side.pokemonLeft > 1 || target.side.pokemonLeft > 1 || target.hp)
      ) {
        this.battle.damage(
    ...
    getDamage(source, target, move, suppressMessages) {
      ...
      if (attack >= 256 || defense >= 256) {
        attack = this.battle.clampIntRange(
          Math.floor(this.battle.clampIntRange(attack, 1, 999) / 4),
          1
        );
        defense = this.battle.clampIntRange(
          Math.floor(this.battle.clampIntRange(defense, 1, 999) / 4),
          1
        );
      }
