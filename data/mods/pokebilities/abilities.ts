export const Abilities: import('../../../sim/dex-abilities').ModdedAbilityDataTable = {
  mummy: {
    ...
    onDamagingHit(damage, target, source, move) {
      if (target.ability === 'mummy') {
        ...
        const possibleAbilities = [source.ability, ...(source.m.innates || [])].filter(
          val => !this.dex.abilities.get(val).flags['cantsuppress'] && val !== 'mummy'
        );
          ...
          if (abil === source.ability) {
            ...
            if (abil) {
              this.add(
                ...
                target,
                'ability: Mummy',
                this.dex.abilities.get(abil).name,
                `[of] ${source}`
              );
            }
          }
  ...
  powerofalchemy: {
    ...
    onAllyFaint(ally) {
      ...
      possibleAbilities = possibleAbilities.filter(
        val =>
          !this.dex.abilities.get(val).flags['noreceiver'] &&
          !additionalBannedAbilities.includes(val)
      );
      if (!possibleAbilities.length) return;
      const ability = this.dex.abilities.get(
        possibleAbilities[this.random(possibleAbilities.length)]
      );
  ...
  trace: {
    ...
    onUpdate(pokemon) {
      ...
      while (possibleTargets.length) {
        ...
        possibleAbilities = possibleAbilities.filter(
          val =>
            !this.dex.abilities.get(val).flags['notrace'] &&
            !additionalBannedAbilities.includes(val)
        );
  ...
  wanderingspirit: {
    ...
    onDamagingHit(damage, target, source, move) {
      ...
      if (isAbility) {
        ...
        const possibleAbilities = [source.ability, ...(source.m.innates || [])].filter(
          val => !this.dex.abilities.get(val).flags['failskillswap']
        );
