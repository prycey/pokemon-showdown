...
export function changeSet(
  ...
  pokemon.set.ivs = ivs;
  if (newSet.nature)
    pokemon.set.nature = Array.isArray(newSet.nature)
      ? context.sample(newSet.nature)
      : newSet.nature;
  ...
  pokemon.set.shiny =
    typeof newSet.shiny === 'number' ? context.randomChance(1, newSet.shiny) : !!newSet.shiny;
  ...
  if (!pokemon.terastallized && newSet.teraType) {
    ...
    pokemon.teraType =
      newSet.teraType === 'Any'
        ? context.sample(allTypes)
        : Array.isArray(newSet.teraType)
          ? context.sample(newSet.teraType)
          : newSet.teraType;
  }
  ...
  pokemon.canTerastallize =
    pokemon.canTerastallize === null ? null : context.actions.canTerastallize(pokemon);
...
export const Scripts: ModdedBattleScriptsData = {
      ...
      case 'scapegoat':
        ...
        if (percent > 66) {
          ...
          this.boost(
            ...
            action.pokemon,
            action.pokemon,
            this.dex.moves.get('scapegoat') as any
          );
        } else if (percent > 33) {
        ...
        this.add(
          `c:|${getName((action.pokemon.illusion || action.pokemon).name)}|Don't worry, if this plan fails we can just blame ${action.target.name}`
        );
  ...
  actions: {
    terastallize(pokemon) {
      ...
      if (!pokemon.illusion && pokemon.name === 'Neko') {
        this.battle.add(
          `c:|${getName('Neko')}|Possible thermal failure if operation continues (Meow on fire ?)`
        );
      }
    ...
    modifyDamage(baseDamage, pokemon, target, move, suppressMessages) {
      ...
      if (move.spreadHit) {
        ...
        const bondModifier =
          this.battle.gen > 6 && !pokemon.hasAbility('Almost Frosty') ? 0.25 : 0.5;
      ...
      if (type !== '???') {
        ...
        if (pokemon.terastallized === 'Stellar') {
          if (!pokemon.stellarBoostedTypes.includes(type)) {
            ...
            if (
              !(
                pokemon.species.name === 'Terapagos-Stellar' ||
    ...
    runMegaEvo(pokemon) {
      ...
      if (speciesid === 'Trapinch' && pokemon.name === 'Arya') {
        this.battle.add(
          `c:|${getName('Arya')}|Oh yeaaaaah!!!!! Finally??!! I can finally Mega-Evolve!!! Vamossss`
        );
      }
        ...
        if (move.ohko) {
          ...
          if (accuracy !== true) {
            ...
            if (!move.ignoreAccuracy) {
              const boosts = this.battle.runEvent('ModifyBoost', pokemon, null, null, {
                ...pokemon.boosts,
              });
              boost = this.battle.clampIntRange(boosts['accuracy'], -6, 6);
            }
            if (!move.ignoreEvasion) {
              const boosts = this.battle.runEvent('ModifyBoost', target, null, null, {
                ...target.boosts,
              });
    ...
    runMove(moveOrMoveName, pokemon, targetLoc, options) {
      ...
      if (move.flags['dance'] && moveDidSomething && !move.isExternal) {
        ...
        for (const currentPoke of this.battle.getAllActive()) {
          ...
          if (
            currentPoke.hasAbility(['dancer', 'virtualidol']) &&
        ...
        for (const dancer of dancers) {
          ...
          const dancersTarget =
            !targetOf1stDance.isAlly(dancer) && pokemon.isAlly(dancer) ? targetOf1stDance : pokemon;
          const dancersTargetLoc = dancer.getLocOf(dancersTarget);
          this.runMove(move.id, dancer, dancersTargetLoc, {
            sourceEffect: dancer.getAbility(),
            externalMove: true,
          });
        }
      }
    ...
    useMoveInner(moveOrMoveName, pokemon, options) {
      ...
      if (
        zMove ||
        (move.category !== 'Status' && sourceEffect && (sourceEffect as ActiveMove).isZ)
      ) {
      ...
      if (
        maxMove ||
        (move.category !== 'Status' && sourceEffect && (sourceEffect as ActiveMove).isMax)
      ) {
      ...
      const callerMoveForPressure =
        sourceEffect && (sourceEffect as ActiveMove).pp ? (sourceEffect as ActiveMove) : null;
      ...
      for (const [i, target] of targets.entries()) {
        if (
          ...
          (pokemon.hasAbility('prankster') ||
            pokemon.hasAbility('youkaiofthedusk') ||
            pokemon.volatiles['irpachuza'] ||
            pokemon.hasAbility('neverendingfhunt')) &&
    ...
    spreadMoveHit(targets, pokemon, moveOrMoveName, hitEffect, isSecondary, isSelf) {
      ...
      if (move.target === 'all' && !isSelf) {
        ...
        (move.target === 'foeSide' || move.target === 'allySide' || move.target === 'allyTeam') &&
        !isSelf
      ) {
        hitResult = this.battle.singleEvent(
          'TryHitSide',
    ...
    getMoveTargets(move, target) {
      ...
      switch (move.target) {
        ...
        default:
          ...
          if (
            !target ||
            (target.fainted && !target.isAlly(this) && this.battle.gameType !== 'freeforall')
          ) {
          ...
          if (this.battle.activePerHalf > 1 && !move.tracksTarget) {
            const isCharging =
              move.flags['charge'] &&
              !this.volatiles['twoturnmove'] &&
              !(
                move.id.startsWith('solarb') &&
                ['sunnyday', 'desolateland'].includes(this.effectiveWeather())
              ) &&
              !(
                move.id === 'fruitfullongbow' &&
                ['sunnyday', 'desolateland'].includes(this.effectiveWeather())
              ) &&
              !(move.id === 'praisethemoon' && this.battle.field.getPseudoWeather('gravity')) &&
              !(
                move.id === 'electroshot' &&
                ['stormsurge', 'raindance', 'primordialsea'].includes(this.effectiveWeather())
              ) &&
  ...
  side: {
    getChoice() {
      if (
        this.choice.actions.length > 1 &&
        this.choice.actions.every(action => action.choice === 'team')
      ) {
        return `team ` + this.choice.actions.map(action => action.pokemon!.position + 1).join(', ');
      }
      return this.choice.actions
        .map(action => {
          switch (action.choice) {
            case 'move':
              let details = ``;
    ...
    chooseSwitch(slotText) {
      ...
      if (isNaN(slot) || slot < 0) {
        ...
        for (const [i, mon] of this.pokemon.entries()) {
          if (
            slotText!.toLowerCase() === mon.name.toLowerCase() ||
            toID(slotText) === mon.species.id
          ) {
            slot = i;
            break;
          }
      ...
      if (this.slotConditions[pokemon.position]['revivalblessing']) {
        ...
        this.choice.forcedSwitchesLeft = this.battle.clampIntRange(
          this.choice.forcedSwitchesLeft - 1,
          0
        );
        pokemon.switchFlag = false;
      ...
      if (this.requestState === 'move') {
        if (pokemon.trapped) {
          ...
          const status = this.emitChoiceError(
    ...
    resolveAction(action, midTurn) {
      ...
      if (!midTurn) {
        if (action.choice === 'move') {
          ...
          action.fractionalPriority = this.battle.runEvent(
            'FractionalPriority',
            action.pokemon,
            null,
            action.move,
