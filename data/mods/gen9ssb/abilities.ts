...
export const Abilities: import('../../../sim/dex-abilities').ModdedAbilityDataTable = {
  ...
  saintlybullet: {
    shortDesc:
      'Snipe Shot always has STAB and heals the user by 1/8 (or 1/6 on a crit) of its max HP.',
    name: 'Saintly Bullet',
    onModifyMove(move) {
  ...
  pesteringassault: {
    shortDesc:
      'Uses Knock Off, Taunt, Torment, Soak, and Confuse Ray with 40% accuracy at turn end.',
  ...
  masquerade: {
    ...
    onUpdate(pokemon) {
      if (['mimikyu', 'mimikyutotem'].includes(pokemon.species.id) && this.effectState.busted) {
        const speciesid =
          pokemon.species.id === 'mimikyutotem' ? 'Mimikyu-Busted-Totem' : 'Mimikyu-Busted';
  ...
  stormsurge: {
    shortDesc:
      'On switch-in, summons rain that causes wind moves to have perfect accuracy and 1.2x Base Power.',
  ...
  hydrostaticpositivity: {
    ...
    onAnyRedirectTarget(target, source, source2, move) {
      ...
      if (move.type !== 'Water' || ['firepledge', 'grasspledge', 'waterpledge'].includes(move.id))
        return;
      const redirectTarget = ['randomNormal', 'adjacentFoe'].includes(move.target)
        ? 'normal'
        : move.target;
  ...
  frozenfortuity: {
    shortDesc:
      "On switch-in, changes the Pokemon to Kyurem-Black if the target's Defense is lower, otherwise Kyurem-White.",
    ...
    onSwitchOut(pokemon) {
      if (pokemon.baseSpecies.baseSpecies !== 'Kyurem' || pokemon.transformed || !pokemon.hp)
        return;
  ...
  thegraceofjesuschrist: {
    ...
    onResidual(pokemon) {
      ...
      const item = this.sample(
        plates.filter(plate => this.toID(plate) !== this.toID(pokemon.item))
      );
      pokemon.item = '';
        ...
        if (!source.m.cycledTreasureBag) {
          ...
          switch (currentItem) {
            case 'Blast Seed':
              ...
              this.add(
                '-message',
                `${source.name} dug through its Treasure Bag and found a ${currentItem}!`
              );
              if (foe) {
            ...
            case 'Oran Berry':
              ...
              this.add(
                '-message',
                `${source.name} dug through its Treasure Bag and found an ${currentItem}!`
              );
            ...
            case 'Reviver Seed':
              ...
              this.add(
                '-message',
                `${source.name} dug through its Treasure Bag and found a Reviver Seed!`
              );
  ...
  hydration: {
    ...
    onResidual(pokemon) {
      if (
        pokemon.status &&
        ['raindance', 'primordialsea', 'stormsurge'].includes(pokemon.effectiveWeather())
      ) {
