...
export const Moves: import('../../../sim/dex-moves').ModdedMoveDataTable = {
  ...
  counter: {
    ...
    damageCallback(pokemon, target) {
      ...
      if (
        this.getCategory(lastAttackedBy.move) === 'Physical' &&
        target.lastMove?.id !== 'sleeptalk'
      ) {
  ...
  encore: {
    ...
    condition: {
      ...
      onStart(target) {
        ...
        if (
          moveIndex < 0 ||
          target.lastMoveEncore?.flags['failencore'] ||
  ...
  mirrorcoat: {
    ...
    damageCallback(pokemon, target) {
      ...
      // Hidden Power counts as physical
      if (
        this.getCategory(lastAttackedBy.move) === 'Special' &&
        target.lastMove?.id !== 'sleeptalk'
      ) {
        return 2 * lastAttackedBy.damage;
      }
    ...
    condition: {
      ...
      onBeforeSwitchOut(pokemon) {
        ...
        for (const source of this.effectState.sources) {
          if (
            source.speed < pokemon.speed ||
            (source.speed === pokemon.speed && this.randomChance(1, 2))
          ) {
