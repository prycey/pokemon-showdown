...
export const Scripts: ModdedBattleScriptsData = {
  ...
  pokemon: {
    ...
    getStat(statName, unboosted, unmodified, fastReturn) {
      ...
      if (
        (['Cubone', 'Marowak'].includes(this.baseSpecies.name) &&
          this.item === 'thickclub' &&
          statName === 'atk') ||
        ...
        this.baseSpecies.name === 'Ditto' &&
        this.item === 'metalpowder' &&
        ['def', 'spd'].includes(statName)
      ) {
  ...
  actions: {
    ...
    runMove(moveOrMoveName, pokemon, targetLoc, options) {
      ...
      if (pokemon.moveThisTurn) {
        ...
        this.battle.debug(
          `${pokemon.fullname} INCONSISTENT STATE, ALREADY MOVED: ${pokemon.moveThisTurn}`
        );
    ...
    moveHit(target, pokemon, move, moveData, isSecondary, isSelf) {
      ...
      if (target) {
        ...
        if (moveData.boosts && !target.fainted) {
          if (
            pokemon.volatiles['lockon'] &&
            target === pokemon.volatiles['lockon'].source &&
      ...
      if (
        ...
        this.battle.runEvent('TrySecondaryHit', target, pokemon, moveData)
      ) {
        for (const secondary of moveData.secondaries) {
          ...
          if (
            secondary.status &&
            ['brn', 'frz'].includes(secondary.status) &&
          ...
          if (
            ...
            ['slp', 'frz'].includes(target.status) &&
            !secondary.kingsrock
          ) {
          ...
          if (!move.multihit || move.lastHit) {
            ...
            if (
              ...
              this.battle.randomChance(effectChance, 256)
            ) {
    ...
    getDamage(source, target, move, suppressMessages) {
      ...
      if (
        ...
        ((type === 'Fire' || move.id === 'solarbeam') &&
          this.battle.field.isWeather('raindance')) ||
