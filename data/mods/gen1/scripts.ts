...
export const Scripts: ModdedBattleScriptsData = {
  ...
  pokemon: {
    ...
    modifyStat(statName, modifier) {
      ...
      const modifiedStats = this.battle.clampIntRange(
        Math.floor(this.modifiedStats![statName] * modifier),
        1
      );
  ...
  actions: {
    ...
    runMove(moveOrMoveName, pokemon, targetLoc, options) {
      ...
      const autoHyperBeam =
        move.id === 'recharge' &&
        !pokemon.volatiles['mustrecharge'] &&
        !pokemon.volatiles['partiallytrapped'];
      if (autoHyperBeam) {
        move = this.battle.dex.getActiveMove('hyperbeam');
        this.battle.hint(
          `In Gen 1, If a faster partial trapping move misses against a user of Hyper Beam during a recharge turn, ` +
            `the user of Hyper Beam will automatically use Hyper Beam during that turn.`,
          true
        );
      }
      ...
      if (
        !lockedMove &&
        (!pokemon.volatiles['partialtrappinglock'] ||
      ...
      if (
        (pokemon.volatiles['partialtrappinglock'] &&
          target !== pokemon.volatiles['partialtrappinglock'].locked) ||
        autoHyperBeam
      ) {
        ...
        if (moveSlot && moveSlot.pp < 0) {
          moveSlot.pp = 63;
          this.battle.hint(
            'In Gen 1, if a player is forced to use a move with 0 PP, the move will underflow to have 63 PP.'
          );
        }
      }
    ...
    useMove(moveOrMoveName, pokemon, options) {
      ...
      if (move.id !== 'metronome') {
        if (
          move.id !== 'mirrormove' ||
          !pokemon.side.foe.active[0]?.lastMove ||
          pokemon.side.foe.active[0].lastMove?.id === 'mirrormove'
        ) {
    ...
    useMoveInner(moveOrMoveName, pokemon, options) {
      ...
      if (
        target.boosts.atk < 6 &&
        (move.selfdestruct || move.id === 'disable') &&
        target.volatiles['rage']
      ) {
        this.battle.boost({ atk: 1 }, target, pokemon, this.dex.conditions.get('rage'));
        this.battle.hint(
          `In Gen 1, using ${move.name} causes the target to build Rage, ` +
            `even if it misses or fails`,
          true
        );
      }
    ...
    tryMoveHit(target, pokemon, move) {
      ...
      if (move.selfdestruct) {
        if (!target.subFainted) {
          ...
          this.battle.hint(
            `In Gen 1, the user of ${move.name} will not take damage if it breaks a Substitute.`
          );
        }
      }
    ...
    moveHit(target, pokemon, move, moveData, isSecondary, isSelf) {
      ...
      const targetHadSub =
        targetSub !== null && targetSub !== false && typeof targetSub !== 'undefined';
      ...
      if (target) {
        ...
        if (
          ...
          moveData.volatileStatus === 'partiallytrapped'
        ) {
          target.addVolatile(moveData.volatileStatus, pokemon, move);
          if (
            !pokemon.volatiles['partialtrappinglock'] ||
            pokemon.volatiles['partialtrappinglock'].duration! > 1
          ) {
      ...
      if (
        moveData.self &&
        moveData.self.volatileStatus !== 'lockedmove' &&
        ...
        for (const secondary of moveData.secondaries) {
          ...
          if (!move.multihit || move.lastHit) {
            ...
            if (
              !(
                secondary.status &&
                ['par', 'brn', 'frz'].includes(secondary.status) &&
                target.hasType(move.type)
              )
            ) {
    ...
    getDamage(source, target, move, suppressMessages) {
      ...
      if (
        source.volatiles['partialtrappinglock'] &&
        target === source.volatiles['partialtrappinglock'].locked
      ) {
      ...
      if (
        (defType === 'def' && defender.volatiles['reflect']) ||
        (defType === 'spd' && defender.volatiles['lightscreen'])
      ) {
      ...
      if (attack >= 256 || defense >= 256) {
        if (attack >= 1024 || defense >= 1024) {
          this.battle.hint(
            'In Gen 1, a stat will roll over to a small number if it is larger than 1024.'
          );
        }
        ...
        if (defense === 0) {
          this.battle.hint(
            'Pokemon Showdown avoids division by zero by rounding defense up to 1. ' +
    ...
    for (i in boost) {
      ...
      if (boost[i] !== 0) {
        ...
        if (boostResult === 0) {
          ...
          if (msg === '-boost') {
            this.hint(
              `In Gen 1, boosting a stat at 999 will apply a -1 drop afterwards, and the stat remains at 999.`,
              true
            );
          } else {
            this.hint(
              `In Gen 1, dropping a stat at 1 will apply a +1 boost afterwards, and the stat remains at 1.`,
              true
            );
          }
          this.runEvent('AfterEachBoost', target, source, effect, currentBoost);
