...
export const Moves: import('../../../sim/dex-moves').ModdedMoveDataTable = {
    ...
    condition: {
      ...
      onBeforeMove(pokemon, t, move) {
        ...
        if (!this.effectState.time) {
          ...
          this.actions.moveHit(target, pokemon, currentMove, {
            damage: this.effectState.damage * 2,
          } as ActiveMove);
          pokemon.removeVolatile('bide');
          return false;
        }
  ...
  bind: {
    ...
    onTryMove(source, target) {
      if (target.volatiles['mustrecharge']) {
        ...
        this.hint(
          'In Gen 1, partial trapping moves negate the recharge turn of Hyper Beam, even if they miss.',
          true
        );
      }
    },
    onHit(target, source) {
      ...
      if (target.volatiles['partiallytrapped']) {
        if (
          ...
          source.volatiles['partialtrappinglock'].duration! > 1
        ) {
  ...
  counter: {
    ...
    damageCallback(pokemon, target) {
      ...
      const lastMoveIsCounterable =
        lastMove &&
        lastMove.basePower > 0 &&
        ['Normal', 'Fighting'].includes(lastMove.type) &&
        lastMove.id !== 'counter';

      const lastSelectedMove =
        target.side.lastSelectedMove && this.dex.moves.get(target.side.lastSelectedMove);
      const lastSelectedMoveIsCounterable =
        lastSelectedMove &&
        lastSelectedMove.basePower > 0 &&
        ['Normal', 'Fighting'].includes(lastSelectedMove.type) &&
        lastSelectedMove.id !== 'counter';
  ...
  disable: {
    ...
    onTryHit(target) {
      ...
      return (
        target.moveSlots.some(ms => ms.pp > 0) && !('disable' in target.volatiles) && undefined
      );
    },
    condition: {
      onStart(pokemon) {
    ...
    condition: {
      ...
      onAfterMoveSelf(pokemon) {
        ...
        if (!damage || toLeech > damage) {
          this.hint(
            'In Gen 1, Leech Seed recovery is not limited by the remaining HP of the seeded Pokemon.',
            true
          );
        }
  ...
  recover: {
    ...
    onHit(target) {
      ...
      if (
        ...
        ((target.hp === target.maxhp - 255 || target.hp === target.maxhp - 511) &&
          target.hp % 256 !== 0)
      ) {
        this.hint(
      ...
      onTryHit(target, source, move) {
        if (move.category === 'Status') {
          ...
          if (
            move.status === 'psn' ||
            move.status === 'tox' ||
            (move.boosts && target !== source) ||
            move.volatileStatus === 'confusion' ||
            SubBlocked.includes(move.id)
          ) {
            return false;
          }
          return;
        ...
        let uncappedDamage =
          move.hit > 1 ? this.lastDamage : this.actions.getDamage(source, target, move);
        ...
        target.volatiles['substitute'].hp -=
          uncappedDamage > target.volatiles['substitute'].hp
            ? target.volatiles['substitute'].hp
            : uncappedDamage;
        ...
        if (target.volatiles['substitute']) {
          ...
          if (move.drain) {
            const amount = this.clampIntRange(
              Math.floor((uncappedDamage * move.drain[0]) / move.drain[1]),
              1
            );
          ...
          if (move.secondary?.volatileStatus === 'confusion') {
            ...
            if (
              secondary.chance === undefined ||
              this.randomChance(Math.ceil((secondary.chance * 256) / 100) - 1, 256)
            ) {
        ...
        if (!lastAttackedBy) {
          target.attackedBy.push({
            source,
            move: move.id,
            damage: uncappedDamage,
            slot: source.getSlot(),
            thisTurn: true,
          });
        } else {
