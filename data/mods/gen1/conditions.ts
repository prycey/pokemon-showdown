...
export const Conditions: import('../../../sim/dex-conditions').ModdedConditionDataTable = {
  brn: {
    ...
    onAfterMoveSelf(pokemon) {
      const toxicCounter = pokemon.volatiles['residualdmg']
      ...
      if (this.randomChance(63, 256)) {
        ...
        if (pokemon.removeVolatile('twoturnmove')) {
          if (pokemon.volatiles['invulnerability']) {
            this.hint(
              `In Gen 1, when a Dig/Fly user is fully paralyzed while semi-invulnerable, ` +
                `it will remain semi-invulnerable until it switches out or fully executes Dig/Fly`,
              true
            );
          }
        }
