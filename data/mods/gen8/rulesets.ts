export const Rulesets: import('../../../sim/dex-formats').ModdedFormatDataTable = {
    ...
    onTeamPreview() {
      ...
      for (const pokemon of this.getAllPokemon()) {
        const details = pokemon.details
          .replace(', shiny', '')
          .replace(
            /(Arceus|Greninja|Gourgeist|Pumpkaboo|Xerneas|Silvally|Urshifu|Dudunsparce)(-[a-zA-Z?-]+)?/g,
            '$1-*'
          )
  ...
  bonustypemod: {
    ...
    onBegin() {
      this.add(
        'rule',
        'Bonus Type Mod: Pok\u00e9mon can be nicknamed the name of a type to have that type added onto their current ones.'
      );
    },
    onModifySpecies(species, target, source, effect) {
  ...
  godlygiftmod: {
    ...
    onValidateTeam(team) {
      ...
      for (const set of team) {
        ...
        if (
          ['Zacian', 'Zamazenta'].includes(species.baseSpecies) &&
          this.toID(set.item).startsWith('rusted')
        ) {
        ...
        if (
          ['ag', 'uber'].includes(
            this.toID(this.ruleTable.has('standardnatdex') ? species.natDexTier : species.tier)
          ) ||
      ...
      const god =
        target.side.team.find(set => {
          ...
          const validator = this.dex.formats.getRuleTable(
            this.dex.formats.get(
              `gen${this.gen}${isNatDex && this.gen >= 8 ? 'nationaldex' : 'ou'}`
            )
          );
