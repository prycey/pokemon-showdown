function checkMegaForme(species: Species, forme: string, battle: Battle) {
  ...
  if (
    altForme.exists &&
    !battle.ruleTable.isBannedSpecies(altForme) &&
...
export const Scripts: ModdedBattleScriptsData = {
  ...
  spreadModify(baseStats, set) {
    ...
    for (statName in modStats) {
      ...
      modStats[statName] = Math.floor(
        (Math.floor(2 * stat + set.ivs[statName]) * set.level) / 100 + 5
      );
    }
    if ('hp' in baseStats) {
      const stat = baseStats['hp'];
      modStats['hp'] = Math.floor(
        (Math.floor(2 * stat + set.ivs['hp'] + 100) * set.level) / 100 + 10
      );
    }
