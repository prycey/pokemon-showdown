export const Scripts: ModdedBattleScriptsData = {
        ...
        if (
          ...
          (pokemon.getAbility().suppressWeather ||
            pokemon.m.innates?.some(
              (k: string) => this.battle.dex.abilities.get(k).suppressWeather
            ))
        ) {
          return true;
        }
  ...
  pokemon: {
      ...
      for (const pokemon of this.battle.getAllActive()) {
        ...
        if (
          (pokemon.ability === ('neutralizinggas' as ID) ||
            pokemon.m.innates?.some((k: string) => k === 'neutralizinggas')) &&
    ...
    formeChange(speciesId, source, isPermanent, message) {
      ...
      if (
        isPermanent &&
        !['disguise', 'iceface', 'ability:disguise', 'ability:iceface'].includes(source.id)
      ) {
        if (this.illusion) {
