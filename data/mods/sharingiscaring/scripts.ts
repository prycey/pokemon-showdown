  ...
  pokemon: {
    ...
    hasItem(item) {
      if (Array.isArray(item)) {
        ...
        if (
          this.battle.toID(item) !== this.item &&
          !this.volatiles['item:' + this.battle.toID(item)]
        )
          return false;
      }
    ...
    useItem(source, sourceEffect) {
      const hasAnyItem =
        !!this.item || Object.keys(this.volatiles).some(v => v.startsWith('item:'));
    ...
    eatItem(force, source, sourceEffect) {
      ...
      if (
        (!this.hp &&
          this.battle.toID(item.name) !== 'jabocaberry' &&
          this.battle.toID(item.name) !== 'rowapberry') ||
        !this.isActive
      )
        return false;
      ...
      if (
        ...
        if (
          RESTORATIVE_BERRIES.has(item.id.startsWith('item:') ? (item.id.slice(5) as ID) : item.id)
        ) {
