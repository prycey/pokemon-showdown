  ...
  galewings: {
    onModifyPriority(priority, pokemon, target, move) {
      for (const poke of this.getAllActive()) {
        if (
          poke.hasAbility('counteract') &&
          poke.side.id !== pokemon.side.id &&
          !poke.abilityState.ending
        ) {
          return;
        }
      }
    ...
    name: 'Gale Wings',
    shortDesc:
      'If this Pokemon has 25% of its max HP or more, its Flying-type moves have +1 priority.',
    rating: 3,
    num: 177,
  },
  magicresistance: {
    ...
    rating: 3.5,
    shortDesc:
      "This Pokemon steals foe's item after hitting them, and takes 50% damage from Fire/Ice.",
  },
  hover: {
    // implemented in moves.ts
    // and also scripts.ts
