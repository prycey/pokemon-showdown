export const Scripts: ModdedBattleScriptsData = {
  ...
  start() {
    ...
    for (const pokemon of this.getAllPokemon()) {
      ...
      if (item.forcedForme && !item.zMove && item.forcedForme !== pokemon.species.name) {
        const rawSpecies = (this.actions as any).getMixedSpecies(
      ...
      case 'start': {
        ...
        for (const pokemon of this.getAllPokemon()) {
          ...
          if (item.id === 'rustedsword') {
            rawSpecies = (this.actions as any).getMixedSpecies(
              pokemon.m.originalSpecies,
              'Zacian-Crowned',
              pokemon
            );
  ...
  actions: {
    ...
    getMixedSpecies(originalForme, formeChange, pokemon) {
      ...
      if (
        originalSpecies.baseSpecies === formeChangeSpecies.baseSpecies &&
        !formeChangeSpecies.isMega &&
        !formeChangeSpecies.isPrimal
      ) {
    ...
    getFormeChangeDeltas(formeChangeSpecies, pokemon) {
      ...
      for (statId in formeChangeSpecies.baseStats) {
        deltas.baseStats[statId] =
          formeChangeSpecies.baseStats[statId] - baseSpecies.baseStats[statId];
      }
      ...
      if (['Arceus', 'Silvally'].includes(baseSpecies.name)) {
        ...
        deltas.type = this.battle.ruleTable.has('mixandmegaoldaggronite')
          ? 'mono'
          : baseSpecies.types[0];
      ...
      if (
        !deltas.formeType &&
        formeChangeSpecies.abilities['H'] &&
        pokemon &&
        pokemon.baseSpecies.abilities['H'] === pokemon.getAbility().name
      ) {
    ...
    mutateOriginalSpecies(speciesOrForme, deltas) {
      ...
      for (const statName in baseStats) {
        baseStats[statName] = this.battle.clampIntRange(
          baseStats[statName] + deltas.baseStats[statName],
          1,
          255
        );
      }
