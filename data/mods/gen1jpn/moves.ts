    ...
    condition: {
      ...
      onTryHit(target, source, move) {
        if (move.drain) {
          ...
          this.hint(
            'In the Japanese versions of Gen 1, draining moves always miss against substitutes.'
          );
          return null;
        }
        ...
        if (target.volatiles['substitute']) {
          if (move.recoil) {
            this.damage(
              Math.round((uncappedDamage * move.recoil[0]) / move.recoil[1]),
              source,
              target,
              'recoil'
            );
          ...
          if (move.drain) {
            this.heal(
              Math.ceil((uncappedDamage * move.drain[0]) / move.drain[1]),
              source,
              target,
              'drain'
            );
