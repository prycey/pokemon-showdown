...
export const Rulesets: import('../sim/dex-formats').FormatDataTable = {
    ...
    onValidateSet(set) {
      ...
      if (requireObtainable) {
        ...
        for (const moveid of set.moves) {
          ...
          if (
            (move.isNonstandard === 'Unobtainable' && move.gen === this.dex.gen) ||
            move.id === 'lightofruin'
          ) {
  ...
  oldunovapokedex: {
    ...
    onValidateSet(set, format) {
      ...
      const isUnova =
        species.num >= 494 &&
        species.num <= 649 &&
        !['Black', 'White', 'Therian', 'Resolute'].includes(species.forme) &&
        species.gen <= 5;
  ...
  blueberrypokedex: {
    ...
    onValidateSet(set, format) {
      ...
      const species = this.dex.species.get(set.species || set.name);
      if (
        !blueberryDex.includes(species.baseSpecies) &&
        !blueberryDex.includes(species.name) &&
        !this.ruleTable.has('+' + species.id)
      ) {
  ...
  evlimits: {
    ...
    onValidateSet(set) {
      ...
      for (const limit of limits.split(' / ')) {
        ...
        if (ev < low || ev > high) {
          problems.push(
            `${set.name || set.species}'s ${this.dex.stats.names[statid]} EV (${ev}) must be ${low}-${high}`
          );
        }
      }
      return problems;
  ...
  teratypepreview: {
    ...
    onValidateRule() {
      if (!this.ruleTable.has('teampreview')) {
        throw new Error(
          `The "Tera Type Preview" rule${this.ruleTable.blame('teratypepreview')} requires Team Preview.`
        );
      }
    },
  ...
  itemclause: {
    ...
    onBegin() {
      this.add(
        'rule',
        `Item Clause: Limit ${this.ruleTable.valueRules.get('itemclause') || 1} of each item`
      );
    },
    onValidateRule(value) {
  ...
  batonpassstatclause: {
    ...
    onValidateTeam(team) {
      ...
      for (const set of team) {
        ...
        const ability = this.toID(set.ability);
        if (
          moves.some(m => boostingEffects.includes(m)) ||
          boostingEffects.includes(item) ||
  ...
  batonpassstattrapclause: {
    ...
    onBegin() {
      this.add(
        'rule',
        'Baton Pass Stat Trap Clause: No Baton Passer may have a way to boost stats or trap Pok\u00e9mon'
      );
    },
    onValidateTeam(team) {
      ...
      if (status.id === 'slp') {
        for (const pokemon of target.side.pokemon) {
          if (pokemon.hp && pokemon.status === 'slp') {
            this.add(
              '-message',
              "Sleep Clause activated. (In official formats, Sleep Clause activates if any of the opponent's Pokemon are asleep, even if self-inflicted from Rest)"
            );
            return false;
          }
        }
  ...
  minsourcegen: {
    ...
    onValidateRule(value) {
      ...
      if (minSourceGen > this.dex.gen) {
        ...
        throw new Error(
          `Invalid generation ${minSourceGen}${this.ruleTable.blame('minsourcegen')} for a Gen ${this.dex.gen} format (${this.format.name})`
        );
      }
    },
  },
  ...
  omunobtainablemoves: {
    ...
    onValidateRule() {
      if (!this.ruleTable.checkCanLearn?.[0]) {
        throw new Error(
          `A format with the "OM Unobtainable Moves"${this.ruleTable.blame('omunobtainablemoves')} rule must also have a special move legality rule.`
        );
      }
    },
  },
  ...
  camomonsmod: {
    ...
    onModifySpecies(species, target, source, effect) {
      ...
      const types = [
        ...new Set(target.baseMoveSlots.slice(0, 2).map(move => this.dex.moves.get(move.id).type)),
      ];
  ...
  openteamsheets: {
    ...
    onValidateRule() {
      if (!(this.ruleTable.has('teampreview') || this.ruleTable.has('teamtypepreview'))) {
        throw new Error(
          `The "Open Team Sheets" rule${this.ruleTable.blame('openteamsheets')} requires Team Preview.`
        );
      }
    },
    onTeamPreview() {
  ...
  forceopenteamsheets: {
    ...
    onValidateRule() {
      if (!(this.ruleTable.has('teampreview') || this.ruleTable.has('teamtypepreview'))) {
        throw new Error(
          `The "Force Open Team Sheets" rule${this.ruleTable.blame('forceopenteamsheets')} requires Team Preview.`
        );
      }
    },
    onTeamPreview() {
  ...
  pickedteamsize: {
    ...
    onValidateRule() {
      if (!(this.ruleTable.has('teampreview') || this.ruleTable.has('teamtypepreview'))) {
        throw new Error(
          `The "Picked Team Size" rule${this.ruleTable.blame('pickedteamsize')} requires Team Preview.`
        );
      }
    },
  },
  ...
  maxtotallevel: {
    ...
    onValidateRule(value) {
      ...
      if (maxTotalLevel >= ruleTable.maxLevel * maxTeamSize) {
        throw new Error(
          `A Max Total Level of ${maxTotalLevel}${ruleTable.blame('maxtotallevel')} is too high (and will have no effect) with ${maxTeamSize}${maxTeamSizeBlame} Pokémon at max level ${ruleTable.maxLevel}${ruleTable.blame('maxlevel')}`
        );
      }
      if (maxTotalLevel <= ruleTable.minLevel * maxTeamSize) {
        throw new Error(
          `A Max Total Level of ${maxTotalLevel}${ruleTable.blame('maxtotallevel')} is too low with ${maxTeamSize}${maxTeamSizeBlame} Pokémon at min level ${ruleTable.minLevel}${ruleTable.blame('minlevel')}`
        );
      }
    },
    // hardcoded in sim/side
  ...
  chimera1v1rule: {
    ...
    onBeforeSwitchIn(pokemon) {
      ...
      pokemon.moveSlots = (pokemon as any).baseMoveSlots = [
        ...
        (move, index, moveSlots) => moveSlots.find(othermove => othermove.id === move.id) === move
      );
  ...
  revelationmonsmod: {
    ...
    onValidateSet(set) {
      ...
      for (const [i, moveid] of set.moves.entries()) {
        ...
        if (i <= slotIndex) {
          problems.push(
            `${move.name} can't be in moveslot ${i + 1} because it's restricted from being in the first ${slotIndex + 1 > 1 ? `${slotIndex + 1} slots` : 'slot'}.`
          );
        }
      }
      return problems;
  ...
  reevolutionmod: {
    ...
    onModifySpecies(species, target) {
      ...
      const baseSpecies = this.dex.species.get(
        (Array.isArray(species.battleOnly) ? species.battleOnly[0] : species.battleOnly) ||
          species.changesFrom ||
          species.name
      );
      if (!newSpecies.prevo) {
      ...
      switch (move.id) {
        ...
        case 'shellsidearm': {
          ...
          const special = Math.floor(
            Math.floor(Math.floor(Math.floor((2 * pokemon.level) / 5 + 2) * 90 * spa) / spd) / 50
          );
  ...
  convergencelegality: {
    ...
    onValidateSet(set, format) {
      ...
      if (!obtainableAbilityPool.has(this.toID(set.ability))) {
        return [
          `${curSpecies.name} doesn't have access to ${this.dex.abilities.get(set.ability).name}.`,
        ];
      }
    },
    checkCanLearn(move, species, setSources, set) {
  ...
  hackmonsformelegality: {
    ...
    onChangeSet(set, format, setHas, teamHas) {
        ...
        .filter(
          s =>
            ...
            !(
              ...
              this.ruleTable.has(`+basepokemon:${this.toID(s.baseSpecies)}`)
            )
        );
      if (problemPokemon.includes(species)) {
        if (species.requiredItem && this.toID(set.item) !== this.toID(species.requiredItem)) {
          return [
            `${set.name ? `${set.name} (${species.name})` : species.name} is required to hold ${species.requiredItem}.`,
          ];
        }
        if (
          species.requiredMove &&
          !set.moves.map(this.toID).includes(this.toID(species.requiredMove))
        ) {
          return [
            `${set.name ? `${set.name} (${species.name})` : species.name} is required to have ${species.requiredMove}.`,
          ];
        }
        set.species = (
          species.id === 'xerneas'
      ...
      if (species.baseSpecies === 'Xerneas' && this.toID(set.ability) !== 'fairyaura') {
        return [
          `${set.name ? `${set.name} (${species.name})` : species.name} is ability-locked into Fairy Aura.`,
        ];
      }
    },
  },
  ...
  franticfusionsmod: {
    ...
    onValidateSet(set) {
      ...
      if (!abilityPool.has(ability.name)) {
        return [
          `${species.name} only has access to the following abilities: ${Array.from(abilityPool).join(', ')}.`,
        ];
      }
    },
    onValidateTeam(team, format) {
  ...
  ferventimpersonationmod: {
    ...
    onValidateTeam(team, format, teamHas) {
      ...
      for (const set of team) {
        ...
        if (
          exhaustedSpecies.has(species.baseSpecies) ||
          (exhaustedSpecies.has(impersonation.baseSpecies) &&
            impersonation.baseSpecies !== species.baseSpecies)
        ) {
          return [
            `You have more than one Pok\u00e9mon nicknamed after ${impersonation.baseSpecies}.`,
          ];
        }
    ...
    onValidateSet(set) {
      ...
      if (
        (this.toID(set.name) !== species.id && this.toID(set.name) !== impersonation.id) ||
        (impersonation.isNonstandard &&
          !(
            rt.has(`+pokemontag:${this.toID(impersonation.isNonstandard)}`) ||
            rt.has(`+pokemon:${impersonation.id}`) ||
            rt.has(`+basepokemon:${this.toID(impersonation.baseSpecies)}`)
          ))
      ) {
        return [
          `All Pok\u00e9mon must either have no nickname or must be nicknamed after a Pok\u00e9mon.`,
        ];
      }
    },
    checkCanLearn(move, species, setSources, set) {
    ...
    onResidual(pokemon) {
      ...
      if (
        pokemon.species.baseSpecies === impersonation.baseSpecies ||
        pokemon.hp > pokemon.maxhp / 2
      )
        return;
      this.add('-activate', pokemon, 'ability: Power Construct');
      const abilitySlot =
        Object.keys(oldPokemon.abilities).find(
          x => (oldPokemon.abilities as any)[x] === pokemon.set.ability
        ) || '0';
