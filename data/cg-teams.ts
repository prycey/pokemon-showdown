...
async function updateLevels(database: SQL.DatabaseManager) {
  ...
  const data: { species_id: ID; wins: number; losses: number; level: number }[] =
    await database.all('SELECT species_id, wins, losses, level FROM gen9computergeneratedteams');
...
export default class TeamGenerator {
  ...
  constructor(format: Format | string, seed: PRNG | PRNGSeed | null) {
    ...
    this.itemPool = this.dex.items
      .all()
      .filter(i => i.exists && i.isNonstandard !== 'Past' && !i.isPokeball);
    this.specialItems = {};
  ...
  protected makeSet(species: Species, teamStats: TeamStats): PokemonSet {
        ...
        if (boosts) {
          for (const stat in boosts) {
            const chance = Math.min(
              100,
              move.secondary?.chance || 100 * (ability === 'Serene Grace' ? 2 : 1)
            );
    ...
    if (species.requiredItem) {
      ...
      item = this.prng.sample(
        species.requiredItems.filter(i => !this.dex.items.get(i).isNonstandard)
      );
    ...
    let attackingTypes = nonStatusMoves.map(m =>
      TeamGenerator.moveType(this.dex.moves.get(m), species)
    );
    let teraType;
    if (species.forceTeraType) {
      ...
      if (hasTeraBlast || hasRevelationDance || noAttacks) {
        attackingTypes = [
          ...this.dex.types
            .names()
            .filter(t => !(noAttacks ? species.types : attackingTypes).includes(t)),
        ];
  ...
  protected static moveIsHazard(move: Move): boolean {
    return (
      !!(move.sideCondition && move.target === 'foeSide') ||
      ['stoneaxe', 'ceaselessedge'].includes(move.id)
    );
  }
  ...
  protected getMoveWeight(
    ...
    if (move.category === 'Status') {
      ...
      if (['defog', 'courtchange', 'tidyup'].includes(move.id) && !teamStats.hazardRemovers) {
        ...
        weight *=
          0.8 ** Object.values(teamStats.hazardSetters).reduce((total, num) => total + num, 0);
      }
    ...
    if (WEIGHT_BASED_MOVES.includes(move.id) || TARGET_HP_BASED_MOVES.includes(move.id))
      basePower = 60;
    ...
    if (!this.specialItems[species.name] && !species.requiredItem) {
      ...
      if (move.id === 'facade') {
        if (
          !['Comatose', 'Purifying Salt', 'Shields Down', 'Natural Cure', 'Misty Surge'].includes(
            ability
          )
        )
          weight *= 1.5;
    ...
    weight *= this.boostWeight(move, movesSoFar, species, ability, level);
    const secondaryChance = Math.min(
      ((move.secondary?.chance || 100) * (ability === 'Serene Grace' ? 2 : 1)) / 100,
      100
    );
  ...
  protected boostWeight(
    ...
    const physicalIsRelevant =
      ...
      movesSoFar.some(
        m =>
          TeamGenerator.moveIsPhysical(m, species) &&
          !m.overrideOffensiveStat &&
          !m.overrideOffensivePokemon
      );
    ...
    const secondaryChance =
      move.secondary && ability !== 'Sheer Force'
        ? Math.min(
            ((move.secondary.chance || 100) * (ability === 'Serene Grace' ? 2 : 1)) / 100,
            100
          ) * accuracy
  ...
  protected getItemWeight(
    ...
    switch (item.id) {
      // Choice Items
      case 'choiceband':
        return moves.every(x => TeamGenerator.moveIsPhysical(x, species) && x.priority < 3)
          ? 50
          : 0;
      case 'choicespecs':
      ...
      case 'choicescarf':
        if (
          moves.some(
            x => x.category === 'Status' || x.secondary?.self?.boosts?.spe || x.priority > 1
          )
        )
          return 0;
      ...
      case 'lifeorb':
        return (
          moves.filter(x => x.category !== 'Status' && !x.damage && !x.damageCallback).length * 8
        );
      case 'focussash':
      ...
      case 'heavydutyboots':
        switch (this.dex.getEffectiveness('Rock', species)) {
          case 2:
            return 50; // double super effective
          case 1:
      ...
      case 'scopelens':
        const attacks = moves.filter(
          x => x.category !== 'Status' && !x.damage && !x.damageCallback && !x.willCrit
        );
      ...
      case 'flameorb':
        ...
        if (moves.some(m => m.id === 'facade')) {
          if (
            !weight &&
            !moves.some(m => TeamGenerator.moveIsPhysical(m, species) && m.id !== 'facade')
          ) {
            weight = 30;
          } else {
            weight *= 2;
      ...
      case 'toxicorb':
        ...
        if (
          ...
          ['Thermal Exchange', 'Water Bubble', 'Water Veil'].includes(ability)
        )
          return 0;
   ...
   */
  weightedRandomPick<T>(choices: T[], weights: number[], options?: { remove?: boolean }) {
