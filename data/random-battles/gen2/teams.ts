...
const PHYSICAL_SETUP = ['bellydrum', 'curse', 'meditate', 'swordsdance'];
  ...
  constructor(format: string | Format, prng: PRNG | PRNGSeed | null) {
    ...
    this.moveEnforcementCheckers = {
      ...
      Psychic: (movePool, moves, abilities, types, counter, species) =>
        !counter.get('Psychic') && species.id !== 'starmie',
      Rock: (movePool, moves, abilities, types, counter, species) =>
        !counter.get('Rock') && species.id !== 'magcargo',
  ...
  override randomMoveset(
    ...
    if (role.includes('Setup')) {
      ...
      const nonSpeedSetupMoves = movePool.filter(
        moveid => SETUP.includes(moveid) && moveid !== 'agility'
      );
