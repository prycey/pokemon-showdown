...
export class RandomCAPTeams extends RandomTeams {
  ...
  override getLevel(species: Species, isDoubles: boolean): number {
    if (this.adjustLevel) return this.adjustLevel;
    return (
      (species.num > 0
        ? this.randomSets[species.id]['level']
        : this.randomCAPSets[species.id]['level']) || 80
    );
  }
  ...
  randomCAPSet(
    ...
    for (const set of sets) {
      ...
      if (
        isLead &&
        (abilities.has('Protosynthesis') || abilities.has('Quark Drive')) &&
  ...
  override randomTeam() {
    ...
    const [capPokemonPool, capBaseSpeciesPool] = this.getPokemonPool(
      type,
      pokemon,
      isMonotype,
      capPokemonList
    ...
    while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
      ...
      if (
        (species.baseSpecies === 'Ogerpon' || species.baseSpecies === 'Terapagos') &&
