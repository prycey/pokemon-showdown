...
const MIXED_SETUP = ['celebrate', 'growth', 'happyhour', 'holdhands', 'shellsmash', 'workup'];
...
const PROTECT_MOVES = ['banefulbunker', 'kingsshield', 'protect', 'spikyshield'];
// Moves that switch the user out
const PIVOT_MOVES = ['partingshot', 'uturn', 'voltswitch'];
...
export class RandomGen7Teams extends RandomGen8Teams {
  ...
  constructor(format: Format | string, prng: PRNG | PRNGSeed | null) {
    ...
    this.moveEnforcementCheckers = {
      Bug: (movePool, moves, abilities, types, counter) =>
        ...
        (!counter.get('Bug') &&
          (abilities.includes('Tinted Lens') || abilities.includes('Adaptability'))),
      Dark: (movePool, moves, abilities, types, counter) => !counter.get('Dark'),
      Dragon: (movePool, moves, abilities, types, counter) =>
        !counter.get('Dragon') && !abilities.includes('Aerilate'),
      ...
      Flying: (movePool, moves, abilities, types, counter, species) =>
        !counter.get('Flying') &&
        !['aerodactyl', 'aerodactylmega', 'mantine'].includes(species.id) &&
      ...
      Ice: (movePool, moves, abilities, types, counter) =>
        !counter.get('Ice') ||
        (moves.has('icebeam') && movePool.includes('freezedry')) ||
  ...
  newQueryMoves(
    ...
    for (const moveid of moves) {
      ...
      if (
        moveid === 'lowkick' ||
        (move.basePower && move.basePower <= 60 && !['nuzzle', 'rapidspin'].includes(moveid))
      ) {
  ...
  cullMovePool(
    ...
    const incompatiblePairs = [
      ...
      [
        ['scald', 'surf'],
        ['hydropump', 'originpulse', 'waterpulse'],
      ],
      ['return', ['bodyslam', 'doubleedge', 'headbutt']],
      [
        ['fierydance', 'firelash', 'lavaplume'],
      ...
      [
        ['gigadrain', 'leafstorm'],
        ['energyball', 'leafstorm', 'petaldance', 'powerwhip'],
      ],
      ['wildcharge', 'thunderbolt'],
      ['gunkshot', 'poisonjab'],
      [
        ['drainpunch', 'focusblast'],
        ['closecombat', 'highjumpkick', 'superpower'],
      ],
  ...
  randomMoveset(
    ...
    if (
      ['Bulky Attacker', 'Bulky Setup', 'Wallbreaker'].includes(role) ||
    ...
    if (
      ['Fast Attacker', 'Setup Sweeper', 'Bulky Attacker', 'Wallbreaker', 'Z-Move user'].includes(
        role
      )
    ) {
  ...
  getPriorityItem(
    ...
    if (
      ...
      !moves.has('sleeptalk') &&
      species.id !== 'stoutland'
    ) {
      return types.includes('Fire') || ability === 'Quick Feet' || ability === 'Toxic Boost'
        ? 'Toxic Orb'
        : 'Flame Orb';
    }
  ...
  getItem(
    ...
    if (counter.get('Special') === 4) {
      return scarfReqs && species.baseStats.spa >= 90 && this.randomChance(1, 2)
        ? 'Choice Scarf'
        : 'Choice Specs';
    }
    ...
    if (
      ...
      return scarfReqs &&
        (species.baseStats.atk >= 100 || ability === 'Pure Power' || ability === 'Huge Power') &&
  ...
  override randomSet(
    ...
    if (
      (!counter.get('Physical') ||
        (counter.get('Physical') <= 1 && (moves.has('foulplay') || moves.has('rapidspin')))) &&
      !moves.has('copycat') &&
      !moves.has('transform')
    ) {
      evs.atk = 0;
    ...
    if (forme === 'Nihilego') {
      while (evs.spd > 1) {
        const spa = Math.floor(
          (Math.floor(2 * species.baseStats.spa + ivs.spa + Math.floor(evs.spa / 4)) * level) /
            100 +
            5
        );
        const spd = Math.floor(
          (Math.floor(2 * species.baseStats.spd + ivs.spd + Math.floor(evs.spd / 4)) * level) /
            100 +
            5
        );
        if (spa >= spd) break;
  ...
  override randomFactoryTeam(side: PlayerOptions, depth = 0): RandomTeamsTypes.RandomFactorySet[] {
    ...
    const isMonotype =
      !!this.forceMonotype || this.dex.formats.getRuleTable(this.format).has('sametypeclause');
