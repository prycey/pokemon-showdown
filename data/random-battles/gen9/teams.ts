...
export class RandomTeams {
  ...
  constructor(format: Format | string, prng: PRNG | PRNGSeed | null) {
    ...
    this.forceTeraType =
      forceTeraType && this.dex.types.get(forceTeraType).exists
        ? this.dex.types.get(forceTeraType).name
        : undefined;
    ...
    this.cachedStatusMoves = this.dex.moves
      .all()
  ...
  randomMoveset(
    ...
    if (isDoubles) {
      ...
      if (
        ...
        (abilities.includes('Prankster') || abilities.includes('Gale Wings'))
      ) {
        counter = this.addMove(
          'tailwind',
          moves,
    ...
    if (
      ...
      for (const moveid of movePool) {
        ...
        if (
          ...
          (move.priority > 0 || (moveid === 'grassyglide' && abilities.includes('Grassy Surge'))) &&
    ...
    if (
      !['AV Pivot', 'Fast Support', 'Bulky Support', 'Bulky Protect', 'Doubles Support'].includes(
        role
      )
    ) {
  ...
  getPriorityItem(
    ...
    if (
      ...
      (species.id === 'calyrexice' && isDoubles)
    )
    ...
    if (
      ...
      (!isDoubles &&
        (species.id === 'luvdisc' || (species.id === 'terapagos' && !moves.has('rest'))))
    )
  ...
  getDoublesItem(
    ...
    if (
      moves.has('flipturn') &&
      moves.has('protect') &&
      (moves.has('aquajet') || moves.has('jetpunch'))
    )
      return 'Mystic Water';
    ...
    if (role === 'Choice Item user') {
      if (
        scarfReqs ||
        (counter.get('Physical') < 4 && counter.get('Special') < 3 && !moves.has('memento'))
      ) {
        return 'Choice Scarf';
      }
    ...
    if (
      ...
      ['fakeout', 'feint', 'firstimpression', 'rapidspin', 'suckerpunch'].every(
        m => !moves.has(m)
      ) &&
    ...
    if (
      ...
      moves.has('irondefense') ||
      moves.has('coil') ||
      doublesLeftoversHardcodes
    )
      return 'Leftovers';
    ...
    if (
      (offensiveRole ||
        (role === 'Tera Blast user' && (species.baseStats.spe >= 80 || moves.has('trickroom')))) &&
  ...
  getItem(
    ...
    if (
      species.id !== 'jirachi' &&
      counter.get('Physical') >= 4 &&
      ['dragontail', 'fakeout', 'firstimpression', 'flamecharge', 'rapidspin'].every(
        m => !moves.has(m)
      )
    ) {
      const scarfReqs =
    ...
    if (role === 'Fast Support' || role === 'Fast Bulky Setup') {
      return counter.get('Physical') + counter.get('Special') >= 3 && !moves.has('nuzzle')
        ? 'Life Orb'
        : 'Leftovers';
    }
  ...
  getForme(species: Species): string {
    ...
    if (
      ['Dudunsparce', 'Magearna', 'Maushold', 'Polteageist', 'Sinistcha', 'Zarude'].includes(
        species.baseSpecies
      )
    ) {
  ...
  randomSet(
    ...
    for (const set of sets) {
      ...
      if (
        isLead &&
        (abilities.includes('Protosynthesis') || abilities.includes('Quark Drive')) &&
    ...
    while (evs.hp > 1) {
      ...
      if (
        (moves.has('substitute') && ['Sitrus Berry', 'Salac Berry'].includes(item)) ||
        species.id === 'minior'
      ) {
    ...
    for (let forme of randomN) {
      ...
      if (this.gen >= 2) {
        do {
          ...
          isIllegalItem =
            this.dex.items.get(item).gen > this.gen || this.dex.items.get(item).isNonstandard;
      ...
      if (species.baseSpecies === 'Terapagos')
        stats = Dex.species.get('terapagosterastal').baseStats;
  ...
  randomFactoryTeam(side: PlayerOptions, depth = 0): RandomTeamsTypes.RandomFactorySet[] {
    ...
    for (const speciesName of pokemonPool) {
      const sortObject = {
        speciesName,
        score:
          this.prng.random() ** (1 / this.randomFactorySets[this.factoryTier][speciesName].weight),
      };
    ...
    while (shuffledSpecies.length && pokemon.length < this.maxTeamSize) {
      ...
      if (!teamData.forceResult && !this.forceMonotype) {
        ...
        for (const typeName of this.dex.types.names()) {
          // it's weak to the type
          if (
            this.dex.getEffectiveness(typeName, species) > 0 &&
  ...
  randomDraftFactoryTeam(side: PlayerOptions): RandomTeamsTypes.RandomDraftFactorySet[] {
    ...
    return team.map(set => {
      ...
      if (species.battleOnly) {
        if (typeof species.battleOnly !== 'string') {
          throw new Error(
            `Invalid species ${species.name} for draft factory matchup ${this.rdfMatchupIndex} team ${this.rdfMatchupSide}`
          );
        }
        species = this.dex.species.get(species.battleOnly);
      }
