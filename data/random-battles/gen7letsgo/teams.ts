...
export class RandomLetsGoTeams extends RandomGen8Teams {
  ...
  constructor(format: Format | string, prng: PRNG | PRNGSeed | null) {
    ...
    this.moveEnforcementCheckers = {
      ...
      Ghost: (movePool, moves, abilities, types, counter) =>
        !types.has('Dark') && !counter.get('Ghost'),
  ...
  override shouldCullMove(
    ...
    switch (move.id) {
      ...
      case 'dragontail':
        return {
          cull:
            ...
            ['encore', 'roar', 'whirlwind'].some(m => moves.has(m)),
        };
      case 'fakeout':
      case 'uturn':
      case 'teleport':
        return {
          cull: !!counter.setupType || !!counter.get('speedsetup') || moves.has('substitute'),
        };
      case 'haze':
      case 'leechseed':
      case 'roar':
      case 'whirlwind':
        return {
          cull: !!counter.setupType || !!counter.get('speedsetup') || moves.has('dragontail'),
        };
      case 'protect':
        return {
          cull: !!counter.setupType || ['rest', 'lightscreen', 'reflect'].some(m => moves.has(m)),
        };
      case 'seismictoss':
        return { cull: counter.damagingMoves.size > 1 || !!counter.setupType };
      case 'stealthrock':
        return {
          cull: !!counter.setupType || !!counter.get('speedsetup') || !!teamDetails.stealthRock,
        };
  ...
  override randomSet(
        ...
        if (
          ...
          if (
            ...
            for (const type of types) {
              if (
                this.moveEnforcementCheckers[type]?.(
                  movePool,
                  moves,
                  [],
                  types,
    ...
    const requiredItem =
      species.requiredItem || (species.requiredItems ? this.sample(species.requiredItems) : null);
    return {
