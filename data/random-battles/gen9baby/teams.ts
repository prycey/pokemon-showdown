...
const SPEED_SETUP = ['agility', 'autotomize', 'flamecharge', 'rockpolish', 'trailblaze'];
...
export class RandomBabyTeams extends RandomTeams {
  ...
  override cullMovePool(
    ...
    const incompatiblePairs = [
      ...
      [
        ['hypnosis', 'thunderwave', 'toxic', 'willowisp', 'yawn'],
        ['hypnosis', 'thunderwave', 'toxic', 'willowisp', 'yawn'],
      ],
      ['roar', 'yawn'],
      ['dragonclaw', 'outrage'],
  ...
  override randomSet(
    ...
    if (hp > targetHP && hp - targetHP <= 3 && targetHP >= minimumHP) {
      ...
      if (
        Math.floor((Math.floor(2 * species.baseStats.hp + ivs.hp + 100) * level) / 100 + 10) >=
        targetHP
      ) {
        evs.hp = 0;
        hp = Math.floor(
