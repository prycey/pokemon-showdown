...
export class RandomGen1Teams extends RandomGen2Teams {
    ...
    for (const pokemon of randomN) {
      ...
      if (this.adjustLevel) {
        ...
        while (level < 100) {
          ...
          mbst += Math.floor(
            ((((stats['atk'] * 2 + 30 + 63 + 100) * level) / 100 + 5) * level) / 100
          );
          mbst += Math.floor(((stats['def'] * 2 + 30 + 63 + 100) * level) / 100 + 5);
          mbst += Math.floor(
            ((((stats['spa'] * 2 + 30 + 63 + 100) * level) / 100 + 5) * level) / 100
          );
      ...
      ivs['hp'] =
        (ivs['atk'] % 2) * 16 + (ivs['def'] % 2) * 8 + (ivs['spe'] % 2) * 4 + (ivs['spa'] % 2) * 2;
      ivs['atk'] *= 2;
      ivs['def'] *= 2;
      ivs['spa'] *= 2;
      ivs['spd'] = ivs['spa'];
  ...
  override randomTeam() {
    ...
    const pokemonPool = Object.keys(
      this.getPokemonPool(type, pokemon, isMonotype, Object.keys(this.randomData))[0]
    );
    while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
      ...
      for (const typeName in weaknessCount) {
        const increaseCount =
          this.dex.getImmunity(typeName, species) &&
  ...
  override randomHCTeam(): PokemonSet[] {
    ...
    for (const forme of randomN) {
      ...
      const calcStat = (statName: StatID, lvl?: number) => {
        if (lvl) {
          return Math.floor(
            (Math.floor(2 * baseStats[statName] + ivs[statName] + Math.floor(evs[statName] / 4)) *
              lvl) /
              100 +
              5
          );
        ...
        return (
          Math.floor(2 * baseStats[statName] + ivs[statName] + Math.floor(evs[statName] / 4)) + 5
        );
      };
      let mbst = 0;
