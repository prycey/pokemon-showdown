...
const PIVOT_MOVES = ['uturn', 'voltswitch'];
...
export class RandomGen5Teams extends RandomGen6Teams {
  ...
  constructor(format: string | Format, prng: PRNG | PRNGSeed | null) {
    ...
    this.moveEnforcementCheckers = {
      ...
      Flying: (movePool, moves, abilities, types, counter, species) =>
        !counter.get('Flying') &&
        !['aerodactyl', 'mantine', 'murkrow'].includes(species.id) &&
  ...
  override getPriorityItem(
    ...
    if (
      moves.has('rest') &&
      !moves.has('sleeptalk') &&
      !['Natural Cure', 'Shed Skin'].includes(ability)
    ) {
  ...
  override getItem(
    ...
    const noExpertBeltMoves = this.noStab.filter(moveid =>
      ['Dragon', 'Normal', 'Poison'].includes(this.dex.moves.get(moveid).type)
    );
    ...
    while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
      ...
      if (!isMonotype && !this.forceMonotype) {
        ...
        if (
          ...
          Object.values(species.abilities).includes('Dry Skin')
        ) {
