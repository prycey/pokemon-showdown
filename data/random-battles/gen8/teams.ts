...
const CONTRARY_MOVES = ['closecombat', 'leafstorm', 'overheat', 'superpower', 'vcreate'];
...
const SPEED_SETUP = ['agility', 'autotomize', 'flamecharge', 'rockpolish'];
...
export class RandomGen8Teams {
  ...
  private poolsCacheKey:
    | [string | undefined, number | undefined, RuleTable | undefined, boolean]
    | undefined;
  ...
  constructor(format: Format | string, prng: PRNG | PRNGSeed | null) {
    ...
    this.moveEnforcementCheckers = {
      ...
      recovery: (movePool, moves, abilities, types, counter, species, teamDetails) =>
        !!counter.get('Status') &&
        !counter.setupType &&
        ['morningsun', 'recover', 'roost', 'slackoff', 'softboiled'].some(moveid =>
      ...
      Dark: (movePool, moves, abilities, types, counter) => {
        ...
        return (
          ...
          (movePool.includes('knockoff') || movePool.includes('wickedblow'))
        );
      },
      ...
      Fairy: (movePool, moves, abilities, types, counter) =>
        !counter.get('Fairy') &&
        ['dazzlinggleam', 'moonblast', 'fleurcannon', 'playrough', 'strangesteam'].some(moveid =>
          movePool.includes(moveid)
        ),
      Fighting: (movePool, moves, abilities, types, counter) =>
        !counter.get('Fighting') || !counter.get('stab'),
      Fire: (movePool, moves, abilities, types, counter, species) => {
        ...
        return (
          !moves.has('bellydrum') &&
          (!counter.get('Fire') || (!enteiException && movePool.includes('flareblitz')))
        );
      },
      Flying: (movePool, moves, abilities, types, counter) =>
        !counter.get('Flying') &&
        !types.has('Dragon') &&
        ['airslash', 'bravebird', 'dualwingbeat', 'oblivionwing'].some(moveid =>
          movePool.includes(moveid)
        ),
      ...
      Psychic: (movePool, moves, abilities, types, counter) => {
        ...
        return (
          abilities.includes('Psychic Surge') ||
          !!counter.setupType ||
    ...
    for (let forme of randomN) {
      ...
      if (this.adjustLevel) {
        ...
        while (level < 100) {
          ...
          mbst += Math.floor(
            ((((stats['atk'] * 2 + 31 + 21 + 100) * level) / 100 + 5) * level) / 100
          );
          mbst += Math.floor(((stats['def'] * 2 + 31 + 21 + 100) * level) / 100 + 5);
          mbst += Math.floor(
            ((((stats['spa'] * 2 + 31 + 21 + 100) * level) / 100 + 5) * level) / 100
          );
    ...
    if (
      ...
      for (const species of this.dex.species.all()) {
        ...
        if (banReason !== '') {
          ...
          if (
            banReason !== '' ||
            this.dex.species.get(species.baseSpecies).isNonstandard !== species.isNonstandard
          ) {
            const nonexistentCheck =
              Tags.nonexistent.genericFilter!(species) && nonexistentBanReason;
  ...
  randomNPokemon(
    ...
    const { pool, speciesPool } = this.getPools(
      requiredType,
      minSourceGen,
      ruleTable,
      requireMoves
    ...
    for (let i = 0; i < n; i++) {
      if (!formes[i].length) {
        throw new Error(
          `Invalid pokemon gen ${this.gen}: ${JSON.stringify(formes)} numbers ${JSON.stringify(hasDexNumber)}`
        );
      }
  ...
  queryMoves(
      ...
      if (
        ...
        if (types.includes(moveType)) {
          ...
          if (
            ...
            (!moveid.startsWith('hiddenpower') || types.length === 1)
          ) {
            counter.add('stab');
          ...
          (moveType === 'Normal' &&
            ['Aerilate', 'Galvanize', 'Pixilate', 'Refrigerate'].some(a =>
              abilities.includes(a)
            )) ||
          (move.priority === 0 &&
            ['Libero', 'Protean'].some(a => abilities.includes(a)) &&
            !this.noStab.includes(moveid)) ||
    ...
    if (counter.get('mixedsetup')) {
      ...
      if (
        categories['Physical'] < 2 &&
        (!counter.get('stab') || !counter.get('physicalpool')) &&
        !(moves.has('rest') && moves.has('sleeptalk')) &&
        !moves.has('batonpass')
      ) {
        counter.setupType = '';
      ...
      if (
        categories['Special'] < 2 &&
        (!counter.get('stab') || !counter.get('specialpool')) &&
  ...
  shouldCullMove(
    ...
    if (
      ...
      (species.id === 'golisopod' &&
        movePool.includes('leechlife') &&
        movePool.includes('firstimpression'))
    ) {
    ...
    switch (move.id) {
      ...
      case 'junglehealing':
        ...
        return {
          cull:
            (species.id.startsWith('rillaboom') && isLead) || (!isDoubles && !counter.setupType),
        };
      case 'dualwingbeat':
      case 'fly':
      ...
      case 'fireblast':
        ...
        return {
          cull:
            ...
            (!moves.has('trick') || counter.get('Status') > 1),
        };
      case 'firepunch':
        ...
        return {
          cull:
            ...
            (moves.has('earthquake') && movePool.includes('substitute')),
        };
      case 'flamecharge':
      ...
      case 'rest':
        const bulkySetup =
          !moves.has('sleeptalk') &&
          ['bulkup', 'calmmind', 'coil', 'curse'].some(m => movePool.includes(m));
      ...
      case 'trick':
        return {
          cull: counter.get('Physical') + counter.get('Special') < 3 || moves.has('rapidspin'),
        };
      case 'trickroom':
        ...
        return {
          cull:
            ...
            movePool.includes('nastyplot'),
        };
      case 'zenheadbutt':
        // Special case for Victini, which should prefer Bolt Strike to Zen Headbutt
        return {
          cull:
            movePool.includes('boltstrike') || (species.id === 'eiscue' && moves.has('substitute')),
        };
      ...
      case 'workup':
        ...
        if (
          counter.damagingMoves.size + counter.get('physicalpool') + counter.get('specialpool') <
          2
        )
          return { cull: true };
        return { cull: false, isSetup: true };
      ...
      case 'fakeout':
        return {
          cull:
            !!counter.setupType ||
            ['protect', 'rapidspin', 'substitute', 'uturn'].some(m => moves.has(m)),
        };
      ...
      case 'rapidspin':
        ...
        return {
          cull:
            ...
            setup ||
            (!!counter.setupType && counter.get('Fighting') >= 2),
        };
      case 'shadowsneak':
        const sneakIncompatible = ['substitute', 'trickroom', 'dualwingbeat', 'toxic'].some(m =>
          moves.has(m)
        );
      ...
      case 'toxic':
        return {
          cull:
            !!counter.setupType ||
            ['sludgewave', 'thunderwave', 'willowisp'].some(m => moves.has(m)),
        };
      case 'toxicspikes':
        return { cull: !!counter.setupType || !!teamDetails.toxicSpikes };
      case 'uturn':
        const bugSwordsDanceCase =
          types.has('Bug') && counter.get('recovery') && moves.has('swordsdance');
        return {
          cull:
            !!counter.get('speedsetup') ||
      ...
      case 'explosion':
        ...
        const otherMoves = ['curse', 'stompingtantrum', 'rockblast', 'painsplit', 'wish'].some(m =>
          moves.has(m)
        );
      ...
      case 'flamethrower':
        ...
        return {
          cull: (moves.has('fireblast') && counter.setupType !== 'Physical') || otherFireMoves,
        };
      case 'flareblitz':
      ...
      case 'gigadrain':
        ...
        const celebiPreferLeafStorm =
          species.id === 'celebi' && !counter.setupType && moves.has('uturn');
      ...
      case 'leafblade':
        ...
        return {
          cull:
            (moves.has('leafstorm') || movePool.includes('leafstorm')) &&
            counter.setupType !== 'Physical',
        };
      case 'leafstorm':
        ...
        return {
          cull:
            ...
            (counter.setupType === 'Physical' &&
              (species.id === 'virizion' || leafBladePossible)) ||
      ...
      case 'bodypress':
        ...
        const turtonatorPressCull =
          species.id === 'turtonator' && moves.has('earthquake') && movePool.includes('shellsmash');
        const pressIncompatible = ['shellsmash', 'mirrorcoat', 'whirlwind'].some(m => moves.has(m));
        return {
          cull: turtonatorPressCull || pressIncompatible || counter.setupType === 'Special',
        };
      case 'circlethrow':
      ...
      case 'drainpunch':
        return {
          cull:
            moves.has('closecombat') ||
            (!types.has('Fighting') && movePool.includes('swordsdance')),
        };
      case 'dynamicpunch':
      case 'thunderouskick':
      ...
      case 'earthquake':
        ...
        const turtQuakeCull =
          species.id === 'turtonator' &&
          movePool.includes('bodypress') &&
      ...
      case 'futuresight':
        return {
          cull: moves.has('psyshock') || moves.has('trick') || movePool.includes('teleport'),
        };
      case 'photongeyser':
      ...
      case 'psychic':
        ...
        return {
          cull: alcremieCase || (moves.has('psyshock') && (!!counter.setupType || isDoubles)),
        };
      case 'psychicfangs':
      ...
      case 'psyshock':
        ...
        return {
          cull:
            moves.has('psychic') ||
            (!counter.setupType && sylveonCase) ||
            (isDoubles && moves.has('psychic')),
        };
      case 'bugbuzz':
      ...
      case 'stoneedge':
        const gutsCullCondition =
          abilities.includes('Guts') && (!moves.has('dynamicpunch') || moves.has('spikes'));
        ...
        return {
          cull:
            gutsCullCondition ||
            (!isDoubles && rockSlidePlusStatusPossible) ||
            otherRockMove ||
            lucarioCull,
        };
      case 'poltergeist':
        ...
        return {
          cull:
            ...
            (abilities.includes('Pixilate') &&
              (!!counter.setupType || counter.get('Status') > 1)) ||
            (!types.has('Ghost') && movePool.includes('focusblast')),
        };
      case 'shadowclaw':
        return {
          cull: types.has('Steel') && moves.has('shadowsneak') && counter.get('Physical') < 4,
        };
      case 'dragonpulse':
      case 'spacialrend':
        return { cull: moves.has('dracometeor') && counter.get('Special') < 4 };
      case 'darkpulse':
        const pulseIncompatible =
          ['foulplay', 'knockoff'].some(m => moves.has(m)) ||
      ...
      case 'clearsmog':
        ...
        return {
          cull:
            moves.has('sludgebomb') ||
            moves.has('trick') ||
            movePool.includes('recover') ||
            toxicCullCondition,
        };
      case 'haze':
        ...
        return {
          cull:
            !teamDetails.stealthRock &&
            (moves.has('stealthrock') || movePool.includes('stealthrock')),
        };
      case 'hypnosis':
      ...
      case 'synthesis':
        return {
          cull:
            moves.has('rest') ||
            moves.has('wish') ||
            (move.id === 'synthesis' && moves.has('gigadrain')),
        };
      case 'roost':
        return {
          cull:
      ...
      case 'substitute':
        const moveBasedCull = ['bulkup', 'nastyplot', 'painsplit', 'roost', 'swordsdance'].some(m =>
          movePool.includes(m)
        );
        ...
        const calmMindCullCondition =
          !counter.get('recovery') && movePool.includes('calmmind') && species.id !== 'calyrex';
  ...
  shouldCullAbility(
    ...
    switch (ability) {
      ...
      case 'Blaze':
        return (
          (isDoubles && abilities.includes('Solar Power')) ||
          (!isDoubles && !isNoDynamax && species.id === 'charizard')
        );
      ...
      case 'Chlorophyll':
        return (
          ...
          (!counter.get('Fire') && !moves.has('sunnyday') && !teamDetails.sun)
        );
      case 'Cloud Nine':
      ...
      case 'Inner Focus':
        return (
          (species.id !== 'glalie' && counter.get('Physical') < 2) ||
          abilities.includes('Iron Fist')
        );
      case 'Infiltrator':
        return (
          (moves.has('rest') && moves.has('sleeptalk')) ||
          (isDoubles && abilities.includes('Clear Body'))
        );
      case 'Intimidate':
      ...
      case 'Lightning Rod':
        return (
          species.types.includes('Ground') || (!isNoDynamax && counter.setupType === 'Physical')
        );
      case 'Limber':
      ...
      case 'Mold Breaker':
        return (
          ...
          abilities.includes('Scrappy') ||
          (abilities.includes('Unburden') && !!counter.setupType) ||
      ...
      case 'Sand Rush':
        return (
          !teamDetails.sand &&
          (isNoDynamax || !counter.setupType || !counter.get('Rock') || moves.has('rapidspin'))
        );
      case 'Sap Sipper':
      ...
      case 'Sheer Force':
        return (
          ...
          (species.id === 'druddigon' && !isDoubles)
        );
      case 'Shell Armor':
      ...
      case 'Sniper':
        ...
        return (
          species.name === 'Inteleon' || (counter.get('Water') > 1 && !moves.has('focusenergy'))
        );
      case 'Solar Power':
      ...
      case 'Sturdy':
        return (
          moves.has('bulkup') ||
          !!counter.get('recoil') ||
          (!isNoDynamax && abilities.includes('Solid Rock'))
        );
      case 'Swarm':
      ...
      case 'Swift Swim':
        if (isNoDynamax) {
          const neverWantsSwim =
            !moves.has('raindance') &&
            ['Intimidate', 'Rock Head', 'Water Absorb'].some(m => abilities.includes(m));
          const noSwimIfNoRain =
            !moves.has('raindance') &&
            [
              'Cloud Nine',
              'Lightning Rod',
        ...
        return (
          !moves.has('raindance') &&
          (['Intimidate', 'Rock Head', 'Slush Rush', 'Water Absorb'].some(abil =>
            abilities.includes(abil)
          ) ||
  ...
  getAbility(
    ...
    if (isDoubles) {
      if (
        abilities.includes('Competitive') &&
        species.id !== 'boltund' &&
        species.id !== 'gothitelle'
      )
        return 'Competitive';
  ...
  getHighPriorityItem(
    ...
    if (
      moves.has('trick') ||
      (moves.has('switcheroo') && !isDoubles) ||
  ...
  getDoublesItem(
    ...
    if (
      ...
      ['fakeout', 'feint', 'rapidspin', 'suckerpunch'].every(m => !moves.has(m)) &&
      (types.has('Dragon') ||
        types.has('Fighting') ||
        types.has('Rock') ||
        moves.has('flipturn') ||
      ...
      return !counter.get('priority') &&
        !abilities.includes('Speed Boost') &&
        species.baseStats.spe >= 60 &&
        species.baseStats.spe <= 100 &&
  ...
  getMediumPriorityItem(
    ...
    if (
      !isDoubles &&
      ((counter.get('Physical') >= 3 && moves.has('defog')) ||
        (counter.get('Special') >= 3 && moves.has('healingwish'))) &&
      !counter.get('priority') &&
      !moves.has('uturn')
    )
      return 'Choice Scarf';
    ...
    if (
      ['clearsmog', 'curse', 'haze', 'healbell', 'protect', 'sleeptalk', 'strangesteam'].some(m =>
        moves.has(m)
      ) &&
      !isDoubles
    )
  ...
  getLowPriorityItem(
    ...
    if (
      isLead &&
      !isDoubles &&
      !['Disguise', 'Sturdy'].includes(ability) &&
      !moves.has('substitute') &&
      !counter.get('drain') &&
      !counter.get('recoil') &&
      !counter.get('recovery') &&
    ...
    const rockWeaknessCase =
      ...
      (!teamDetails.defog ||
        ...
        moves.has('voltswitch'));
    const spinnerCase =
      moves.has('rapidspin') && (ability === 'Regenerator' || !!counter.get('recovery'));
    ...
    if (
      ...
      (counter.get('speedsetup') ||
        ...
        (counter.get('drain') &&
          (!isNoDynamax || species.id !== 'buzzwole' || moves.has('roost'))) ||
        moves.has('trickroom') ||
        moves.has('psystrike') ||
  ...
  getLevel(species: Species, isDoubles: boolean, isNoDynamax: boolean): number {
    ...
    // No Dmax levelling
    if (isNoDynamax) {
      const tier = species.name.endsWith('-Gmax')
        ? this.dex.species.get(species.changesFrom).tier
        : species.tier;
  ...
  randomSet(
    ...
    const randMoves =
      (isDoubles && data.doublesMoves) || (isNoDynamax && data.noDynamaxMoves) || data.moves;
    ...
    do {
      ...
      for (const moveid of moves) {
        ...
        if (
          ...
          const stabs =
            counter.get(species.types[0]) + (species.types[1] ? counter.get(species.types[1]) : 0);
    ...
    while (evs.hp > 1) {
      const hp = Math.floor(
        (Math.floor(2 * species.baseStats.hp + ivs.hp + Math.floor(evs.hp / 4) + 100) * level) /
          100 +
          10
      );
      const multipleOfFourNecessary =
        ...
        (item === 'Sitrus Berry' || item === 'Salac Berry' || ability === 'Power Construct');
    ...
    for (const baseSpecies of Object.keys(pokemonPool)) {
      ...
      const weight =
        baseSpecies === 'Squawkabilly'
          ? 1
          : Math.min(Math.ceil(pokemonPool[baseSpecies].length / 3), 3);
  ...
  randomTeam() {
    ...
    for (const poke of Object.keys(this.randomData)) {
      if (
        (isDoubles && this.randomData[poke]?.doublesMoves) ||
        (!isDoubles && this.randomData[poke]?.moves)
      ) {
    ...
    while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
      ...
      if (
        ...
        ['Zacian', 'Zacian-Crowned', 'Zamazenta', 'Zamazenta-Crowned', 'Eternatus'].includes(
          species.name
        )
      ) {
        continue;
  ...
  randomFactorySet(
    ...
    return {
      ...
      shiny:
        typeof setData.set.shiny === 'undefined' ? this.randomChance(1, 1024) : setData.set.shiny,
      level,
