...
export class RandomBDSPTeams extends RandomGen8Teams {
  ...
  override getHighPriorityItem(
    ...
    if (
      moves.has('solarbeam') &&
      !(moves.has('sunnyday') || ability === 'Drought' || teamDetails.sun)
    )
      return 'Power Herb';
  ...
  override shouldCullMove(
    ...
    switch (move.id) {
       ...
       */
      case 'explosion':
        const otherMoves = ['curse', 'stompingtantrum', 'painsplit', 'wish'].some(m =>
          moves.has(m)
        );
        return { cull: !!counter.get('speedsetup') || !!counter.get('recovery') || otherMoves };
      case 'quickattack':
        return {
          cull: !!counter.get('speedsetup') || (types.has('Rock') && !!counter.get('Status')),
        };
      case 'flamethrower':
      case 'lavaplume':
      ...
      case 'facade':
        ...
        return {
          cull:
            moves.has('dynamicpunch') &&
            species.types.includes('Fighting') &&
      ...
      case 'bugbuzz':
        return {
          cull: moves.has('uturn') && !counter.setupType && !abilities.includes('Tinted Lens'),
        };
      case 'leechlife':
        return {
          cull:
      ...
      case 'substitute':
        const moveBasedCull =
          ...
          species.id !== 'breloom' &&
          ['bulkup', 'nastyplot', 'painsplit', 'roost', 'swordsdance'].some(m =>
            movePool.includes(m)
          );
        const shayminCase =
          abilities.includes('Serene Grace') &&
          movePool.includes('airslash') &&
          !moves.has('airslash');
    ...
    switch (ability) {
      ...
      case 'Blaze':
        return (
          ...
          (!isDoubles && species.id === 'charizard')
        );
      case 'Chlorophyll':
        return (
      ...
      case 'Guts':
        return (
          (!moves.has('facade') && !moves.has('sleeptalk') && !species.nfe) ||
      ...
      case 'Sand Rush':
        return (
          !teamDetails.sand &&
          (!counter.setupType || !counter.get('Rock') || moves.has('rapidspin'))
        );
      case 'Scrappy':
      ...
      case 'Technician':
        return (
          !counter.get('technician') || moves.has('tailslap') || abilities.includes('Punk Rock')
        );
      case 'Tinted Lens':
        return (
          // For Butterfree
