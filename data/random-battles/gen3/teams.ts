...
export class RandomGen3Teams extends RandomGen4Teams {
  ...
  constructor(format: string | Format, prng: PRNG | PRNGSeed | null) {
    ...
    this.moveEnforcementCheckers = {
      ...
      Steel: (movePool, moves, abilities, types, counter, species) =>
        !counter.get('Steel') && species.id !== 'forretress',
  ...
  override randomMoveset(
    ...
    if (
      ['Fast Attacker', 'Setup Sweeper', 'Bulky Attacker', 'Wallbreaker', 'Berry Sweeper'].includes(
        role
      )
    ) {
  ...
  override getItem(
    ...
    if (
      ...
      (moves.has('batonpass') || (role === 'Wallbreaker' && counter.get('Special')))
    ) {
      return 'Choice Band';
    }
    ...
    const salacReqs =
      species.baseStats.spe >= 60 && species.baseStats.spe <= 100 && !counter.get('priority');
    ...
    if (moves.has('swordsdance') && counter.get('Status') === 1) {
      ...
      if (species.baseStats.spe > 100) {
        return counter.get('Physical') >= 3 && this.randomChance(1, 2)
          ? 'Liechi Berry'
          : 'Lum Berry';
      }
    }
  ...
  override randomSet(
    ...
    while (evs.hp > 1) {
      ...
      if (moves.has('substitute') && ['flail', 'reversal'].some(m => moves.has(m))) {
        ...
        moves.has('substitute') &&
        (item === 'Salac Berry' || item === 'Petaya Berry' || item === 'Liechi Berry')
      ) {
    ...
    // Prepare optimal HP
    let hp = Math.floor(
      (Math.floor(2 * species.baseStats.hp + ivs.hp + Math.floor(evs.hp / 4) + 100) * level) / 100 +
        10
    );
  ...
  override randomTeam() {
    ...
    if (
      pokemon.length < this.maxTeamSize &&
      !isMonotype &&
      !this.forceMonotype &&
      pokemon.length < 12
    ) {
