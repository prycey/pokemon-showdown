...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
  ...
  naturalcure: {
    onCheckShow(pokemon) {
      ...
      if (!cureList.length || !noCureCount) {
        ...
        this.add(
          '-message',
          `(${cureList.length} of ${pokemon.side.name}'s pokemon ${cureList.length === 1 ? 'was' : 'were'} cured by Natural Cure.)`
        );
  ...
  pickup: {
    ...
    onResidual(pokemon) {
      ...
      const pickupTargets = this.getAllActive().filter(
        target => target.lastItem && target.usedItemThisTurn && pokemon.isAdjacent(target)
      );
  ...
  truant: {
    onStart(pokemon) {
      ...
      if (
        pokemon.activeTurns &&
        (pokemon.moveThisTurnResult !== undefined || !this.queue.willMove(pokemon))
      ) {
