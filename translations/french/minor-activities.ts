...
export const translations: Translations = {
  strings: {
    ...
    'You are not allowed to use filtered words in announcements.':
      "Tu n'es pas autorisé à utiliser des mots filtrés dans les annonces.",
    'There is already a poll or announcement in progress in this room.':
      'Il y a déjà un sondage ou une annonce dans cette room.',
    ...
    'There is no announcement running in this room.':
      "Il n'y a actuellement pas d'annonce dans cette room.",
    ...
    'you will not be able to vote after viewing results':
      'Tu ne seras plus capable de voter après avoir vu les résultats',
    ...
    'The poll has ended &ndash; scroll down to see the results':
      'Le sondage est terminé &ndash; descends dans le chat pour voir les résultats.',
    ...
    'Too many options for poll (maximum is 8).':
      "Il y a trop d'options pour un sondage (le maximum est de 8).",
    'There are duplicate options in the poll.':
      'Il y a plusieurs options identiques dans le sondage.',
    ...
    'Can\'t delete poll at slot ${slotString} - "${slotString}" is not a number.':
      'Impossible de supprimer le sondage en position ${slotString} - "${slotString}" n\'est pas un nombre.',
    'There is no poll in queue at slot ${slot}.':
      "Il n'y a pas de sondage dans la file d'attente en position ${slot}.",
    '(${user.name} deleted the queued poll in slot ${slot}.)':
      '${user.name} a supprimé le poll en attente en position ${slot}.',
    'There is no poll running in this room.':
      "Il n'y a pas de sondage actuellement dans cette room.",
    ...
    'The poll timer was turned on: the poll will end in ${timeout} minute(s).':
      'Le minuteur du sondage a été activé : le sondage va se terminer dans ${timeout} minute(s).',
    'The poll timer was set to ${timeout} minute(s) by ${user.name}.':
      'Le minuteur du sondage a été fixé à ${timeout} minute(s) par ${user.name}.',
    'The poll timer is on and will end in ${poll.timeoutMins} minute(s).':
      'Le minuteur du sondage est activé et se terminera dans ${poll.timeoutMins} minute(s).',
