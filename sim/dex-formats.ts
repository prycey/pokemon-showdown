...
export class RuleTable extends Map<string, string> {
  ...
  resolveNumbers(format: Format, dex: ModdedDex) {
    const gameTypeMinTeamSize = ['triples', 'rotation'].includes(format.gameType as 'triples')
      ? 3
    ...
    if (this.valueRules.get('pickedteamsize') === 'Auto') {
      this.pickedTeamSize = ['doubles', 'rotation'].includes(format.gameType)
        ? 4
        : format.gameType === 'triples'
          ? 6
          : 3;
    }
    ...
    if (this.maxTeamSize > 24) {
      throw new Error(
        `Max team size ${this.maxTeamSize}${this.blame('maxteamsize')} is unsupported (we only support up to 24).`
      );
    }
    if (this.maxLevel > 99999) {
      throw new Error(
        `Max level ${this.maxLevel}${this.blame('maxlevel')} is unsupported (we only support up to 99999)`
      );
    }
    if (this.maxMoveCount > 24) {
      ...
      throw new Error(
        `Max move count ${this.maxMoveCount}${this.blame('maxmovecount')} is unsupported (we only support up to 24)`
      );
    }
    ...
    if (!this.defaultLevel) {
      ...
      if (
        this.maxTotalLevel &&
        this.maxLevel > 100 &&
        this.maxLevel * maxTeamSize > this.maxTotalLevel
      ) {
    ...
    if (this.minTeamSize && this.minTeamSize < gameTypeMinTeamSize) {
      throw new Error(
        `Min team size ${this.minTeamSize}${this.blame('minteamsize')} must be at least ${gameTypeMinTeamSize} for a ${format.gameType} game.`
      );
    }
    if (this.pickedTeamSize && this.pickedTeamSize < gameTypeMinTeamSize) {
      throw new Error(
        `Chosen team size ${this.pickedTeamSize}${this.blame('pickedteamsize')} must be at least ${gameTypeMinTeamSize} for a ${format.gameType} game.`
      );
    }
    if (this.minTeamSize && this.pickedTeamSize && this.minTeamSize < this.pickedTeamSize) {
      throw new Error(
        `Min team size ${this.minTeamSize}${this.blame('minteamsize')} is lower than chosen team size ${this.pickedTeamSize}${this.blame('pickedteamsize')}.`
      );
    }
    if (!this.minTeamSize)
    ...
    if (this.maxTeamSize < gameTypeMinTeamSize) {
      throw new Error(
        `Max team size ${this.maxTeamSize}${this.blame('maxteamsize')} must be at least ${gameTypeMinTeamSize} for a ${format.gameType} game.`
      );
    }
    if (this.maxTeamSize < this.minTeamSize) {
      throw new Error(
        `Max team size ${this.maxTeamSize}${this.blame('maxteamsize')} must be at least min team size ${this.minTeamSize}${this.blame('minteamsize')}.`
      );
    }
    if (this.minLevel > this.maxLevel) {
      throw new Error(
        `Min level ${this.minLevel}${this.blame('minlevel')} should not be above max level ${this.maxLevel}${this.blame('maxlevel')}.`
      );
    }
    if (this.defaultLevel > this.maxLevel) {
      throw new Error(
        `Default level ${this.defaultLevel}${this.blame('defaultlevel')} should not be above max level ${this.maxLevel}${this.blame('maxlevel')}.`
      );
    }
    if (this.defaultLevel < this.minLevel) {
      throw new Error(
        `Default level ${this.defaultLevel}${this.blame('defaultlevel')} should not be below min level ${this.minLevel}${this.blame('minlevel')}.`
      );
    }
    if (this.adjustLevelDown && this.adjustLevelDown >= this.maxLevel) {
      throw new Error(
        `Adjust Level Down ${this.adjustLevelDown}${this.blame('adjustleveldown')} will have no effect because it's not below max level ${this.maxLevel}${this.blame('maxlevel')}.`
      );
    }
    if (this.adjustLevel && this.valueRules.has('minlevel')) {
      throw new Error(
        `Min Level ${this.minLevel}${this.blame('minlevel')} will have no effect because you're using Adjust Level ${this.adjustLevel}${this.blame('adjustlevel')}.`
      );
    }
    if (this.evLimit && this.evLimit >= 1512) {
      throw new Error(
        `EV Limit ${this.evLimit}${this.blame('evlimit')} will have no effect because it's not lower than 1512, the maximum possible combination of 252 EVs in every stat (if you currently have an EV limit, use "! EV Limit" to remove the limit).`
      );
    }
    if (this.evLimit && this.evLimit < 0) {
      throw new Error(
        `EV Limit ${this.evLimit}${this.blame('evlimit')} can't be less than 0 (you might have meant: "! EV Limit" to remove the limit, or "EV Limit = 0" to ban EVs).`
      );
    }

    if (timer.starting !== undefined && (timer.starting < 10 || timer.starting > 1200)) {
      throw new Error(
        `Timer starting value ${timer.starting}${this.blame('timerstarting')} must be between 10 and 1200 seconds.`
      );
    }
    if (timer.grace && timer.grace > 300) {
      throw new Error(
        `Timer grace value ${timer.grace}${this.blame('timergrace')} must be at most 300 seconds.`
      );
    }
    if (timer.addPerTurn && timer.addPerTurn > 30) {
      throw new Error(
        `Timer add per turn value ${timer.addPerTurn}${this.blame('timeraddperturn')} must be at most 30 seconds.`
      );
    }
    if (timer.maxPerTurn !== undefined && (timer.maxPerTurn < 10 || timer.maxPerTurn > 1200)) {
      throw new Error(
        `Timer max per turn value ${timer.maxPerTurn}${this.blame('timermaxperturn')} must be between 10 and 1200 seconds.`
      );
    }
    if (
      timer.maxFirstTurn !== undefined &&
      (timer.maxFirstTurn < 10 || timer.maxFirstTurn > 1200)
    ) {
      throw new Error(
        `Timer max first turn value ${timer.maxFirstTurn}${this.blame('timermaxfirstturn')} must be between 10 and 1200 seconds.`
      );
    }

    if ((format as any).cupLevelLimit) {
      throw new Error(
        `cupLevelLimit.range[0], cupLevelLimit.range[1], cupLevelLimit.total are now rules, respectively: "Min Level = NUMBER", "Max Level = NUMBER", and "Max Total Level = NUMBER"`
      );
    }
    if ((format as any).teamLength) {
      throw new Error(
        `teamLength.validate[0], teamLength.validate[1], teamLength.battle are now rules, respectively: "Min Team Size = NUMBER", "Max Team Size = NUMBER", and "Picked Team Size = NUMBER"`
      );
    }
...
export class Format extends BasicEffect implements Readonly<BasicEffect> {
  ...
  declare readonly validateSet?: (
    this: TeamValidator,
    set: PokemonSet,
    teamHas: AnyObject
  ) => string[] | null;
  declare readonly validateTeam?: (
    this: TeamValidator,
    team: PokemonSet[],
    options?: {
  ...
  if (custom !== undefined) {
    for (const element of custom) {
      ...
      if (element.section) {
        ...
        // otherwise, adds the element to its section.
        current.formats.push(element as FormatData);
      }
    }
  }
...
export class DexFormats {
  ...
  load(): this {
    ...
    try {
      ...
      if (!Array.isArray(customFormats)) {
        throw new TypeError(
          `Exported property 'Formats' from "./config/custom-formats.ts" must be an array`
        );
      }
    } catch (e: any) {
    ...
    if (!Array.isArray(Formats)) {
      throw new TypeError(
        `Exported property 'Formats' from "./config/formats.ts" must be an array`
      );
    }
    ...
    for (const [i, format] of Formats.entries()) {
      ...
      if (!id) {
        throw new RangeError(
          `Format #${i + 1} must have a name with alphanumeric characters, not '${format.name}'`
        );
      }
  ...
  get(name?: string | Format, isTrusted = false): Format {
    ...
    if (this.dex.data.Rulesets.hasOwnProperty(id)) {
      effect = new Format({
        name,
        ...(this.dex.data.Rulesets[id] as any),
        ...supplementaryAttributes,
      });
    } else {
  ...
  isPokemonRule(ruleSpec: string) {
    return (
      ruleSpec.slice(1).startsWith('pokemontag:') ||
      ruleSpec.slice(1).startsWith('pokemon:') ||
  ...
  getRuleTable(format: Format, depth = 1, repeals?: Map<string, number>): RuleTable {
    ...
    for (let ruleSpec of ruleSpecs) {
      ...
      if (ruleSpec.startsWith('!') && !ruleSpec.startsWith('!!')) {
        ...
        if (repealDepth === depth && !noWarn) {
          throw new Error(
            `Rule "${ruleSpec}" did nothing because "${ruleSpec.slice(1)}" is not in effect`
          );
        }
      ...
      if ('+*-'.includes(ruleSpec.charAt(0))) {
        if (ruleTable.has(ruleSpec)) {
          throw new Error(
            `Rule "${ruleSpec}" in "${format.name}" already exists in "${ruleTable.get(ruleSpec) || format.name}"`
          );
        }
        if (skipPokemonBans) {
          if (ruleSpec === '-pokemontag:allpokemon' || ruleSpec === '+pokemontag:allpokemon') {
            ...
            if (!format.customRules) {
              throw new Error(
                `Rule "${ruleSpec}" must go after any "All Pokemon" rule in ${format.name} ("+All Pokemon" should go in ruleset, not unbanlist)`
              );
            }
            continue;
          }
      ...
      if (subformat.hasValue) {
        if (value === undefined)
          throw new Error(
            `Rule "${ruleSpec}" should have a value (like "${ruleSpec} = something")`
          );
        ...
        if (subformat.hasValue === 'positive-integer') {
          if (parseInt(value) === 0) {
            throw new Error(
              `In rule "${ruleSpec}", "${value}" must be positive (to remove it, use the rule "! ${subformat.name}").`
            );
          }
          if (parseInt(value) <= 0) {
        ...
        if (oldValue === value) {
          if (!noWarn) {
            throw new Error(
              `Rule "${ruleSpec}" is redundant with existing rule "${subformat.id}=${value}"${ruleTable.blame(subformat.id)}.`
            );
          }
        } else if (repealAndReplace) {
          if (oldValue === undefined) {
            if (
              subformat.mutuallyExclusiveWith &&
              ruleTable.valueRules.has(subformat.mutuallyExclusiveWith)
            ) {
              ...
              throw new Error(
                `Rule "${ruleSpec}" is not replacing anything (it should not have "!!")`
              );
            }
          }
        } else {
          if (oldValue !== undefined) {
            throw new Error(
              `Rule "${ruleSpec}" conflicts with "${subformat.id}=${oldValue}"${ruleTable.blame(subformat.id)} (Use "!! ${ruleSpec}" to override "${subformat.id}=${oldValue}".)`
            );
          }
          if (
            subformat.mutuallyExclusiveWith &&
        ...
        if (ruleTable.has(subformat.id) && !repealAndReplace && !noWarn) {
          throw new Error(
            `Rule "${ruleSpec}" in "${format.name}" already exists in "${ruleTable.get(subformat.id) || format.name}"`
          );
        }
      }
      ...
      if (depth > 16) {
        throw new Error(
          `Excessive ruleTable recursion in ${format.name}: ${ruleSpec} of ${format.ruleset}`
        );
      }
      ...
      for (const [ruleid, sourceFormat] of subRuleTable) {
        ...
        if (newValue !== undefined) {
          ...
          if (
            subSubFormat.mutuallyExclusiveWith &&
            ruleTable.valueRules.has(subSubFormat.mutuallyExclusiveWith)
          ) {
            // mutually exclusive conflict!
            throw new Error(
              `Rule "${ruleid}=${newValue}" from ${subformat.name}${subRuleTable.blame(ruleid)} conflicts with "${subSubFormat.mutuallyExclusiveWith}=${ruleTable.valueRules.get(subSubFormat.mutuallyExclusiveWith)}"${ruleTable.blame(subSubFormat.mutuallyExclusiveWith)} (Repeal one with ! before adding another)`
            );
          }
          if (newValue !== oldValue) {
            if (oldValue !== undefined) {
              // conflict!
              throw new Error(
                `Rule "${ruleid}=${newValue}" from ${subformat.name}${subRuleTable.blame(ruleid)} conflicts with "${ruleid}=${oldValue}"${ruleTable.blame(ruleid)} (Repeal one with ! before adding another)`
              );
            }
    ...
    if (!hasPokemonBans && warnForNoPokemonBans) {
      throw new Error(
        `"+All Pokemon" rule has no effect (no species are banned by default, and it does not override obtainability rules)`
      );
    }
    ...
    if (
      ruleTable.has('obtainableformes') &&
      canMegaEvo &&
    ...
    for (const rule of ruleTable.keys()) {
      ...
      if (subFormat.exists) {
        const value = subFormat.onValidateRule?.call(
          ...
          ruleTable.valueRules.get(rule as ID)!
        );
  ...
  validateBanRule(rule: string) {
    ...
    if (matches.length > 1) {
      throw new Error(
        `More than one thing matches "${rule}"; please specify one of: ` + matches.join(', ')
      );
    }
    if (matches.length < 1) {
