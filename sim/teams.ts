...
export const Teams = new (class Teams {
  pack(team: PokemonSet[] | null): string {
    ...
    for (const set of team) {
      ...
      if (set.evs) {
        evs =
          `|${set.evs['hp'] || ''},${set.evs['atk'] || ''},${set.evs['def'] || ''},` +
      ...
      if (set.ivs) {
        ivs =
          `|${getIv(set.ivs, 'hp')},${getIv(set.ivs, 'atk')},${getIv(set.ivs, 'def')},` +
  ...
  unpack(buf: string): PokemonSet[] | null {
    ...
    for (let count = 0; count < 24; count++) {
      ...
      set.moves = buf
        .substring(i, j)
        .split(',', 24)
        .map(name => this.unpackName(name, Dex.moves));
      i = j + 1;
    ...
    if (!hideStats) {
      ...
      if (set.ivs) {
        const stats = Dex.stats
          .ids()
          .map(stat =>
            set.ivs[stat] !== 31 && set.ivs[stat] !== undefined
