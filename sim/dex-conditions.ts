...
export interface EventMethods {
  ...
  onCriticalHit?:
    | ((this: Battle, pokemon: Pokemon, source: null, move: ActiveMove) => boolean | void)
    | boolean;
  onDamage?: (
    this: Battle,
    damage: number,
  ...
  onFoeNegateImmunity?:
    | ((this: Battle, pokemon: Pokemon, type: string) => boolean | void)
    | boolean;
  onFoeOverrideAction?: (
    this: Battle,
    pokemon: Pokemon,
...
export class Condition
  extends BasicEffect
  implements Readonly<BasicEffect & SideConditionData & FieldConditionData & PokemonConditionData>
{
  ...
  declare readonly durationCallback?: (
    this: Battle,
    target: Pokemon,
    source: Pokemon,
    effect: Effect | null
  ...
  constructor(data: AnyObject) {
    super(data);
    this.effectType = ['Weather', 'Status'].includes(data.effectType)
      ? data.effectType
      : 'Condition';
...
export class DexConditions {
  ...
  get(name?: string | Effect | null): Condition {
    ...
    return this.getByID(
      name.startsWith('item:') || name.startsWith('ability:') ? (name as ID) : toID(name)
    );
  }
