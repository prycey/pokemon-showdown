...
export class Side {
  ...
  addSlotCondition(
    ...
    if (this.slotConditions[target][status.id]) {
      ...
      return this.battle.singleEvent(
        'Restart',
        status,
        this.slotConditions[target][status.id],
        this,
        source,
        sourceEffect
      );
    ...
    if (status.durationCallback) {
      conditionState.duration = status.durationCallback.call(
        this.battle,
        this.active[0],
        source,
        sourceEffect
  ...
  chooseMove(
    ...
    const maxMove =
      event === 'dynamax' || pokemon.volatiles['dynamax']
    ...
    const canDynamax = (this.activeRequest as MoveRequest)?.active[this.active.indexOf(pokemon)]
      .canDynamax;
  ...
  chooseTeam(data = '') {
    ...
    if (ruleTable.maxTotalLevel) {
      ...
      if (totalLevel > ruleTable.maxTotalLevel) {
        if (!data) {
          // autoChoose
          positions = [...this.pokemon.keys()]
            .sort((a, b) => this.pokemon[a].level - this.pokemon[b].level)
            .slice(0, pickedTeamSize);
        } else {
          return this.emitChoiceError(
            `Your selected team has a total level of ${totalLevel}, but it can't be above ${ruleTable.maxTotalLevel}; please select a valid team of ${pickedTeamSize} Pokémon`
          );
        }
      }
    }
    if (ruleTable.valueRules.has('forceselect')) {
      ...
      if (!data) {
        // autoChoose
        positions = [...this.pokemon.keys()]
          .filter(pos => this.pokemon[pos].species.name === species.name)
          .concat(
            [...this.pokemon.keys()].filter(pos => this.pokemon[pos].species.name !== species.name)
          )
          .slice(0, pickedTeamSize);
      } else {
  ...
  clearChoice() {
    ...
    if (this.battle.requestState === 'switch') {
      ...
      const canSwitchIn = this.pokemon
        .slice(this.active.length)
        .filter(pokemon => pokemon && !pokemon.fainted).length;
